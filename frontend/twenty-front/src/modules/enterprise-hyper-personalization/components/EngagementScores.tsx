...
const TREND_COLORS: Record<string, string> = { rising: themeCssVariables.color.green, stable: themeCssVariables.color.blue, declining: themeCssVariables.color.red };
...
const StyledTrend = styled.span<{ color: string }>` color: ${({ color }) => color}; font-size: ${themeCssVariables.font.size.sm}; font-weight: ${themeCssVariables.font.weight.medium}; `;
...
export const EngagementScores = () => {
  ...
  const scores: EngagementScoreData[] = data?.hyperpersonalizationData?.scores ?? [];
  return (
    <StyledContainer>
      <StyledTitle>{t`Engagement Scores`}</StyledTitle>
      <StyledTable>
        <thead><tr><StyledTh>{t`Contact`}</StyledTh><StyledTh>{t`Score`}</StyledTh><StyledTh>{t`Trend`}</StyledTh><StyledRHH>{t`Channel`}</StyledRHH><StyledRHH>{t`Last Activity`}</StyledRHH></tr></thead>
        <tbody>{scores.map((s) => (<tr key={s.id}><StyledTd>{s.contactName}</StyledTd><StyledTd>{s.score}</StyledTd><StyledTd><StyledTrend color={TREND_COLORS[s.trend] ?? themeCssVariables.color.gray50}>{s.trend}</StyledTrend></StyledTd><StyledRH>{s.topChannel}</StyledRH><StyledRH>{s.lastActivity}</StyledRH></tr>))}</tbody>
      </StyledTable>
    </StyledContainer>
  );
};
