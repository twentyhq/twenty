...
- Twenty uses PostgreSQL as a database. If you don't a PostgreSQL instance available, we also provide a one-line command to provision a PostgreSQL instance through Docker.
...
To do so, make a copy of the `.env.example` files:
```
...
You can also provision the project with Docker. This come with a few advantages:
...
Before running the project, you need to initialize the database by running the migrations and seeds.
