...
To do so, copy the .env.example files:
```
...
**Note:** the configuration is stored `infra/dev` folder but you can run `make` commands directly from the root folder.
