...
Refrain from introducing extra colors; instead, use the existing palette from the theme. Should there be a situation where the palette does not align, please leave a comment so that the team can rectify it.
...
The codebase has a coding standard to disallow type imports. This helps maintain consistency and readability in the TypeScript code. To enforce this standard, an ESLint rule checks for and reports any type imports.
...
A specially configured ESLint rule, `@typescript-eslint/consistent-type-imports`, enforces the no-type import standard. This rule will generate errors or warnings for any type import violations.
