...
React and Recoil handle state management in Twenty's codebase.
...
They are quite useful when you need to store a list of items.
...
Avoid supplying the default value for an optional prop, as it generally doesn’t contribute too much.
