...
Most of our modules are directly related to a model in the database. For instance, we have a module for users, which is directly related to the user model in the database. 
...
In addition to NestJS recommended rules, we use custom ESLint rules to organize the code and make it easy to maintain. If you import a library that's not used in the file, it's going to be removed automatically. We also have rules to reorder `import` statements. We have the NestJS imports first, followed by external imports and then internal imports.    
