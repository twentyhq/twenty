...
:::info
Right now `npm` does not support local packages satisfactorily. That's why it's better to use `yarn` instead.
:::
...
It's better to use `nvm` to install the correct `node` version. The `server/.nvmrc` ensures all contributors use the same version.
