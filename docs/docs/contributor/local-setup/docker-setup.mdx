...
- It provides the exact same environment as Twenty's core development team.
...
The configuration for the build is present in the `infra/dev` folder, but you can run `make` commands directly from the root folder.
