...
function main() {
  ...
  if (blocks.length === 0) {
    console.log(
      'render-mermaid: no Mermaid blocks found yet (expected before final validation)',
    );
    process.exit(0);
  }
  if (blocks.length !== EXPECTED_COUNT) {
    console.error(
      `render-mermaid: expected ${EXPECTED_COUNT} diagrams, found ${blocks.length}`,
    );
  ...
  if (skipRender) {
    console.log(
      `render-mermaid: ${blocks.length} blocks found (skipped render via --skip-render)`,
    );
  ...
  for (const b of blocks) {
    ...
    try {
      execSync(`${MERMAID_CLI} -i "${inFile}" -o "${outFile}"`, {
        timeout: 30000,
        stdio: 'pipe',
      });
  ...
  if (failures > 0) {
    console.error(
      `render-mermaid: ${failures} of ${blocks.length} diagrams failed to render`,
    );
    process.exit(1);
  }
  console.log(
    `render-mermaid: all ${blocks.length} diagrams rendered successfully`,
  );
