...
describe('Source provenance', () => {
  it('source-manifest.json exists with two sources and correct hashes', () => {
    ...
    const matrix = manifest.sources.find(s => s.logicalName === 'directus-compatibility-matrix');
    assert.ok(matrix, 'matrix source missing');
    assert.match(matrix.sha256, /^[0-9a-f]{64}$/, 'matrix sha256 must be 64 hex chars');
...
describe('Collection map', () => {
  ...
  it('collection set matches source set exactly', () => {
    ...
    const setNames = readJson('sources/directus-matrix-collection-set.json').sort();
    assert.deepStrictEqual(mapNames, setNames);
  });

  it('every row has valid authority, disposition, and normalization', () => {
    const validAuth = ['DIRECTUS_AUTHORITATIVE','TWENTY_AUTHORITATIVE','APPEND_ONLY_BOTH_WITH_SHARED_IDEMPOTENCY','DERIVED_IN_TWENTY_FROM_DIRECTUS','DERIVED_IN_DIRECTUS_FROM_TWENTY','REFERENCE_ONLY_NO_REPLICATION','NOT_ALLOWED_TO_SYNC'];
    const validDisp = ['MAPPED','REFERENCE_ONLY','PORTAL_ONLY','LEGACY','OUT_OF_SCOPE'];
    const { rows } = readCsv('directus-collection-map.csv');
    for (const r of rows) {
      assert.ok(validAuth.includes(r.canonical_authority), `bad authority ${r.canonical_authority} for ${r.collection}`);
      assert.ok(validDisp.includes(r.disposition), `bad disposition ${r.disposition} for ${r.collection}`);
      assert.ok(r.normalization_status, `missing normalization_status for ${r.collection}`);
      assert.ok(r.normalization_loss, `missing normalization_loss for ${r.collection}`);
  ...
  it('SPLIT_BY_FIELD rows withhold collection authority', () => {
    const { rows } = readCsv('directus-collection-map.csv');
    const splitRows = rows.filter(r => r.raw_authority.toLowerCase().startsWith('split'));
    for (const r of splitRows) {
      assert.strictEqual(r.canonical_authority, 'NOT_ALLOWED_TO_SYNC',
  ...
  it('CSV is formula-safe (no dangerous leading chars)', () => {
    const text = readFileSync(join(ROOT, 'directus-collection-map.csv'), 'utf8');
    const lines = text.split('\n');
    for (let i = 1; i < lines.length; i++) {
      if (!lines[i].trim()) continue;
      const cells = parseCsvLine(lines[i]);
      for (const cell of cells) {
        ...
        if (t && /^[=+\-@\t\r]/.test(t) && !t.match(/^-?\d/)) {
          assert.fail(`CSV formula injection at line ${i + 1}: "${t.substring(0, 20)}"`);
        }
      }
    }
  });
...
describe('Inventory JSON', () => {
  ...
  it('all unavailable schema attributes use exact sentinel', () => {
    ...
    for (const c of inv.collections) {
      assert.strictEqual(c.fieldType, SENTINEL, `fieldType for ${c.collection}`);
      assert.strictEqual(c.isNullable, SENTINEL, `isNullable for ${c.collection}`);
      assert.strictEqual(c.isUnique, SENTINEL, `isUnique for ${c.collection}`);
      assert.strictEqual(c.relationTarget, SENTINEL, `relationTarget for ${c.collection}`);
      assert.strictEqual(c.deleteBehavior, SENTINEL, `deleteBehavior for ${c.collection}`);
      assert.strictEqual(c.aiEligibility, SENTINEL, `aiEligibility for ${c.collection}`);
    }
  });

  it('no attribute is blank, null, or inferred instead of sentinel', () => {
    const inv = readJson('directus-schema-inventory.json');
    const sentinelFields = ['fieldType', 'isNullable', 'isUnique', 'relationTarget', 'deleteBehavior', 'aiEligibility'];
    for (const c of inv.collections) {
...
describe('Field ownership registries', () => {
  it('directus-field-ownership.csv has required columns and at least one owner per row', () => {
    ...
    assert.ok(header.includes('canonical_authority'), 'missing canonical_authority column');
    assert.ok(rows.length > 0, 'no field ownership rows');
    for (const r of rows) {
      assert.ok(r.collection, 'row missing collection');
      assert.ok(r.canonical_authority, `row for ${r.collection} missing authority`);
    }
  });
...
describe('Commercial firewall', () => {
  it('commercial-selection-firewall.csv has no PERMITTED status entries', () => {
    ...
    for (const r of rows) {
      assert.notStrictEqual(r.status.toLowerCase(), 'permitted', `${r.prohibited_selector} in ${r.context} must not be PERMITTED`);
    }
  });
  ...
  it('denylist and firewall have no overlapping selectors that contradict', () => {
    ...
    if (!existsSync(join(ROOT, 'candidate-facing-nonreplication-denylist.csv'))) return;
...
describe('Event schema fixtures', () => {
  ...
  it('invalid event fixture is missing required fields', () => {
    ...
    assert.ok(!evt.sourceCollection || !evt.idempotencyKey || !evt.workspaceKey,
...
describe('Core/app object ownership', () => {
  it('ownership CSV has valid columns, enums, and unique keys when present', () => {
    if (!existsSync(join(ROOT, 'core-app-object-ownership.csv'))) {
      console.log('  (core-app-object-ownership.csv not yet created — skipping)');
      return;
    }
    const { header, rows } = readCsv('core-app-object-ownership.csv');
    const requiredCols = ['object_family_key','conceptual_objects','owner','lifecycle_on_disable','lifecycle_on_uninstall','identifier_namespace','migration_owner','rationale','source_section'];
    for (const col of requiredCols) {
      assert.ok(header.includes(col), `missing column ${col}`);
    }
    const validOwners = ['CORE_STANDARD','APP_TECHNICAL'];
    const validLifecycle = ['PRESERVE','DISABLE_RUNTIME','REMOVE_APP_METADATA'];
    const validMigration = ['CORE_UPGRADE_COMMAND','APP_INSTALL_HOOK','NOT_APPLICABLE'];
    const keys = new Set();
    for (const r of rows) {
      assert.ok(!keys.has(r.object_family_key), `duplicate key ${r.object_family_key}`);
      keys.add(r.object_family_key);
      assert.ok(validOwners.includes(r.owner), `bad owner for ${r.object_family_key}`);
      assert.ok(validLifecycle.includes(r.lifecycle_on_disable), `bad lifecycle_on_disable for ${r.object_family_key}`);
      assert.ok(validLifecycle.includes(r.lifecycle_on_uninstall), `bad lifecycle_on_uninstall for ${r.object_family_key}`);
      assert.ok(validMigration.includes(r.migration_owner), `bad migration_owner for ${r.object_family_key}`);
    }
  });
});
...
describe('Generator reproducibility', () => {
  it('generator --check passes (inventory matches committed sources)', () => {
    ...
    assert.ok(existsSync(join(ROOT, 'directus-schema-inventory.json')), 'inventory JSON must exist');
  });
});
