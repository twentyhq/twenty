...
const CreateFieldMetadataInputSchema = z.object({
  ...
  calculation: z
    .unknown()
    .optional()
    .describe('Calculated field definition stored under settings.calculation'),
...
export class FieldMetadataToolsFactory {
  constructor(
    ...
    private readonly calculatedFieldService: CalculatedFieldService,
  ) {}
    ...
    return {
      ...
      create_field_metadata: {
        ...
        execute: async (parameters: {
          ...
          try {
            ...
            const mergedSettings = this.mergeCalculationSettings(
              existingSettings,
              calculation,
            );
          ...
          try {
            ...
            const flatFieldMetadata =
              await this.fieldMetadataService.updateOneField({
                updateFieldInput: { id, ...update, settings: mergedSettings } as Parameters<
        ...
        execute: async (parameters: {
          ...
          try {
            await this.fieldMetadataService.createManyFields({
              createFieldInputs: parameters.fields.map((field) => {
                const { calculation, settings, ...createFieldInput } = field;

                return {
                  ...createFieldInput,
                  settings: this.mergeCalculationSettings(settings, calculation),
                };
              }) as Parameters<
      ...
      update_many_field_metadata: {
        ...
        execute: async (parameters: {
          ...
          try {
            await Promise.all(
              parameters.fields.map(async ({ id, calculation, settings: existingSettings, ...update }) => {
                const mergedSettings = this.mergeCalculationSettings(
                  existingSettings,
                  calculation,
                );
      ...
      preview_calculated_field: {
        ...
        execute: async (parameters: {
          ...
          const fieldMetadata = await this.fieldMetadataService.findOneWithinWorkspace(
            workspaceId,
            {
              where: {
          ...
          if (!fieldMetadata) {
            throw new Error(`Field metadata ${parameters.fieldId} not found`);
          }

          return this.calculatedFieldService.previewFieldValue({
            fieldMetadata,
  ...
  private mergeCalculationSettings(
    settings: unknown,
    calculation: unknown,
  ): unknown {
    if (!calculation) {
