import { Optional, UseFilters, UseGuards, UsePipes } from '@nestjs/common';
...
import { ConfigVariableVersionDTO } from 'src/engine/core-modules/admin-panel/dtos/config-variable-version.dto';
...
export class AdminPanelResolver {
  ...
  @UseGuards(AdminPanelGuard)
  @Query(() => [ConfigVariableVersionDTO])
  async getDatabaseConfigVariableVersions(
    @Args('key', { type: () => String }) key: keyof ConfigVariables,
    @Args('limit', { type: () => Int, nullable: true, defaultValue: 50 })
    limit?: number,
  ): Promise<ConfigVariableVersionDTO[]> {
    if (!this.configVariableVersionService) {
      return [];
    }

    return await this.configVariableVersionService.getHistory(key, limit);
  }
