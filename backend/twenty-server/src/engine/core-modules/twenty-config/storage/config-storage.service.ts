...
export class ConfigStorageService implements ConfigStorageInterface {
  ...
  async toAppValue<T extends keyof ConfigVariables>(
    dbValue: unknown,
    key: T,
  ): Promise<ConfigVariables[T] | undefined> {
    return await this.convertAndSecureValue(dbValue, key, true);
  }

  async toStorageValue<T extends keyof ConfigVariables>(
    appValue: ConfigVariables[T] | null | undefined,
    key: T,
  ): Promise<unknown> {
    return await this.convertAndSecureValue(appValue, key, false);
  }
  ...
  async get<T extends keyof ConfigVariables>(
    ...
    try {
      ...
      return await this.toAppValue(result.value, key);
    } catch (error) {
  ...
  async set<T extends keyof ConfigVariables>(
    ...
    try {
      const dbValue = await this.toStorageValue(value, key);
    ...
    try {
      ...
      for (const configVar of configVars) {
        if (configVar.value !== null) {
          ...
          try {
            const value = await this.toAppValue(
              configVar.value,
              key,
            );
