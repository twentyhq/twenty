...
Runs Twenty from the official image (see `docs/adr/0001-deploy-from-official-twenty-image-not-source.md`). Only this directory goes on the VPS, not the monorepo.
...
2. Run `scripts/backup.sh`, then `scripts/restore-test.sh`.
3. Try the new `TAG` on a local copy with synthetic data first.
4. Change `TAG`, `docker compose pull && docker compose up -d`.
