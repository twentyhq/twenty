...
import { SALES_NOTES_LIST_ON_PERSON_FRONT_COMPONENT_UID } from 'src/constants/universal-identifiers';
import {
  type RelatedSalesNote,
  fetchSalesNotesForPerson,
...
const SalesNotesListOnPerson = () => {
  ...
  useEffect(() => {
    ...
    setRecords(null);
    setError(null);
    fetchSalesNotesForPerson(personId)
  ...
  if (records.length === 0) {
    return (
      <div style={emptyStyle}>
        <span>
          No call reports for this contact yet. Click <b>+ Call report</b> in the
...
export default defineFrontComponent({
  universalIdentifier: SALES_NOTES_LIST_ON_PERSON_FRONT_COMPONENT_UID,
  name: 'sales-notes-list-on-person',
  description:
    'Lists call reports the focal Person is an attendee on (via salesNoteAttendee). Each row navigates to the call report.',
  component: SalesNotesListOnPerson,
});
