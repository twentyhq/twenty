...
// Drag + resize against the parent bounds. During interaction the shell
// mutates imperatively (transform/width/height) for frame-rate; React
// state commits on release. Ported verbatim.
...
export function useWindowPointerInteractions({
  ...
  shellRef,
  size,
}: UseWindowPointerInteractionsOptions) {
  const dragStateRef = useRef<WindowDragState | null>(null);
  const resizeStateRef = useRef<WindowResizeState | null>(null);
  const latestPositionRef = useRef<WindowPosition | null>(position);
  latestPositionRef.current = position;
  const latestSizeRef = useRef<WindowSize | null>(size);
  ...
  useEffect(() => {
    ...
    const handleMove = (event: PointerEvent) => {
      ...
      const nextPosition = windowGeometry.clampPosition({
        bounds,
        candidate: {
  ...
  }, [edgeGap, getBounds, isDragging, setPosition, shellRef]);
  ...
  }, [edgeGap, getBounds, isResizing, minSize, setPosition, setSize, shellRef]);
