...
import { SalesfarceWindow } from './salesfarce-window';
import { SalesfarceWrongChoicePopup } from './salesfarce-wrong-choice-popup';
import {
  type SalesfarceAddonRowType,
  type SalesfarcePricingPanelType,
  type SalesfarceWrongChoicePopupType,
...
const getPopupPosition = (
  ...
  const maxLeft = Math.max(
    POPUP_MARGIN,
    containerRect.width - WRONG_CHOICE_POPUP.widthPx - POPUP_MARGIN,
  );
...
type SalesfarceFlowProps = {
  pricing: SalesfarcePricingPanelType;
};

export function SalesfarceFlow({ pricing }: SalesfarceFlowProps) {
  ...
  const handleAddonToggle = useCallback(
    (addon: SalesfarceAddonRowType, anchorRect: DOMRect | null) => {
      if (addon.disabled) {
        return;
      }
