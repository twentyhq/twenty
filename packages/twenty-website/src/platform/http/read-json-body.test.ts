import { readJsonBody } from '@/platform/http/read-json-body';
...
function jsonRequest(
  ...
  return new Request('https://example.com/', { method: 'POST', headers, body });
}
...
describe('readJsonBody', () => {
  ...
  it('rejects when declared content-length exceeds maxBytes (fast path)', async () => {
    const result = await readJsonBody(
  ...
  it('rejects when actual body length exceeds maxBytes (no content-length)', async () => {
