...
} from '@/platform/visuals/engine/create-visual-frame-loop';
import { WEBGL_CONTEXT_LOST_EVENT } from '@/platform/visuals/engine/webgl-context-lost-event';
import { isWebGlSupported } from '@/platform/visuals/engine/webgl-policy';
import { createVisualRenderer } from '@/platform/visuals/three-runtime/create-visual-renderer';

import { HALFTONE_FOOTPRINT, type HalftoneImageFit } from './footprint';
import { HALFTONE_IMAGE_CONSTANTS } from './image-constants';
import {
  HALFTONE_INTERACTION,
...
import { HALFTONE_SHADERS } from './shaders';
import type {
  HalftoneExportPose,
...
const DEFAULT_IMAGE_INTERACTION_SETTINGS: HalftoneImageInteractionSettings = {
  hoverFadeIn: HALFTONE_IMAGE_CONSTANTS.hoverFadeIn,
  hoverFadeOut: HALFTONE_IMAGE_CONSTANTS.hoverFadeOut,
  pointerFollow: HALFTONE_IMAGE_CONSTANTS.pointerFollow,
  pointerVelocityDamping: HALFTONE_IMAGE_CONSTANTS.pointerVelocityDamping,
};
...
function updateMaterial(
  ...
  HALFTONE_MATERIALS.applySettings(
    resources.material,
    settings.material,
...
export function HalftoneCanvas({
  ...
  settings,
  snapshotRef,
  virtualRenderHeight = HALFTONE_FOOTPRINT.virtualRenderHeight,
}: HalftoneCanvasProps) {
  ...
  const interactionReference = useRef<HalftoneInteractionState>(
    HALFTONE_INTERACTION.create(initialPose),
  );
  ...
  useEffect(() => {
    ...
    interactionReference.current = HALFTONE_INTERACTION.create(initialPose);
  }, [initialPose]);
  ...
  useEffect(() => {
    ...
    if (
      ...
      HALFTONE_INTERACTION.reset(interactionReference.current, next);
    }
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    let renderLoop: VisualFrameLoop | null = null;
    ...
    void (async () => {
      const materialAssets = await HALFTONE_MATERIALS.createAssets(renderer);

      if (cancelled) {
        HALFTONE_MATERIALS.disposeAssets(materialAssets);
        cleanup();
      ...
      const material = HALFTONE_MATERIALS.create();
      HALFTONE_MATERIALS.applySettings(
        material,
      ...
      const blurHorizontalMaterial = new THREE.ShaderMaterial({
        ...
        vertexShader: HALFTONE_SHADERS.passThroughVertex,
        fragmentShader: HALFTONE_SHADERS.blurFragment,
      });
      ...
      const halftoneMaterial = new THREE.ShaderMaterial({
        ...
        vertexShader: HALFTONE_SHADERS.passThroughVertex,
        fragmentShader: HALFTONE_SHADERS.halftoneFragment,
      });
      ...
      const imageMaterial = new THREE.ShaderMaterial({
        uniforms: {
          ...
          zoom: {
            value: HALFTONE_FOOTPRINT.getImagePreviewZoom(
        ...
        vertexShader: HALFTONE_SHADERS.passThroughVertex,
        fragmentShader: HALFTONE_SHADERS.imagePassthrough,
      });
      ...
      const getImageHalftoneScale = (
        ...
        return HALFTONE_FOOTPRINT.getImageFootprintScale({
      ...
      const getMeshHalftoneScale = (
        ...
        return HALFTONE_FOOTPRINT.getMeshFootprintScale({
          camera,
          localBounds,
          lookAtTarget,
      ...
      const captureSnapshot: HalftoneSnapshotFn = async (
        ...
        if (isImage) {
          imageMaterial.uniforms.zoom.value =
            HALFTONE_FOOTPRINT.getImagePreviewZoom(
        ...
        if (isImage) {
          ...
          HALFTONE_MATERIALS.renderScene({
            camera,
      ...
      const renderFrame = ({ deltaSeconds, elapsedSeconds }: VisualFrame) => {
        ...
        const elapsedTime =
          (initialPoseReference.current?.timeElapsed ?? 0) + elapsedSeconds;
        ...
        if (isImageMode) {
          ...
          imageMaterial.uniforms.zoom.value =
            HALFTONE_FOOTPRINT.getImagePreviewZoom(baseDistance);
          imageMaterial.uniforms.viewportSize.value.set(
            logicalWidth,
            logicalHeight,
          );
        ...
        if (!isImageMode) {
          ...
          if (activeSettings.animation.springReturnEnabled) {
            const springX = HALFTONE_INTERACTION.springStep({
            ...
            const springY = HALFTONE_INTERACTION.springStep({
            ...
            const springZ = HALFTONE_INTERACTION.springStep({
      ...
      if (renderStrategy !== 'static') {
        ...
        canvas.addEventListener(WEBGL_CONTEXT_LOST_EVENT, handleContextLost);
        ...
        renderLoop = createVisualFrameLoop({
          renderFrame,
          target: container,
