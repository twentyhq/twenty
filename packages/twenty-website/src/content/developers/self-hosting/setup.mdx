...
### Mode 1: Database Configuration (Default - Recommended)
```bash
IS_CONFIG_VARIABLES_IN_DB_ENABLED=true  # default
```

**How it works:**
- Admin panel can modify most settings through the UI
- Changes take effect immediately (within 15 seconds for multi-container deployments)
- Environment variables are only needed for infrastructure settings
- Multi-container deployments share configuration automatically

### Mode 2: Environment-Only Configuration
```bash
...
- Admin panel shows current values but cannot modify them
- Requires container restart for changes
- Full control over configuration versioning

---

## Admin Panel Configuration (Mode 1 - Default)
...
**Who can access:** Only users with admin privileges (`canAccessFullAdminPanel: true`)
...
## Environment-Only Configuration (Mode 2)
...
## Infrastructure Variables (Both Modes)

Some **infrastructure variables** can only be set via environment variables for security reasons and require container restart:

**Required infrastructure variables** (automatically set by install script):
...
**When you need to edit your .env file:**

**Database Configuration Mode** (default - `IS_CONFIG_VARIABLES_IN_DB_ENABLED=true`):
- Only for variables marked as `isEnvOnly: true` in the code (infrastructure, logging, performance settings)
- Application settings (auth providers, email, storage, integrations) use the admin panel

**Environment-Only Mode** (`IS_CONFIG_VARIABLES_IN_DB_ENABLED=false`):
- All configuration changes require editing .env and container restart
- Admin panel shows values but cannot modify them

**Infrastructure variables** (cannot be changed via admin panel):
- **Database:** `PG_DATABASE_URL`, `PG_SSL_ALLOW_SELF_SIGNED`, `PG_ENABLE_POOL_SHARING`, `PG_POOL_*`
- **Server:** `SERVER_URL`, `APP_SECRET`, `NODE_PORT`, `NODE_ENV`, `FRONTEND_URL`
- **Cache:** `REDIS_URL`, `IS_CONFIG_VARIABLES_IN_DB_ENABLED`
- **SSL:** `SSL_KEY_PATH`, `SSL_CERT_PATH`
- **Logging/Monitoring:** `EXCEPTION_HANDLER_DRIVER`, `LOG_LEVELS`, `LOGGER_DRIVER`, `METER_DRIVER`, `OTLP_COLLECTOR_ENDPOINT_URL`
- **Performance:** `SERVERLESS_TYPE`, `LLM_CHAT_MODEL_DRIVER`, `LLM_TRACING_DRIVER`, `CAPTCHA_DRIVER`
- **Enterprise:** `IS_MULTIWORKSPACE_ENABLED`, `APP_VERSION`
