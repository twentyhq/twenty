...
**If it does not work:**  
If you tried to run only `npx nx run twenty-server:start` on WSL and its failing with the below memory error:
...
Please note that system memory requirement is minimum 2Gb of RAM but by setting 8GB here in WSL it should work.
