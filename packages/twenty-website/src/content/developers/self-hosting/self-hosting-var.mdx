...
<ArticleTable options={[
  ['CUSTOM_CODE_ENGINE_DRIVER_TYPE', 'local', "Custom code engine driver: 'local' or 'lambda'"],
  ['LAMBDA_REGION', '', 'Lambda Region'],
  ['LAMBDA_ROLE', '', 'Lambda Role'],
  ['LAMBDA_ACCESS_KEY_ID', '', 'Optional depending on the authentication method'],
  ['LAMBDA_SECRET_ACCESS_KEY', '', 'Optional depending on the authentication method'],
]}></ArticleTable>
