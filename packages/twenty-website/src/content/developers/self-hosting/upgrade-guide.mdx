...
To fix a desynchronized workspace version, you will have to pull the corresponding twenty's version and manually run both `migrate` and `upgrade` commands and so on until it reaches desired version:
...
#### I have a workspace version between `0.52.0` and `0.52.6`

Unfortunately `0.52.0` and `0.52.6` have been completly removed from dockerHub.
You will have to manually udpate your workspace version to `0.51.0` and pull twenty `0.52.11` to run the migrate and upgrade command from.
