...
To fix a desynchronized workspace version, you will have to pull the corresponding twenty's version sequentially and so on until it reaches desired version.

For Twenty's version that are anterior to `0.53` you will have to manually run:
