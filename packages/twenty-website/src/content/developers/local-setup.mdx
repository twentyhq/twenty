...
> ### __Note: Run all the below commands in wsl__.
> Run `wsl -d ${distroName}` to start wsl from windows terminal
...
<ArticleTabs label1="Linux" label2="Mac OS" label3="Windows (WSL)">
  ...
  <ArticleTab>
    ...
    ```bash
     ...
     sudo psql postgres -c "CREATE DATABASE \"default\";" -c "CREATE DATABASE test;" -c "CREATE USER twenty PASSWORD 'twenty';" -c "ALTER ROLE twenty superuser;"
    ```
