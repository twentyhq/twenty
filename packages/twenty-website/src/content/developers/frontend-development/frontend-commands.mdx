...
```bash npx nx start twenty-front
 ```
...
```bash
npx nx graphql:generate twenty-front --configuration=metadata
```
OR
```bash
...
```bash
npx nx test twenty-front# run jest tests
npx nx storybook:serve:dev twenty-front# run storybook
npx nx storybook:test twenty-front# run tests # (needs yarn storybook:serve:dev to be running)
npx nx storybook:coverage twenty-front # (needs yarn storybook:serve:dev to be running)
```
