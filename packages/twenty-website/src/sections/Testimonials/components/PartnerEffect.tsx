...
import { useAsyncImage } from '@/lib/visual-runtime/hooks/use-async-image';
...
const EffectFrame = styled.div`
  ...
  /*
   * Defensive cursor reset for the inner WebGL canvas. HalftoneCanvas
   * itself sets canvas.style.cursor imperatively from getCanvasCursor(),
   * which always returns 'default' for our (sourceMode: 'image',
   * followDragEnabled: false) settings — so the inline style and this
   * rule already agree. This rule exists as a CSS-side fallback for the
   * brief window before the imperative path runs.
   */
  & canvas {
    cursor: default;
  }
