...
const PRODUCT_INTRO_IMAGE_URL =
  '/illustrations/generated/product-background-skyline.webp?v=7';
...
async function mountProductIntroCanvas({
  container,
}: {
  container: HTMLDivElement;
}): Promise<() => void> {
  const image = await loadVisualImage(PRODUCT_INTRO_IMAGE_URL, {
    label: 'product intro background',
  });
  ...
  const imageMaterial = new THREE.ShaderMaterial({
    ...
    uniforms: {
      ...
      horizontalPixelOffset: { value: HORIZONTAL_OFFSET_PX },
      imageSize: { value: new THREE.Vector2(image.width, image.height) },
      tImage: { value: imageTexture },
      verticalAnchor: { value: VERTICAL_ANCHOR },
      verticalPixelOffset: { value: VERTICAL_OFFSET_PX },
      viewportSize: {
  ...
  const renderFrame = (
    ...
    pointer.smoothedMouseX +=
      (pointer.mouseX - pointer.smoothedMouseX) * POINTER_FOLLOW;
    pointer.smoothedMouseY +=
      (pointer.mouseY - pointer.smoothedMouseY) * POINTER_FOLLOW;
...
export function useProductIntroHalftone({
  mountRef,
}: {
