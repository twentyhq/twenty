...
import { type TerminalSendButtonHintPosition } from "./terminal-send-button-hint-position";

const FINGER_INK = APP_PREVIEW_STAGE.fingerHintInk;

const FINGER_ROTATION = -21;
const FINGER_SIZE = 51;
const FINGER_HINT_Z_INDEX = 20;

const FingerHint = styled.span`
  pointer-events: none;
  position: fixed;
  z-index: ${FINGER_HINT_Z_INDEX};
`;
...
// The old site's authored pointing-hand artwork, verbatim.
function FingerIcon({ size }: { size: number }) {
	return (
		<svg
			width={size}
			height={size}
		...
		>
			<g filter="url(#finger_shadow)">
				<path
					...
					fill={FINGER_INK.fill}
					stroke={FINGER_INK.outline}
					strokeWidth="0.75"
					strokeLinecap="round"
...
export function TerminalSendButtonFingerHint({
	position,
	ready,
	visible,
}: {
