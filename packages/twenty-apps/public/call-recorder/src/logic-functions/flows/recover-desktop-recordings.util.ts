...
export const hydrateDesktopRecordingTimestamps = async (
  ...
  if (
    !isDesktopAudioRecording(recording.desktopRecordingSession) ||
...
export const recoverDesktopRecordings = async (
  ...
  for (const { node } of result.callRecordings?.edges ?? []) {
    if (!node || !isDesktopAudioRecording(node.desktopRecordingSession))
      continue;
    const session = asRecord(node.desktopRecordingSession);
      ...
      if (now.getTime() - Date.parse(String(node.createdAt)) > 60 * 60_000)
        await updateCallRecording(client, {
          ...
          data: {
            ...
            callRecorderFailureReason: 'desktop_setup_interrupted',
          },
        });
      continue;
    }
    const upload = await recallBotApiRequest<{
      ...
      path: `/sdk_upload/${sdkUploadId}/`,
      method: 'GET',
      maxAttempts: 1,
    });
    if (!upload.ok) continue;
    const { status, recording_id: recordingId } = upload.data;
    if (
      status.code === 'failed' ||
      (status.code === 'pending' &&
      ...
      const updated = await updateCallRecording(client, {
        id: String(node.id),
        expectedStatuses: NON_TERMINAL_CALL_RECORDING_STATUSES,
        data: {
