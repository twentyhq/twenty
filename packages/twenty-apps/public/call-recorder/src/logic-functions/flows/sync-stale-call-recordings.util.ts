...
const SYNC_BOT_LIST_LOOKBACK_DAYS = SYNC_LOOKBACK_DAYS + 1;
const SYNC_BOT_LIST_LOOKAHEAD_MS = 60 * 60 * 1000;
const SYNC_PER_ID_FALLBACK_LIMIT = 25;
const SYNC_PER_ID_FALLBACK_ROTATION_INTERVAL_MS = 15 * 60 * 1000;

type StaleCallRecordingCandidate = SyncableCallRecording & {
...
type ActionableCallRecordingCandidate = StaleCallRecordingCandidate & {
  externalBotId: string;
};

type StaleCallRecordingNode = {
  id: string;
  status?: string | null;
...
type SyncActionableCandidateOutcome =
  | { outcome: 'synced'; updated: boolean; requestedTranscript: boolean }
  | { outcome: 'marked-failed-after-bot-loss' }
  | { outcome: 'unsynced-after-bot-loss' }
  | { outcome: 'bot-state-unavailable' };

export const syncStaleCallRecordings = async ({
  client,
  now,
}: {
  client: CoreApiClient;
  now: Date;
}): Promise<SyncStaleCallRecordingsResult> => {
  const candidates = await fetchStaleCallRecordingCandidates(client);
  const triage = triageSyncCandidates({ candidates, now });
  const result: SyncStaleCallRecordingsResult = {
    candidateCount: candidates.length,
    updatedCallRecordingIds: [],
    markedFailedCallRecordingIds: [],
    requestedTranscriptCallRecordingIds: [],
    unsyncedCallRecordingIds: triage.unsyncedCallRecordingIds,
  ...
  const listedBotsById = await listRecallBotsByIdForSync(now);
  ...
  let remainingPerIdFallbackCount = SYNC_PER_ID_FALLBACK_LIMIT;
  ...
  for (const candidate of orderedActionableCandidates) {
    ...
    if (isUndefined(listedBot) && remainingPerIdFallbackCount === 0) {
      console.warn(
        `[call-recorder] skipping Recall bot ${candidate.externalBotId} for call recording ${candidate.id}: per-id sync fallback budget exhausted`,
      );
    ...
    const candidateOutcome = await syncActionableCandidate({
      client,
      candidate,
      listedBot,
      now,
    ...
    if (candidateOutcome.outcome === 'synced') {
    ...
    if (candidateOutcome.outcome === 'unsynced-after-bot-loss') {
      result.unsyncedCallRecordingIds.push(candidate.id);
    }
  }
...
const triageSyncCandidates = ({
  candidates,
  now,
}: {
  candidates: StaleCallRecordingCandidate[];
  now: Date;
}): SyncCandidateTriage => {
  const syncLookbackBoundary = new Date(
    now.getTime() - SYNC_LOOKBACK_DAYS * 24 * 60 * 60 * 1000,
  );
  const triage: SyncCandidateTriage = {
  ...
  for (const candidate of candidates) {
    if (isOutsideSyncLookback(candidate, syncLookbackBoundary)) {
      console.warn(
        `[call-recorder] stale call recording ${candidate.id} falls outside the ${SYNC_LOOKBACK_DAYS}-day sync window; it will not sync automatically`,
      );
      triage.unsyncedCallRecordingIds.push(candidate.id);
    ...
    if (isUndefined(candidate.externalBotId)) {
      console.warn(
        `[call-recorder] stale call recording ${candidate.id} has no Recall bot id; it will not sync automatically`,
      );
      triage.unsyncedCallRecordingIds.push(candidate.id);
...
const syncActionableCandidate = async ({
  client,
  candidate,
  listedBot,
  now,
  ...
  now: Date;
}): Promise<SyncActionableCandidateOutcome> => {
  ...
  return { outcome: 'synced', ...syncResult };
};
...
const markCallRecordingFailedAfterBotLoss = async ({
  ...
  console.warn(
    `[call-recorder] Recall bot ${candidate.externalBotId} for call recording ${candidate.id} no longer exists; the recording cannot sync automatically`,
  );
  ...
  if (
    ...
    return { outcome: 'unsynced-after-bot-loss' };
  }
...
const rotateActionableCandidatesForFallback = ({
  ...
  if (candidates.length <= SYNC_PER_ID_FALLBACK_LIMIT) {
    return candidates;
  }

  const rotationOffset =
    Math.floor(now.getTime() / SYNC_PER_ID_FALLBACK_ROTATION_INTERVAL_MS) %
    candidates.length;
...
const listRecallBotsByIdForSync = async (
  now: Date,
  ...
  if (isUndefined(currentWorkspaceId)) {
    console.warn(
      '[call-recorder] workspace id unavailable for Recall bot list fetch; using capped per-id sync fallback',
    );
  ...
  const listResult = await listScheduledRecallBots({
    joinAtAfter: new Date(
      now.getTime() - SYNC_BOT_LIST_LOOKBACK_DAYS * 24 * 60 * 60 * 1000,
    ).toISOString(),
    joinAtBefore: new Date(
      now.getTime() + SYNC_BOT_LIST_LOOKAHEAD_MS,
    ).toISOString(),
  ...
  if (!listResult.ok) {
    console.warn(
      `[call-recorder] Recall bot list fetch failed; deferring stale recording sync to the next run: ${listResult.errorMessage}`,
    );
...
const fetchStaleCallRecordingCandidates = async (
  client: CoreApiClient,
): Promise<StaleCallRecordingCandidate[]> => {
  ...
  const candidateNodes = await fetchAllNodes<StaleCallRecordingNode>(
    async (afterCursor) => {
      ...
      return (queryResult.callRecordings ?? undefined) as
        | ConnectionPage<StaleCallRecordingNode>
        | undefined;
    },
  );
...
const isOutsideSyncLookback = (
  candidate: StaleCallRecordingCandidate,
  syncLookbackBoundary: Date,
): boolean => {
  ...
  return (
    ...
    new Date(boundReference).getTime() < syncLookbackBoundary.getTime()
  );
};
