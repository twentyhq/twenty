...
type DesktopRecord = {
  id: string;
  status: string;
  desktopRecordingSession: DesktopRecordingSession | null;
};
const getSession = async (
  ...
  const result = await client.query({
    callRecordings: {
      ...
      edges: {
        node: { id: true, status: true, desktopRecordingSession: true },
      },
    },
  });
...
export const createDesktopRecordingUpload = async (
  ...
  if (existing) {
    if (
      !isDesktopAudioRecording(existing.desktopRecordingSession) ||
      existing.desktopRecordingSession?.userWorkspaceId !== userWorkspaceId
    )
    ...
    if (!existing.desktopRecordingSession.sdkUploadId)
      throw new Error('Recording setup is still in progress. Please retry.');
    const upload = await recallBotApiRequest<{ upload_token: string }>({
      config,
      path: `/sdk_upload/${existing.desktopRecordingSession.sdkUploadId}/`,
      method: 'GET',
    });
    if (!upload.ok || !upload.data?.upload_token)
    ...
    createCallRecording: {
      __args: {
        data: {
          ...
          desktopRecordingSession: session,
        },
      },
      id: true,
    },
    ...
    await client.mutation({
      updateCallRecording: {
        __args: {
          ...
          data: {
            status: 'FAILED',
            callRecorderFailureReason: 'desktop_upload_creation_failed',
          },
        },
        id: true,
      },
  ...
  await client.mutation({
    updateCallRecording: {
      __args: {
        id: sessionId,
        data: {
          desktopRecordingSession: { ...session, sdkUploadId: upload.data.id },
        },
      },
      id: true,
    },
  ...
  if (recording.status === 'JOINING')
    await updateCallRecording(client, {
      ...
      data: {
        ...
        callRecorderFailureReason: 'desktop_capture_did_not_start',
      },
    });
