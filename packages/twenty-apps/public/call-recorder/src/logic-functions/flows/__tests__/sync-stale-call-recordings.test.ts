...
describe('syncStaleCallRecordings', () => {
  ...
  it('defers syncState without per-id fetches when the Recall bot list fails', async () => {
    listScheduledRecallBotsMock.mockResolvedValue({
      ok: false,
      status: 429,
  ...
  it('completes and charges when syncState lands the last artifact', async () => {
    getRecallBotMock.mockResolvedValue({
      ok: true,
      bot: {
        statusChanges: [
  ...
  it('does not use old createdAt as the syncState bound when calendar data is missing', async () => {
    getRecallBotMock.mockResolvedValue({
      ok: true,
      bot: {
        statusChanges: [
  ...
  it('fills a completed Recall transcript artifact during syncState', async () => {
