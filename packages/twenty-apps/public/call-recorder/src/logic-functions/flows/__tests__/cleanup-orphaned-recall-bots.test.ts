...
describe('cleanupOrphanedRecallBots', () => {
  ...
  it('lists only bots claimed by the current workspace', async () => {
    ...
    const listRequestParams = new URL(listRequestUrl).searchParams;
    expect(listRequestParams.get('join_at_after')).toBe(JOIN_AT_AFTER);
    expect(listRequestParams.get('join_at_before')).toBe(JOIN_AT_BEFORE);
