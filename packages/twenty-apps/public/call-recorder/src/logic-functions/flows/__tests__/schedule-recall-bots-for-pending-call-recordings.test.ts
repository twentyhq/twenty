...
describe('scheduleRecallBotsForPendingCallRecordings', () => {
  ...
  it('attaches an existing bot claiming the recording instead of scheduling a duplicate', async () => {
    ...
    const lookupParams = new URL(listBotRequestUrls()[0]).searchParams;
    expect(lookupParams.get('metadata__twentyWorkspaceId')).toBe(WORKSPACE_ID);
    expect(lookupParams.get('metadata__twentyCallRecordingId')).toBe(
      'call-recording-1',
    );
  ...
  it('widens the existing-bot lookup window when the join-early offset exceeds the 24h floor', async () => {
    ...
    const lookupParams = new URL(listBotRequestUrls()[0]).searchParams;
    expect(lookupParams.get('join_at_after')).toBe(
      new Date(
