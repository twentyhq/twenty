...
describe('handleRecallWebhook', () => {
  ...
  it('marks the recording NOT_RECORDED when nobody joined the meeting', async () => {
    ...
    expect(requestArtifactContinuationMock).not.toHaveBeenCalled();
    expect(client.mutations).toEqual([
      {
        id: 'call-recording-1',
        data: {
  ...
  it('marks the recording NOT_RECORDED when the bot was never admitted', async () => {
    const client = new FakeCoreApiClient([
      {
        id: 'call-recording-1',
        status: 'JOINING',
  ...
  it('skips a late done event once the recording is NOT_RECORDED', async () => {
    const client = new FakeCoreApiClient([
      {
        id: 'call-recording-1',
        status: 'NOT_RECORDED',
