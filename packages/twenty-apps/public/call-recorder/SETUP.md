...
> default (`CALL_RECORDER_RECORD_BY_DEFAULT`, on by default; turn it off to
> record only meetings explicitly set to **On**), display name, join timing,
> lobby and leave timeouts, the transcription provider
> (`CALL_RECORDER_TRANSCRIPT_PROVIDER`) and the summary settings
> (`CALL_RECORDER_SUMMARY_ENABLED`, `CALL_RECORDER_ADDITIONAL_SUMMARY_PROMPT`)
> are **application variables** that a workspace admin tunes inside the app —
> not server variables. See [PRIVACY.md](./PRIVACY.md) for what the app
...
|---|---|---|
| No bot joined a meeting | The event's **Recording Bot** field was Off, the workspace turned `CALL_RECORDER_RECORD_BY_DEFAULT` off without setting the field to On, the event had no conference link, it wasn't synced from a connected calendar, or `RECALL_API_KEY` isn't set | Confirm the event is upcoming, has a video link, and came from a synced calendar; set the **Recording Bot** field to On (or leave the workspace default on); confirm `RECALL_API_KEY` is set |
...
| Bot joined a meeting you didn't want recorded | Recording is on by default (`CALL_RECORDER_RECORD_BY_DEFAULT`) | Set the event's **Recording Bot** field to Off (the scheduled bot is canceled); turn off `CALL_RECORDER_RECORD_BY_DEFAULT` to make recording opt-in per meeting workspace-wide |
