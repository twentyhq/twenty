...
> timeouts) and the summary prompt (`CALL_RECORDER_SUMMARY_PROMPT`) are
> **application variables** that a workspace admin tunes inside the app — not
> server variables.
...
| Summary stays empty after the transcript arrives | Summaries are disabled, or the summarizer run failed (for example, out of AI credits) | Confirm `CALL_RECORDER_SUMMARY_PROMPT` isn't `false` and the workspace has AI credits |
| Summary shows "No summary available." | The transcript was too short or unintelligible to summarize | No action needed; this is the expected verdict for low-quality transcripts |
