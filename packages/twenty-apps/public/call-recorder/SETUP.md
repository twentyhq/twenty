...
| Recording is `NOT_RECORDED` with reason `workspace_out_of_credits`, `workspace_without_subscription` or `workspace_suspended` | The workspace could not spend credits about ten minutes before the meeting, so the scheduled bot was canceled | Top up credits or fix the subscription in **Settings → Billing**; later meetings are recorded again |
