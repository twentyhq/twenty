...
2. A **webhook** from Recall.ai back to your deployment, so the app learns when
   a recording is ready and can import it.
...
| `CALL_RECORDER_RECORDING_RETENTION_HOURS` | No       | How long Recall.ai retains the source media after processing. Defaults to `166` hours (6 days 22 hours), just under Recall's 168-hour free-storage window. Values above `168` may incur Recall storage charges. Twenty's imported copy is unaffected. |
...
`PROCESSING`, or `FAILED`), and — once the recording finishes — imports the
...
1. In the Recall.ai dashboard, create a webhook endpoint pointing at your
   ...
   **recording**, and **transcript** events (`bot.status_change`,
   ...
   transcript events trigger media and transcript import. Subscribing to
   status changes alone leaves import to the scheduled sync backstop.
...
imported. Recall produces only the artifacts requested at bot creation (mixed
...
default) to stay inside its free-storage window. Twenty imports and stores the
video, audio, and transcript in its own storage, so they remain available after
Recall's media expires.

A scheduled sync runs as a safety net, pulling the latest status from Recall and
keeping Call Recordings and Recall Bots in sync when a real-time webhook update
is missed.
...
| Recording never reaches `COMPLETED`                                              | A Recall webhook was missed, or only one of audio/video was produced                                                               | The scheduled sync pulls the latest status from Recall within a few minutes; if it is marked `FAILED`, inspect the bot in the Recall dashboard |
| Transcript empty, or marked pending/failed                                       | Recall hasn't finished async transcription yet, or transcription failed for that call                                              | Wait for the scheduled sync to import the transcript; a persistent failure leaves a marker in the transcript                                   |
