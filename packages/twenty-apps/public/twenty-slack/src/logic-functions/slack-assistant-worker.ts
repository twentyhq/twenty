...
export const slackAssistantWorkerHandler = async (
  ...
  // In channels the reply goes to the mention's thread; in DMs it goes to the
  // conversation itself unless the request was already inside a thread.
  ...
  if (!placeholderResult.success || !isNonEmptyString(placeholderResult.slackTs)) {
    await updateSlackAssistantRequest(client, {
      id: record.id,
