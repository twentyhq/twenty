...
const downloadTranscriptContent = async (
  ...
  try {
    ...
    if (!response.ok) {
      console.warn(
        `[twenty-meeting-bot] transcript download responded with HTTP ${response.status}`,
      );
    ...
    console.warn(
      `[twenty-meeting-bot] transcript download failed: ${error instanceof Error ? error.message : String(error)}`,
    );
