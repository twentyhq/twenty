...
- Resolve every company's most recently contacted person with a single grouped query instead of one indexed lookup per company.
- Recognise a rate-limited API response from the extensions the server attaches rather than from the error message, and honour the `retryAfterMs` it reports. The throttler answers HTTP 200 with a `BAD_USER_INPUT` code and a `LIMIT_REACHED` sub-code, so the message never matched and the app was not retrying the one limit it actually hits.
