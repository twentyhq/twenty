...
describe("updateRelatedLastContact", () => {
	it("updates the company and point-of-contact opportunities for an email", async () => {
		...
		expect(
			opportunityCall?.[0].updateOpportunities.__args.filter.and[0],
		).toEqual({ pointOfContactId: { eq: PERSON_ID } });
	...
	it("only guards against newer contacts", async () => {
		...
		expect(
			opportunityCall?.[0].updateOpportunities.__args.filter.and[1],
		).toEqual(expectedGuard);
	});
