...
- One Granola connection per Twenty workspace, shared by every member. The app detects whether the saved key is a workspace or personal key, then creates and manages its signed webhook automatically.
...
There is no per-user Granola synchronisation yet. The whole Twenty workspace syncs from this one key, so every member's Call Recordings come from the notes that key can see. To cover a team, use a workspace key and enable API access on the spaces you need; a personal key syncs only its owner's notes, for everyone.
...
- The saved key powers every sync and action for the whole Twenty workspace, whether it is a workspace or personal Granola key. Choose it deliberately; members cannot connect their own Granola accounts yet.
