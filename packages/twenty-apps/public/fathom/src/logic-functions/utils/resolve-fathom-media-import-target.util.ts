...
export const resolveFathomMediaImportTarget = async ({
  ...
  if (!isNonEmptyString(mediaState.fathomRecordingImportId)) {
    return { status: 'skipped', reason: 'call recording has no Fathom import' };
  }

  const recordingId = Number(mediaState.recordingId);
  ...
  return {
    status: 'proceed',
    fathomRecordingImportId: mediaState.fathomRecordingImportId,
