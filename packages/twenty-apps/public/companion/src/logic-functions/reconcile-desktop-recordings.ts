...
export const reconcileDesktopRecordingsHandler = async () => {
  ...
  const now = new Date();
  // Media outages must not prevent completed recordings from retrying billing.
  const [media, billing, summaries] = await Promise.allSettled([
    recoverDesktopRecordings(client, now),
    recoverRecordingCharges(client, now),
    recoverRecordingSummaries(client, now),
  ]);
  if (media.status === 'rejected') throw media.reason;
  if (billing.status === 'rejected') throw billing.reason;
