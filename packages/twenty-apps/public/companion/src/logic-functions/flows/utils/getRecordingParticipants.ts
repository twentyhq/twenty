...
import { type RecordingParticipant } from 'src/logic-functions/types/RecordingParticipant';
import { type RecordingWithParticipants } from 'src/logic-functions/types/RecordingWithParticipants';
import { type RecordingViewer } from 'src/logic-functions/types/RecordingViewer';
...
const transcriptParticipants = (
  ...
  return transcript.flatMap((entry) => {
    ...
    if (
      ...
      /^(you|me|host|guest|unknown( speaker| participant)?|(speaker|participant|user)([\s_-]*\d+)?)$/i.test(
        name,
      )
    )
      return [];
