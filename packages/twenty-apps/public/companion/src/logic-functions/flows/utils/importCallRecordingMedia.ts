...
import { CALL_RECORDING_AUDIO_FIELD_UNIVERSAL_IDENTIFIER } from 'src/constants/CALL_RECORDING_AUDIO_FIELD_UNIVERSAL_IDENTIFIER';
...
import { COMPANION_MAX_MEDIA_FILE_SIZE_BYTES } from 'src/logic-functions/constants/COMPANION_MAX_MEDIA_FILE_SIZE_BYTES';
...
import { extractRecallMediaUrls } from 'src/logic-functions/recall-api/utils/extractRecallMediaUrls';
...
type CallRecordingMediaUpdateFields = Pick<
  ...
  'audio' | 'video' | 'companionFailureReason'
>;
...
export const importCallRecordingMedia = async ({
  ...
  if (!recordingResult.ok) {
    console.warn(
      `[companion] failed to fetch Recall recording ${externalRecordingId} while importing media for call recording ${callRecordingId}: ${recordingResult.errorMessage}`,
    );
  ...
  for (const descriptor of MEDIA_ARTIFACT_DESCRIPTORS) {
    ...
    const importResult = await importMediaArtifact({
      ...
      maxMediaFileSizeBytes: COMPANION_MAX_MEDIA_FILE_SIZE_BYTES,
    });
  ...
  if (tooLargeFailureReasons.length > 0) {
    updateFields.companionFailureReason = tooLargeFailureReasons.join(',');
  }
...
const importMediaArtifact = async ({
  ...
  try {
    ...
    if (download.outcome === 'too-large') {
      console.warn(
        `[companion] media-import phase=artifact-too-large callRecordingId=${callRecordingId} fileName=${fileName} sizeBytes=${download.sizeBytes} maxMediaFileSizeBytes=${maxMediaFileSizeBytes}`,
      );
    ...
    console.warn(
      `[companion] failed to import ${fileName} for call recording ${callRecordingId}: ${error instanceof Error ? error.message : String(error)}`,
    );
...
const openMediaDownload = async ({
  ...
  console.log(
    `[companion] media-import phase=artifact-download-response callRecordingId=${callRecordingId} fileName=${fileName} responseStatus=${response.status} contentLengthBytes=${contentLengthBytes ?? 'unknown'} ${formatMemoryUsageForLog()}`,
  );
...
const uploadMediaStreamToStorage = async ({
  ...
  console.log(
    `[companion] media-import phase=artifact-upload-start callRecordingId=${callRecordingId} fileName=${fileName} declaredBytes=${sizeBytes} ${formatMemoryUsageForLog()}`,
  );
...
const cancelMediaDownloadBody = async ({
  ...
  await body.cancel().catch((error) => {
    console.warn(
      `[companion] media-import phase=download-body-cancel-failed callRecordingId=${callRecordingId} fileName=${fileName}: ${error instanceof Error ? error.message : String(error)}`,
    );
  });
};
