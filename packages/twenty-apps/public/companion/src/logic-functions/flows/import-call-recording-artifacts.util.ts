...
export const importCallRecordingArtifacts = async ({
  ...
  try {
    ...
    callRecording = await findCallRecordingForArtifactsImport(client, request.callRecordingId);
    if (!callRecording) return { status: 'skipped', callRecordingId: request.callRecordingId, reason: 'recording removed' };
    const upload = await getOwnedDesktopUpload(callRecording);
    if (!upload || upload.status.code !== 'complete' || !upload.recording_id ||
        upload.recording_id !== callRecording.externalRecordingId) {
      return { status: 'skipped', callRecordingId: callRecording.id, reason: 'recording does not match the owned desktop upload' };
    }
    ...
    if (callRecording.status === 'COMPLETED') {
      await chargeCompletedCallRecording(client, { callRecordingId: callRecording.id });
    }
