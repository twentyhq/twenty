...
const findMatchingCallRecording = async ({
  ...
  if (!isUndefined(webhookEvent.callRecordingIdFromMetadata)) {
    ...
    callRecording = (await findCallRecordingsByFilter(client, {
  ...
  if (!upload ||
      (webhookEvent.externalSdkUploadId && upload.id !== webhookEvent.externalSdkUploadId) ||
      (webhookEvent.externalRecordingId && upload.recording_id !== webhookEvent.externalRecordingId)) return undefined;
