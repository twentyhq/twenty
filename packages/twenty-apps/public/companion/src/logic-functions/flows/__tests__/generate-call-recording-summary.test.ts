...
const enqueuePersistence = vi.hoisted(() => vi.fn());
vi.mock('src/logic-functions/data/enqueue-summary-persistence.util', () => ({
  enqueueSummaryPersistence: enqueuePersistence,
}));
...
describe('generateCallRecordingSummary', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    enqueuePersistence.mockResolvedValue(undefined);
    stored.clear();
  ...
  it('queues the paid text when database writes keep failing', async () => {
    saveCache
    ...
    expect(enqueuePersistence).toHaveBeenCalledWith(
      'companion-summary:call-recording-1:automatic',
