...
describe('verifyRecallWebhookSignature', () => {
  ...
  it('allows a delayed durable retry only when its original signature still verifies', () => {
    const body = JSON.stringify({ event: 'recording.done' });
    const request = {
      rawBody: body,
      secret: SECRET,
      checkTimestamp: false,
      now: new Date(Number(WEBHOOK_TIMESTAMP) * 1000 + 24 * 60 * 60 * 1000),
      headers: {
        'webhook-id': WEBHOOK_ID,
        'webhook-timestamp': WEBHOOK_TIMESTAMP,
        'webhook-signature': `v1,${sign(body)}`,
    ...
    expect(verifyRecallWebhookSignature(request)).toEqual({ valid: true });
    expect(
      verifyRecallWebhookSignature({ ...request, rawBody: '{}' }).valid,
    ).toBe(false);
  });
