...
describe('completeCallRecordingImport', () => {
  it('guards the flip with non-terminal statuses and returns true when the row is claimed', async () => {
    ...
    const mutation = vi.fn(async (mutationArg: { updateCallRecordings: { __args: { filter: unknown; data: unknown } } }) => {
    ...
    expect(capturedArgs?.data).toEqual({ status: 'COMPLETED', companionBillingState: { status: 'PENDING' } });
  });

  it('returns false when the row was already COMPLETED, without clearing its existing billing state', async () => {
