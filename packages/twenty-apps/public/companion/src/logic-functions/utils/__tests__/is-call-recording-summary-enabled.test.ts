...
describe('isCallRecordingSummaryEnabled', () => {
  beforeEach(() => {
    delete process.env[COMPANION_SUMMARY_ENABLED_ENV_VAR_NAME];
  });
  ...
  it.each(['false', '0', 'no', 'off', '  False  '])(
    'is disabled for %s',
    (value) => {
      process.env[COMPANION_SUMMARY_ENABLED_ENV_VAR_NAME] = value;
  ...
  it('falls back to the default for unrecognized values', () => {
    process.env[COMPANION_SUMMARY_ENABLED_ENV_VAR_NAME] = 'maybe';
