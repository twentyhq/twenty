...
export const createAppRuntimeMock = ({
	...
	let runAgentImplementation: (input: RunAgentInput) => RunAgentResult =
		() => ({
			success: true,
	...
	return {
		...
		get lastAgentMessages(): NonNullable<RunAgentInput["messages"]> {
			return agentRuns[agentRuns.length - 1]?.messages ?? [];
		},
