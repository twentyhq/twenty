...
- **Slack Web API** (`create-slack-api-mock.ts`) - a small stateful fake of the
...
- **App runtime services** (`create-app-runtime-mock.ts`) - `kv`,
...
- **Everything else is real**: Slack Assistant Request records are created,
  read and updated on the live server through `CoreApiClient`, and every
  request MSW does not recognize is passed through untouched.
