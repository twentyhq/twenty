...
export default defineLogicFunction({
  ...
  description:
    'Receives Slack Events API callbacks, verifies the request signature in the owner workspace, answers the url_verification handshake, and resolves the target workspace plus the function that handles the event (assistant enqueue, the channel welcome on member_joined_channel, the suggested prompts on app_home_opened, the record link unfurl on link_shared, or the team release on app_uninstalled and tokens_revoked).',
  timeoutSeconds: 15,
