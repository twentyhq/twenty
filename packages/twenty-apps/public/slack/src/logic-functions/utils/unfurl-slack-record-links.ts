...
export const unfurlSlackRecordLinks = async (
  ...
  // The card is visible to the whole channel, so it only ever shows what the
  // poster themselves may read; nothing is fetched with the app's own role.
  const posterClient = await createWorkspaceMemberCoreApiClient({
    workspaceMemberId,
  });

  if (!isDefined(posterClient)) {
    return { ok: true, skipped: 'Could not act as the poster' };
  }
