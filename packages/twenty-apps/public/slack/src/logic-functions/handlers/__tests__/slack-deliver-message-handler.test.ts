...
describe('slackDeliverMessageHandler', () => {
  ...
  it('should replay the preview scope the answer was queued with', async () => {
    sendSlackMessageMock.mockResolvedValue({ success: true });

    await slackDeliverMessageHandler({
      ...ANSWER_PAYLOAD,
      recordPreviewScope: {
    ...
    expect(sendSlackMessageMock).toHaveBeenCalledWith(
      expect.not.objectContaining({ recordPreviewScope: expect.anything() }),
      {
        waitOutRateLimit: false,
        recordPreviewScope: {
          kind: 'workspaceMember',
