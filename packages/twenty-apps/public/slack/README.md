...
**Open by default, restrict when you want.** By default the assistant answers anyone. In the app's **Settings** tab, a member with the workspace members permission can flip **Restrict to linked members** on, and the assistant then only answers Slack accounts linked to a workspace member; everyone else gets a short reply telling them to ask an admin to link them.
