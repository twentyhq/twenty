...
- **Timed-out questions.** A question the agent cannot finish inside its time budget is stopped mid-run, so nothing writes a result. A sweep runs every five minutes, moves those Slack Assistant Request records from Processing to Failed, and replies in the thread so the requester is not left waiting on an answer that is never coming.
