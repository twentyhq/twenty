...
Installation and functional tests use a disposable Twenty workspace. Configure
`TWENTY_API_URL` and `TWENTY_API_KEY` for that workspace, then run `yarn test`.
The suite checks that the list action is deployed and exercises the real handler
against an in-memory Microsoft Graph. The test setup installs and uninstalls the
application; do not use the shared QA workspace. Public apps use the monorepo CI
workflows.
