...
5. In Teams admin center > Meetings > Meeting settings > Transcript API access,
   enable **Microsoft Graph access**. Without it, Graph answers every transcript
   request with `GraphAccessToTranscriptsDisabled`.
...
1. Open **Settings > Applications > Teams Transcripts > Settings**, select
   **Add connection**, and sign in with the Microsoft work account that
   organizes your meetings.
2. Schedule a Teams meeting with that account, join it, start transcription,
   speak, and end the meeting. Microsoft publishes the transcript a few minutes
   later.
...
which defaults to the last 31 days; `nextPageUrl` continues a paginated
result.
...
[transcript permissions](https://learn.microsoft.com/en-us/graph/api/onlinemeeting-list-transcripts?view=graph-rest-1.0)
and [Teams transcript API access settings](https://learn.microsoft.com/en-us/microsoftteams/meeting-transcript-api-access).
