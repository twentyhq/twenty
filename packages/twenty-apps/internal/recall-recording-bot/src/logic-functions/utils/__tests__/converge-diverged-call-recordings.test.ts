...
import { convergeDivergedCallRecordings } from 'src/logic-functions/utils/converge-diverged-call-recordings.util';
...
describe('convergeDivergedCallRecordings', () => {
  ...
  it('logs candidates older than the lookback bound instead of converging them', async () => {
    const client = buildClient([
      buildStuckRecordingNode({ createdAt: '2026-06-01T12:00:00.000Z' }),
    ]);
