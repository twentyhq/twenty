...
const RECALL_RECORDING_BOT_RELEVANT_CALENDAR_EVENT_FIELDS = [
  'title',
...
const RECALL_RECORDING_BOT_KEY_CALENDAR_EVENT_FIELDS = [
  'conferenceLink',
  'startsAt',
  'iCalUid',
];
...
const hasRelevantFieldChange = (updatedFields: string[]): boolean =>
  updatedFields.some((updatedField) =>
    RECALL_RECORDING_BOT_RELEVANT_CALENDAR_EVENT_FIELDS.includes(updatedField),
  );

const hasKeyFieldChange = (updatedFields: string[]): boolean =>
  updatedFields.some((updatedField) =>
    RECALL_RECORDING_BOT_KEY_CALENDAR_EVENT_FIELDS.includes(updatedField),
  );
...
export default defineLogicFunction({
  ...
  name: 'reconcile-recall-recording-bot-calendar-event',
  description:
