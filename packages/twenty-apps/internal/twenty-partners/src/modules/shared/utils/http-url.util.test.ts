...
import { isHttpUrl, optionalHttpUrl } from "./http-url.util";
...
describe("optionalHttpUrl", () => {
	...
	it("passes a valid https url through", () => {
		expect(optionalHttpUrl.parse("https://example.com")).toBe(
			"https://example.com",
		);
	});

	it("passes a valid http url through", () => {
		expect(optionalHttpUrl.parse("http://example.com")).toBe(
			"http://example.com",
		);
	});
