...
async function findOrCreateCompanyId(
	...
	const name = isNonEmptyString(company?.name)
		? company.name.trim()
		: undefined;
	const domain = isNonEmptyString(company?.domain)
		? company.domain.trim()
		: undefined;
...
async function findOrCreatePersonId(
	...
	const lastName = isNonEmptyString(pointOfContact?.lastName)
		...
		: "";
	if (email === undefined && firstName === "" && lastName === "")
		return undefined;
	...
	const personData: CoreSchema.PersonCreateInput = {
...
export async function importOpportunityFromTft(
	...
	try {
		...
		if (existingId !== undefined)
			return { ok: true, created: false, id: existingId };
