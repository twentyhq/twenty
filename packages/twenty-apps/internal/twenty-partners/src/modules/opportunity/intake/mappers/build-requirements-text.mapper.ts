...
export function buildRequirementsText(
	...
	const base = isNonEmptyString(input.requirements)
	...
	if (isNonEmptyString(input.seatCount))
		bullets.push(`• Seats: ${input.seatCount.trim()}`);
	if (isNonEmptyString(input.country))
		bullets.push(`• Country: ${input.country.trim()}`);
	...
	if (isNonEmptyString(input.timeline))
		bullets.push(`• Timeline: ${input.timeline.trim()}`);
	if (isNonEmptyString(input.budgetRange))
		bullets.push(`• Budget: ${input.budgetRange.trim()}`);
