...
const fieldNamed = (embed: Record<string, unknown>, name: string) =>
	(embed.fields as { name: string; value: string }[]).find(
		(f) => f.name === name,
	);
...
describe("buildBriefEmbed", () => {
	it("puts the need in the description and links the record", () => {
		...
		expect(embed.url).toBe(
			"https://partners.twenty.com/object/opportunity/opp-1",
		);
	});
		...
		const embed = buildBriefEmbed(
			{
				opportunityId: "opp-1",
				input,
				referringPartner: { id: "p-9", name: "Acme Consulting" },
			},
			"https://partners.twenty.com",
		);
		...
		const embed = buildBriefEmbed(
			{
				...
				input: {
					...input,
					country: "c".repeat(2000),
					companyName: "x".repeat(2000),
				},
	...
	it("pads each inline row to three columns so rows never reflow", () => {
		const embed = buildBriefEmbed(
			{
				opportunityId: "opp-1",
				input: { ...input, seatCount: "40" },
				referringPartner: null,
			},
			"https://partners.twenty.com",
		);
		const inlineRun = (embed.fields as { inline?: boolean }[]).filter(
			(f) => f.inline === true,
		);
