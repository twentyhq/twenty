...
export type LinkPartnerUserResult =
	...
	| {
			linked: false;
			reason: "already_linked_same" | "partner_already_linked_other";
	  };

const collectIds = (
	edges: ({ node?: { id?: string | null } | null } | null)[] | null | undefined,
): string[] =>
	(edges ?? [])
		.map((e) => e?.node?.id)
...
export async function linkPartnerUser(
	...
	if (companyId) {
		const companyOwner = (await getCompanyPartnerUser(client, companyId))
			.company?.partnerUserId;
	...
	const results = await Promise.allSettled([
		...
		...applicationIds.map((id) =>
			updateApplicationPartnerUser(client, id, memberId),
		),
		...partnerLinkIds.map((id) =>
			updatePartnerLinkPartnerUser(client, id, memberId),
		),
		...partnerServiceIds.map((id) =>
			updatePartnerServicePartnerUser(client, id, memberId),
		),
		...partnerContentIds.map((id) =>
			updatePartnerContentPartnerUser(client, id, memberId),
		),
		...companyWrites,
	]);
	...
	const claimant = (await getPartnerOwner(client, partnerId)).partner
		?.partnerUserId;
	...
	await updatePartnerPartnerUser(
		client,
		partnerId,
		memberId,
