...
describe("linkPartnerUser", () => {
		...
		query.mockImplementation((q: Record<string, unknown>) => {
			...
			if ("company" in q)
				return Promise.resolve({
					company: {
						id: "company-1",
						partnerUserId: opts.companyOwner ?? null,
					},
				});
			if ("partner" in q)
				return Promise.resolve({
					partner: {
						id: "partner-1",
						partnerUserId: opts.ownerRecheck ?? null,
					},
				});
			return Promise.resolve({});
		});
	...
	it("stamps the partner and cascades to persons, company, applications, links, services and content", async () => {
		routeQueries({
			cascade: {
				partner: {
					...
					persons: {
						edges: [
							{ node: { id: "person-1", partnerUserId: null } },
							{ node: { id: "person-2", partnerUserId: null } },
						],
					},
				},
		...
		const result = await linkPartnerUser(client, {
			partnerId: "partner-1",
			memberId: "member-1",
		});
		...
		expect(mutation).toHaveBeenCalledWith({
			updatePerson: {
				__args: { id: "person-1", data: { partnerUserId: "member-1" } },
				id: true,
			},
		});
		expect(mutation).toHaveBeenCalledWith({
			updatePerson: {
				__args: { id: "person-2", data: { partnerUserId: "member-1" } },
				id: true,
			},
		});
		expect(mutation).toHaveBeenCalledWith({
			updateApplication: {
				__args: { id: "app-1", data: { partnerUserId: "member-1" } },
				id: true,
			},
		});
		expect(mutation).toHaveBeenCalledWith({
		...
		expect(mutation).toHaveBeenCalledWith({
			updatePartnerLink: {
				__args: { id: "link-1", data: { partnerUserId: "member-1" } },
				id: true,
			},
		});
		expect(mutation).toHaveBeenCalledWith({
		...
		expect(mutation).toHaveBeenCalledWith({
			updatePartnerContent: {
				__args: { id: "content-1", data: { partnerUserId: "member-1" } },
				id: true,
			},
		});
		expect(mutation).toHaveBeenCalledWith({
			updatePartner: {
				__args: {
					...
					data: {
						...
						partnerUserLinkedAt: expect.any(String),
					},
				},
				id: true,
			},
	...
	it("skips persons already linked to a member", async () => {
		routeQueries({
			cascade: {
				partner: {
					...
					persons: {
						edges: [
							{ node: { id: "person-1", partnerUserId: "member-7" } },
							{ node: { id: "person-2", partnerUserId: null } },
						],
					},
				},
		...
		expect(mutation).not.toHaveBeenCalledWith(
			expect.objectContaining({
				updatePerson: expect.objectContaining({
					__args: expect.objectContaining({ id: "person-1" }),
				}),
			}),
		);
	});
	...
	it("does not steal a company already owned by a different member", async () => {
		routeQueries({
			...
			cascade: {
				partner: {
					...
					partnerUserId: null,
					persons: {
						edges: [{ node: { id: "person-1", partnerUserId: null } }],
					},
				},
				applications: { edges: [] },
				partnerLinks: { edges: [] },
		...
		expect(mutation).not.toHaveBeenCalledWith(
			expect.objectContaining({ updateCompany: expect.anything() }),
		);
		expect(mutation).toHaveBeenCalledWith(
			expect.objectContaining({ updatePartner: expect.anything() }),
		);
	});
	...
	it("aborts before stamping when another member claimed the partner during the cascade", async () => {
		...
		expect(result).toEqual({
			linked: false,
			reason: "partner_already_linked_other",
		});
	...
	it("throws when a cascade write fails (retry semantics)", async () => {
		...
		await expect(
			linkPartnerUser(client, { partnerId: "partner-1", memberId: "member-1" }),
		).rejects.toThrow(/cascade write/);
		expect(mutation).not.toHaveBeenCalledWith(
			expect.objectContaining({ updatePartner: expect.anything() }),
		);
	});
