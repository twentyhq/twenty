...
vi.mock("./services/resolve-partner-by-email.service", () => ({
	resolvePartnerByEmail: vi.fn(),
}));
vi.mock("./services/link-partner-user.service", () => ({
	linkPartnerUser: vi.fn(),
}));
...
describe("on-workspace-member-created handler", () => {
	...
	it("skips when partner is already claimed by another member", async () => {
		...
		vi.mocked(linkPartnerUser).mockResolvedValue({
			linked: false,
	...
	it("links the member on a fresh match", async () => {
		...
		const res = await handler(makePayload("A@Acme.com"));
		expect(resolvePartnerByEmail).toHaveBeenCalledWith(
			expect.anything(),
			"a@acme.com",
		);
		expect(linkPartnerUser).toHaveBeenCalledWith(expect.anything(), {
			partnerId: "partner-1",
			memberId: "member-1",
		});
