...
export function validateProfileOptionValues(
	...
	if (input.languagesSpoken !== undefined) {
		const unknown = input.languagesSpoken.find(
			(value) => !LANGUAGE_VALUES.has(value),
		);
		if (unknown !== undefined) return { error: `Unknown language: ${unknown}` };
	}
	if (input.partnerScope !== undefined) {
		const unknown = input.partnerScope.find(
			(value) => !PARTNER_SCOPE_VALUES.has(value),
		);
		if (unknown !== undefined)
			return { error: `Unknown partner scope: ${unknown}` };
	}
...
export function buildPartnerUpdateData(
	...
	if (input.country !== undefined) {
		data.country =
			input.country === null
				? null
				: (input.country as CoreSchema.PartnerCountryEnum);
	}
	if (input.languagesSpoken !== undefined) {
	...
	if (input.region !== undefined) {
		data.region = input.region.map(
			(value) => value as CoreSchema.PartnerRegionEnum,
		);
	}
	...
	if (input.hourlyRate !== undefined) {
		data.hourlyRate =
			input.hourlyRate === null ? null : { ...input.hourlyRate };
	}
	...
	if (input.website !== undefined) {
		data.website =
			input.website === null ? null : { primaryLinkUrl: input.website };
	}
	if (input.linkedin !== undefined) {
		data.linkedin =
			input.linkedin === null ? null : { primaryLinkUrl: input.linkedin };
	}
	if (input.calendarLink !== undefined) {
		data.calendarLink =
			input.calendarLink === null
				? null
				: { primaryLinkUrl: input.calendarLink };
	}
