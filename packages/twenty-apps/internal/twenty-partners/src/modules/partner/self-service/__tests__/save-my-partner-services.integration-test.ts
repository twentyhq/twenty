...
import { handler } from "src/modules/partner/self-service/save-my-partner-services.logic-function";
...
describe("save-my-partner-services handler", () => {
	...
	beforeAll(async () => {
		...
		const partnerCreated = await client.mutation({
			createPartner: {
				__args: {
					data: {
						name: "Services Integration Test Partner",
		...
		otherServiceId = requireId(
			otherService.createPartnerService?.id,
			"createPartnerService",
		);
	});
	...
	afterAll(async () => {
		...
		await client.mutation({
			destroyPartnerService: { __args: { id: keepId }, id: true },
		});
		await client.mutation({
			destroyPartnerService: { __args: { id: otherServiceId }, id: true },
		});
		await client.mutation({
	...
	it("keeps+edits one, creates one, and drops the omitted one", async () => {
		const result = await handler({
			...
			requestContext: {
				http: { method: "POST", path: "/save-my-partner-services" },
			},
			userWorkspaceId,
		});
		...
		expect(result.services.some((service) => service.id === dropId)).toBe(
			false,
		);
		expect(
			result.services.some((service) => service.title === "Brand new service"),
		).toBe(true);
	});
