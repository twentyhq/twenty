...
import { MY_CASE_STUDIES_FRONT_COMPONENT_ID } from "src/modules/partner/self-service/constants/my-case-studies.constants";
...
type LoadResult =
	| { ok: true; profile: MyProfilePayload }
...
const MyCaseStudies = () => {
		...
		try {
			...
			if (res.ok) {
				...
				await enqueueSnackbar({
					message: `Could not load case studies: ${res.reason}`,
					variant: "error",
				});
			}
		} catch (error) {
	...
	const handleDelete = async (key: string) => {
		...
		const confirmed = await openCommandConfirmationModal({
			...
			subtitle:
				"It will be removed from your public profile. This cannot be undone.",
		...
		<div style={pageStyle}>
			...
			{loading ? (
				...
				<div style={emptyStyle}>
					No case studies yet. Add one to show clients the work you have
					delivered.
				</div>
			) : (
				<div>
...
export default defineFrontComponent({
	...
	description:
		"Self-service page for a partner to create and edit their case studies.",
