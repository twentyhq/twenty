...
import { handler } from "src/modules/partner/self-service/save-my-partner-content.logic-function";
...
describe("save-my-partner-content handler", () => {
	...
	beforeAll(async () => {
		...
		if (!member?.userId) {
			throw new Error(
				"No workspace member with a linked userId found to drive this test.",
			);
		}
		...
		const partnerCreated = await client.mutation({
			createPartner: {
				__args: {
					data: {
						name: "Content Integration Test Partner",
		...
		otherPartnerId = requireId(
			otherPartnerCreated.createPartner?.id,
			"createPartner",
		);
		...
		otherContentId = requireId(
			otherContent.createPartnerContent?.id,
			"createPartnerContent",
		);
	});
	...
	afterAll(async () => {
		...
		await client.mutation({
			destroyPartnerContent: { __args: { id: keepId }, id: true },
		});
		await client.mutation({
			destroyPartnerContent: { __args: { id: otherContentId }, id: true },
		});
		...
		if (dropId) {
			await client
				.mutation({
					destroyPartnerContent: { __args: { id: dropId }, id: true },
				})
				.catch(() => {});
		}
		await client.mutation({
			destroyPartner: { __args: { id: partnerId }, id: true },
		});
		await client.mutation({
			destroyPartner: { __args: { id: otherPartnerId }, id: true },
		});
	});
	...
	it("keeps+edits one (status untouched), creates one (WIP/CASE_STUDY), drops the omitted one", async () => {
		const result = await handler({
			...
			requestContext: {
				http: { method: "POST", path: "/save-my-partner-content" },
			},
			userWorkspaceId,
		});
		...
		const created = result.caseStudies.find(
			(row) => row.name === "Brand new case study",
		);
