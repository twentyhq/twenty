...
const makeRouteEvent = (
	...
	requestContext: {
		http: { method: "POST", path: "/save-my-partner-profile" },
	},
	userWorkspaceId,
});
...
describe("save-my-partner-profile", () => {
	...
	afterEach(async () => {
		...
		for (const id of createdPartnerIds.splice(0))
			await destroyPartner(client, id);
	});
	...
	it("rejects an invalid body before writing anything", async () => {
		...
		const result = await handler(
			makeRouteEvent(userWorkspaceId, { website: "not-a-url" }),
		);
	...
	it("rejects an unknown enum value without writing anything", async () => {
		...
		const result = await handler(
			makeRouteEvent(userWorkspaceId, { country: "NOPE" }),
		);
	...
	it("writes only the editable fields provided in the body", async () => {
		...
		expect(partner.hourlyRate).toEqual({
