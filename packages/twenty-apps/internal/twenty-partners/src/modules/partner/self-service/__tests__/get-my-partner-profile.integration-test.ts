...
async function destroyPartnerLink(client: CoreApiClient, id: string) {
	await client
		.mutation({ destroyPartnerLink: { __args: { id }, id: true } })
		.catch(() => {});
}
...
async function destroyPartnerService(client: CoreApiClient, id: string) {
	await client
		.mutation({ destroyPartnerService: { __args: { id }, id: true } })
		.catch(() => {});
}
...
async function destroyPartnerContent(client: CoreApiClient, id: string) {
	await client
		.mutation({ destroyPartnerContent: { __args: { id }, id: true } })
		.catch(() => {});
}
...
describe("get-my-partner-profile", () => {
	...
	afterEach(async () => {
		...
		for (const id of createdLinkIds.splice(0))
			await destroyPartnerLink(client, id);
		for (const id of createdServiceIds.splice(0))
			await destroyPartnerService(client, id);
		for (const id of createdContentIds.splice(0))
			await destroyPartnerContent(client, id);
	...
	it("returns the calling partner's profile with links, services, and case studies", async () => {
		...
		const contentId = await createPartnerContent(
			client,
			partnerId,
			member.id,
			caseStudyName,
