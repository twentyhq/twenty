...
import {
	mapMyProfilePayload,
	type PartnerNode,
} from "./get-my-partner-profile.mapper";
...
describe("mapMyProfilePayload", () => {
	it("unwraps LINKS fields to their primaryLinkUrl", () => {
		...
		expect(mapped.caseStudies[0].caseStudyLink).toBe(
	...
	it("prefers the uploaded profile picture file url over the legacy link", () => {
		...
		expect(mapped.profilePictureUrl).toBe(
			"https://images.example.com/uploaded.png",
		);
	});

	it("falls back to the legacy profile picture link when there is no uploaded file", () => {
		const mapped = mapMyProfilePayload(makeNode({ profilePictureFile: null }));
	...
	it("maps case study body.markdown to bodyMarkdown and passes coverImageUrl through", () => {
		...
		expect(mapped.caseStudies[0].bodyMarkdown).toBe(
			"Moved 12 teams to Twenty.",
		);
					...
					edges: [
						{
							node: {
								...
								coverImageUrl: "https://images.example.com/case-study.png",
								caseStudyLink: {
									primaryLinkUrl: "https://example.com/case-study",
								},
								status: "APPROVED",
								contentType: ["CASE_STUDY"],
							},
