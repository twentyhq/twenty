...
describe("case-study-rows", () => {
	...
	it("treats an untouched new draft as blank, but not a filled or persisted row", () => {
		...
		expect(isBlankDraft(newDraftRow())).toBe(true);
		expect(
			isBlankDraft(persisted({ id: undefined, key: "draft-x", headline: "x" })),
		).toBe(false);
	...
	it("builds the save body: drops blank drafts, keeps id for persisted, carries published + derived name", () => {
		const rows = [
			persisted(),
			newDraftRow(),
			{ ...newDraftRow(), headline: "Fresh", published: true },
		];
		const body = toSaveBody(rows) as {
			caseStudies: Array<Record<string, unknown>>;
		};
		expect(body.caseStudies).toHaveLength(2);
		expect(body.caseStudies[0]).toMatchObject({
			id: "r1",
			name: "Migration",
			clientName: "Acme",
			bodyMarkdown: "## Hi",
		...
		expect(body.caseStudies[1]).toMatchObject({
			name: "Fresh",
			published: true,
		});
	...
	it("carries coverImageUrl into the save body and reads it from the load payload", () => {
		const rows = buildInitialRows([
			{
				...
				headline: "H",
				bodyMarkdown: "B",
				coverImageUrl: "https://img.example.com/a.png",
				caseStudyLink: "https://y",
				status: "APPROVED",
			},
		]);
		...
		const body = toSaveBody([
			persisted({ coverImageUrl: "https://img.example.com/c.png" }),
		]) as {
			caseStudies: Array<Record<string, unknown>>;
		};
		expect(body.caseStudies[0].coverImageUrl).toBe(
	...
	it("a new draft with only a cover URL set is not treated as blank", () => {
		expect(
			isBlankDraft({
				...newDraftRow(),
				coverImageUrl: "https://img.example.com/x.png",
			}),
		).toBe(false);
	});
});
