	...
	beforeAll(async () => {
		...
		const partnerCreated = await client.mutation({
			createPartner: {
				__args: {
					data: {
						name: "Submit-For-Review Integration Test Partner",
		...
		approvedId = requireId(
			approved.createPartnerContent?.id,
			"createPartnerContent",
		);
		...
		otherWipId = requireId(
			otherWip.createPartnerContent?.id,
			"createPartnerContent",
		);
	});
		...
		for (const id of [wipId, approvedId, otherWipId]) {
			if (id)
				await client.mutation({
					destroyPartnerContent: { __args: { id }, id: true },
				});
		}
