...
describe("save-my-partner-links handler", () => {
	...
	beforeAll(async () => {
		...
		const partnerCreated = await client.mutation({
			createPartner: {
				__args: {
					data: {
						name: "Links Integration Test Partner",
		...
		otherLinkId = requireId(
			otherLink.createPartnerLink?.id,
			"createPartnerLink",
		);
	});
	...
	afterAll(async () => {
		...
		await client.mutation({
			destroyPartnerLink: { __args: { id: keepId }, id: true },
		});
		await client.mutation({
			destroyPartnerLink: { __args: { id: otherLinkId }, id: true },
		});
		await client.mutation({
	...
	it("keeps+edits one, creates one, and drops the omitted one", async () => {
		const result = await handler({
			...
			requestContext: {
				http: { method: "POST", path: "/save-my-partner-links" },
			},
			userWorkspaceId,
		});
		...
		expect(result.links.some((link) => link.name === "Brand new link")).toBe(
			true,
		);
	});
