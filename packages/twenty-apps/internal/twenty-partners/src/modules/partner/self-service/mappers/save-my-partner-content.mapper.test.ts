...
import {
	...
	saveContentSchema,
} from "./save-my-partner-content.mapper";
import { canSubmitForReview } from "./submit-partner-content-for-review.mapper";

describe("saveContentSchema", () => {
	it("accepts a minimal case study (name only)", () => {
		expect(
			saveContentSchema.safeParse({ caseStudies: [{ name: "Acme rollout" }] })
				.success,
		).toBe(true);
	});
	...
	it("rejects a case study missing name", () => {
		expect(
			saveContentSchema.safeParse({ caseStudies: [{ clientName: "Acme" }] })
				.success,
		).toBe(false);
	});
});
...
describe("buildContentCreateData", () => {
	...
	it("wraps bodyMarkdown into { markdown } (empty string when omitted)", () => {
		expect(buildContentCreateData({ name: "x" }).body).toEqual({
			markdown: "",
		});
		expect(
			buildContentCreateData({ name: "x", bodyMarkdown: "Hello **world**" })
				.body,
	...
	it("wraps caseStudyLink into { primaryLinkUrl } when present", () => {
		...
		expect(data.caseStudyLink).toEqual({
	...
	it("passes coverImageUrl through when provided, and omits it when absent", () => {
		expect(
			buildContentCreateData({
				name: "x",
				coverImageUrl: "https://img.example.com/c.png",
			}).coverImageUrl,
		).toBe("https://img.example.com/c.png");
...
describe("buildContentUpdateData", () => {
	...
	it("wraps bodyMarkdown and caseStudyLink the same way as create", () => {
		...
		expect(data.caseStudyLink).toEqual({
			primaryLinkUrl: "https://example.com/updated",
		});
	});
...
describe("partner-controlled publish", () => {
	...
	it("creates a draft (published false or omitted) as WIP", () => {
		expect(buildContentCreateData({ name: "x", published: false }).status).toBe(
			"WIP",
		);
	...
	it("updates status from the published flag", () => {
		expect(buildContentUpdateData({ name: "x", published: true }).status).toBe(
			"APPROVED",
		);
		expect(buildContentUpdateData({ name: "x", published: false }).status).toBe(
			"WIP",
		);
	});
