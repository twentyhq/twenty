...
const MyProfile = () => {
		...
		try {
			...
			if (res.ok) {
				...
				await enqueueSnackbar({
					message: `Could not load profile: ${res.reason}`,
					variant: "error",
				});
			}
		} catch (error) {
	...
	const save = useCallback(async () => {
		...
		try {
			...
			if (res.ok) {
				...
				await enqueueSnackbar({
					message: `Save failed: ${res.reason}`,
					variant: "error",
				});
			}
		} catch (error) {
	...
	if (!form || !options) {
		return (
			<div style={styles.root}>No partner profile found for your account.</div>
		);
	}
...
export default defineFrontComponent({
	...
	description:
		"Self-service page for a partner to view and edit their profile.",
	component: MyProfile,
});
