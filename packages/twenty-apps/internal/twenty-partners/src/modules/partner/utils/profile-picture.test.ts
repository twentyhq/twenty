...
describe("firstFileUrl", () => {
	it("returns the first file url", () => {
		expect(
			firstFileUrl([{ url: "https://x/a.png" }, { url: "https://x/b.png" }]),
		).toBe("https://x/a.png");
	});
	it("skips entries without a url", () => {
		expect(firstFileUrl([{ url: null }, { url: "https://x/b.png" }])).toBe(
			"https://x/b.png",
		);
	});
...
describe("resolvePartnerPictureUrl", () => {
	...
	it("falls back to the legacy link when there is no file", () => {
		expect(resolvePartnerPictureUrl(null, "https://x/legacy.png")).toBe(
			"https://x/legacy.png",
		);
		expect(resolvePartnerPictureUrl([], "https://x/legacy.png")).toBe(
			"https://x/legacy.png",
		);
		expect(
			resolvePartnerPictureUrl([{ url: null }], "https://x/legacy.png"),
		).toBe("https://x/legacy.png");
	});
