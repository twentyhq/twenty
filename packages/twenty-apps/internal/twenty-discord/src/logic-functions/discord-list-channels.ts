...
export default defineLogicFunction({
  ...
  description:
    'List the text-postable channels (types `GUILD_TEXT` and `GUILD_ANNOUNCEMENT`) in a single Discord server, sorted by Discord position. Each entry exposes `id`, `name`, `type`, `parentId` (category), and `position`. Discord bot tokens are global, so the listing is scoped per-guild — pass the server (guild) ID. Voice channels, threads, forums, and categories are excluded.',
  timeoutSeconds: 30,
  toolTriggerSettings: {
