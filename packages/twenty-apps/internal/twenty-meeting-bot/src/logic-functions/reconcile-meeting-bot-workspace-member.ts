...
const MEETING_BOT_RELEVANT_WORKSPACE_MEMBER_FIELDS = [
...
export const reconcileMeetingBotWorkspaceMemberHandler = async (
...
const isRelevantWorkspaceMemberChange = ({
  ...
  if (action === 'updated') {
    ...
    return updatedFields.some((updatedField) =>
      MEETING_BOT_RELEVANT_WORKSPACE_MEMBER_FIELDS.includes(updatedField),
    );
  }
...
export default defineLogicFunction({
  ...
  name: 'reconcile-meeting-bot-workspace-member',
  description:
  ...
  timeoutSeconds: 60,
  handler: reconcileMeetingBotWorkspaceMemberHandler,
