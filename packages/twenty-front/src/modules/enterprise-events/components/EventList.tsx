...
const STATUS_COLORS: Record<EventStatus, string> = {
  upcoming: themeCssVariables.color.blue, live: themeCssVariables.color.turquoise,
  completed: themeCssVariables.color.gray50, cancelled: themeCssVariables.color.red,
};
...
const StyledBarFill = styled.div<{ percent: number }>`
  height: 100%; width: ${({ percent }) => percent}%; background: ${themeCssVariables.color.blue}; border-radius: 3px;
`;
...
export const EventList = () => {
  ...
  const events: EventData[] = data?.eventsData ?? [];
        ...
        {events.map((event) => {
          ...
          return (
            <StyledCard key={event.id}>
              <StyledBadge color={STATUS_COLORS[event.status] ?? themeCssVariables.color.gray50}>{event.status}</StyledBadge>
              ...
              <StyledBar><StyledBarFill percent={fillPercent} /></StyledBar>
            </StyledCard>
          );
        })}
      </StyledGrid>
