...
import { type WorkflowCreateOrUpdateRecordAction } from '@/workflow/types/Workflow';
...
type WorkflowEditActionCreateOrUpdateRecordProps = {
  action: WorkflowCreateOrUpdateRecordAction;
  actionOptions:
    | {
        readonly: true;
      }
    | {
        readonly?: false;
        onActionUpdate: (action: WorkflowCreateOrUpdateRecordAction) => void;
      };
};
...
type CreateOrUpdateRecordFormData = {
  objectName: string;
  matchFields: string[];
...
export const WorkflowEditActionCreateOrUpdateRecord = ({
  action,
  actionOptions,
}: WorkflowEditActionCreateOrUpdateRecordProps) => {
  ...
  const [formData, setFormData] = useState<CreateOrUpdateRecordFormData>(() => {
    const baseFormData: CreateOrUpdateRecordFormData = {
      objectName: action.settings.input.objectName,
      matchFields: action.settings.input.upsertCriteria.matchFields || [],
      ...(action.settings.input.fieldsToUpdate && { fieldsToUpdate: action.settings.input.fieldsToUpdate }),
  ...
  const handleFieldChange = (
    fieldName: keyof CreateOrUpdateRecordFormData,
    updatedValue: JsonValue,
  ) => {
    if (fieldName === 'matchFields' || fieldName === 'fieldsToUpdate') {
      const newFormData: CreateOrUpdateRecordFormData = {
        ...formData,
  ...
  const saveAction = useDebouncedCallback(
    async (formData: CreateOrUpdateRecordFormData) => {
        ...
        <Select
          ...
          onChange={(updatedObjectName) => {
            const newFormData: CreateOrUpdateRecordFormData = {
              objectName: updatedObjectName,
              matchFields: [],
              ...(formData.fieldsToUpdate !== undefined && { fieldsToUpdate: [] }),
            };
