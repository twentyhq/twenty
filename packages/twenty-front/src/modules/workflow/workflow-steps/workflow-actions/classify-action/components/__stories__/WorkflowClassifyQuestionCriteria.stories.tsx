  ...
  render: function Render(args) {
    ...
    return (
      <>
        ...
        <Button
          onClick={() =>
            setCriteria([{ id: 'lawyer', name: 'Accountant', description: '' }])
          }
        >
          Reload criteria
        </Button>
...
export const ReloadsExistingRow: Story = {
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement);
    await canvas.findByText('Lawyer', { selector: 'p' });
    await userEvent.click(
      await canvas.findByRole('button', { name: 'Reload criteria' }),
    );
    await expect(
      await canvas.findByText('Accountant', { selector: 'p' }),
    ).toBeVisible();
    await expect(
