...
type WorkflowDiagramIteratorLoopEmptyTriggerEdgeEditableProps =
...
const StyledTopLabelContainer = styled.div<{
  ...
  transform: translate(0%, -50%)
    translate(
      ${({ sourceX, sourceY }) => `${Math.floor(sourceX)}px, ${sourceY}px`}
    )
    translateX(10px);
  width: fit-content;
`;

export const WorkflowDiagramIteratorLoopEmptyTriggerEdgeEditable = ({
  source,
  target,
  sourceX,
  sourceY,
  ...
  markerStart,
  markerEnd,
}: WorkflowDiagramIteratorLoopEmptyTriggerEdgeEditableProps) => {
    ...
    <>
      ...
      <EdgeLabelRenderer>
        <StyledTopLabelContainer sourceX={sourceX} sourceY={sourceY}>
          <WorkflowDiagramEdgeLabel label={t`loop`} />
