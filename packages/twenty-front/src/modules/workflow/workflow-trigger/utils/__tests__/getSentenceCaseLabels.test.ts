...
const labels = [
  ...
  getTriggerStepName({
    type: 'MANUAL',
    settings: { objectType: 'company', outputSchema: {}, icon: 'IconHandMove' },
  }),
  getTriggerStepName({
    type: 'WEBHOOK',
    settings: { httpMethod: 'GET', authentication: null, outputSchema: {} },
  }),
...
describe('All UI labels use sentence case', () => {
  it.each(labels)(
    '"%s" should only have the first letter uppercase',
    (label) => {
