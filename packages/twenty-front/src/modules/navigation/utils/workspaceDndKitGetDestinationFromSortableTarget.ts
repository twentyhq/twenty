...
import type { SortableTargetDestination } from '@/navigation/utils/workspaceDndKitSortableTargetDestination';
...
export const getDestinationFromSortableTarget = (
  ...
  if (!isDefined(group) || !isDefined(index)) return null;
  ...
  const dropTargetId = getDndKitDropTargetId(destDroppableId, index as number);
  ...
  const destination: DropDestination = {
    droppableId: isTargetFolder
      ? `${NavigationMenuItemDroppableIds.WORKSPACE_FOLDER_HEADER_PREFIX}${target.id}`
      : destDroppableId,
    index: isTargetFolder ? 0 : (index as number),
  };
  return {
    destination,
