...
export const useMapRecordRelationConnectionsToRelationRecords = () => {
  const objectMetadataItems = useRecoilValue(objectMetadataItemsState);

  const mapConnectionToRecords = useMapConnectionToRecords();

  const mapRecordRelationConnectionsToRelationRecords = useCallback(
      ...
      return produce(objectRecord, (objectRecordDraft) => {
        for (const relationField of relationFields) {
          ...
          const relatedObjectMetadataSingularName =
            ...
            relationField.fromRelationMetadata?.toObjectMetadata.nameSingular ??
            null;
          ...
          const relationConnection = objectRecordDraft?.[relationField.name] as
    ...
    [objectMetadataItems, mapConnectionToRecords],
  );

  return { mapRecordRelationConnectionsToRelationRecords };
};
