...
export const useMergePreview = ({
  ...
  const mergeInProgress = useRecoilValue(mergeInProgressState);
  ...
  useEffect(() => {
    const fetchPreview = async () => {
      if (selectedRecords.length < 2 || mergeInProgress) return;
    ...
    if (selectedRecords.length > 0 && !mergeInProgress) {
      fetchPreview();
    }
