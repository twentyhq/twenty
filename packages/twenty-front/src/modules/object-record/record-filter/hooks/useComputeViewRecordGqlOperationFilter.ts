...
const useComputeFilterRecordGqlOperationFilter = () => {
  ...
  const computeFilterRecordGqlOperationFilter = (
    ...
    switch (filter.definition.type) {
      ...
      case 'DATE':
      case 'DATE_TIME': {
        const resolvedFilterValue = resolveFilterValue(
      ...
      case 'RELATION': {
        if (!isEmptyOperand) {
          const recordIds = resolveFilterValue(
            filter.definition.type,
            filter.value,
            filter.operand,
          );
  ...
  return { computeFilterRecordGqlOperationFilter };
};

const useComputeViewFilterGroupRecordGqlOperationFilter = () => {
  const { computeFilterRecordGqlOperationFilter } =
    useComputeFilterRecordGqlOperationFilter();
  ...
  return { computeViewFilterGroupRecordGqlOperationFilter };
};

export const useComputeViewRecordGqlOperationFilter = () => {
  const { computeFilterRecordGqlOperationFilter } =
    useComputeFilterRecordGqlOperationFilter();
  const { computeViewFilterGroupRecordGqlOperationFilter } =
    useComputeViewFilterGroupRecordGqlOperationFilter();
