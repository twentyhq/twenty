...
describe('buildExcludedRecipientReasons', () => {
  it('returns nothing when no recipient was excluded', () => {
    expect(buildExcludedRecipientReasons(NO_EXCLUSIONS, formatNumber)).toEqual(
      [],
    );
  });
  ...
  it('names only the reasons that actually excluded someone', () => {
    expect(
      buildExcludedRecipientReasons(
        { ...NO_EXCLUSIONS, hardSuppressed: 3 },
        formatNumber,
      ),
  ...
  it('pluralises the duplicate count', () => {
    expect(
      buildExcludedRecipientReasons(
        { ...NO_EXCLUSIONS, duplicateEmails: 1 },
        formatNumber,
      ),
    ).toEqual(['1 duplicate']);
    expect(
      buildExcludedRecipientReasons(
        { ...NO_EXCLUSIONS, duplicateEmails: 2 },
        formatNumber,
      ),
