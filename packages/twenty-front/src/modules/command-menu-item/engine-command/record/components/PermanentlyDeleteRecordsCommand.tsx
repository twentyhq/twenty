...
export const PermanentlyDeleteRecordsCommand = () => {
  ...
  if (!isDefined(recordIndexId) || !isDefined(objectMetadataItem)) {
    throw new Error(
      'Record index ID and object metadata are required to permanently delete records',
    );
  }
  ...
  const handleExecute = async () => {
    ...
    if (!isDefined(graphqlFilter)) {
      throw new Error(
        'Cannot permanently delete records without a valid filter',
      );
    }
  ...
  const title = t`Permanently delete ${objectLabel}`;
  ...
  const confirmButtonText = `${t`Permanently delete`} ${objectLabel}`;
