...
describe('getUpdatedTabLayouts', () => {
  ...
  it('should treat missing breakpoint layouts as empty', () => {
    const layoutsWithMissingBreakpoints: TabLayouts = {
      'tab-1': {},
    };

    const result = getUpdatedTabLayouts(
      layoutsWithMissingBreakpoints,
      'tab-1',
      newLayout,
    );

    expect(result['tab-1'].desktop).toEqual([newLayout]);
    expect(result['tab-1'].mobile).toEqual([{ ...newLayout, w: 1, x: 0 }]);
  });
});
