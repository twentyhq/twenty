...
describe('formatUpgradeCommandName', () => {
  it('should format fast instance command with timestamp before version', () => {
    ...
    ).toBe('DropWorkspaceVersionColumn 1785000000000 (1.23.0)');
  });

  it('should append (slow) only for slow instance commands', () => {
    expect(
      formatUpgradeCommandName(
        '1.23.0_BackfillWorkspaceIdSlowInstanceCommand_1785000000001',
      ),
    ).toBe('BackfillWorkspaceId 1785000000001 (1.23.0) (slow)');
  });

  it('should format workspace command names', () => {
    expect(
      formatUpgradeCommandName(
        '1.22.0_BackfillStandardSkillsCommand_1780000002000',
      ),
    ).toBe('BackfillStandardSkills 1780000002000 (1.22.0)');
  });

  it('should return null for null or undefined', () => {
    expect(formatUpgradeCommandName(null)).toBeNull();
    expect(formatUpgradeCommandName(undefined)).toBeNull();
  });
