...
const viewsAtom = metadataStoreState.atomFamily('views');
const viewFieldsAtom = metadataStoreState.atomFamily('viewFields');
const viewFiltersAtom = metadataStoreState.atomFamily('viewFilters');
const viewSortsAtom = metadataStoreState.atomFamily('viewSorts');
const viewGroupsAtom = metadataStoreState.atomFamily('viewGroups');
const viewFilterGroupsAtom = metadataStoreState.atomFamily('viewFilterGroups');
const viewFieldGroupsAtom = metadataStoreState.atomFamily('viewFieldGroups');

const coreViewsAtom = atom((get) => {
  const viewsEntry = get(viewsAtom) as MetadataStoreItem;
  const viewFieldsEntry = get(viewFieldsAtom) as MetadataStoreItem;
  ...
  const flatViewFilters = viewFiltersEntry.current as FlatViewFilter[];
  const flatViewSorts = viewSortsEntry.current as FlatViewSort[];
  const flatViewGroups = viewGroupsEntry.current as FlatViewGroup[];
  const flatViewFilterGroups =
    viewFilterGroupsEntry.current as FlatViewFilterGroup[];
  const flatViewFieldGroups =
    viewFieldGroupsEntry.current as FlatViewFieldGroup[];
...
coreViewsAtom.debugLabel = 'coreViewsSelector';

export const coreViewsSelector: Selector<CoreViewWithRelations[]> = {
  type: 'Selector',
