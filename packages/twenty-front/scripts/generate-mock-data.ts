...
type MetadataField = {
  ...
  morphRelations?: { type: string }[] | null;
};
...
const buildFindManyQuery = (object: MetadataObject): string => {
...
const main = async () => {
  ...
  const metadata = (await graphqlRequest(
    ...
    METADATA_QUERY,
    token,
  )) as { objects: { edges: { node: MetadataObject }[] } };
  ...
  const companyQuery = buildFindManyQuery(companyObject);
  console.log('Fetching company records from /graphql ...');

  const companyData = (await graphqlRequest(
    '/graphql',
    companyQuery,
    token,
  )) as { companies: { edges: { node: Record<string, unknown> }[] } };

  const companyRecords = companyData.companies.edges.map((edge) => edge.node);

  console.log(`Fetched ${companyRecords.length} company records.`);
