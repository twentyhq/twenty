...
This file is the authoritative compliance matrix. Every row maps an official requirement (Codex build guide, Codex best-practices guide, or OpenAI reference implementation) to either an **automated** check (a `scripts/validate-source.js` assertion function) or a **manual** sign-off.
...
| #   | Requirement                                                                                                                          | Source                    | Check                                   |
| --- | ------------------------------------------------------------------------------------------------------------------------------------ | ------------------------- | --------------------------------------- |
| D1  | `templates/marketplace.example.json` has marketplace identity, plugin name/category, a local distribution source, and valid policies | Codex marketplace schema  | [automated] `assertMarketplaceTemplate` |
| D2  | Optional repo-level `.agents/plugins/marketplace.json` points at `./packages/twenty-agent-skills/dist`                               | Build guide §Distribution | [automated] `assertJsonMetadata`        |
...
| #   | Requirement                                                     | Source              | Check                                                               |
| --- | --------------------------------------------------------------- | ------------------- | ------------------------------------------------------------------- |
| P1  | `validate-source.js` runs in CI on every PR touching the plugin | Reference impl      | [manual] confirm `.github/workflows/ci-agent-skills.yaml` is active |
| P2  | `validate-source.js` has its own unit tests                     | Internal convention | [manual] confirm `scripts/__tests__/` exists and runs               |
