...
Official, harness-agnostic [Agent Skills](https://skills.sh/) for building Twenty apps. Works with every harness supported by the `skills` CLI: Claude Code, Codex, Cursor, Pi, GitHub Copilot, Windsurf, OpenCode, and others.

Each skill directory is fully self-contained: installing a single skill also installs every reference doc and the cross-skill operating rules it needs, with no broken relative links and nothing to fix by hand afterwards.
...
npx skills add twentyhq/twenty/packages/twenty-agent-skills --skill develop-app
```

Install everything:

```bash
npx skills add twentyhq/twenty/packages/twenty-agent-skills --skill '*'
```
...
Both forms scope discovery to this package. Pointing the CLI at the bare repository instead (`twentyhq/twenty`) discovers every skill in the monorepo, including internal contributor skills that are not meant for app development; see [`SKILLS.md`](../../SKILLS.md) for what those are.
