...
- Use `Heading` from `twenty-ui/primitives/typography` for compact headings, or `SectionHeader` from `twenty-ui/components` for a heading with a description or action.
