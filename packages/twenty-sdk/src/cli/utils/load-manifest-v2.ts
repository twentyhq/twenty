...
import path, { posix, relative, sep } from 'node:path';
import {
  type Application,
...
/**
 * Check if the app uses the new folder structure (app/ folder exists).
 */
export const hasNewFolderStructure = async (appPath: string): Promise<boolean> => {
  const appFolder = path.join(appPath, 'app');
  return fs.pathExists(appFolder);
};
 ...
 * ```
 */
export const loadManifestV2 = async (
  appPath: string,
  ...
  const applicationConfigPath = path.join(appPath, 'app', 'application.config.ts');
