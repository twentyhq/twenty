...
import { loadManifestV2, hasNewFolderStructure } from '../load-manifest-v2';
...
describe('loadManifestV2 (integration)', () => {
  ...
  it('should detect new folder structure', async () => {
    const hasNew = await hasNewFolderStructure(appDirectory);
    expect(hasNew).toBe(true);
  });

  it('should detect legacy structure (no app/ folder)', async () => {
    const legacyDir = join(tmpdir(), 'test-legacy-' + Date.now());
    ensureDirSync(legacyDir);
    write(legacyDir, 'package.json', JSON.stringify(packageJsonMock, null, 2));

    const hasNew = await hasNewFolderStructure(legacyDir);
  ...
  it('should load manifest with imported constants resolved', async () => {
    const { manifest, packageJson, warnings } =
      await loadManifestV2(appDirectory);
...
`;
    ...
    const { manifest } = await loadManifestV2(appDirectory);
...
`;
  ...
  it('should throw error when app/ folder is missing', async () => {
    ...
    await expect(loadManifestV2(noAppDir)).rejects.toThrow(
      'Missing app/ folder',
    );
  ...
  it('should throw error when application.config.ts is missing', async () => {
    ...
    await expect(loadManifestV2(noConfigDir)).rejects.toThrow(
...
`;
    ...
    await expect(loadManifestV2(appDirectory)).rejects.toThrow(
...
`;
    ...
    const { shouldGenerate } = await loadManifestV2(appDirectory);
