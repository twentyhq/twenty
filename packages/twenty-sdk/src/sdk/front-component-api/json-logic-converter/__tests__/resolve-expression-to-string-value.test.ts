...
describe('resolveExpressionToStringValue', () => {
  it('resolves a string literal directly', () => {
    expect(resolveExpressionToStringValue(getExpression('"hello";'))).toBe(
      'hello',
    );
  });
  ...
  it('resolves a FeatureFlagKey constant to string', () => {
    expect(
      resolveExpressionToStringValue(
        getExpression('FeatureFlagKey.IS_RECORD_PAGE_LAYOUT_EDITING_ENABLED;'),
      ),
  ...
  it('throws for unresolvable expression', () => {
    expect(() =>
      resolveExpressionToStringValue(getExpression('someUnknownVar;')),
    ).toThrow(JsonLogicConversionError);
  });

  it('throws for numeric expressions', () => {
    expect(() => resolveExpressionToStringValue(getExpression('42;'))).toThrow(
