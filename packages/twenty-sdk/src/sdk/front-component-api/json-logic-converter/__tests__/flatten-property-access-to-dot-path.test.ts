...
describe('flattenPropertyAccessToDotPath', () => {
  it('returns the identifier name for a simple identifier', () => {
    expect(flattenPropertyAccessToDotPath(getExpression('foo;'))).toBe('foo');
  });

  it('flattens a single property access', () => {
    expect(flattenPropertyAccessToDotPath(getExpression('foo.bar;'))).toBe(
      'foo.bar',
    );
  });

  it('flattens deeply nested property access', () => {
    expect(flattenPropertyAccessToDotPath(getExpression('a.b.c.d;'))).toBe(
      'a.b.c.d',
    );
  });
  ...
  it('strips chained non-null assertions', () => {
    expect(flattenPropertyAccessToDotPath(getExpression('a!.b!.c;'))).toBe(
      'a.b.c',
    );
  });
  ...
  it('throws for unsupported node kinds', () => {
    ...
    expect(() => flattenPropertyAccessToDotPath(expression)).toThrow(
