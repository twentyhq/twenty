...
if [ "${ENABLE_DB_MIGRATIONS}" = "true" ] && [ ! -f /app/docker-data/db_status ]; then
    ...
    echo "SELECT 'CREATE DATABASE default' WHERE NOT EXISTS (SELECT FROM pg_database WHERE datname = 'default')\gexec" | PGPASSWORD=${PGPASS} psql -h localhost -p 5432 -U ${PGUSER} -d postgres
