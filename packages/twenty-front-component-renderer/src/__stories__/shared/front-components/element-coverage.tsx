...
import { type HtmlElementName } from './html-element-names';
...
export type ElementCoverageEventName = 'click' | 'focus-blur';

type ElementCoverageScenarioProps = {
  tag: HtmlElementName;
  eventName: ElementCoverageEventName;
};

export const ElementCoverageScenario = ({
  tag,
  eventName,
}: ElementCoverageScenarioProps) => {
  const [interactionCount, setInteractionCount] = useState(0);
  const { entries, pushEvent } = useEventLog();

  const spec = ELEMENT_REGISTRY[tag];
  ...
  const wrapped = isDefined(spec.wrapper) ? spec.wrapper(subject) : subject;
