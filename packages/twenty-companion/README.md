...
- **Continue in Twenty.** See your five most recent recordings, open a recording directly, or choose **View all in Twenty** to browse and search your history in the workspace.
...
- Home shows the next three events grouped by local calendar day, with large day numbers and meeting time ranges, followed by the five most recent recordings. See more expands the event list when additional meetings are available. View all in Twenty opens Call Recordings in the connected workspace. Clicking any recording opens its record directly in that workspace, including processing or failed recordings. Finishing capture keeps the current page open and updates the recording status in the list.
...
- Controls, cards, tags, avatars, and interface typography use Twenty UI directly in both themes. Emphasized actions use its blue primary accent. All typography uses Inter. Native popover positioning, desktop layouts, and the onboarding cloud are desktop-specific compositions using shared tokens.
...
The desktop reuses the CLI OAuth client and browser sign-in flow used by `twenty remote:add`, with PKCE and a local callback. Tokens stay in the desktop's secure store. After sign-in, the desktop checks the backend and offers installation if it is missing. An administrator installs the integration in each target workspace. Existing completed desktop recordings can be transferred using the integration's one-time migration script without replacing their media or record IDs.

## Code structure

- `src/main/services/Companion.ts` owns workspace and recording state. Only the main process holds credentials and Recall upload tokens.
- `src/main/services/TwentyClient.ts` discovers the workspace endpoint, refreshes OAuth credentials, and validates backend responses. Credential writes are serialized so sign-out wins over an in-flight refresh.
...
- `src/renderer/components/CompanionApp.tsx` composes the shell. `Onboarding`, `Home`, `RecordingList`, and `Settings` own their screen layout and local UI state, using the same shared controls.
...
Desktop Recorder uses the existing SDK billing path and does not change platform pricing, credit accounting, or billing infrastructure. Validate deductions and exhausted-credit behavior in a billing-enabled test workspace before production distribution. Automatic retries of ambiguous charges are intentionally excluded from this delivery.

Signing, notarization, update delivery, and supported-provider recording checks are also required before distribution.
