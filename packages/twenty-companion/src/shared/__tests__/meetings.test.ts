...
describe('meeting scheduling', () => {
  ...
  it('matches participant-specific join links and duplicate calendar copies', () => {
    expect(
      matchMeeting({
        meetings: [MEETING, { ...MEETING, id: 'copy' }],
        url: `${MEETING.url}?authuser=1`,
        now: NOW,
      })?.usesCalendarBot,
    ).toBe(true);
    expect(
...
it('matches Zoom host aliases but keeps distinct ports separate', () => {
  ...
  expect(
    matchMeeting({ meetings: [zoom], url: 'https://zoom.us/j/123', now: NOW })
      ?.id,
  ).toBe(zoom.id);
  expect(
    matchMeeting({
      meetings: [zoom],
      url: 'https://zoom.us:8443/j/123',
      now: NOW,
    }),
  ).toBeUndefined();
});
