...
export class Companion {
  ...
  private joiningMeetings = new Set<string>();
  ...
  async refresh(): Promise<void> {
    ...
    try {
      const agenda = await this.client.companion<Agenda>({ action: 'agenda' });
      if (
        this.state.connection !== 'connected' ||
  ...
  private async join(meetingId: string): Promise<void> {
    ...
    if (this.joiningMeetings.has(meeting.id)) return;
    if (
      this.stopped ||
      this.state.connection !== 'connected' ||
      connectionVersion !== this.connectionVersion
      ...
      return;
    this.joiningMeetings.add(meeting.id);
    try {
      await shell.openExternal(url);
      if (this.stopped || connectionVersion !== this.connectionVersion) return;
      await this.markHandled(meeting.id);
      this.state.notice = { type: 'opening-meeting', title: meeting.title };
    } finally {
      this.joiningMeetings.delete(meeting.id);
    }
  }
