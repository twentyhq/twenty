...
import { type RecordingUpload } from '../../shared/types/RecordingUpload';
import { type RecordingConfiguration } from '../../shared/types/RecordingConfiguration';
import { type RecordingResult } from '../../shared/types/RecordingResult';
import { type CompanionState } from '../../shared/types/CompanionState';
import { matchMeeting } from '../../shared/utils/matchMeeting';
import { type TwentyClient } from './TwentyClient';
...
export class RecordingController {
  ...
  private async startSdk(region?: string): Promise<void> {
    ...
    const apiUrl =
      region ??
      (
        await this.client.companion<RecordingConfiguration>({
  ...
  private async startRecording(windowId?: string): Promise<void> {
    ...
    try {
      ...
      const upload = await this.client.companion<RecordingUpload>({
  ...
  listenToRecording(): void {
    ...
    const handleDetection = (
      ...
      if (
        ...
        void this.record(window.id)
          .catch((error) => this.events.fail(error))
          .finally(() => {
            if (this.state.activeRecording?.windowId !== window.id)
