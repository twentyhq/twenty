...
it('disconnects consistently if resetting the SDK fails after credentials were saved', async () => {
  mocks.oauth.mockResolvedValueOnce({ serverUrl: 'https://other.twenty.com' });
  mocks.shutdown.mockRejectedValueOnce(new Error('SDK reset failed'));
  await companion.command({
    type: 'connect',
    serverUrl: 'https://other.twenty.com',
  });
  expect(companion.state.connection).toBe('disconnected');
  expect(companion.state.workspace).toBeNull();
  expect(companion.state.meetings).toEqual([]);
  expect(companion.state.error?.message).toBe('SDK reset failed');
});

it('retries a failed browser launch on the next auto-join tick', async () => {
  companion.state.settings.setupCompleted = true;
  companion.state.meetings = [
    {
      id: 'event-1',
      title: 'Meeting',
  ...
  await vi.advanceTimersByTimeAsync(1000);
  expect(mocks.handled).not.toHaveBeenCalled();
  await vi.advanceTimersByTimeAsync(1000);
  expect(mocks.open).toHaveBeenCalledTimes(2);
  expect(mocks.handled).toHaveBeenCalledOnce();
});

it('allows a later detection update to retry after capture fails', async () => {
  allowPermissions();
  companion.state.settings.setupCompleted = true;
  companion.state.settings.autoRecord = true;
  const event = {
  ...
  mocks.start.mockRejectedValueOnce(new Error('Capture unavailable'));
  mocks.listeners.get('meeting-detected')?.(event);
  await vi.advanceTimersByTimeAsync(0);
  expect(mocks.start).toHaveBeenCalledOnce();
  expect(companion.state.activeRecording).toBeNull();
  mocks.listeners.get('meeting-updated')?.(event);
  await vi.advanceTimersByTimeAsync(0);
  expect(mocks.start).toHaveBeenCalledTimes(2);
  expect(companion.state.activeRecording?.windowId).toBe('retry-call');
});
