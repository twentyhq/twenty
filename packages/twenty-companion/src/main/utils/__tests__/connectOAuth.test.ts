...
describe('Twenty OAuth', () => {
  ...
  it('does not open the browser or register an app when CLI discovery is missing', async () => {
    ...
    ).rejects.toThrow('does not support CLI sign-in');
  ...
  it('uses the existing CLI client with state validation and PKCE through real HTTP boundaries', async () => {
    let origin = '';
    let challenge = '';
    ...
    const server = createServer(async (request, response) => {
      ...
      if (request.url === '/.well-known/oauth-authorization-server')
        ...
        expect(parameters.get('client_id')).toBe('cli-client');
        expect(
          createHash('sha256')
