...
import { type RecordingConfiguration, type Agenda } from '../shared/types';
...
export class TwentyClient {
  ...
  async connect(credentials: Credentials, signal?: AbortSignal): Promise<void> {
    ...
    try {
      const configuration = await requestJson<RecordingConfiguration>(
        context.endpoint,
        {
          signal,
          method: 'POST',
      ...
      validateServerUrl(configuration.apiUrl);
    } catch (error) {
  ...
  async companion<TResponse>(body: object): Promise<TResponse> {
    const connectionVersion = this.connectionVersion;
    if (!this.credentials)
    ...
    try {
      const response = await requestJson<TResponse>(this.context.endpoint, {
        method: 'POST',
        headers: {
  ...
  private async discoverEndpoint(
    ...
    const [configuration, metadata] = await Promise.all([
      requestJson<{ publicFunctionDomain?: string | null }>(
        `${credentials.serverUrl}/client-config`,
        { signal },
      ),
      requestJson<{
        ...
        data: {
          currentWorkspace: {
            ...
            logo?: string | null;
            subdomain: string;
            workspaceUrls: { subdomainUrl: string; customUrl?: string | null };
          };
        };
      }>(`${credentials.serverUrl}/metadata`, {
        signal,
        method: 'POST',
        headers: {
          Authorization: `Bearer ${credentials.accessToken}`,
    ...
    const { publicFunctionDomain } = configuration;
    const { currentWorkspace } = metadata.data;
