...
Para crear una clave de API, ve a **Settings → MCP & APIs → API → + Create key**. Consulta [APIs](/es/developers/extend/api#create-an-api-key) para más detalles.
...
* **API Key**: Asigna un rol a la clave de API en **Settings → Members → Roles**. Consulta [Permisos](/es/user-guide/permissions-access/capabilities/permissions).
