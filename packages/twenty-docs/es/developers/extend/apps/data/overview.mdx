...
<CardGroup cols={2}>
  <Card title="Objetos" icon="table" href="/es/developers/extend/apps/data/objects">
    `defineObject` — declara nuevos tipos de registros con sus propios campos.
  </Card>
  <Card title="Extender objetos" icon="wand-magic-sparkles" href="/es/developers/extend/apps/data/extending-objects">
    `defineField` — agrega campos a objetos estándar o de otras aplicaciones.
  </Card>
  <Card title="Relaciones" icon="diagram-project" href="/es/developers/extend/apps/data/relations">
    Conexiones bidireccionales `MANY_TO_ONE` / `ONE_TO_MANY` entre objetos.
  </Card>
  <Card title="Tipos de actividad de la cronología" icon="clock-rotate-left" href="/es/developers/extend/apps/data/timeline-activity-types">
    Defina eventos de auditoría automáticos y eventos explícitos de la aplicación para cronologías de registros.
  </Card>
</CardGroup>
...
| **Tipo de actividad de la cronología** | Un evento de auditoría automático o un evento de dominio explícito mostrado en las líneas de tiempo de los registros.                    | `defineTimelineActivityType()`           |

El SDK detecta estos mediante análisis AST en tiempo de compilación, por lo que la organización de archivos depende de ti; la convención es `src/objects/`, `src/fields/`, `src/indexes/` y `src/timeline-activity-types/`. Los UUID `universalIdentifier` estables vinculan todo a través de los despliegues.
...
<Note>
¿Buscas **Application Config** o **Roles & Permissions**? Esos describen la propia aplicación en lugar de los datos que agrega; se encuentran en [Config](/es/developers/extend/apps/config/overview). ¿Buscas **Connections** (Linear, GitHub, Slack OAuth)? Estas existen para ser llamadas *desde* las funciones lógicas y se encuentran en [Logic](/es/developers/extend/apps/logic/connections).
</Note>
