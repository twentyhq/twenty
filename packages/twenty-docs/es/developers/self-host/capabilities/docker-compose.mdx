...
<Warning>
Los contenedores de Docker son para alojamiento en producción o autoalojamiento. Para contribuir, consulta la [Configuración local](/es/developers/contribute/capabilities/local-setup).
</Warning>
...
Consulta [Configurar variables de entorno](/es/developers/self-host/capabilities/setup) para configuraciones avanzadas. Todas las variables de entorno deben declararse en el archivo `docker-compose.yml` a nivel de servidor y/o trabajador, dependiendo de la variable.
...
2. **Generar una clave de cifrado**
   ...
   **Importante:** Mantén este valor en secreto / no lo compartas. Perder `ENCRYPTION_KEY` significa perder el acceso a todos los secretos almacenados en la base de datos (tokens OAuth, variables de la aplicación, secretos TOTP, etc.).
...
3. **Actualiza el `.env`**
   ...
   Consulta la [guía de rotación de claves](/es/developers/self-host/capabilities/key-rotation) para obtener instrucciones sobre cómo rotarla sin tiempo de inactividad.
...
Por defecto, Twenty se ejecuta en `localhost` en el puerto `3000`. La interfaz de usuario resuelve la API desde el propio origen del navegador, por lo que se carga desde cualquier nombre de host que llegue al contenedor, pero aun así establece `SERVER_URL` en tu URL pública: el servidor la usa para los enlaces generados (correos electrónicos, callbacks de OAuth) y para detectar HTTPS para las cookies seguras.
...
<Tip>
  ¿Prefieres no ejecutar y mantener Twenty por tu cuenta? [Encuentra un partner de alojamiento certificado de Twenty](https://twenty.com/partners/list?categories=HOSTING\&ref=docs-docker-compose) para implementar, alojar y gestionar tu instancia de principio a fin. *(O contacta con nuestro equipo: [contact@twenty.com](mailto:contact@twenty.com).)*
</Tip>
