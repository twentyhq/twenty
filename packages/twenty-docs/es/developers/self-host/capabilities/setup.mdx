...
<Warning>
**¿Instalando por primera vez?** Siga la [guía de instalación de Docker Compose](/es/developers/self-host/capabilities/docker-compose) para ejecutar Twenty, luego regrese aquí para la configuración.
</Warning>
...
Para rotar `ENCRYPTION_KEY` sin tiempo de inactividad, consulta la [Guía de rotación de claves](/es/developers/self-host/capabilities/key-rotation).
...
y `FRONTEND_URL` ya son de confianza. El servidor registra una advertencia cuando un navegador
lo llama desde un origen no declarado.
...
<Warning>
El front-end envía credenciales en cada solicitud, y los navegadores rechazan una
respuesta con credenciales desde un origen no declarado, por lo que la aplicación no se carga hasta
que configures esto.
</Warning>
...
Con varios espacios de trabajo habilitados, no listes aquí todos los subdominios de los espacios de trabajo.
En su lugar, enruta las rutas de la API (`/graphql`, `/metadata`, `/rest`, `/auth/`,
`/client-config`, `/file`) al back-end en cada nombre de host de cada espacio de trabajo, de modo que cada
espacio de trabajo permanezca en el mismo origen — así es como funciona Twenty Cloud.
