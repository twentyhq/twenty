...
3. Replace the three "replace_me_with_a_random_string_access" with the three random strings you just generated.
    1. If Twenty will be running behind reverse proxy, add `SERVER_URL=https://<your-api-url.com>` to your `.env`
4. Create a `docker-compose.yml` file from the example below.
    1. If Twenty will be running behind reverse proxy, uncomment `SERVER_URL=${SERVER_URL}` in your `docker-compose.yml`
