...
`RECIPIENT_FIELD_ID` должен идентифицировать связь `MANY_TO_ONE` или морфную связь в промежуточном объекте. Это сообщает Twenty, какая связанная запись является целевой, когда функция, например [распределение действий временной шкалы](/ru/developers/extend/apps/data/timeline-activity-types#automatic-and-explicit-events), проходит через промежуточный объект.
...
Вы также можете объявить связь напрямую внутри [`defineObject`](/ru/developers/extend/apps/data/objects). При встроенном объявлении опустите `objectUniversalIdentifier` — он наследуется от родительского объекта:
