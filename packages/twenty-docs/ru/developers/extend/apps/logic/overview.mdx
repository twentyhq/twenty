...
<CardGroup cols={2}>
  <Card title="Логические функции" icon="bolt" href="/ru/developers/extend/apps/logic/logic-functions">
    Основной строительный блок — типы триггеров, полезные данные (payloads) и типизированный клиент API.
  </Card>
  <Card title="Навыки и агенты" icon="robot" href="/ru/developers/extend/apps/logic/skills-and-agents">
    Повторно используемые инструкции агента ИИ и ассистенты с пользовательскими системными подсказками.
  </Card>
  <Card title="Подключения" icon="plug" href="/ru/developers/extend/apps/logic/connections">
    OAuth-учетные данные, которые ваше приложение хранит для сторонних сервисов — Linear, GitHub, Slack и других.
  </Card>
  <Card title="Хранилище ключ-значение" icon="database" href="/ru/developers/extend/apps/logic/key-value-store">
    Сохраняйте состояние между запусками логических функций — кэши, курсоры и утверждения между рабочими пространствами.
  </Card>
  <Card title="Фоновые задания" icon="layer-group" href="/ru/developers/extend/apps/logic/background-jobs">
    Поставьте функцию Logic в очередь на исполнение воркерами, чтобы обойти таймаут на один запуск.
  </Card>
</CardGroup>
...
Функции выполняются в изолированных процессах Node.js в песочнице и получают доступ к рабочему пространству через типизированный клиент API с областью действия, ограниченной ролью, объявленной в [`defineApplication()`](/ru/developers/extend/apps/config/application).

<Note>
**Хуки времени установки** — код, который выполняется до или после установки, — используют то же окружение выполнения, но свои собственные функции `define*` и находятся в разделе [Config → Install Hooks](/ru/developers/extend/apps/config/install-hooks).
</Note>
