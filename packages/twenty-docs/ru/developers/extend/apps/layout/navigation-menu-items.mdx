...
**Элемент навигационного меню** — это запись в левой боковой панели. Используйте `defineNavigationMenuItem()` для добавления пользовательских ссылок на боковую панель — обычно по одной на каждое [представление](/ru/developers/extend/apps/layout/views), которое вы поставляете, — или для указания на внешние URL-адреса.
...
* Перечисление также содержит `NavigationMenuItemType.RECORD`, используемый внутренне для добавляемых пользователем в «Избранное» записей — он не может использоваться из манифеста приложения (нет поля для ссылки на запись).
...
<Note>
**Распространенная ошибка:** создание объекта без связанного элемента навигационного меню делает этот объект недоступным через боковую панель. Если только это не технический/внутренний объект, у каждого настраиваемого объекта должен быть элемент боковой панели, указывающий на него.
</Note>
