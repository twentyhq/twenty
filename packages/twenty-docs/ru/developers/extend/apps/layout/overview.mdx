...
<CardGroup cols={2}>
  <Card title="Представления" icon="list" href="/ru/developers/extend/apps/layout/views">
    `defineView` — сохранённые конфигурации списков: видимые столбцы, фильтры, группы.
  </Card>
  <Card title="Элементы меню навигации" icon="bars" href="/ru/developers/extend/apps/layout/navigation-menu-items">
    `defineNavigationMenuItem` — элементы боковой панели, ссылающиеся на представления или внешние URL.
  </Card>
  <Card title="Макеты страниц" icon="table-columns" href="/ru/developers/extend/apps/layout/page-layouts">
    `definePageLayout` и `definePageLayoutTab` — вкладки и виджеты на странице деталей записи.
  </Card>
  <Card title="Компоненты фронтенда" icon="window-maximize" href="/ru/developers/extend/apps/layout/front-components">
    `defineFrontComponent` — изолированные React‑компоненты, которые отображаются внутри Twenty.
  </Card>
  <Card title="Элементы меню команд" icon="terminal" href="/ru/developers/extend/apps/layout/command-menu-items">
...
| **Настройки приложения**                 | Настраиваемый раздел конфигурации на вкладке «Настройки» приложения, используемый вместо стандартного интерфейса переменных | `defineSettingsFrontComponent`            |
...
Они все еще находятся в активной разработке: песочница реализует частичный DOM, поэтому сложные варианты использования могут завершаться сбоем. См. раздел [Текущие ограничения](/ru/developers/extend/apps/layout/front-components#current-limitations).
