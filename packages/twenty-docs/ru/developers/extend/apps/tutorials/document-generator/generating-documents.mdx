...
Теперь ядро: [логическая функция](/ru/developers/extend/apps/logic/logic-functions)
...
(`yarn test:unit`). См. раздел [Тестирование](/ru/developers/extend/apps/operations/testing).
</Tip>

## Обработчик

Обработчик использует [`CoreApiClient`](/ru/developers/extend/apps/logic/logic-functions)
...
Смотрите [Roles](/ru/developers/extend/apps/config/roles) для получения более четких разрешений.
...
<Card title="Далее: HTTP-маршруты →" icon="земля" href="/ru/developers/extend/apps/tutorials/document-generator/http-routes">
