...
1. **Создайте ключ API**: перейдите в **Настройки → APIs** и создайте новый ключ API. Вам понадобится этот токен для логической функции.
...
<Tip>
С ручным триггером, пользователи могут запускать этот рабочий процесс, используя кнопку, которая появляется в правом верхнем углу после выбора записи, которая запускает рабочий процесс для создания и прикрепления PDF.
</Tip>

### Шаг 2: Добавьте логическую функцию

1. Добавить действие **код** (логическая функция)
...
| 4   | Логическая функция       | Скачать и прикрепить сгенерированный PDF   |
...
Каждый сервис предоставляет API, который возвращает URL-адрес PDF, который затем можно передать в логическую функцию.
...
* [Триггеры рабочего процесса](/ru/user-guide/workflows/capabilities/workflow-triggers)
* [Логические функции](/ru/user-guide/workflows/capabilities/workflow-actions#code)
* [Создание коммерческого предложения или счета в Twenty](/ru/user-guide/workflows/how-tos/connect-to-other-tools/generate-quote-or-invoice-from-twenty)
