...
Use the collapse button at the top of the sidebar, or choose **Collapse sidebar** from the command menu (`Cmd+K` on Mac or `Ctrl+K` on Windows). When collapsed, use **Expand sidebar** to show it again.
...
Accessible via the Command Menu, the top of the sidebar, or by pressing `/`. Search works across all objects.
...
<Card title="Full Layout guide" icon="arrow-right" href="/user-guide/layout/overview">
  Sidebar, views, record pages — detailed reference and how-tos.
</Card>
