---
title: Dezvoltatori
description: Creați aplicații, utilizați API-ul, autogăzduiți sau contribuiți la baza de cod.
---

import { CardTitle } from "/snippets/card-title.mdx"

<CardGroup cols={3}>
  <Card href="/ro/developers/extend/apps/getting-started/quick-start" img="/images/user-guide/halftone/dev-apps.png">
    <CardTitle>Aplicații</CardTitle>
    Extindeți Twenty cu obiecte personalizate, logică pe partea de server, componente UI și agenți AI — toate sub formă de pachete TypeScript.
  </Card>

  <Card href="/ro/developers/extend/api" img="/images/user-guide/halftone/dev-api.png">
    <CardTitle>API</CardTitle>
    API-uri REST și GraphQL, webhook-uri și OAuth.
  </Card>

  <Card href="/ro/developers/self-host/capabilities/docker-compose" img="/images/user-guide/halftone/dev-self-host.png">
    <CardTitle>Autogăzduire</CardTitle>
    Rulați Twenty pe propria infrastructură.
  </Card>

  <Card href="/ro/developers/contribute/capabilities/local-setup" img="/images/user-guide/halftone/dev-contribute.png">
    <CardTitle>Contribuiți</CardTitle>
    Configurați monorepo-ul local și trimiteți PR-uri.
  </Card>
</CardGroup>
