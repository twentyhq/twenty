...
<Warning>
Containerele Docker sunt destinate găzduirii în producție sau auto-găzduirii. Pentru a contribui, consultați [Configurarea locală](/ro/developers/contribute/capabilities/local-setup).
</Warning>
...
Consultați [Setup Environment Variables](/ro/developers/self-host/capabilities/setup) pentru configurare avansată. Toate variabilele de mediu trebuie declarate în fișierul `docker-compose.yml` la nivel de server și/sau de lucru, în funcție de variabilă.
...
3. **Actualizați `.env`**
   ...
   Vezi [ghidul pentru rotația cheii](/ro/developers/self-host/capabilities/key-rotation) pentru instrucțiuni despre cum să o rotești fără întreruperi.
