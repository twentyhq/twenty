...
Pentru a crea o cheie API, mergi la **Settings → MCP & APIs → API → + Create key**. Vezi [API-uri](/ro/developers/extend/api#create-an-api-key) pentru detalii.
...
* **Cheie API**: Atribuie un rol cheii API în **Settings → Members → Roles**. Vezi [Permisiuni](/ro/user-guide/permissions-access/capabilities/permissions).
