...
<Warning>
**هل هي المرة الأولى التي تقوم فيها بالتثبيت؟** اتبع [دليل تثبيت Docker Compose](/ar/developers/self-host/capabilities/docker-compose) لتشغيل Twenty، ثم عد هنا للإعداد.
</Warning>
...
قم بإنشاء القيم باستخدام الأمر `openssl rand -base64 32` وخزنها في مكان آمن (مثل مدير الأسرار، إعدادات مُشفَّرة، إلخ). فقدان `ENCRYPTION_KEY` يعني فقدان الوصول إلى كل سر مخزن في قاعدة البيانات.

لتدوير `ENCRYPTION_KEY` بدون وقت توقف، راجع [دليل تدوير المفاتيح](/ar/developers/self-host/capabilities/key-rotation).
...
<Note>
"Mail.ReadWrite" و "Mail.Send" إلزاميان فقط إذا كنت ترغب في إرسال رسائل بريد إلكتروني باستخدام إجراءات سير العمل الخاصة بنا. يمكنك استخدام "Mail.Read" بدلاً من ذلك إذا كنت ترغب فقط في تلقي الرسائل الإلكترونية.
</Note>

<Note>
"Calendars.ReadWrite" إلزامي إذا كنت تريد إنشاء أحداث تقويم باستخدام إجراءات سير العمل الخاصة بنا. إذا كنت تريد فقط مزامنة أحداث التقويم، فاستخدم "Calendars.Read".
</Note>
...
## واجهة برمجة التطبيقات (API) والواجهة الأمامية على مضيفين منفصلين

ينطبق فقط إذا كنت لا تدع الواجهة الخلفية تقوم بخدمة الواجهة الأمامية. عمليات النشر ذات الأصل نفسه لا تحتاج إلى أي شيء هنا.

صرّح بجميع أصول المتصفحات التي تتواصل مع واجهة برمجة التطبيقات (API)، مفصولة بفواصل. `SERVER_URL`
و`FRONTEND_URL` موثوق بهما بالفعل. يقوم الخادم بتسجيل تحذير عندما يتصل به متصفح
من مصدر غير مصرح به.
...
<Warning>
ترسل الواجهة الأمامية بيانات الاعتماد مع كل طلب، وتقوم المتصفحات برفض الاستجابة التي تحتوي على بيانات اعتماد إذا جاءت من مصدر غير مصرح به، لذلك يفشل التطبيق في التحميل حتى تقوم بتعيين هذا الإعداد.
</Warning>

إذا كان المضيفان على نطاقات قابلة للتسجيل مختلفة (وليس مجرد نطاقات فرعية مختلفة)، قم أيضًا بتعيين `AUTH_COOKIE_SAME_SITE=none`، والذي يتطلب HTTPS.

مع تمكين ميزة تعدد مساحات العمل، لا تُدرِج كل نطاق فرعي لكل مساحة عمل هنا.
بدلًا من ذلك، قم بتوجيه مسارات واجهة برمجة التطبيقات (API) (`/graphql`، `/metadata`، `/rest`، `/auth/`،
`/client-config`، `/file`) إلى الواجهة الخلفية على كل اسم مضيف لمساحة عمل، بحيث تبقى كل مساحة عمل من نفس المصدر (same-origin) — هكذا تعمل Twenty Cloud.
