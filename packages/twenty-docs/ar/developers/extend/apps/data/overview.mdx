...
<CardGroup cols={2}>
  <Card title="كائنات" icon="table" href="/ar/developers/extend/apps/data/objects">
    `defineObject` — صرّح عن أنواع سجلات جديدة مع حقولها الخاصة.
  </Card>
  <Card title="توسيع الكائنات" icon="wand-magic-sparkles" href="/ar/developers/extend/apps/data/extending-objects">
    `defineField` — أضِف حقولًا إلى كائنات قياسية أو كائنات تطبيقات أخرى.
  </Card>
  <Card title="العلاقات" icon="diagram-project" href="/ar/developers/extend/apps/data/relations">
    اتصالات ثنائية الاتجاه من نوع `MANY_TO_ONE` / `ONE_TO_MANY` بين الكائنات.
  </Card>
  <Card title="أنواع أنشطة الجدول الزمني" icon="clock-rotate-left" href="/ar/developers/extend/apps/data/timeline-activity-types">
...
<Note>
هل تبحث عن **Application Config** أو **Roles & Permissions**؟ هذه تصف التطبيق نفسه بدلًا من البيانات التي يضيفها — وتوجد ضمن [Config](/ar/developers/extend/apps/config/overview). هل تبحث عن **Connections** (Linear, GitHub, Slack OAuth)؟ هذه وُجِدت ليتم استدعاؤها *من* دوال المنطق وتوجد ضمن [Logic](/ar/developers/extend/apps/logic/connections).
</Note>
