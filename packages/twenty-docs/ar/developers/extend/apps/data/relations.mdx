...
يجب أن يحدد `RECIPIENT_FIELD_ID` علاقة `MANY_TO_ONE` أو علاقة morph في كائن الربط. يخبر Twenty بالسجل المرتبط الذي يمثل الهدف عندما تعبر ميزة مثل [توزيع نشاط المخطط الزمني](/ar/developers/extend/apps/data/timeline-activity-types#automatic-and-explicit-events) كائن الربط.
...
يمكنك أيضًا إعلان علاقة مباشرة داخل [`defineObject`](/ar/developers/extend/apps/data/objects). عند كونها مضمَّنة، احذِف `objectUniversalIdentifier` — إذ يُورَّث من الكائن الأب:
