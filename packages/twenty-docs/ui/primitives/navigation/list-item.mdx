...
Render the row as a label and place a [Switch](/ui/primitives/input/switch) in `endIcon` to keep it visible. The label names the switch and makes the whole row clickable. Keep the change handler on the switch so each interaction updates the value once.

<StoryEmbed
  storyId="ui-navigation-listitem--with-switch"
...
For a disabled setting, pass `disabled` to both `ListItem` and `Switch`. Avoid adding other interactive controls to the label.
