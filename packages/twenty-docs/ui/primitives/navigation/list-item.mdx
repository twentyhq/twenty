...
String descriptions keep an overflow tooltip. Set `actionsVisibility="always"` for persistent row actions. When the host expands a submenu row, pass `submenuOpen` alongside `hasSubmenu` to rotate the chevron.
...
Compose [Tag](/ui/primitives/data-display/tag) as the row content for tagged values, or put `ColorSample` in `startIcon` for a color choice. A row can also contain plain text or an avatar.
