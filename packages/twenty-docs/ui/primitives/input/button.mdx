...
Compose `Button` with `AnimatedIconCrossfade` to change its icon when application state changes. This replaces `AnimatedButton`: pass the animated element to `startIcon` instead of `animatedSvg`, and the label as children instead of `title`.
...
`AnimatedLightIconButton` is also removed. When an icon is itself an action, use `IconButton` or `LightIconButton` and animate its icon content. When a disclosure header owns expansion, keep its chevron decorative and expose `aria-expanded` on the header control. Keep animation styles with the feature and respect `prefers-reduced-motion`.

<StoryEmbed
  title="Button with an animated icon"
  storyId="ui-input-button-button--animated-icon"
  height={120}
/>
