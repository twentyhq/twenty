...
Set `attached={false}` to keep each control's rounded corners and a 2px gap. Shared size, surface, and color still apply through wrappers and tooltip anchors. Use [IconButtonGroup](/ui/components/input/icon-button-group) for a compact floating frame.
