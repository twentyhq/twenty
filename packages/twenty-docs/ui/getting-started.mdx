...
For a standalone React application, install the library and its React 19 peer dependencies:

```bash
yarn add twenty-ui react@^19 react-dom@^19
```

Twenty UI also declares `monaco-editor` as a peer dependency. Install it if you use the code editor components.
