...
The code editor has a separate entry point, `twenty-ui/components/code-editor`. Other components do not require Monaco. If you use the code editor, install its optional peer dependencies:

```bash
yarn add @monaco-editor/react monaco-editor
```

Import `CodeEditor`, `CoreEditorHeader`, and the editor theme helpers from `twenty-ui/components/code-editor`. These exports have moved out of `twenty-ui/primitives/input`, `twenty-ui/primitives`, and the package root. Configure Monaco workers for your bundler before mounting the editor.
