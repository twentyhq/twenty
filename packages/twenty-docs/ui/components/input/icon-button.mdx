...
## Surface, shadow, and blur

Control the surface and elevation independently:

- `variant="surface"` supplies the background, border, icon, and interaction colors for a bordered surface.
- `elevated` adds a shadow and blurs the content behind the button where its background is transparent.

`elevated` defaults to `false` and works with every variant. These props do not change positioning, visibility, or size. The size remains `md` (32px) by default; set `size="sm"` for a 24px control.
...
export const AddWidgetButton = () => (
  <IconButton variant="surface" elevated size="sm" aria-label="Add widget">
    <IconPlus />
  </IconButton>
);
```
...
<StoryEmbed
  title="Icon action with surface, shadow, and blur"
  storyId="ui-components-iconbutton--surface"
  height={180}
/>

Omit `elevated` to keep the surface without shadow or blur. Use `elevated` on its own to retain the default outline colors. All combinations support the same tooltips, loading states, links, and keyboard interactions.

<StoryEmbed
  title="Surface icon action with tooltip"
  storyId="ui-components-iconbutton--surface-tooltip"
  height={180}
/>

Replace existing `FloatingIconButton` controls with `IconButton variant="surface" elevated`. Pass the icon as children, use native `aria-label`, and map `small` to `sm` and `medium` to `md`. Supply `size="sm"` when the previous control used its default size. Native focus and `aria-pressed` replace synthetic focus and active flags.
