...
`ListItem` accepts JSX through `actions`. Pass one `LightIconButton` directly, or use `ButtonGroup` for multiple buttons. The row controls their placement and hover visibility; each button owns its accessible name, click handler, and disabled state. Wrap dropdown triggers directly around the button.
