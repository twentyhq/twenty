...
Use `MainButton` for a prominent action. It composes
[Button](/ui/components/input/button) with a solid surface, elevation, semibold
...
Use `href` for a link or compose a router link through `render`,
`nativeButton={false}`, and `role="link"`. Routing belongs to the application.
