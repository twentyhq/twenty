...
`Header` displays a title with an optional leading action. Pass `action={{ icon: 'close', label: 'Close', onClick: handleClose }}` for a close button, or use `icon: 'back'` for a back button. The label names the action for assistive technology. The header calls your handler without changing dropdown state, so it can also be used outside a root. Use `Dropdown.Back` when navigating the root's page history.
...
By default, Enter in `Search` does nothing until an arrow key moves focus to a result. Set `enterSelects="first-match"` to activate the first enabled result immediately, including with an empty query. Use `enterSelects="first-match-while-searching"` to enable this only for a nonblank query. Both options use the rendered result order, including action rows, and leave the popup open when no enabled results exist. Filtering remains caller-owned.
...
<StoryEmbed
  storyId="ui-components-dropdown--nested"
  title="Sort picker with a nested direction picker"
  height={420}
/>
