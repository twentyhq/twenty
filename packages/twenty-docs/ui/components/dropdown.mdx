...
Pickers allow creation actions and selectable values in the same list, in any order. Options expose their selected state with `aria-pressed`; commands keep their button semantics. Name `Content` with `aria-label`, or with a `Title` inside a `Header`, and give `Search` an accessible name.
...
`Header` lays out a row at the top of the popup. Put a `Title` in it to name the popup: the title labels `Content` for assistive technology, so `Content` no longer needs `aria-label`. `Close` closes the popup the same way an outside press does. It renders an icon button with an X by default, so give it an `aria-label`, or pass `render` and children for another control. Other controls, such as a button that returns to a previous step, can sit in the same row. Use `Dropdown.Back` when navigating the root's page history.
