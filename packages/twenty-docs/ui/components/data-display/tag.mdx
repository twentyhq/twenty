...
Use `Tag` for categories, priorities, or other short attributes. Keep a text label so the meaning does not depend on color. Use [Status](/ui/components/data-display/status) when you need a state indicator and loading feedback.

<StoryEmbed storyId="ui-data-display-tag--default" title="Tag example" />
