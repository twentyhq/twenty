...
<CardGroup cols={2}>
  <Card title="Oggetti" icon="table" href="/it/developers/extend/apps/data/objects">
    `defineObject` — dichiara nuovi tipi di record con i propri campi.
  </Card>
  <Card title="Estendere gli oggetti" icon="wand-magic-sparkles" href="/it/developers/extend/apps/data/extending-objects">
    `defineField` — aggiunge campi agli oggetti standard o di altre app.
  </Card>
  <Card title="Relazioni" icon="diagram-project" href="/it/developers/extend/apps/data/relations">
    Connessioni bidirezionali `MANY_TO_ONE` / `ONE_TO_MANY` tra oggetti.
  </Card>
  <Card title="Tipi di attività della timeline" icon="clock-rotate-left" href="/it/developers/extend/apps/data/timeline-activity-types">
    Definisci eventi di audit automatici ed eventi espliciti dell'app per le timeline dei record.
  </Card>
</CardGroup>
...
| **Tipo di attività della timeline** | Un evento di audit automatico o un evento di dominio esplicito visualizzato nelle timeline dei record                                   | `defineTimelineActivityType()`           |

L'SDK rileva questi elementi tramite analisi AST in fase di build, quindi l'organizzazione dei file dipende da te — la convenzione è `src/objects/`, `src/fields/`, `src/indexes/` e `src/timeline-activity-types/`. Gli UUID `universalIdentifier` stabili collegano tutto tra le varie distribuzioni.
...
<Note>
Cerchi **Application Config** o **Roles & Permissions**? Quelli descrivono l'app in sé piuttosto che i dati che aggiunge — si trovano sotto [Config](/it/developers/extend/apps/config/overview). Cerchi **Connections** (Linear, GitHub, Slack OAuth)? Quelle esistono per essere chiamate *dalle* funzioni di logica e si trovano sotto [Logic](/it/developers/extend/apps/logic/connections).
</Note>
