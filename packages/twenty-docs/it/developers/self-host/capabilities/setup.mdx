...
<Warning>
**Prima volta che installi?** Segui la [guida all'installazione di Docker Compose](/it/developers/self-host/capabilities/docker-compose) per far funzionare Twenty, quindi torna qui per la configurazione.
</Warning>
...
Per ruotare `ENCRYPTION_KEY` senza tempi di inattività, consulta la [guida alla rotazione delle chiavi](/it/developers/self-host/capabilities/key-rotation).
...
## API e front-end su host separati

Rilevante solo se il back-end non serve il front-end. Le distribuzioni con la stessa origine
non richiedono nulla qui.

Dichiara tutte le origini del browser che comunicano con l'API, separate da virgole. `SERVER_URL`
...
<Warning>
Il front-end invia le credenziali su ogni richiesta, e i browser rifiutano una risposta credenziale
da un'origine non dichiarata, quindi l'applicazione non riesce a caricare fino a quando
hai impostato questo.
</Warning>

Se i due host si trovano su domini registrabili diversi (non solo sottodomini
diversi), imposta anche `AUTH_COOKIE_SAME_SITE=none`, che richiede HTTPS.
