...
<Warning>
I container Docker sono destinati all'hosting in produzione o al self-hosting. Per contribuire, consulta [Configurazione locale](/it/developers/contribute/capabilities/local-setup).
</Warning>
...
Consulta [Configurazione delle variabili di ambiente](/it/developers/self-host/capabilities/setup) per configurazioni avanzate. Tutte le variabili di ambiente devono essere dichiarate nel file `docker-compose.yml` a livello di server e/o di worker, a seconda della variabile.
...
3. **Aggiorna il `.env`**
   ...
   Consulta la [Guida alla rotazione delle chiavi](/it/developers/self-host/capabilities/key-rotation) per le istruzioni su come ruotarla senza tempi di inattività.
...
Per impostazione predefinita, Twenty gira su `localhost` alla porta `3000`. L'interfaccia utente risolve l'API dall'origine del browser, quindi viene caricata da qualsiasi hostname che raggiunga il container — ma imposta comunque `SERVER_URL` sul tuo URL pubblico: il server lo usa per i link generati (email, callback OAuth) e per rilevare l'HTTPS per i cookie sicuri.
...
Se riscontri problemi, controlla [Risoluzione dei problemi](/it/developers/self-host/capabilities/troubleshooting) per le soluzioni.

## Hosting gestito

<Tip>
  Preferisci non gestire e mantenere Twenty in autonomia? [Trova un partner Twenty certificato](https://twenty.com/partners/list?categories=HOSTING\&ref=docs-docker-compose) per distribuire, ospitare e gestire la tua istanza end-to-end. *(Oppure contatta il nostro team: [contact@twenty.com](mailto:contact@twenty.com).)*
</Tip>
