...
description: Comprendere le funzionalità di integrazione delle email in Twenty.
---

**Nota**: Per connettere i tuoi account email e configurare le impostazioni di sincronizzazione, visita [Configurazione Email e Calendario](/it/user-guide/calendar-emails/overview).
...
### Oggetti in cui è possibile trovare le email
...
* **Oggetto + Metadati**: Riga dell'oggetto, mittente, destinatario e timestamp
...
Twenty importa email a circa **400 messaggi al minuto**. Questa velocità è attualmente limitata dalle limitazioni di frequenza dell'API di Gmail (e vale in modo analogo per altri provider).

A titolo di riferimento:

* 1.000 email → ~2,5 minuti
* 10.000 email → ~25 minuti
* 50.000 email → ~2 ore
...
* **Email interne**: Per impostazione predefinita, le email tra colleghi (stesso dominio) restano private (configurabile in Impostazioni → Generale → Sicurezza)
...
2. Attiva/disattiva **Sincronizza email interne**
...
2. Configura le cartelle nella sezione Email
