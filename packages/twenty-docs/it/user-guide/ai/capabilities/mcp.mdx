...
Per creare una chiave API, vai su **Impostazioni → MCP e API → API → + Crea chiave**. Vedi [API](/it/developers/extend/api#create-an-api-key) per i dettagli.

## Avvio rapido

### 1. Copia la configurazione

Vai su **Impostazioni → MCP e API → MCP** in Twenty. Scegli il metodo di autenticazione (OAuth o Chiave API), copia lo snippet JSON (userà già l'URL del tuo spazio di lavoro) e incollalo nel file di configurazione del tuo client MCP.
...
1. **`learn_tools`** — ottenere lo schema di input per strumenti specifici
2. **`execute_tool`** — eseguire uno strumento

Non è necessario ricordare i nomi degli strumenti. Chiedi al tuo assistente AI cosa può fare e chiamerà `learn_tools` automaticamente.
...
* **Chiave API**: assegna un ruolo alla chiave API in **Impostazioni → Membri → Ruoli**. Vedi [Autorizzazioni](/it/user-guide/permissions-access/capabilities/permissions).
