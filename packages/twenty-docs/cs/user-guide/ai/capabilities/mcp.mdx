...
Chcete-li vytvořit klíč API, přejděte do **Settings → MCP & APIs → API → + Create key**. Podrobnosti viz [Rozhraní API](/cs/developers/extend/api#create-an-api-key).
...
* **Klíč API**: Přiřaďte klíči API roli v **Settings → Members → Roles**. Viz [Oprávnění](/cs/user-guide/permissions-access/capabilities/permissions).
