...
Jakmile je vaše aplikace [sestavena a otestována lokálně](/cs/developers/extend/apps/getting-started/concepts), máte dvě cesty, jak ji distribuovat:
...
Podívejte se na [sekci defineApplication](/cs/developers/extend/apps/config/application#marketplace-metadata) na stránce Building Apps pro úplný seznam polí tržiště (`author`, `category`, `aboutDescription`, `websiteUrl`, `termsUrl` atd.).
