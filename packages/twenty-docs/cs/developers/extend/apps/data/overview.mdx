...
<CardGroup cols={2}>
  <Card title="Objekty" icon="table" href="/cs/developers/extend/apps/data/objects">
    `defineObject` — deklarujte nové typy záznamů s jejich vlastními poli.
  </Card>
  <Card title="Rozšiřování objektů" icon="wand-magic-sparkles" href="/cs/developers/extend/apps/data/extending-objects">
    `defineField` — přidejte pole ke standardním objektům nebo objektům jiných aplikací.
  </Card>
  <Card title="Vztahy" icon="diagram-project" href="/cs/developers/extend/apps/data/relations">
    Obousměrná `MANY_TO_ONE` / `ONE_TO_MANY` propojení mezi objekty.
  </Card>
  <Card title="Typy aktivit na časové ose" icon="clock-rotate-left" href="/cs/developers/extend/apps/data/timeline-activity-types">
    Definujte automatické události auditu a explicitní události aplikace pro časové osy záznamů.
  </Card>
</CardGroup>
...
| **Typ aktivity na časové ose** | Automatická událost auditu nebo explicitní událost domény zobrazená na časových osách záznamů                                      | `defineTimelineActivityType()`         |

SDK je detekuje pomocí analýzy AST v době buildu, takže organizace souborů je na vás — zažitou konvencí je `src/objects/`, `src/fields/`, `src/indexes/` a `src/timeline-activity-types/`. Stabilní UUID `universalIdentifier` vše propojují napříč nasazeními.
...
<Note>
Hledáte **Application Config** nebo **Roles & Permissions**? Ty popisují samotnou aplikaci, nikoli data, která přidává — najdete je pod [Config](/cs/developers/extend/apps/config/overview). Hledáte **Connections** (Linear, GitHub, Slack OAuth)? Ty existují proto, aby byly volány *z* logických funkcí, a najdete je pod [Logic](/cs/developers/extend/apps/logic/connections).
</Note>
