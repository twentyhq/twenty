...
* `writability` určuje, kdo vůbec smí zapisovat záznamy objektu, ještě předtím, než se uplatní oprávnění rolí: `MetadataWritability.OPEN` (výchozí – rozhodují role ve workspace), `MetadataWritability.APPLICATION` (záznamy mohou vytvářet, aktualizovat nebo mazat pouze logické funkce samotné aplikace; použijte pro objekty podobné konfiguraci, jejichž záznamy určují chování, aby je členové workspace s širokým přístupem k záznamům nemohli upravovat přes API) nebo `MetadataWritability.SYSTEM` (vyhrazeno pro data spravovaná platformou). Čtení nejsou ovlivněna – toto je vynucováno na straně serveru, na rozdíl od `isUIEditable`, které pouze skrývá prvky uživatelského rozhraní. Existuje také na úrovni jednotlivých polí, kde může být pouze přísnější než na úrovni objektu.
* Pole definovaná zde inline **nepotřebují** `objectUniversalIdentifier` — dědí se z nadřazeného objektu. Pomocí [`defineField()`](/cs/developers/extend/apps/data/extending-objects) můžete přidávat pole k objektům, které nevlastníte.
* Nové objekty můžete vygenerovat pomocí `yarn twenty dev:add object`, který vás provede pojmenováním, poli a vztahy. Viz [Architektura → Scaffolding entit](/cs/developers/extend/apps/getting-started/scaffolding).
...
| Identifikátory a relace | `UUID`, `RELATION`, `MORPH_RELATION` (viz [Relations](/cs/developers/extend/apps/data/relations))                                  |
...
* **Propojte tento objekt s ostatními** — vzor obousměrných vztahů najdete v části [Relations](/cs/developers/extend/apps/data/relations).
* **Přidávejte pole k objektům z jiných aplikací** — viz [Extending Objects](/cs/developers/extend/apps/data/extending-objects) pro `defineField()`.
* **Zobrazit tento objekt v uživatelském rozhraní** — viz [Položky navigační nabídky](/cs/developers/extend/apps/layout/navigation-menu-items) pro přidání položky do postranního panelu; viz [Zobrazení](/cs/developers/extend/apps/layout/views) pro přidání vlastních konfigurací seznamu.
