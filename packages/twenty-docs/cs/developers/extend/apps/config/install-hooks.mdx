...
* Hook je při sestavení automaticky připojen k manifestu aplikace (`preInstallLogicFunction` / `postInstallLogicFunction`) — v [`defineApplication()`](/cs/developers/extend/apps/config/application) není potřeba na nic odkazovat.
