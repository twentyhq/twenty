---
title: Vývojáři
description: Vytvářejte aplikace, používejte API, hostujte sami nebo přispívejte do kódu.
---

import { CardTitle } from "/snippets/card-title.mdx"

<CardGroup cols={3}>
  <Card href="/cs/developers/extend/apps/getting-started/quick-start" img="/images/user-guide/halftone/dev-apps.png">
    <CardTitle>Aplikace</CardTitle>
    Rozšiřte Twenty pomocí vlastních objektů, serverové logiky, UI komponent a AI agentů — to vše jako balíčky TypeScriptu.
  </Card>

  <Card href="/cs/developers/extend/api" img="/images/user-guide/halftone/dev-api.png">
    <CardTitle>API</CardTitle>
    Rozhraní API REST a GraphQL, webhooky a OAuth.
  </Card>

  <Card href="/cs/developers/self-host/capabilities/docker-compose" img="/images/user-guide/halftone/dev-self-host.png">
    <CardTitle>Hostujte sami</CardTitle>
    Provozujte Twenty na vlastní infrastruktuře.
  </Card>

  <Card href="/cs/developers/contribute/capabilities/local-setup" img="/images/user-guide/halftone/dev-contribute.png">
    <CardTitle>Přispějte</CardTitle>
    Nastavte si monorepo lokálně a odesílejte PR.
  </Card>
</CardGroup>
