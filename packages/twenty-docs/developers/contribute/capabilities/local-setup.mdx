...
If you want a contract-driven path instead of running the setup commands manually, install [Ota](https://ota.run/docs/install) and use the repo contract in `ota.yaml`.

```bash
ota doctor .
ota up --workflow native-dev --native .
ota up --workflow verify --native .
ota up --workflow verify --container .
ota tasks --use .
```

See the [Ota docs](https://ota.run/docs) for command reference and the [Ota blog](https://ota.run/blogs) for engineering notes and rollout examples.
