...
2. **Generate Secret Tokens**
    ...
    Generate **two** independent random strings — one for `APP_SECRET` (signs session cookies, fallback for legacy JWTs) and one for `ENCRYPTION_KEY` (encrypts secrets at rest):

    ```bash
    openssl rand -base64 32  # value for APP_SECRET
    openssl rand -base64 32  # value for ENCRYPTION_KEY
    ```
    **Important:** Keep both values secret / do not share them. Losing `ENCRYPTION_KEY` means losing access to every secret stored in the database (OAuth tokens, application variables, TOTP secrets, etc.).
...
3. **Update the `.env`**
    ...
    ENCRYPTION_KEY=second_random_string
    ```

    See the [Key rotation guide](/developers/self-host/capabilities/key-rotation) for instructions on rotating either of these values without downtime.
