...
<Warning>
**Security Notice:** The local serverless driver (`SERVERLESS_TYPE=LOCAL`) uses JavaScript-level isolation via V8 isolates ([isolated-vm](https://github.com/laverdet/isolated-vm)). While this prevents most attacks by sandboxing code execution with memory limits (128MB) and timeouts (30s), it is **not** equivalent to the hardware-level isolation provided by AWS Lambda. For production deployments handling untrusted code, we highly recommend using `SERVERLESS_TYPE=LAMBDA` or `SERVERLESS_TYPE=DISABLED`.
</Warning>
...
| Local | `SERVERLESS_TYPE=LOCAL` | Development and trusted environments | Medium (V8 isolate sandboxing) |
