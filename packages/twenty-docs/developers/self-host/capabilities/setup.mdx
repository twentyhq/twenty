...
3. Set these variables:
   ...
   - `AUTH_MICROSOFT_TENANT_ID={tenant}` (optional, defaults to `common`): use `common` for multi-tenant apps, or your directory (tenant) GUID or a verified domain name (e.g. `contoso.onmicrosoft.com`) for single-tenant apps
