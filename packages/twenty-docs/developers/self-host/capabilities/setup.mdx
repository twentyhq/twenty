...
Some infrastructure settings like database connections (`PG_DATABASE_URL`), server URLs (`SERVER_URL`), and secrets (`APP_SECRET`, `ENCRYPTION_KEY`, `FALLBACK_ENCRYPTION_KEY`) can only be configured via `.env` file.
...
Twenty uses three closely related env-only secrets:

| Variable | Purpose | Required |
| --- | --- | --- |
| `APP_SECRET` | Signs session cookies and serves as a fallback for legacy JWTs. | Yes |
| `ENCRYPTION_KEY` | Primary key used to encrypt secrets at rest (OAuth tokens, application variables, signing-key private keys, TOTP secrets, sensitive config values). | Recommended for new installs |
...
If `ENCRYPTION_KEY` is unset, Twenty falls back to `APP_SECRET` for at-rest encryption — matching the legacy behaviour for older deployments. New installs should always set a dedicated `ENCRYPTION_KEY`.
...
To rotate either secret without downtime, see the [Key rotation guide](/developers/self-host/capabilities/key-rotation).
