...
Twenty talks to the shared `common` endpoint by default, which only accepts
multi-tenant app registrations. If yours is single-tenant, Microsoft rejects
every sign-in with `AADSTS50194` and you have two ways out:

- set **Supported account types** to *Accounts in any organizational directory* in Azure, or
- keep the app single-tenant and set `AUTH_MICROSOFT_TENANT_ID` to your tenant ID (or a verified domain).

`organizations` and `consumers` also work if you want to accept one of those
audiences instead.
...
3. Set these variables:
   ...
   - `AUTH_MICROSOFT_TENANT_ID={tenant-id}` if your app registration is single-tenant (see below)
