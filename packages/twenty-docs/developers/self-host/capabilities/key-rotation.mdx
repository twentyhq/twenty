...
<Note>Campaign click links also use `ENCRYPTION_KEY` for signing. Unlike encrypted rows, links already sent in emails cannot be re-signed. They remain valid only while their signing key is either `ENCRYPTION_KEY` or `FALLBACK_ENCRYPTION_KEY`. Dropping the fallback invalidates links signed with the old key; rotating again also invalidates links signed before the previous rotation. Plan a link lifetime before rotating if old campaign links must continue to work.</Note>
