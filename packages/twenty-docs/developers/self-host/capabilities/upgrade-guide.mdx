...
By default the server container still starts when the upgrade fails on some workspaces, and logs a warning. Set `EXIT_ON_UPGRADE_FAILURE=true` to make the container exit with code 1 instead, so your orchestrator surfaces the failed upgrade rather than serving a partially migrated instance.
