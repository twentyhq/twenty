...
If you want to use a dedicated `ENCRYPTION_KEY` separate from `APP_SECRET`, set it **before** the v2.5 upgrade so the backfill writes rows under the new key from the start. Switching keys after the backfill requires a [rotation](/developers/self-host/capabilities/key-rotation).
