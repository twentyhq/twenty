...
Without `emit.through`, the event is written on the source record's own timeline. To fan it out to related records, set `emit.through.relationFieldUniversalIdentifier` to a [one-to-many junction relation](/developers/extend/apps/data/relations#junction-relations) on the source object. Its `universalSettings.junctionTargetFieldUniversalIdentifier` must identify the relation from the junction object to the target. Creating or deleting a junction row then produces `linked` or `unlinked`, while changes to the source record produce its normal action on every related target timeline.
