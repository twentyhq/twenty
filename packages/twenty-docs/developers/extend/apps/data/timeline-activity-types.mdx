...
Set both `action` and `objectUniversalIdentifier` when this type should describe the object's automatic audit events. The supported actions are `created`, `updated`, `deleted`, `restored`, `linked`, and `unlinked`. Only one effective type can handle the same action/object/target-relation combination in a workspace.
...
An app owns automatic timeline contracts for its own objects. To customize an event on an object owned by another app, declare the existing type explicitly with `overridesTimelineActivityTypeUniversalIdentifier`:
...
The referenced type must belong to the target object's application and describe the same action and route. Removing the overriding app restores the base type; removing the base type disables the override.
...
| `overridesTimelineActivityTypeUniversalIdentifier` | No       | Existing type explicitly replaced when targeting another app's object       |
