...
Without `emit.through`, the event is written on the source record's own timeline. To fan it out to related records, set `emit.through.relationFieldUniversalIdentifier` to a one-to-many junction relation on the source object. Creating or deleting a junction row then produces `linked` or `unlinked`, while changes to the source record produce its normal action on every related target timeline.

`linked` and `unlinked` events require `emit.through`, because their trigger is the creation or deletion of the junction row.
...
| `emit.through.relationFieldUniversalIdentifier`    | No        | Junction relation used to fan events out to related record timelines  |
