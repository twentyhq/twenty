...
Workspace administrators can toggle installed activity types and restore their
application defaults from **Settings → Applications → [App] → Timeline activity
types**. Label and icon overrides remain available through the metadata API.
