...
- To add a tab to a standard page layout (e.g. the Task or Company detail page), use [`definePageLayoutTab`](/developers/extend/apps/layout/page-layouts#definepagelayouttab) with `STANDARD_PAGE_LAYOUT_UNIVERSAL_IDENTIFIERS` from `twenty-sdk/define`; to add a single widget to one of its existing tabs, use [`definePageLayoutWidget`](/developers/extend/apps/layout/page-layouts#definepagelayoutwidget).
