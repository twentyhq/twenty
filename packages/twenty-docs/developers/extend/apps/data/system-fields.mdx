...
<Note>
  Alongside the system `INDEX` view, the server also provisions a regular table
  view stored with `ViewKey.DEFAULT`. That view belongs to the workspace, not to
  the engine: it is never reconciled, so users can rename, filter, group or
  delete it. Applications should keep targeting `SYSTEM_VIEW_KEYS.INDEX`, which
  stays the stable, unfiltered view that deep links resolve against.
</Note>
