...
- `viewKey` is the system view discriminator: `ViewKey.INDEX` for the main list view, `FIELDS_WIDGET_SYSTEM_VIEW_KEY` for the record-page fields widget view. The latter only keys the derivation and is never stored on the view row.
