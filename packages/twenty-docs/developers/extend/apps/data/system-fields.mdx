...
The server also provisions a **system view** on every object: the main list view (`All {objectLabelPlural}`, keyed on `ViewKey.INDEX` (derived with `SYSTEM_VIEW_KEYS.INDEX`)), with one column per displayable field. Like system relation fields, their identifiers are derived **name-free**, so renaming an object or a field never changes them.
...
- `viewKey` is the system view discriminator: `SYSTEM_VIEW_KEYS.INDEX` for the main list view, `SYSTEM_VIEW_KEYS.FIELDS_WIDGET` for the record-page fields widget view. It keys the derivation; only `INDEX` is also stored on the view row.

<Note>
  A workspace may also hold a regular table view stored with `ViewKey.DEFAULT`,
  alongside the system `INDEX` view. It belongs to the workspace, not to the
  engine: it is never reconciled, so users can rename, filter, group or delete
  it. It is not part of an object's guaranteed system metadata — objects created
  through your application's manifest do not get one — so never derive or target
  it. Applications should keep targeting `SYSTEM_VIEW_KEYS.INDEX`, which always
  exists and stays the stable, unfiltered view that deep links resolve against.
</Note>
