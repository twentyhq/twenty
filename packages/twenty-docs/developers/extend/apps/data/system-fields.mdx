...
- `viewKey` is the system view key: `ViewKey.INDEX` for the main list view, `ViewKey.FIELDS_WIDGET` for the record-page fields widget view.
...
<Warning>
  ...
  ignored, so a manifest view can never claim the `INDEX` or `FIELDS_WIDGET`
  keys, and the server
