...
- `defaultOpenRecordIn` sets where a new [view](/developers/extend/apps/layout/views) on this object opens its records: `ViewOpenRecordIn.USER_PREFERENCE` (the default, which follows each workspace member's own setting), `ViewOpenRecordIn.SIDE_PANEL`, or `ViewOpenRecordIn.RECORD_PAGE`. Set it to `RECORD_PAGE` for records that need a full page to be usable. Individual views can override it, and so can workspace admins in Settings.
