...
- `inheritance` goes with `MetadataReadability.INHERITED`, and only with it: it names the outgoing to-one relations a record inherits its access through, and whether `ANY` or `ALL` of them must grant it.

  ```ts
  inheritance: {
    match: ObjectAccessInheritanceMatch.ANY,
    through: [
      // an ordinary relation is named by its field universal identifier
      { kind: ObjectAccessInheritanceRelationKind.FIELD, fieldUniversalIdentifier: '...' },
      // a morph relation is named by its morph id, which covers every variant,
      // including ones added later
      { kind: ObjectAccessInheritanceRelationKind.MORPH, morphId: '...' },
    ],
  }
  ```

  Each entry is one branch: an ordinary branch grants access when its parent exists and is readable, a morph branch when the single concrete parent it points at is readable. A record whose branch has no parent is not readable, even when the parent object is `OPEN`, so clearing the link hides the record instead of exposing it. A parent counts as readable only through its whole effective authorization: the role's object permission, its row-level predicates, its share rows and, when the parent is itself `INHERITED`, its own parents. Creating a record under a parent, or moving it onto another one, additionally requires write access on every parent supplied. An `INHERITED` object whose declaration cannot be resolved refuses reads instead of falling back to `OPEN`, and deleting a relation it inherits through is refused unless the same change declares a valid replacement.
