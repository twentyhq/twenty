...
App-declared indexes are intentionally limited:

- **No unique indexes** — those belong to the data model and stay under admin control.
- **No partial `WHERE` clauses** — keep them simple and predictable.
- **Capped at 10 indexes per object** — same cap as user-created indexes; the framework's own indexes don't count against it.
- **Removed on uninstall** — indexes go away with the app, just like its objects.

Order the `fields` array the way Postgres should — leftmost column first, like a phone book. Indexes are not free: every write to the table updates them. Add one only when you have a query that needs it.
