...
- `defineIndex` doesn't accept `isUnique` or a partial `WHERE` clause — single-field uniqueness goes on the field with `defineField({ isUnique: true })`; richer constraints stay under admin control.
