...
**Generate document** command, and two record-page front components — one to
**preview** a document, one to **edit** a template in Markdown.
...
styles via a small [`Markdown`](https://github.com/twentyhq/twenty/blob/main/packages/twenty-apps/public/document-generator/src/utils/markdown-to-react.tsx)
helper.
...
const DocumentViewer = () => {
  ...
  // ...load { name, content, file } for recordId, then render:
  return (
...
## Edit a template in Markdown

The same pattern gives templates an **Editor** tab: a Markdown textarea on the
left, a live preview on the right. Editing and saving writes straight back to the
record with `CoreApiClient`.

```tsx filename="src/front-components/template-editor.front-component.tsx"
const TemplateEditor = () => {
  const recordId = useFrontComponentExecutionContext((c) => c.recordId ?? null);
  const [body, setBody] = useState('');
  // ...load the template body for recordId

  const save = async () => {
    await new CoreApiClient().mutation({
      updateDocumentTemplate: { __args: { id: recordId, data: { body } }, id: true },
    });
  ...
  return (
    <div style={styles.split}>
      ...
      <div style={styles.preview}><Markdown content={body} /></div>
    </div>
  );
};
```

Mount it with a `RECORD_PAGE` page layout on `documentTemplate`, exactly like the
viewer — see
[`template-record.page-layout.ts`](https://github.com/twentyhq/twenty/blob/main/packages/twenty-apps/public/document-generator/src/page-layouts/template-record.page-layout.ts).
Now editors write templates with instant feedback:

<Frame caption="The Editor tab: Markdown on the left, live preview on the right.">
  <img src="/images/docs/developers/extends/apps/document-generator/10-template-editor.png" alt="Template editor front component with live Markdown preview" />
</Frame>
