...
puts that view in the sidebar. The server already provisions each object's
default "All ..." view automatically, so the views an app defines are
additional, curated ones.
...
`availabilityType: CommandMenuItemAvailabilityType.RECORD_SELECTION` shows up when a Person is selected, and
