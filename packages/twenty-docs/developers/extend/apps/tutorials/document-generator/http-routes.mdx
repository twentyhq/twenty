...
or the app's custom domain — the URL Twenty injects as `TWENTY_FUNCTIONS_URL`,
with no `/s` prefix. The `/s` prefix is deprecated there and only remains for
self-hosted and local instances.
