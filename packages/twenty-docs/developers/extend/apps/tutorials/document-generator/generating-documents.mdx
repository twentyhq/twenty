...
Then upload it and store the reference on the record. `uploadFileToFilesField`
sends the bytes straight to file storage and attaches them to your app-owned
files field; the returned `id` is what you save:
...
const uploaded = await new MetadataApiClient().uploadFileToFilesField({
  file: bytes,
  filename: fileName,
...
<Note>
`uploadFileToFilesField` only targets **app-owned** files fields (so uploads always require an
