...
First, give the `document` object a `FILES` field to hold the PDF. Uploads target
a files field, so this field is what routes the upload:
...
Then upload it and store the reference on the record. `uploadFile` sends the
bytes straight to file storage for the `file` field; the returned `id` is what
you save:
...
<Note>
`uploadFile` needs the `UPLOAD_FILE` role flag plus update access to the object
that owns the target files field, the same access the app needs to attach the file
to a record. The default role above grants both, so the PDF can land on the
record's own `file` field. The
[call-recorder app](https://github.com/twentyhq/twenty/tree/main/packages/twenty-apps/public/call-recorder)
uploads recordings the same way.
</Note>
