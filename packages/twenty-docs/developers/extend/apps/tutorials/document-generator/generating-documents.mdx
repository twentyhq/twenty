...
Generate the PDF bytes. Logic functions are bundled and run in a sandbox, so for
plain-text output a small **dependency-free** writer beats pulling a PDF library
(no install at cold start). The full helper is
...
it emits a valid multi-page PDF using the built-in Courier fonts.
