...
#### One-shot sync with `yarn twenty sync`

If you do not want a watcher running in the background (for example in a CI pipeline, a git hook, or a scripted workflow), use `yarn twenty sync` instead. It runs the same pipeline as `yarn twenty dev` — build manifest, bundle files, upload, sync, regenerate the typed API client — but **exits as soon as the sync completes**:
...
| `yarn twenty sync` | Performs a single build + sync, then exits with code `0` on success or `1` on failure. | Scripts, CI, pre-commit hooks, AI agents, and any non-interactive workflow. |

Both commands require a Twenty server running in development mode and an authenticated remote — the same prerequisites apply.
...
You can now run `yarn twenty dev`, `yarn twenty sync`, `yarn twenty help`, and all other commands.
