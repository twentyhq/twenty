...
<Warning>
Dev mode is only available on Twenty instances running in development (`NODE_ENV=development`). Production instances reject dev sync requests. Use `yarn twenty deploy` followed by `yarn twenty install` to publish and install on production servers — `deploy` registers the app in the server's registry, while `install` activates it on your workspace. See [Publishing Apps](/developers/extend/apps/publishing) for details.
</Warning>
