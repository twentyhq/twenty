...
- `--minimal` — scaffold only the essential files (no examples)
- `--exhaustive` — scaffold all example entities (default)
...
If port 2020 is already in use (e.g., another Twenty instance or a different service), you can start on a different port:
