...
export default defineConnectionProvider({
  ...
  oauth: {
    ...
    scopes: ['read', 'write'],
    // Names of serverVariables on this app's defineApplication. The server
    // admin fills these in once on the app registration; all workspaces
    // installing the app share the same OAuth client.
...
- `clientIdVariable` / `clientSecretVariable` reference **server variables** declared on `defineApplication.serverVariables` (not application variables). The server admin fills them on the app registration, and all installed workspaces share them.
...
<Accordion title="serverVariables for OAuth credentials" description="Declare the client_id / client_secret slots on the app">

OAuth client credentials live on the **app registration** (one OAuth app per Twenty server), not on each workspace install. Declare them as `serverVariables` on `defineApplication` so the server admin can configure them once:
...
Until those values are filled in, the per-app settings tab shows a "needs server admin" hint and the "Add connection" button is disabled.
...
export const createLinearIssueHandler = async (input: {
  ...
  // user-scoped one. For HTTP-route triggers you typically pick the request
  // user's connection via event.userWorkspaceId instead.
  const connection =
...
| `scope`           | `'user'` (private to one workspace member) or `'workspace'` (shared with all members)                    |
...
- Pass `{ providerName }` to scope by provider; omit it to get all connections this app owns across all providers.
...
<Accordion title="Per-user vs workspace-shared scope" description="How users choose between private and shared credentials">

When a user clicks "Add connection," they're prompted to pick a scope:
...
const conn = connections.find((c) => c.scope === 'workspace');
```
