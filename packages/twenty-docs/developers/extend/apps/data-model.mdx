...
<Accordion title="defineApplicationRole" description="Mark a role as the app's default role">

`defineApplicationRole()` is a thin wrapper around `defineRole()` that flags **the** role used as your application's default role at install time. Validation is identical to `defineRole`, but the build pipeline auto-wires its `universalIdentifier` into the application manifest's `defaultRoleUniversalIdentifier` — so you do not need to reference it from `defineApplication()` yourself.
...
- All other roles in your app must use `defineRole()`.
- If your app uses `defineApplicationRole()`, the `defaultRoleUniversalIdentifier` field on `defineApplication()` is optional. Setting it explicitly is still supported for backward compatibility.
...
- **Permissions**: which role its functions and front components use (auto-detected from `defineApplicationRole()`).
...
- The default role is detected automatically from the role file marked with `defineApplicationRole()` (see above). You can still set `defaultRoleUniversalIdentifier` explicitly here if you prefer.
...
The role declared with `defineApplicationRole()` designates the default role used by your app's logic functions and front components. See `defineApplicationRole` above for details.
...
When you scaffold a new app, the CLI creates a default role file using `defineApplicationRole`:
...
The build pipeline detects the `defineApplicationRole()` call and wires its `universalIdentifier` into the application manifest automatically — you do not need to reference it from `application-config.ts`.
