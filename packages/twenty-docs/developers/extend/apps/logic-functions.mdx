...
- `toolTriggerSettings.inputSchema` is JSON Schema — declare the shape of the params object your handler expects, with `description` fields the AI agent can read.
- `workflowActionTriggerSettings.inputSchema` is Twenty's `InputSchema` (an array of property descriptors using `FieldMetadataType` for rich editors like `CURRENCY`, `RELATION`, `EMAILS`, etc.).
- You can combine these triggers with `cronTriggerSettings` / `databaseEventTriggerSettings` / `httpRouteTriggerSettings`.
...
<Note>
  **CoreApiClient is generated at dev/build time.** If you use it without
  running `yarn twenty dev` or `yarn twenty build` first, it throws an error.
  The generation happens automatically — the CLI introspects your workspace's
  GraphQL schema and generates a typed client using `@genql/cli`.
</Note>
