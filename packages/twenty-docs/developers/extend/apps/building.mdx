...
The SDK provides functions to define your app entities. You must use `export default defineEntity({...})` for the SDK to detect your entities:
...
import { CoreApiClient, type Person } from 'twenty-client-sdk';
...
```tsx src/front-components/my-widget.tsx
...
{/* TODO: add screenshot of a front component rendered on a record page */}
...
| `isHeadless` | No | Set to `true` if the component has no visible UI |
| `command` | No | Register the component as a command in the command menu (see below) |
...
You can make a front component available from the **command menu** (Cmd+K) by adding a `command` field:

```tsx src/front-components/quick-action.tsx
import { defineFrontComponent } from 'twenty-sdk';

const QuickAction = () => {
  return <div>Running quick action...</div>;
};

export default defineFrontComponent({
  universalIdentifier: 'c3d4e5f6-a7b8-9012-cdef-345678901234',
  name: 'quick-action',
  description: 'A command-triggered component',
  component: QuickAction,
  command: {
    universalIdentifier: 'd4e5f6a7-b8c9-0123-defa-456789012345',
    label: 'Run Quick Action',
    shortLabel: 'Quick Action',
    icon: 'IconBolt',
    isPinned: true,
...
{/* TODO: add screenshot of a front component appearing in the Cmd+K command menu */}
...
| `label` | Yes | Full label shown in the command menu |
| `shortLabel` | No | Shorter label for compact views |
| `icon` | No | Icon displayed next to the label |
| `isPinned` | No | Pin the command for quick access |
| `availabilityType` | No | `'GLOBAL'`, `'RECORD_SELECTION'`, or `'FALLBACK'` |
| `availabilityObjectUniversalIdentifier` | No | Restrict to a specific object type |
| `conditionalAvailabilityExpression` | No | Boolean or expression to control visibility |

#### Placing a front component on a page

Front components are not visible by default — you need to place them using a `definePageLayout` (see the definePageLayout section below). In the page layout, add a widget with `type: 'FRONT_COMPONENT'`:
...
widgets: [
  {
    ...
    configuration: {
      ...
      frontComponentUniversalIdentifier: 'a1b2c3d4-e5f6-7890-abcd-ef1234567890',
    },
  },
],
```

{/* TODO: add screenshot of a record page with a custom tab containing a front component widget */}
...
- **During `yarn twenty build`**: all files in `public/` are copied to `.twenty/output/public/` and listed in the manifest as `publicAssets` with their checksums.
- **During `yarn twenty dev`**: a file watcher monitors `public/` for changes. When you add, update, or delete a file, it is synced to the server automatically.
- **On the server**: files are uploaded via the `uploadApplicationFile` API and classified as `PublicAsset`.

### Supported file types

The asset pipeline supports a wide range of formats:

| Category | Formats |
|----------|---------|
| Images | jpg, jpeg, png, gif, webp, svg, bmp, ico |
| Documents | pdf, doc, docx, xls, xlsx, ppt, pptx, txt, csv |
| Data | json, xml, zip, tar, gz |
| Media | mp3, mp4, avi, mov |

Any file type not in this list is uploaded with a generic `application/octet-stream` MIME type.
...
<Note>
  ...
  - `APP_ACCESS_TOKEN_ENV_KEY` — Short-lived key scoped to your application's default function role
