...
- **Execution model**: post-install is **enqueued on the message queue** (with `retryLimit: 3`) and runs asynchronously after the install returns. The install is considered successful as soon as the job is enqueued — a failing post-install will be retried by the worker, but it will not roll back the install itself.
- If the hook throws or returns an error, make sure your handler is idempotent, especially when `shouldRunOnVersionUpgrade: true` or because the queue may retry it — it may run more than once over the lifetime of the app.
...
- **Not executed in dev mode**: when an app is registered locally (via `yarn twenty dev`), the server skips the install flow entirely and syncs files directly through the CLI watcher — so post-install never runs in dev mode. Use `yarn twenty exec --postInstall` to trigger it manually against a running workspace.
