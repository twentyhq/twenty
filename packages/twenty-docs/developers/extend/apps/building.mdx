...
- If the hook throws or returns an error, the install (or upgrade) is treated as failed and the error is surfaced to the caller. Make sure your handler is idempotent, especially when `shouldRunOnVersionUpgrade: true` — it may run more than once over the lifetime of the app.
...
- The function's `universalIdentifier` and `shouldRunOnVersionUpgrade` are automatically attached to the application manifest under the `postInstallLogicFunction` field during the build — you do not need to reference either of them in `defineApplication()`.
...
A pre-install function is a logic function that runs automatically during installation, **just before the workspace migrations are applied**. It shares the same payload shape as post-install (`InstallPayload`), but it is positioned earlier in the install flow so it can prepare state that the upcoming migrations depend on — typical uses include backing up data, validating compatibility with the new schema, or archiving records that are about to be restructured.
...
- **When the hook runs**: positioned just before `synchronizeFromManifest` in the install flow. On **upgrades** it resolves against the previously-installed version's logic function (the new version is not yet in metadata), which is the natural pre-upgrade hook. On **fresh installs** there is no prior logic function to resolve, so the hook is a no-op. This makes pre-install primarily useful for upgrade scenarios.
