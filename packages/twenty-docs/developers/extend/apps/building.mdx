...
The quickest way to see a front component in action is to register it as a **quick action command**. This makes it accessible from a button on the top right of the page:
...
After syncing with `yarn twenty dev`, you can see the `⚡️ Hello` quick action on the top right of the page:

<div style={{textAlign: 'center'}}>
  <img src="/images/docs/developers/extends/apps/quick-action.png" alt="Installed app — Content tab" />
</div>

Click on the button to see the component in action:
...
Front components are not visible by default — you need to place them on a page using a **page layout**. See the [definePageLayout](#definelayout) section for how to add a `FRONT_COMPONENT` widget to a record page tab.
...
export default defineFrontComponent({
  ...
  command: {
    universalIdentifier: 'bd0505af-97c6-4edf-b0b6-8c2b3fdf9e44',
