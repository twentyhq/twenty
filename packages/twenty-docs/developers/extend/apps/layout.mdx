...
| **Page Layout Tab** | A standalone tab attached to an existing page layout (standard or another app's) | `definePageLayoutTab` |
...
`definePageLayoutTab` lets your app attach a single tab — with optional widgets — to an **existing** page layout that you do not own. The most common use case is adding a custom tab (for example, an analytics or AI summary tab) to one of Twenty's built-in record pages, but it also works against page layouts shipped by another app or by your own app.
...
// Replace this with the universalIdentifier of the page layout you want to
// extend. Standard page layouts have stable identifiers exposed by Twenty.
...
Key points:
- `pageLayoutUniversalIdentifier` is **required** when using `definePageLayoutTab` and must point to a page layout that already exists at install time (standard, your app's, or another app's). When the parent page layout is missing, installation fails with a clear validation error.
