...
Front components can also share dependencies through a [vendor bundle](/developers/extend/apps/layout/front-components#sharing-dependencies-with-a-vendor-bundle). Packages listed in `defineVendor()` are built once into a separate file and loaded from it at runtime instead of being inlined into each component, so a component that imports only vendored packages carries almost no dependency code of its own.
