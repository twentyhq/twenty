...
| Regenerate local source from the workspace | `yarn twenty pull` | Writes the installed app back to define files. See below. |
...
Pull needs an existing project — scaffold one with `npx create-twenty-app@latest` first, then pull into it. Without `-u`, pull targets the application your local `defineApplication()` file declares.

**What it writes.** The application config, objects with their fields, fields your app added to objects it does not own, and authored indexes. Each entity goes to the file that already defines it, so a second pull rewrites only what changed on the server; new entities are placed beside existing files of their kind.

**What it does not write.** Roles and permissions, views, page layouts, navigation, command menu items, logic functions and front components are not exported yet, and neither is the stored source of your functions. Rows the engine derives from your metadata (system fields, default relations, default views, backing indexes) are never written: the server rebuilds them on apply. An object may still point its label identifier at one of those derived fields, as junction objects created in the UI do; the pointer is written and the build resolves it without adding a field. Workspace runtime state — member and API-key role assignments, webhooks, per-user navigation — is excluded by design, and rows another application owns are reported rather than pulled.
