...
| Ship a tarball to a server | `yarn twenty app:publish --private` | Requires a **strictly higher** `package.json` version, or pass `--version` to override it (e.g. computed by your CD pipeline) — see [Publishing](/developers/extend/apps/operations/publishing). |
