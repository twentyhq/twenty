...
| Ship a tarball to a server | `yarn twenty app:publish --private` | Requires a **strictly higher** `package.json` version, or pass `--version auto` to generate one — see [Publishing](/developers/extend/apps/operations/publishing). |
