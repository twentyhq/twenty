...
- **Concurrent operation error** — an install, upgrade or uninstall is rejected with `APPLICATION_OPERATION_IN_PROGRESS` because another operation is already running on that app; wait for it to finish and retry.
- **Failed install** — an install that started but could not finish leaves the app in the `FAILED` state, with the reason recorded on the app and shown on its page; fix the cause and install again, or uninstall to clear the failed attempt. An upgrade or uninstall that fails leaves the app installed and working, and records the reason the same way. An operation whose worker never reports back is failed automatically after 30 minutes.
