...
- **Placed on a default or foreign page layout** — widgets added to a tab of an engine default page layout, or of a page layout owned by another application, have no source form yet and are reported. Tabs added there are written as standalone files, since the layout's identifier is known.
