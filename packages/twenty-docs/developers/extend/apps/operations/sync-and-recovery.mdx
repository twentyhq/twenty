...
- **Failed install** — an install that started but could not finish leaves no application behind, so the app simply is not installed; fix the cause and install again. An upgrade or uninstall that fails leaves the app installed and working on the version it had. An operation whose worker never reports back is rolled back automatically after 30 minutes.
