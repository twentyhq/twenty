...
- **Dependencies size error** — the sync or install fails because the app's production `dependencies` are too large to install as a runtime layer; move packages your logic functions do not import at runtime (UI libraries, dev tooling) to `devDependencies`.
