...
The `logo` and each `galleryImages` file must not exceed **20 MB**. Larger files are skipped when the marketplace rehosts your published assets, so they will not be displayed.
