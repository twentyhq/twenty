...
- Installing, upgrading or uninstalling while another operation is already running on the same app is rejected with an `APPLICATION_OPERATION_IN_PROGRESS` error; wait for it to complete.
