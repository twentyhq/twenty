...
A `universalIdentifier` is unique across the whole instance and a registration belongs to exactly one workspace, so installing an app and developing one are two different rights:
