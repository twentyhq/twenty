...
Manually bumping `version` on every deploy makes sense for an app distributed to many workspaces, but it's unnecessary overhead for a private app that you continuously deploy to your own server (CI runs, on green you merge, and the pipeline deploys). For that workflow, let the CLI generate the version instead with `--version`:

```bash filename="Terminal"
# Generate a monotonic version automatically (uses the current timestamp,
# with the short git SHA attached as build metadata for traceability):
yarn twenty app:publish --private --version auto

# Or pass an explicit version derived from your CI environment:
yarn twenty app:publish --private --version "0.0.${CI_RUN_NUMBER}"
```

`--version auto` produces a version like `0.0.1700000000000+a1b2c3d`. The timestamp guarantees each deploy is strictly higher than the last (so the server's version check passes without you touching `package.json`), and the `+a1b2c3d` build metadata records the deployed commit — semver ignores build metadata when comparing versions, so it never affects ordering.

The override only changes the version inside the packaged tarball; your source `package.json` is left untouched. This applies to `--private` deploys only — npm publishing still uses the version in `package.json`, since marketplace consumers depend on meaningful version numbers.
