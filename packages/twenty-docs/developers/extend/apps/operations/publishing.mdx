...
- **The identifier belongs to no workspace.** Apps imported by the marketplace catalog sync start unowned. Claim yours through npm trusted publishing before developing on it: the refusal links to the **Claim an application** section of **Settings → Applications → Developer**, with the identifier prefilled.

The database enforces the rule too: a development application row can only reference a registration owned by its own workspace, so a transfer of the registration is refused while the owning workspace still has a development application synced from it. Uninstall that development application first.
