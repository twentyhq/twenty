...
- **The identifier belongs to no workspace.** Apps imported by the marketplace catalog sync start unowned. Claim yours through npm trusted publishing before developing on it.
...
The `logo` declared in `defineApplication()` is uploaded alongside the tarball, so a deployed app shows its configured logo like a marketplace one. The path is resolved against your app source, since the build output does not carry it. If the file is missing, the deploy still succeeds and any previously uploaded logo is kept.
...
- If `engines.twenty` is set and the target workspace's version does not satisfy the range, the install is rejected with a `SERVER_VERSION_INCOMPATIBLE` error and a message indicating both the required range and the actual server version.
...
<Note>
The server is the authoritative check — it validates `engines.twenty` at workspace install. Publishing is never gated on it: the same artifact can be installed into workspaces at different upgrade cursors, and a server too old when you published may satisfy the range later.
</Note>
