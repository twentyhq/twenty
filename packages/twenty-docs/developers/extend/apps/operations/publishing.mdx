...
The Twenty server populates its marketplace catalog from the npm registry by running a catalog sync. This sync is **not scheduled by default** — an operator triggers it (or registers a recurring cron job) on each server.
