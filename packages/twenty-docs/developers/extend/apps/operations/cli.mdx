...
The output is self-contained TypeScript source (no compiled bundles), so your own toolchain compiles it and plain `tsc` works in CI without a running Twenty instance. It includes a `README.md` recording where it was generated from, and the vendored [genql](https://github.com/remorses/genql) runtime ships with its MIT license (`runtime/LICENSE`). Treat the committed output like a lockfile: regenerate it when your data model changes (or after upgrading Twenty), review the diff, and let the type checker surface anything that broke.
