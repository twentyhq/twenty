...
<Note>
The uninstall runs in a background job on the server. The command waits for it
to complete (polling every 2 seconds, up to 10 minutes) and exits non-zero if
the uninstall fails — in that case the app stays installed.
</Note>
