...
### Committing the generated client (`--output`)

To emit the generated client as committable TypeScript source instead of replacing the copy inside `node_modules`, pass `--output`:

```bash filename="Terminal"
# Emit the full client source to a repo path
yarn twenty dev:generate-client --output src/generated/twenty

# Emit only the schema types (response types, enums, scalar aliases)
yarn twenty dev:generate-client --output src/generated/twenty --types-only
```

The output is self-contained TypeScript source (no compiled bundles), so your own toolchain compiles it and plain `tsc` works in CI without a running Twenty instance. Treat the committed output like a lockfile: regenerate it when your data model changes (or after upgrading Twenty), review the diff, and let the type checker surface anything that broke.

Unlike the `node_modules` mode, the emitted client does not include the `CoreApiClient` wrapper: the wrapper authenticates from in-app environment variables (`TWENTY_APP_ACCESS_TOKEN`), which out-of-app consumers don't have. Construct the client yourself with your own URL and auth:

```typescript
import { createClient } from './generated/twenty';

const client = createClient({
  url: 'https://my-instance.example.com/graphql',
  headers: async () => ({
...
const { people } = await client.query({
  people: { edges: { node: { id: true, name: true } } },
});
```

#### Authentication

The emitted client is the genql runtime, which gives you three hooks, in increasing order of control:

1. **`headers`** — a static object, or a sync/async function. A function is re-evaluated **on every request**, so it is the natural place for per-request tokens: return a fresh `Authorization` header from your auth layer each time. A static object fits server-side consumers using a long-lived API key.
2. **`fetch`** — swap the fetch implementation; URL, body building, and header merging stay genql's.
3. **`fetcher`** — full transport override: genql hands you the built GraphQL operation and you do the HTTP call yourself.

There is **no built-in token refresh**. Error semantics: a non-2xx response throws a plain `Error`; a 200 response carrying GraphQL errors throws `GenqlError` (exported from the generated `index.ts`), so an `UNAUTHENTICATED` extension code surfaces there rather than as a 401. Two patterns:

- **Proactive (recommended)**: refresh inside the async `headers` callback. If `getValidAccessToken()` checks expiry and refreshes before returning, every request carries a valid token and you never handle 401s in the normal path.
- **Reactive**: retry on 401 with a custom `fetcher` — the only hook that lets you act on the response and control the retry before the client throws (a custom `fetch` also sees the raw response, but returns straight into genql's error handling):
...
const client = createClient({
  fetcher: async (operation) => {
    ...
    if (response.status === 401) {
      response = await post(await forceRefreshAccessToken());
    }
...
`--types-only` emits a single `schema.ts` with the response types, enum constant maps, and scalar aliases. It contains no request/selection types and no runtime client, so it is safe to consume from packages that must stay dependency-free.

One bundling caveat: the full client's type map (`types.ts`) is a single object consumed at module init, so bundlers cannot tree-shake unused objects out of it. If you only need types, use `--types-only` (which erases completely at build time) and bring your own GraphQL layer.

<Note>
Committed output is for consumers living outside the app: sibling workspace packages, a custom frontend, or a standalone service. Code that runs inside Twenty (logic functions, front components) should keep importing `twenty-client-sdk/core`. The server regenerates that client from the installing workspace's schema at install time, while a committed client is baked from the schema of the remote you generated it from.
</Note>
