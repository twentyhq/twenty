...
The server enforces semver versioning on every deploy. Each new deploy must use a `version` in `package.json` that is **strictly higher** (per [semver](https://semver.org) ordering) than the version currently deployed for this app. Re-deploying the same version, or pushing a lower one, is rejected before the tarball is stored — you'll see a `VERSION_NOT_BUMPED` error from the CLI.
...
<Note>
Pre-release tags work as expected: bumping `1.0.0-rc.1` → `1.0.0-rc.2` is allowed, and a final release like `1.0.0` is correctly recognized as higher than `1.0.0-rc.5`. The version in `package.json` must itself be a valid semver string — values like `v1.0.0` (with a leading `v`) are rejected as `INVALID_INPUT`.
</Note>
...
<Note>
Install is idempotent: if the version of the app currently registered on the server is already installed in your workspace, the command exits successfully as a no-op — file writes, manifest sync, and SDK regeneration are all skipped. To install a newer version, deploy or publish it first, then re-run `yarn twenty install`.
</Note>
