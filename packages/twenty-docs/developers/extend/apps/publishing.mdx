...
```bash filename="Terminal"
yarn twenty remote add --api-url https://your-twenty-server.com --api-key <api-key> --as production
```
...
| `yarn twenty remote add` | Add a server connection | `--url`, `--api-key`, `--as`, `--local` |
