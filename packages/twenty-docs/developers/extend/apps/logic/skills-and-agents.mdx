...
workspace member. Only calls authenticated with an app access token — which is
what `runAgent()` uses from a logic function — may name a member. A token
issued on behalf of a specific user, as a front component receives, may only
name that user's own member.
