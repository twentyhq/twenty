...
<Warning>
  The run fails when the workspace member cannot be resolved — a missing or
  deactivated member, or one with no role. It never falls back to the agent's
  own role, since that would grant more access than the caller asked for.
</Warning>
