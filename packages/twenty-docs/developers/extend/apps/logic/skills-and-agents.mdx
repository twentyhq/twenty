...
workspace member. When the run was triggered by someone, `getCaller()` already
gives you that person: a user caller carries `workspaceMemberId`, which is
exactly what `runAsWorkspaceMemberId` expects. See [Caller identity and
permission checks](/developers/extend/apps/logic/logic-functions#caller-identity-and-permission-checks).

Naming a member requires an app access token, and what a token may name depends
on whether it carries a user:
