...
Each user holds at most one connection per provider. Adding a second one is rejected; the **Reconnect** action on the connection page refreshes the credential or changes its visibility.
