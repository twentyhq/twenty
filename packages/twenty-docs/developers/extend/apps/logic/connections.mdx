...
export default defineConnectionProvider({
  ...
  // provider in the per-app settings tab.
...
- `logo` is a path to an asset bundled in your app's `public/` folder, served through the app's public assets. When omitted, the settings tab falls back to a placeholder built from `displayName`.
