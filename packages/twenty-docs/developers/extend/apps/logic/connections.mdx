...
Anything an app persisted at connect time has to be undone when the connection goes away. The Slack app, for example, releases the server-scoped `slack-team:<team_id>` claim it took on connect so another workspace can connect the same Slack team. Set `onDisconnectLogicFunction` to reference a logic function in the same app, and it runs right after the `ConnectedAccount` is deleted.
