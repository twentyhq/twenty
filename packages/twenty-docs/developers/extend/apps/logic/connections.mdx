...
| `handle`          | The connected upstream account's email, refreshed on each reconnect; read from the `email`, `upn` or `preferred_username` claim of the OIDC `id_token` returned at token exchange, falling back to the connecting Twenty user's email when the provider returns none of them |
