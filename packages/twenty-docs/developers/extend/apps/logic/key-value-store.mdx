...
- **What to store.** Any JSON-serializable value — numbers, strings, arrays, objects. Keep entries small; this is for coordination and caching, not large blobs or files. For files, use a `FILES` field and [`uploadFileToFilesField`](/developers/extend/apps/logic/logic-functions#uploading-files).
