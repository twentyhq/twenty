...
<CardGroup cols={2}>
  ...
  <Card title="Messaging Channels" icon="messages" href="/developers/extend/apps/logic/messaging-channels">
    Bring conversations from any provider into Twenty's Message and Message Thread records — threading and de-duplication included.
  </Card>
  ...
  <Card title="Charging Credits" icon="coins" href="/developers/extend/apps/logic/credits">
    Declare what your app bills for and charge workspace credits for it.
  </Card>
</CardGroup>
...
| **Server route** | A third-party provider calls one URL shared by every install; a resolver picks the workspace | `serverRouteTriggerSettings` |
| **Server cron** | A CRON expression matches, once per server; the function dispatches work to each workspace | `serverCronTriggerSettings` |
...
The [**health check**](/developers/extend/apps/logic/logic-functions#health-check) shares it too. It declares no trigger: Twenty calls it on demand when the app's settings page opens.
</Note>
