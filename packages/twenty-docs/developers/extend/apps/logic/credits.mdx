...
A workspace can run out of credits. Your app is not stopped when that happens, but the platform's own work is: AI calls, workflow runs and outbound email are refused once the balance is gone. So an app that does expensive work first and calls `runAgent` at the end will fail late, on a call it did not write.
