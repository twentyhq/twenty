...
A workspace can run out of credits. Your app is not stopped when that happens, but some of the platform's own work is: AI calls and outbound email are refused once the balance is gone. So an app that does expensive work first and calls `runAgent` at the end will fail late, on a call it did not write.
...
- Not everything the platform does is gated on credits today. Workflow runs are not, so a `false` here does not mean every downstream call will fail.
