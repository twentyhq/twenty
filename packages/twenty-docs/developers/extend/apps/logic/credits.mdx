...
A workspace can run out of credits. When it does, the platform refuses to start your app's next logic function: the run fails before your handler is called, and so do AI entry points, workflow steps and outbound email. Charges themselves are never refused — a charge reports work you have already paid a provider for, so it is always recorded — which means the moment you find out is the *next* thing your app tries to do.

Ask first, and your app decides how to stop rather than being stopped mid-way:
...
- It is a snapshot. Credits can run out between the check and the work — including because of your own charges, since every charge you make moves the same counter this reads.
- A `false` tells you the workspace is out of credits. It does not predict which specific call will fail: not every outbound path is gated identically.

### Batch work

One check at the top of a run is not enough when the run charges as it goes. A batch that enriches 3,000 records charges after every chunk, so it can exhaust the workspace halfway through its own records and keep spending against a balance that is already gone. Check inside the loop instead:

```ts
for (const recordIdsChunk of chunk({ items: recordIds, size: 100 })) {
  const { hasAvailableCredits } = await getCreditAvailability();
  ...
  await enrichChunk({ recordIds: recordIdsChunk });
}
```

Report the records you skipped rather than failing the whole run — the ones you already enriched were charged for, and the caller needs to know which are left. The People Data Labs app does this in `runBatchEnrichment`.

The cost of the check is one request per chunk, against a chunk that is already doing real work. Size your chunks so that ratio stays sensible.
