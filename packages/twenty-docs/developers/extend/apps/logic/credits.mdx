...
A workspace can run out of credits. When that happens your function is refused before your code starts, and the platform's own work stops too: `runAgent` returns an error instead of running, and the other AI entry points refuse outright. Uninstall and disconnect hooks are the exception and always run, so an app can still be removed cleanly. An execution that does get through can still fail late, on a platform call it did not write, if the workspace runs out mid-run.
...
- The gate is not applied uniformly. Not every outbound email path is credit-gated, so a `false` here tells you the workspace is out of credits rather than predicting exactly which calls will fail.
