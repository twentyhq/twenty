...
const handler = async () => {
  ...
  // caller is null, or:
  // { kind: 'user', userId, userWorkspaceId, workspaceMemberId? }
...
`callerHasPermissionFlag` checks the caller's role server-side and returns `false` when there is no caller. Use it to gate privileged operations inside your own logic: your function's data operations always run with the app's own access, so this is how you decide whether the person who triggered the function is allowed to make the function act.
