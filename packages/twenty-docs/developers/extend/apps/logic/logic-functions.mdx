...
It runs server-side, so it reads secret variables like any other logic function. The handler receives `HealthCheckPayload` (`{ version?: string }`), the installed version of the app.
...
| --- | --- | --- |
| `status` | Yes | `ok`, `warning` or `error` |
| `message` | For `warning` / `error` | Shown in the banner on the app's settings page |
