...
## Caller identity and permission checks

When a logic function is invoked by someone — a workspace member executing it from the UI, an authenticated HTTP route call, a record mutation firing a database event trigger, or an AI agent acting for a user — the runtime exposes who that caller is. Cron triggers and unauthenticated webhooks have no caller.

```ts
import { getCaller, callerHasPermissionFlag, PermissionFlagType } from 'twenty-sdk/logic-function';

const handler = async () => {
  const caller = getCaller();
  // caller is null, or:
  // { type: 'user', userId, userWorkspaceId, workspaceMemberId? }
