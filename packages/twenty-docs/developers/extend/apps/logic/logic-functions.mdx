...
## Permission checks on the invoking user

When a logic function is invoked by a signed-in person — a workspace member executing it from the UI, an authenticated HTTP route call, a record mutation firing a database event trigger, or an AI agent acting for a user — your function can check that person's permissions. Cron triggers and unauthenticated webhooks have no invoking user.

```ts
import { invokingUserHasPermissionFlag, PermissionFlagType } from 'twenty-sdk/logic-function';

const handler = async () => {
  if (!(await invokingUserHasPermissionFlag(PermissionFlagType.WORKSPACE_MEMBERS))) {
...
`invokingUserHasPermissionFlag` checks the invoking user's role server-side and returns `false` when there is none. The identity it checks is the one the server recorded when it started the run, carried by the app access token. Your function never passes it in, so the answer cannot be steered from inside the runtime. Use it to gate privileged operations inside your own logic: your function's data operations always run with the app's own access, so this is how you decide whether the person who triggered the function is allowed to make the function act.
