...
Some runs serve a person the platform cannot see: a chat bot answering a message, a webhook from a tool where you have mapped the sender to a workspace member yourself. Name that member and the client acts as them:

```ts
import { CoreApiClient } from 'twenty-client-sdk/core';

const asMember = new CoreApiClient({
  runAs: { workspaceMemberId: '20202020-0687-4c41-b707-ed1bfca972a7' },
});

// only the records that member may read come back
const { people } = await asMember.query({ ... });
```

On its first call the client exchanges your application's token for a short-lived access token bound to that member, through the `generateApplicationTokenForWorkspaceMember` mutation, and keeps it for its lifetime. The exchange needs the application token, so it only works inside a logic function run, and it needs your application's role to declare the impersonate permission flag (`SystemPermissionFlag.IMPERSONATE` in `permissionFlagUniversalIdentifiers`), which a workspace admin sees and grants when installing your app. The result is the member's role intersected with your application's, the same access the default client has when that member triggers a run themselves: never more than the member could see, never more than your app declared. `RestApiClient` and `MetadataApiClient` take the same option. Pass a member you have verified: the mapping from an external identity to a workspace member is your app's responsibility.
