...
| `action` | No | `{ label, location }` for the banner's button. `location` is the settings tab it opens, today `general`, `variables` or `settings`; omit it to land on the app's configuration tab |
...
The button is only rendered when the app reports an action and the location it points at exists for that app, so a check on an app with no variables and no custom settings tab shows its message without a dead button.
