...
`ok` shows no banner at all; every other status renders the message in a banner coloured after it, so pick the one that matches how much the person needs to act.
