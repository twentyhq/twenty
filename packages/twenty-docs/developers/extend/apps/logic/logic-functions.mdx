...
Queue-executed runs (database event, cron, queued server-route dispatch) swallow handler failures by default: the failure is recorded in execution logs but the run is not repeated. Set `shouldRetryOnFailure: true` on a function to have the queue retry its failed runs. Only declare it on idempotent handlers — a retry re-runs the whole handler, and every attempt is billed as an execution.
...
| `retryLimit` | `number` (optional) | Queue attempts after the first run in `'queued'` mode, `0`–`10`, default `3`. Platform errors always retry up to this limit; a failure inside the target handler is only retried when the target declares `shouldRetryOnFailure`. Ignored in `'sync'` mode. |
