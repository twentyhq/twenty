...
- **serverWebhook**: Exposes a single registration-scoped webhook endpoint. A **resolver** function (declared with `serverWebhookTriggerSettings`) runs in the owner workspace, picks the target workspace from the incoming request, and the platform then runs a regular workspace **target** function in that workspace. See [Server webhook trigger](#server-webhook-trigger).
...
`httpRouteTriggerSettings` exposes a function under `/s/` and resolves the workspace from the request host — which works when each workspace has its own domain. Third-party providers, however, deliver every tenant's events to **one** webhook URL. For that case, use `serverWebhookTriggerSettings`.

The trigger has two parts:

1. A **resolver** logic function — declared with `serverWebhookTriggerSettings` — runs in your **owner workspace** (the workspace that owns the application registration). Its only job is to inspect the incoming request and return `{ workspaceId, payload? }`.
2. A **target** logic function — a regular per-workspace logic function — then runs in the resolved workspace with the payload returned by the resolver (or the original request payload if the resolver didn't transform it). Its return value becomes the HTTP response. **The target is where you verify the webhook signature** — it's reachable like any public HTTP endpoint, so treat it as one.

```ts src/logic-functions/resolve-workspace-from-webhook.logic-function.ts
import { defineLogicFunction } from 'twenty-sdk/define';
import type { RoutePayload } from 'twenty-sdk/logic-function';

// Runs in the owner workspace. Reads the workspaceId off the request and
// passes the original payload through so the target can verify the
// signature.
const handler = async (event: RoutePayload) => {
  const body = (event.body ?? {}) as { metadata?: { twentyWorkspaceId?: string } };

  return { workspaceId: body.metadata?.twentyWorkspaceId ?? '' };
};

export default defineLogicFunction({
  universalIdentifier: 'b3c2f0a1-7d4e-4c9a-9f2b-2e1d6a4c8e10',
  name: 'resolve-workspace-from-webhook',
  handler,
...
// Runs in the resolved workspace. Receives the full RoutePayload (headers
// and rawBody included), verifies the webhook signature, and does the
// real work.
const handler = async (event: RoutePayload) => {
  // verify signature here — see example below.
  return { received: true };
};
...
```
POST https://your-twenty-server.com/webhooks/server/:resolverLogicFunctionUniversalIdentifier/:targetLogicFunctionUniversalIdentifier
```

Both identifiers are the `universalIdentifier`s from your manifest — the resolver's, then the target's. Register that URL with the provider.

**Resolver contract.** The SDK's `LogicFunctionConfig` type enforces this at compile time: as soon as you set `serverWebhookTriggerSettings`, your handler is constrained to return `{ workspaceId: string; payload?: object }` (or a `Promise` of it). The `workspaceId` must be a workspace where your target function is installed, otherwise the request is rejected with `404`.

| Field | Type | Notes |
|-------|------|-------|
| `workspaceId` | `string` | Workspace UUID where the target will run. |
| `payload` | `object` (optional) | If set, replaces the request body sent to the target. Leave it out (the default) when the target needs the original `rawBody` and headers to verify the signature. |

<Warning>
**Signature verification is the target's responsibility.** The platform does not verify webhook signatures — the endpoint behaves like any public HTTP webhook, and the caller controls which target function in the resolved workspace runs. Verify the signature in the **target** using `event.rawBody` and the headers you listed in the resolver's `forwardedRequestHeaders`. Reject (throw or return a non-2xx) **before** any side effect, and use a constant-time comparison. Don't rely on the resolver for verification — when it returns a `payload`, the target loses `rawBody` and headers.
</Warning>
...
Verify the signature **in the target** before any side effect:
...
<Note>
The target runs **synchronously** and its returned value becomes the HTTP response, so providers see your status code and can retry on non-2xx. Keep both handlers fast — some providers (e.g. Slack) time out in a few seconds. Because the resolver is unauthenticated by design, protect this endpoint with rate limiting at your edge.
</Note>
