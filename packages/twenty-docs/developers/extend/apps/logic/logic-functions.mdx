...
Queue-executed runs (database event, cron, server-route dispatch) are retried when the handler fails. The retry budget is set per trigger with `retryLimit` (`0`–`10`): `cronTriggerSettings.retryLimit` (default `10`) and `databaseEventTriggerSettings.retryLimit` (default `3`); server-route dispatch always uses `3`. Set `retryLimit: 0` on triggers whose handler is not idempotent — a retry re-runs the whole handler, and every attempt is billed as an execution.
