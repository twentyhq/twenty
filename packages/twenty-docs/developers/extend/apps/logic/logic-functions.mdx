...
- **ingress**: Receives inbound webhooks from a third-party service (Stripe, GitHub, Svix, …) at a single registration-scoped endpoint and resolves the target workspace from the payload. See [Ingress trigger](#ingress-trigger).
...
`httpRouteTriggerSettings` exposes a function under `/s/` and resolves the workspace from the request host — which works when each workspace has its own domain. Third-party providers, however, deliver every tenant's events to **one** webhook URL. For that case, use `ingressTriggerSettings`: the function is reachable at a registration-scoped endpoint and the workspace is resolved from the payload.
...
```
POST https://your-twenty-server.com/webhooks/application-registrations/:applicationRegistrationUniversalIdentifier/:logicFunctionUniversalIdentifier
```
...
**Workspace resolution.** Because one endpoint serves every workspace, your integration must put the target `workspaceId` somewhere in the delivery, and `workspaceId.{ source, path }` tells the platform where to read it:
