...
`MetadataApiClient` includes an `uploadFileToFilesField` method for attaching files to file-type fields. The file goes straight to Twenty's file storage: the API issues an upload target, the client sends the bytes there, then the API confirms the upload.
...
const uploadedFile = await metadataClient.uploadFileToFilesField({
...
|-----------|------|-------------|
| `file` | `Blob \| ArrayBuffer \| ArrayBufferView` | The file contents, for example a `Buffer` |
...
<Note>
`uploadFile(fileBuffer, filename, contentType, fieldMetadataUniversalIdentifier)` still works but is deprecated: it streams the file through the API, which applies a lower size limit. New code should call `uploadFileToFilesField`.
</Note>
