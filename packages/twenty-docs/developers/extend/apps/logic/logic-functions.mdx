...
Queue-executed runs (database event, cron, server-route dispatch) are retried when the handler fails. Write these handlers to be idempotent — a retry re-runs the whole handler, and every attempt is billed as an execution.
