...
<Note>
`deprecatedUploadFile(fileBuffer, filename, contentType, fieldMetadataUniversalIdentifier)` keeps the previous signature and streams the file through the API, which applies a lower size limit. `uploadFile` falls back to it on its own when the direct upload fails.
</Note>
