...
1. A **resolver** logic function — declared with `serverRouteTriggerSettings` — runs in your **owner workspace** (the workspace that owns the application registration). It inspects the incoming request and returns `{ workspaceId, targetLogicFunctionUniversalIdentifier, payload?, dispatchMode? }`, picking *both* the target workspace and the target function. The resolver is the single point of authorization — the URL only carries the resolver's identifier. **This is the preferred place to verify request signatures**: the resolver runs before any side effect, has access to the original `rawBody` and forwarded headers, and can reject without ever touching the target.
...
| `dispatchMode` | `'sync' \| 'queued'` (optional) | `'sync'` (default) runs the target inline and returns its result as the HTTP response. `'queued'` acknowledges with `202` as soon as the resolver returns and runs the target on the worker queue, retrying it there on failure — the caller cannot observe the target's result. |
...
<Note>
With the default `dispatchMode: 'sync'`, the target runs **synchronously** and its returned value becomes the HTTP response, so callers see your status code and can retry on non-2xx. With `dispatchMode: 'queued'`, the route responds `202` right after the resolver returns and the target runs on the worker queue — the caller never observes the target's latency, result, or failures (the queue retries those internally). Prefer `'queued'` for high-volume webhook ingestion so sender redeliveries can't amplify processing slowdowns; keep `'sync'` when the caller needs the target's response. In both modes keep the resolver fast — some providers (e.g. Slack) time out in a few seconds. Because the resolver is reachable as a public endpoint, protect it with rate limiting at your edge.
</Note>
