...
// Runs in the owner workspace. Verify the signature, then map the
// incoming request to the target workspace and (optionally) reshape
// the payload that the target will receive.
const handler = async (event: RoutePayload) => {
  const body = (event.body ?? {}) as { metadata?: { twentyWorkspaceId?: string } };
  const workspaceId = body.metadata?.twentyWorkspaceId ?? '';

  return { workspaceId, payload: body };
};
...
// Runs in the resolved workspace. Receives the resolver's `payload`.
const handler = async (event: RoutePayload) => {
  // Do the real work here, in the right workspace.
  return { received: true };
};
...
| `payload` | `object` (optional) | If set, replaces the request body sent to the target. Useful for stripping signing fields or pre-parsing the event. |

<Warning>
**Signature verification is your responsibility.** The platform does not verify webhook signatures — it only runs your resolver, then your target. Verify the signature in the **resolver** (it runs first, in the owner workspace, with access to your application variables) using `event.rawBody` and the headers you listed in `forwardedRequestHeaders`. Throw before returning a `workspaceId` if the signature is invalid. Always verify **before** any side effect and use a constant-time comparison.
</Warning>
...
Verify the signature in the **resolver**, then return the `workspaceId`:
...
const handler = async (event: RoutePayload) => {
  ...
  return { workspaceId: body.metadata?.twentyWorkspaceId ?? '', payload: body };
};
