...
| `action` | No | `{ label, settingsTab }` for the banner's button. `settingsTab` is where it sends the user: `general`, `variables` or `settings` |
...
The button is only rendered when the app reports an action and the tab it points at exists for that app, so a check on an app with no variables and no custom settings tab shows its message without a dead button.
