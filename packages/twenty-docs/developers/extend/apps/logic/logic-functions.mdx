...
- Your app's role needs the `UPLOAD_FILE` permission flag and update access to the object owning the field, the same access it needs to attach the file to a record. System objects such as `attachment` and `callRecording` are exempt from the object check.
