...
Twenty runs the check when the app's settings page opens and stores the result on the application, so it never blocks the page on a third party.

A check that throws, times out or returns a shape Twenty cannot read is recorded as **unknown**, never as an error, and no banner is shown. A third-party outage should not paint every installed app red — if it did, people would learn to ignore the signal.
