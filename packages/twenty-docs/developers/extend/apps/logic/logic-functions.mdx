...
<Note>
  ...
  - `TWENTY_APP_ACCESS_TOKEN` — Short-lived key scoped to the triggering person's role intersected with your application's. **Absent when nobody triggered the run**, which is why the default client refuses in cron schedules, install hooks and unauthenticated webhooks.
  - `TWENTY_APP_APPLICATION_ACCESS_TOKEN` — Short-lived key scoped to your application's own role alone. Always injected, and what `runAs: 'application'` uses.
