...
export default defineHealthCheck({
  ...
  handler: async () => {
    ...
    if (response.status === 401) {
      return {
        status: 'ERROR',
        message: 'Your Stripe key was revoked',
...
| `message` | For every status but `OK` | Shown in the banner on the app's settings page |
...
`OK` shows no banner at all; every other status renders the message in a banner coloured after it, so pick the one that matches how much the person needs to act. The statuses are Twenty's own `ApplicationHealthStatus`, so the plain string and the enum member are interchangeable:

```ts
import { ApplicationHealthStatus } from 'twenty-sdk/define';

return { status: ApplicationHealthStatus.WARNING, message: 'Quota almost reached' };
```
