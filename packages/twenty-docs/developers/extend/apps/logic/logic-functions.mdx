...
| `action` | No | `{ label, location }` for the banner's button. `location` is where the button redirects to, an absolute path inside Twenty such as `/settings/billing`, optionally with a hash to select a tab; omit it to land on the app's own configuration tab |
