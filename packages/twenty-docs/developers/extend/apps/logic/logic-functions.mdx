...
| `batchMode` | Optional boolean. When `true`, the handler is called once per batch of records instead of once per record. The server decides how many records a batch holds. |
...
<Note>
Batch size is not something a handler can rely on. The server decides how many events a call carries, and a batch holds anything from a single event up to that maximum, depending on how many the originating write produced. Write the handler to loop over `events` whatever its length.
</Note>
