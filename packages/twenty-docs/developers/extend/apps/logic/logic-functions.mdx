...
On its first call the client exchanges your application's token for one bound to that member, through the `generateApplicationTokenForWorkspaceMember` mutation, and keeps it for its lifetime. The exchange needs the application token, so it only works inside a logic function run. The result is the member's role intersected with your application's, the same access the default client has when that member triggers a run themselves: never more than the member could see, never more than your app declared. `RestApiClient` and `MetadataApiClient` take the same option. Pass a member you have verified: the mapping from an external identity to a workspace member is your app's responsibility.
