...
A client acts as the person who triggered the run: their role intersected with your application's, so the call can never do more than either of you. That is the default, and it means a person can never use your app to exceed their own permissions.
...
Build both once, at module scope, and every call site then says which access it uses by which client it calls.
