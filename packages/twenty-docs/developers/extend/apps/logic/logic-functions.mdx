...
When a database event trigger invokes your logic function, it receives one `DatabaseEventPayload` per changed record (or a `DatabaseEventBatchPayload` covering up to `batchSize` records when batching is enabled). The payload combines metadata about the source workspace and object with the record-level event.
...
| `batchSize` | Optional integer greater than or equal to 1. When set, the handler is called once per chunk of up to `batchSize` records instead of once per record. |
...
A bulk write emits many events at once: importing a mailbox, for instance, can update thousands of records in a single operation, and one call per record floods the queue. Set `batchSize` to receive them together:
...
Events are grouped by the user who caused them before being chunked, so every call covers a single actor and `context.workspaceMemberId` stays meaningful. `updatedFields` filtering is applied before chunking, so a batch only contains records you asked for. The server caps `batchSize` at its own maximum, which means a batch can be smaller than requested but never larger.

Without `batchSize`, the handler keeps receiving one `DatabaseEventPayload` per record.
