...
Some calls legitimately need the application's own access instead — reading your app's configuration records, or doing work the triggering person could not do themselves. Build a second client for those:
...
<Note>
**A run nobody triggered has no one to act as.** Cron schedules, install hooks and unauthenticated webhooks have no person behind them, so the default client throws rather than quietly falling back to your application's access. Use `runAs: 'application'` in those functions, or check `context.workspaceMemberId` first when a function can be reached both ways.
</Note>

This applies to record access. The SDK helpers that reach your application's own resources always use its access and are unaffected by `runAs`: the [key-value store](/developers/extend/apps/logic/key-value-store), [connections](/developers/extend/apps/logic/connections), `runAgent`, `getPublicAssetUrl` and credit charging all work the same in a run nobody triggered.
...
<Note>
  ...
  - `TWENTY_APP_ACCESS_TOKEN` — Short-lived key scoped to a person's role intersected with your application's: whoever triggered the run in a logic function, whoever is looking at the page in a front component. In a logic function it is **absent when nobody triggered the run**, which is why the default client refuses in cron schedules, install hooks and unauthenticated webhooks.
