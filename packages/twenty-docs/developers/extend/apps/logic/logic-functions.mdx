...
Queue-executed runs (database event, cron, server-route dispatch) are retried when the handler fails: up to 10 times for cron triggers and 3 times for database-event triggers and server-route dispatch. Write these handlers to be idempotent — a retry re-runs the whole handler, and every attempt is billed as an execution.
