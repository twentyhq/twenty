...
- The usual logic-function environment (`APPLICATION_ID`, `API_URL`) is injected, so you can call the Twenty API with your app's token. Nobody triggers an install, so there is no person to act as: build every client with `runAs: 'application'`. See [Whose access a call uses](/developers/extend/apps/logic/logic-functions#whose-access-a-call-uses).
