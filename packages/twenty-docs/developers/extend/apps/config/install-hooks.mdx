...
- **Not executed in dev mode**: when an app is registered locally (via `yarn twenty dev`), the server skips the install flow entirely and syncs files directly through the CLI watcher — so post-install never runs in dev mode, regardless of `shouldRunSynchronously`. Use `yarn twenty dev:fn-exec --postInstall` to trigger it manually against a running workspace.
...
```bash filename="Terminal"
yarn twenty dev:fn-exec --preInstall
```
...
- **Not executed in dev mode**: same as post-install — the install flow is skipped entirely for locally-registered apps, so pre-install never runs under `yarn twenty dev`. Use `yarn twenty dev:fn-exec --preInstall` to trigger it manually.
