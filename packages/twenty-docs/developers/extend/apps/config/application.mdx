...
`applicationVariables` also accept an optional `category` string that groups them under a named section in the workspace settings UI. Variables without a `category` fall under the default `Configuration` section.
