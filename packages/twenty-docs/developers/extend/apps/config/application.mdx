...
- `settingsCustomTabFrontComponentUniversalIdentifier` *(optional)* takes the `universalIdentifier` of a [front component](/developers/extend/apps/layout/front-components). When set, the app's **Settings** tab renders that front component instead of the default variable configuration UI.
