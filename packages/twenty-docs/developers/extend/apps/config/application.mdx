...
- To render a custom front component as the app's **Settings** tab instead of the default variable configuration UI, declare it with [`defineSettingsTabFrontComponent()`](/developers/extend/apps/layout/front-components#custom-settings-tab) in its own file. Only one is allowed per app. (The legacy `settingsCustomTabFrontComponentUniversalIdentifier` field on `defineApplication()` is deprecated but still honored.)
