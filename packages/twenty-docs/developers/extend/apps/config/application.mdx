...
- Both kinds of variable accept `isRequired: true`. Use it for the values an app cannot ship a default for and cannot run without — an instance-wide API key on `serverVariables`, or, on `applicationVariables`, whatever each workspace has to supply for itself (its own tenant id, its own credentials). The app's settings screen then says how many required variables are still empty, instead of an unconfigured installation looking exactly like a finished one.
