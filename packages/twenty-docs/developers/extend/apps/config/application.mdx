...
- To add your own pages to the app's settings (in place of the generated **Variables** page), declare a front component and point a [`defineSettingPage()`](/developers/extend/apps/layout/front-components#settings-pages) at it, in its own file. Declare as many as you need and order them with `position`; `scope` puts a page under the workspace settings or under each member's own settings. The system-managed **General** page (auto-upgrade, App URL, connections) always comes first, and App URL only shows with **Advanced mode** on.
