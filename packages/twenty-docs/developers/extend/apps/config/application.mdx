...
- `settingsCustomTabFrontComponentUniversalIdentifier` *(optional)* takes the `universalIdentifier` of a [front component](/developers/extend/apps/layout/front-components). When set, the app's settings page gains a **Custom** tab that renders that front component.
