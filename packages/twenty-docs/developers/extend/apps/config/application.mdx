...
- To add your own tabs to the app's settings page (in place of the generated **Variables** tab), declare a front component with [`defineSettingsFrontComponent()`](/developers/extend/apps/layout/front-components#custom-settings-tabs) in its own file. Declare as many as you need and order them with `tab.position`. The system-managed **General** tab (auto-upgrade, App URL, connections) always comes first, and App URL only shows with **Advanced mode** on.
