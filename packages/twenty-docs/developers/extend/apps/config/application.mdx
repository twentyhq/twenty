...
Values are **always stored and injected as strings** — this is inherent to environment variables (`process.env.*` is string-only). Any typed value you declare in the manifest is serialized before storage: booleans to `"true"` / `"false"`, numbers to their decimal string, and arrays / objects / rich-text to JSON. It is up to your logic function or front component to parse the string back into the type you expect:
