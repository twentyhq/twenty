...
If your app charges credits, declare what it charges for. Each entry maps an operation name you choose onto the billing category the platform meters, plus the label shown to workspace admins in **Settings > Usage**.
...
await chargeCredits({
  ...
  creditsUsedMicro: 250_000,
  quantity: 12,
  userWorkspaceId: storedOwnerUserWorkspaceId,
});
```
