...
decides which records they _see_ by granting a principal (everyone, a role, or a workspace member)
`READ` or `READ_WRITE` access to the records that match its criteria. Criteria reuse the same
