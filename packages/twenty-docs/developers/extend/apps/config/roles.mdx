...
Declare predicates with `rowLevelPermissionPredicates` on the role. Each predicate references an
object and a field by `universalIdentifier`, an `operand`, and (optionally) a workspaceMember field
whose value is injected at query time — so you can express "the record's owner relation **is** the
...
export default defineRole({
  ...
  rowLevelPermissionPredicates: [
    {
      // "this Company's accountOwner IS the current workspace member"
      objectUniversalIdentifier:
        STANDARD_OBJECT_UNIVERSAL_IDENTIFIERS.company.universalIdentifier,
...
- Predicate `universalIdentifier`s are derived for you; you don't manage uuids for them.
- To combine multiple predicates with `AND`/`OR`, declare a `rowLevelPermissionPredicateGroups`
  entry (with its own `universalIdentifier` and `logicalOperator`) and point each predicate at it
  via `predicateGroupUniversalIdentifier`.
