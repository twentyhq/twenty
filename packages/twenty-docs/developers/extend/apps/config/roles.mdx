...
- Row-level security is enforced for workspaces on the **Organization plan or above**. On other
  plans the predicates declared in an app manifest still sync, they are simply not enforced.
- The Metadata API behaves differently from the manifest here: without the entitlement,
  `upsertRowLevelPermissionPredicates` is rejected with
  `ROW_LEVEL_PERMISSION_FEATURE_DISABLED` rather than syncing. A workspace on a plan that does not
  include row-level security therefore cannot create predicates outside of an app manifest, even
  though the `rowLevelPermissionPredicates` field is present on the `Role` type when you introspect
  the schema.
...
- Setting `defaultRoleUniversalIdentifier` explicitly on `defineApplication()` is still supported for backward compatibility, but is deprecated in favor of `defineApplicationRole()`. The identifier must match a role your app defines, or the build fails.
- Widening the application role in a new version blocks automatic upgrades: workspaces that installed an earlier version keep it until a workspace admin reviews the added permissions and approves the upgrade from the app's settings. Restricting the role never needs approval.

### Agent roles

The application role is what a workspace reviews when it installs your app, so it must cover everything the app itself can do. Any role you assign to an agent through `roleUniversalIdentifier` on `defineAgent()` must not grant more than the application role: no object action, field access, permission flag or row-level scope the application role lacks. If it does, the build fails with an error listing the excess grants. Roles your app ships for workspace members or API keys are not held to this rule, since the app never runs with them.
