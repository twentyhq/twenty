...
Roles decide what a member may _open_; on objects whose readability is `PRIVATE`, a **sharing rule**
decides which records they _see_ by granting a principal (everyone or a role; a single workspace
member can only be granted from the workspace itself, since a member id has no meaning in another
workspace) `READ` or `READ_WRITE` access to the records that match its criteria. Criteria reuse the same
`rowLevelPermissionPredicates` and `rowLevelPermissionPredicateGroups` shape as roles, nested under
the rule instead of a role; a rule without criteria matches every record of the object. Rules ship at
the top level of the manifest with `defineSharingRule`, and a rule with criteria only syncs on plans
that include row-level security:

```ts src/sharing-rules/sales-reads-deals.ts
import {
  defineSharingRule,
...
import { SALES_ROLE_UNIVERSAL_IDENTIFIER } from '../roles/sales-role';

export default defineSharingRule({
  universalIdentifier: 'f6e5d4c3-2b1a-4098-8765-666666666666',
  name: 'Sales reads open deals',
  objectUniversalIdentifier:
    STANDARD_OBJECT_UNIVERSAL_IDENTIFIERS.opportunity.universalIdentifier,
  granteePrincipalType: RecordSharePrincipalType.ROLE,
  granteeRoleUniversalIdentifier: SALES_ROLE_UNIVERSAL_IDENTIFIER,
  accessLevel: SharingRuleAccessLevel.READ,
  rowLevelPermissionPredicates: [
    {
      universalIdentifier: 'a7f6e5d4-3c2b-4109-9876-777777777777',
      objectUniversalIdentifier:
        STANDARD_OBJECT_UNIVERSAL_IDENTIFIERS.opportunity.universalIdentifier,
      fieldUniversalIdentifier:
...
A `PRIVATE` object whose only readable path is one rule cannot lose it: if a sync would delete that
rule, deactivate it, or give it criteria, and no other active criteria-free rule granting everyone or
a role remains on the object, the sync fails, with `INVALID_SHARING_RULE_INPUT` when the rule is
deleted or deactivated and `INVALID_ROW_LEVEL_PERMISSION_PREDICATE_DATA` when it gains criteria. Ship
another such rule in the same sync, or move the object off `PRIVATE`.

The same guard applies on the way in: syncing a new object with `readability: PRIVATE`, or moving an
existing object to `PRIVATE`, fails with `INVALID_OBJECT_INPUT` unless that same sync leaves at least
one active criteria-free rule granting everyone or a role on the object. Define the rule alongside
the object rather than in a later sync.

Rules are materialized into `recordShare` rows by a background job rather than at read time, so
they are eventually consistent: creating, updating or deleting a rule, changing a record's criteria
fields, or making an object `PRIVATE` queues a recalculation, and the records a grantee sees catch up
once that job has run. Because a rule is materialized once for every grantee, its criteria cannot use
a value taken from the current workspace member (the "me" predicates roles allow); grant a role
instead.
