...
<CardGroup cols={2}>
  ...
  <Card title="Roles & Permissions" icon="shield-halved" href="/developers/extend/apps/config/roles">
    `defineRole` and `defineSharingRule` — declare what your app's logic functions can read and write, and which records a role or everyone can see.
  </Card>
...
- **Sharing Rules** decide which records of a `PRIVATE` object a role, or everyone, may read. Declare them with `defineSharingRule()` at the top level of the manifest.
