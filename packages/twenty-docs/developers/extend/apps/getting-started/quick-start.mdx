...
Official Twenty [Agent Skills](https://skills.sh/twentyhq/twenty) teach coding agents the full app workflow: scaffolding, entity development, sync and deploy operations, and marketplace publishing. They install into Claude Code, Codex, Cursor, and any other harness supported by the `skills` CLI, and work against any Twenty instance — cloud, self-hosted custom domains, or localhost.
...
Browse the collection on [skills.sh](https://skills.sh/twentyhq/twenty).
