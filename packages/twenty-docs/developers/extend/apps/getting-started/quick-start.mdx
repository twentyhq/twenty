...
<Note>
Every mode syncs into the workspace that owns your app's registration. A `universalIdentifier` is unique across the whole instance, and syncing code under it is a claim of authorship, so a workspace can only develop an app whose registration it owns. If the identifier is registered to another workspace, or to no workspace at all, the sync is refused. See [Registration ownership](/developers/extend/apps/operations/publishing#registration-ownership).
</Note>
