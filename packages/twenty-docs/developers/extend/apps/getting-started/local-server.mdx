...
Data persists across restarts in three Docker volumes (`twenty-app-dev-data` for PostgreSQL, `twenty-app-dev-clickhouse` for ClickHouse logs and analytics, `twenty-app-dev-storage` for files). Use `reset` to wipe everything.
...
The test instance has its own container (`twenty-app-dev-test`), volumes (`twenty-app-dev-test-data`, `twenty-app-dev-test-clickhouse`, `twenty-app-dev-test-storage`), and config — it runs alongside your main instance without conflicts. Combine `--test` with `--port` to override 2021.
