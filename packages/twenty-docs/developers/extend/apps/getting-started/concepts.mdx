...
│   ├── workflows/               ← defineWorkflow (experimental)
...
| **Workflow (experimental)** | App-owned manual workflows composed of logic-function steps | [Workflows](/developers/extend/apps/logic/workflows) |
...
| **Shared Dependencies** | Dependencies shared by the app's front components, declared in package.json | [Shared Dependencies](/developers/extend/apps/layout/front-components#sharing-dependencies-across-front-components) |
| **Command Menu Item** | Quick actions and Cmd+K entries | [Command Menu Items](/developers/extend/apps/layout/command-menu-items) |
| **Settings Menu Item** | Entries in the app's settings menu, each rendering a front component | [Settings Menu Items](/developers/extend/apps/layout/front-components#settings-menu-items) |

## Sandboxing

- **Logic functions** run in isolated Node.js processes on the server. They only access data through the typed API client, which acts either as the person who triggered the run or as the app itself. See [Whose access a call uses](/developers/extend/apps/logic/logic-functions#whose-access-a-call-uses).
...
- **Permissions** are enforced at the API level. Two runtime tokens are injected, both derived from the role declared with `defineApplicationRole()`; the default one is narrowed further to what the triggering person can do whenever someone is behind the run.
