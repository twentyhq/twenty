...
`twenty build` compiles the catalogs and serves the right language for the
...
re-running `twenty build` (and redeploying), the same as any other change.

<Note>
Translations are compiled by `twenty build` (and `twenty dev --once`). The
