...
- Each `tab` defines a section of the page with a `title`, `position`, and `layoutMode` (`VERTICAL_LIST` for record pages, `GRID` for dashboards). A tab holding a single full-page widget renders full-bleed automatically; you no longer set a layout mode for that.
