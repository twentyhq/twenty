...
- `viewerControls` is optional and only supported in `TABLE` display mode. Set `filter` and `sort` to `true` to expose viewer controls for temporary filters and sorts; each defaults to `false`. These controls are session-local and do not change the backing view or page layout configuration. Sort controls are unavailable when the embedded view uses a calendar layout.
...
- `viewerControls` is optional. Set `filter` and `sort` to `true` to expose viewer controls for temporary filters and sorts; each defaults to `false`. These controls are session-local and do not change the backing view or page layout configuration. Sort controls are unavailable for calendar layouts.
