...
- `fieldDisplayMode` is one of `'FIELD'`, `'CARD'`, `'EDITOR'` or `'TABLE'`. `TABLE` embeds a view listing the records of a one-to-many relation field.
- `nestedRelationFieldMetadataId` is optional and takes the universal identifier of a one-to-many relation field on the relation target object, to list records two relation hops away (e.g. a Company page listing the opportunities of the company's people). Both hops must be one-to-many relation fields, and the widget renders as an embedded view.
