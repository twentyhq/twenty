...
- Each `tab` defines a section of the page with a `title`, `position`, and `layoutMode`: `VERTICAL_LIST` for record pages and standalone pages, `GRID` for dashboards, and `CANVAS` for a single widget that should fill the tab viewport. A `VERTICAL_LIST` tab stacks widgets vertically. Built-in widgets that manage their own scrolling, such as timelines, files, notes, tasks, and workflows, fill one viewport; fields, front components, graphs, and other fit-content widgets render at their content or configured height. A `GRID` tab always lays its widgets out as cards on a 12-column grid. A `CANVAS` widget has no explicit position; if a canvas tab contains multiple widgets, they render at their content height instead of filling the viewport.
...
A `FIELD` widget renders one field of the **current record**. For relation fields it can also embed a list of related records — the rows are always scoped to the record whose page you are on (e.g. this company's people). To embed an arbitrary view of an object, unscoped to the current record, use a [record table widget](#record-table-widgets) instead.
...
- `isUIEditable` is optional and defaults to `true`. Set it to `false` to make inline editing unavailable in the widget.
...
A `RECORD_TABLE` widget embeds a view of any object, on any layout surface — dashboards, record pages, and standalone pages. Despite the name, it renders the view in the view's own layout: a table view renders as a table, a kanban view as a board, and list and calendar views as their respective layouts.

The difference with a field widget in `TABLE` display mode: a field widget lists records **related to the current record** through a relation field, while a record table widget renders **a view**, independent of any record — which is why it is the widget you use on dashboards.
...
```ts
{
  ...
  configuration: {
    ...
    viewId: OPPORTUNITIES_VIEW_UNIVERSAL_IDENTIFIER,
    recordLimit: 8,
    isUIEditable: false,
  },
}
...
- `viewId` takes the universal identifier of the view whose records, fields, filters, sorts and layout the widget renders.
- `recordLimit` is optional and caps how many records the widget displays.
- `isUIEditable` is optional and controls inline record editing in the widget: cell editing on tables, and card editing, dragging between columns and per-column record creation on kanban boards. When omitted, it defaults to `true` on `RECORD_PAGE` layouts and `false` everywhere else. It only gates record data: the widget's view configuration (visible fields, filters, sorts, aggregates, column widths) is always edited through the page layout editor, never inline on the rendered widget.
