...
- `isWidgetContentEditable` is optional and defaults to `true`. It applies when `fieldDisplayMode` is `'TABLE'`; set it to `false` to make the embedded view read-only, with no inline editing, dragging between columns, or record creation.
