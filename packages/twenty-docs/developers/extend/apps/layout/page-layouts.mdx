...
- `yarn twenty pull` does not write `definePageLayoutWidget()` files yet. A widget on a tab your app does not own is carried by the export but reported rather than written, so keep pushing such a tree with `yarn twenty apply --no-delete`; see [what pull does not write](/developers/extend/apps/operations/sync-and-recovery#pulling-an-installed-app-back-to-source).
