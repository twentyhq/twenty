...
- Widgets are ordered by their declaration order in the `widgets` array, which install turns into a `position` matching the tab `layoutMode` (`{ layoutMode: 'VERTICAL_LIST', index }` for vertical lists, the widget `gridPosition` for grids). Set `position` on a widget explicitly if you need to override that, keeping its `layoutMode` in sync with the tab's.
