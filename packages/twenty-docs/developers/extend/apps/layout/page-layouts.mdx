...
- Widgets are ordered by their declaration order in the `widgets` array. `yarn twenty dev:build` resolves that into an explicit `position` matching the tab `layoutMode` (`{ layoutMode: 'VERTICAL_LIST', index }` for vertical lists, the widget `gridPosition` for grids), so the built manifest states where every widget goes. Set `position` on a widget yourself if you need to override that, keeping its `layoutMode` in sync with the tab's.
