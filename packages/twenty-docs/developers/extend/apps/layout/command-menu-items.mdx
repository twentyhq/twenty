...
| `availabilityType` | No | Controls where the command appears — a `CommandMenuItemAvailabilityType` enum value: `GLOBAL` (always available), `GLOBAL_OBJECT_CONTEXT` (only on pages with an object context — index and record pages), `RECORD_SELECTION` (only when records are selected), or `FALLBACK` (shown when no other commands match) |
