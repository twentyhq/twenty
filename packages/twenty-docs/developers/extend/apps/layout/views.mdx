...
| `openRecordIn` | `ViewOpenRecordIn.USER_PREFERENCE` (default), `ViewOpenRecordIn.SIDE_PANEL`, `ViewOpenRecordIn.RECORD_PAGE` | Where clicking a record opens it. `USER_PREFERENCE` defers to each workspace member's own setting. The default is `RECORD_PAGE` for objects that need a full page to be usable (workflows, workflow versions, workflow runs, dashboards, message campaigns) and `USER_PREFERENCE` everywhere else. |
