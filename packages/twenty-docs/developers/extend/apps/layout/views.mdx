...
| `openRecordIn` | `ViewOpenRecordIn.USER_PREFERENCE` (default), `ViewOpenRecordIn.SIDE_PANEL`, `ViewOpenRecordIn.RECORD_PAGE` | Where clicking a record opens it. `USER_PREFERENCE` defers to each workspace member's own setting. Omit it and the view inherits the object's [`defaultOpenRecordIn`](/developers/extend/apps/data/objects), which is `USER_PREFERENCE` unless the object declares otherwise. |
