...
| `toggleMineFilterFieldMetadataUniversalIdentifier` | Actor field or workspace member relation | All/Mine button that toggles a "Me" filter on this field. |
...
| `IS` "Me" on workspace member `RELATION` / `ACTOR` | object | `{ isCurrentWorkspaceMemberSelected: true, selectedRecordIds: [] }` |
