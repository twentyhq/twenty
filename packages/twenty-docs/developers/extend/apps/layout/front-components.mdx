...
> **Calling a route from outside Twenty (webhooks, third parties)?** On Twenty Cloud, HTTP-triggered logic functions are also served from a dedicated, isolated domain at `https://<your-workspace-subdomain>.withtwenty.com<path>` — with no `/s/` prefix. Because that origin is isolated from your workspace, functions served there may return arbitrary response headers (custom headers, `Permissions-Policy`, `Cross-Origin-Opener-Policy`, `Cross-Origin-Embedder-Policy`, ...). You can copy the exact URL from **Settings → Functions → your function → HTTP trigger**, or from the application's **Settings** tab. The `/s/` route shown above keeps working for self-hosting and for authenticated calls — including the front-component `RestApiClient` calls below, which send the app access token. On Twenty Cloud, only **public** (`isAuthRequired: false`) routes created after the cutoff date stop being served on `/s/` (they return `410 Gone`); point external callers at the `withtwenty.com` URL.
