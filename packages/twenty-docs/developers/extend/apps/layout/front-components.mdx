...
Use `useSelectedObjectMetadata()` to identify the object associated with a record-page widget, a front component opened in the side panel with record context, or an object-scoped command. This lets an integration select templates or other configuration for the current object without searching across objects for a record ID.
