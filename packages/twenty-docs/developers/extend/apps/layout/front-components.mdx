...
- **`CommandOpenSidePanelPage`** — Opens content in the side panel. Workspace routes use the same typed `to`, `params`, and `queryParams` props as `CommandLink`, plus an optional `hash`; purpose-built panel tools use their own page-specific props.
