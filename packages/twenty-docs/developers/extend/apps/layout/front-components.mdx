...
| `uploadFile(file, { fieldMetadataId, fileName? })` | Upload a `Blob` into a FILES field as the application acting for the viewer: the application's role needs `UPLOAD_FILE` and update access to the object owning the field, on top of the viewer's own access. Resolves to `{ status: 'uploaded', file: { fileId, path, url, size, mimeType } }` or `{ status: 'failed', reason: 'invalid-params' \| 'upload-failed' }` |
