...
| `document.activeElement` | Follows the page's focus inside the component; `focus()` / `blur()` calls made inside the sandbox move it until the page's focus changes again | `onFocus` / `onBlur` to react to the page's focus |
