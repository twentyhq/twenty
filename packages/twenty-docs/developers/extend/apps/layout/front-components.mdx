...
<Note>
Declaring and syncing menu items works today, and the rows they create are what the rendering builds on. The settings menu itself does not display app-declared items yet — it still shows the single tab from the deprecated `defineSettingsFrontComponent()`. Until that follow-up lands, an item you declare is persisted but not visible.
</Note>
...
- **`WORKSPACE`** (default) — configured once for the whole workspace, alongside the other workspace settings.
