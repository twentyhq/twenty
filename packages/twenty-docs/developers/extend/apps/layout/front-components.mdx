...
A logic function declared with `httpRouteTriggerSettings` is reachable over HTTP at its route path. Your front component calls it with the `RestApiClient` from `twenty-client-sdk/rest`, which authenticates with the `TWENTY_APP_ACCESS_TOKEN` that Twenty injects into the worker.

The `RestApiClient` is built for exactly this. It reads `TWENTY_FUNCTIONS_URL` (the base your functions are served from) and `TWENTY_APP_ACCESS_TOKEN` from the worker environment, attaches the `Authorization: Bearer` header, serializes and parses JSON, and throws a `RestApiClientError` when the token or URL is missing or the response is non-2xx — so you don't reimplement that boilerplate in every component.

> **On Twenty Cloud, HTTP-triggered logic functions are served on a dedicated domain** at `https://<your-workspace-subdomain>.withtwenty.com<path>`. `RestApiClient` uses it automatically, so the code below is unchanged. For external callers, copy the exact URL from the function's **HTTP trigger** settings or the application's **Settings** tab — on Twenty Cloud new routes must use it, as the legacy `/s/` route returns `410 Gone` (it stays only for self-hosting).
...
const SyncPrs = () => {
  const execute = async () => {
    ...
    await client.post('/github/fetch-prs', {
      owner: 'twentyhq',
      repo: 'twenty',
    });
  };
...
The path passed to the client is the logic function's `httpRouteTriggerSettings.path`. Keep `isAuthRequired: true`; the client supplies the app access token Twenty mints for your component:
...
The function base URL and token are resolved from the environment by default. Pass overrides to the constructor when needed — for example in tests, or `workspaceUrl` for the rare case of calling the core REST API instead of your functions:
...
try {
  const prs = await client.get('/github/fetch-prs', {
    query: { state: 'open' },
  });
} catch (error) {
