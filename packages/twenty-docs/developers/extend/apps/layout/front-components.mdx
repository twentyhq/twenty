...
| `window.matchMedia()` | Works for `width`, `height`, `orientation`, `device-pixel-ratio`, `resolution` and `prefers-color-scheme`, with `change` events; widths and heights describe **your widget's box**, not the browser viewport, so they follow `document.body.clientWidth` rather than `window.innerWidth`. Any other feature evaluates to `false` |
