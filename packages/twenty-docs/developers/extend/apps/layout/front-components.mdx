...
Application variables defined in [`defineApplication()`](/developers/extend/apps/config/application) with `isSecret: false` are automatically available as `process.env.VARIABLE_NAME` inside front components:

```tsx src/front-components/greeting.tsx
import { defineFrontComponent } from 'twenty-sdk/define';

const Greeting = () => {
  const recipientName = process.env.DEFAULT_RECIPIENT_NAME ?? 'World';
