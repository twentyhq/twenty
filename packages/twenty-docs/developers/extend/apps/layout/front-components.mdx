...
> `TWENTY_FUNCTIONS_URL` resolves to your app's custom domain if one is configured, a dedicated per-workspace domain on Twenty Cloud (`https://<your-workspace-subdomain>.withtwenty.com`), or `<server-url>/s` on self-hosted instances. For external callers, copy the exact URL from the function's **HTTP trigger** settings or the application's **Settings** tab.
