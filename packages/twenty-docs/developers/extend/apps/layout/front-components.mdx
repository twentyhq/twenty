...
- **App settings** — Pointed at by a [`defineSettingPage()`](#settings-pages) declaration, the front component renders one of the app's settings pages. An app can declare several.
...
- **Point a [`defineSettingPage()`](#settings-pages) at it** — renders it as one of the app's settings pages.
...
An app can add its own pages to its settings, each rendering a front component. Declare a page with `defineSettingPage()` in its own file, pointing at the component that renders it:

```tsx src/setting-pages/sync-settings.ts
import { defineSettingPage } from 'twenty-sdk/define';
...
The page points at the front component rather than the component declaring itself a settings page. One component can therefore back several pages, and a page can be repointed at a different component without touching either.

Declare as many pages as you need. Twenty's own **General** page — auto-upgrade, App URL and connections — always comes first and cannot be overridden by an app; App URL only shows with **Advanced mode** on.
...
| `frontComponentUniversalIdentifier` | The `universalIdentifier` of the front component this page renders |
| `title` | The page's label. `General` is reserved and fails the build |
...
`position` is a decimal, so a page can be slotted between two existing ones without renumbering. Pages sort by ascending `position`, ties broken on `universalIdentifier` so the order is the same in every workspace. Two pages of one app cannot share a position in the same scope — the sync refuses it rather than letting install order decide which renders first.

### Workspace and user scope

`scope` says who a page is configured by:
...
A `USER` page runs as the member viewing it. Note that `applicationVariables` are workspace-wide: one value shared by everyone. A page that stores something per person should persist it through the app's own objects rather than a variable, otherwise one member's value overwrites another's.

<Note>
`defineSettingsFrontComponent()` is deprecated in favor of `defineSettingPage()`. An app still using it keeps working — its component is treated as a single **Settings** page — but it can only ever declare one, which is what `defineSettingPage()` exists to lift.
</Note>
...
`getApplicationVariable` returns the values injected when the component loaded, so a value written afterwards only shows up on the next load. To read the stored values, or to write one (what a [settings page](#settings-pages) does when it saves what the user typed), go through `MetadataApiClient`:
