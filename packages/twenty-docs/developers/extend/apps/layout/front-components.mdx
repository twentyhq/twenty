...
| `recordAudio(params)` | Record audio with the device microphone and upload the result |
| `recordVideo(params)` | Record video with the device camera and microphone and upload the result |
| `captureMedia(params)` | Generic form of the two functions above (`mediaType: 'audio' \| 'video'`) |

### Recording audio or video

`recordAudio` and `recordVideo` open a host-owned recorder: the user consents, records, reviews the result, and only then is the file uploaded. The component never touches the raw media stream.
...
```tsx
import { recordAudio } from 'twenty-sdk/front-component';
...
if (result.status === 'captured') {
  ...
  await new CoreApiClient().mutation({
    createMediaNote: {
      __args: {
        data: { recording: [{ fileId: result.file.fileId, name: result.file.path }] },
      },
      id: true,
    },
  });
...
The result is `captured`, `cancelled`, or `failed` with a reason (`permission-denied`, `no-device`, `device-in-use`, `blocked`, `busy`, `upload-failed`, or `unknown`).
