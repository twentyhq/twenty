...
If one of these blocks you, say so on the [Twenty Discord](https://discord.com/channels/1130383047699738754/1130386664812982322) so it gets prioritized.
...
Only `id`, `className`, `style`, `title`, `tabIndex`, `role`, `draggable` and any `aria-*` / `data-*` attribute reach the host DOM (hyphenated, so `ariaLabel` is dropped). Everything else is silently discarded, so express custom state as `data-*`.
...
- Calls to the Twenty API and your app's routes are proxied by the host, so prefer [`RestApiClient`](#calling-the-twenty-rest-api). On proxied calls, `AbortSignal` and the other `RequestInit` options are dropped, and only string bodies are supported.
