...
| `uploadFile(file, { fieldMetadataId, fileName? })` | Upload a `Blob` into a FILES field with the application's own permissions: its role needs `UPLOAD_FILE` and update access to the object owning the field. Resolves to `{ status: 'uploaded', file: { fileId, path, url, size, mimeType } }` or `{ status: 'failed', reason: 'invalid-params' \| 'upload-failed' }` |
