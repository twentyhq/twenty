...
> **On Twenty Cloud these calls go to an isolated domain automatically.** HTTP-triggered logic functions are served from a dedicated, cookieless domain at `https://<your-workspace-subdomain>.withtwenty.com<path>` (no `/s/` prefix). `RestApiClient` detects this and transparently routes the `/s/...` calls below to that domain, so your front-component code doesn't change. Because the origin is isolated from your workspace, functions served there may return arbitrary response headers (custom headers, `Permissions-Policy`, `Cross-Origin-Opener-Policy`, `Cross-Origin-Embedder-Policy`, ...). For external callers (webhooks, third parties), copy the exact URL from **Settings → Functions → your function → HTTP trigger**, or from the application's **Settings** tab. The legacy `/s/` route still serves self-hosted instances; on Twenty Cloud, routes created after the cutoff date are no longer served on `/s/` (they return `410 Gone`) and must use the `withtwenty.com` URL.
