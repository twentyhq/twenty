...
- **`CommandOpenSidePanelPage`** — Opens content in the side panel. Use `RoutedPage` with a canonical in-app `path` for records, views, and settings pages; purpose-built panel tools use their own page-specific props.
...
const OpenCompanyEmails = () => {
  ...
  return (
    <CommandOpenSidePanelPage
      ...
      path={`/object/company/${recordId}#company-tab-emails`}
