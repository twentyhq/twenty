...
- **App settings** — When referenced via `settingsCustomTabFrontComponentUniversalIdentifier` in [`defineApplication()`](/developers/extend/apps/config/application), the front component renders as the app's **Settings** tab content, replacing the default variable configuration UI.
...
- **Render it as the app's settings** — set `settingsCustomTabFrontComponentUniversalIdentifier` in [`defineApplication()`](/developers/extend/apps/config/application) to render it as the app's **Settings** tab content, replacing the default variable configuration UI.
