...
| `captureMedia(params)` | Record audio or video with the device microphone or camera and upload the result |

### Capturing audio or video

`captureMedia({ mediaType: 'audio' | 'video', maxDurationSeconds?, fieldMetadataId? })` opens a host-owned recorder: the user consents, records, reviews the result, and only then is the file uploaded. The component never touches the raw media stream.

```tsx
import { captureMedia } from 'twenty-sdk/front-component';

const result = await captureMedia({ mediaType: 'audio', maxDurationSeconds: 120 });
