...
- **App settings banner** — Defined with [`defineSettingsBanner()`](#settings-banner), a headless front component that drives a banner shown at the top of the app's settings page.
...
- **Define it with [`defineSettingsBanner()`](#settings-banner)** — drives a banner at the top of the app's settings page.
...
The component is the whole content of a dedicated **Settings** tab, and it replaces the auto-generated **Variables** tab: your app is expected to render its own variables. Twenty's system-managed sections — auto-upgrade, App URL, and connections — live in the app's **General** tab and cannot be overridden by the app.
...
To guide users through configuring your app, define a settings banner with `defineSettingsBanner`. It takes the same [configuration fields](#configuration-fields) as `defineFrontComponent` (except `isHeadless`, which is always `true`).

The component mounts as soon as the user opens the app's settings page, on any tab, and renders nothing itself. Through the `useSettingsBanner` hook it decides whether the banner is shown, what it says, and what happens when the action button is clicked.

```tsx src/front-components/settings-banner.tsx
import { defineSettingsBanner } from 'twenty-sdk/define';
import {
  enqueueSnackbar,
...
const SettingsBanner = () => {
  const apiKey = getApplicationVariable('API_KEY');
    ...
    apiKey
      ...
      : {
          ...
          action: {
            ...
            onClick: () =>
              enqueueSnackbar({
                message: 'Add your API key in the Variables tab',
                variant: 'info',
              }),
          },
        },
...
export default defineSettingsBanner({
  universalIdentifier: 'f3b6a4d2-9c81-4e0a-b5c7-2d1e8f4a6b03',
  name: 'settings-banner',
  description: 'Tells the user what is left to configure',
  component: SettingsBanner,
});
```

`useSettingsBanner` takes the banner to display, or `null` to hide it. The banner is also hidden when the user leaves the settings page. `onClick` runs inside your component, so it can call any front-component SDK function — open a side panel page, navigate, or hit one of your logic functions.

| Field | Required | Description |
| --- | --- | --- |
| `variant` | No | `info` (blue), `warning` (orange) or `error` (red). Defaults to `info` |
| `text` | Yes | The message shown in the banner |
| `action` | No | `{ label, onClick }` for the banner's action button |

Only one settings banner is allowed per app; declaring more than one fails the build.
