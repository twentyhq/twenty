...
| `MutationObserver` | Works, including `subtree`, `attributeFilter`, old values and `takeRecords()`, with one gap noted below |
...
The one `MutationObserver` gap: a `class` write on an element you rendered reaches the page through the `className` property rather than the attribute, so it never produces a record. Watch your own state instead of observing the attribute.
