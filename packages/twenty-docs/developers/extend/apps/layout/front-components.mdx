...
The result is `captured`, `cancelled`, or `failed` with a reason (`invalid-params`, `permission-denied`, `no-device`, `device-in-use`, `blocked`, `busy`, `upload-failed`, or `unknown`).
