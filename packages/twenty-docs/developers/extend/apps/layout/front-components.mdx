...
const result = await recordAudio({ maxDurationSeconds: 120 });
...
The result is `captured`, `cancelled`, or `failed` with a reason (`permission-denied`, `no-device`, `device-in-use`, `blocked`, `busy`, `upload-failed`, or `unknown`). Pass a `fieldMetadataId` pointing at a FILES field to upload the recording where it can be attached to records; otherwise it lands in the generic `media-capture` folder.
