...
> **On Twenty Cloud, HTTP-triggered logic functions are also available on a dedicated domain** at `https://<your-workspace-subdomain>.withtwenty.com<path>`. `RestApiClient` uses it automatically, so the code below is unchanged. For external callers, copy the exact URL from the function's **HTTP trigger** settings or the application's **Settings** tab.
