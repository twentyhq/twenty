...
| `window.matchMedia()` | Works for `width`, `height`, `orientation`, `device-pixel-ratio`, `resolution` and `prefers-color-scheme`, in the `min-`/`max-` form, the Level 4 range form (`(width >= 600px)`, `(400px <= width <= 800px)`) and the valueless form (`(width)`), with `change` events. Widths, heights and `orientation` describe **your widget's box**, not the browser viewport, so they follow `document.body.clientWidth` rather than `window.innerWidth`; `em` and `rem` count as 16px. Any other feature makes the whole query (or comma-separated member) evaluate to `false`, even under `not`, the same way browsers treat unknown features |
...
Because that stylesheet reaches the real page, a `@media (min-width: ...)` rule is evaluated by the host against the browser viewport, while `matchMedia('(min-width: ...)')` answers for your widget's box. Use `@container` when you want CSS to follow the same box your JavaScript sees.
