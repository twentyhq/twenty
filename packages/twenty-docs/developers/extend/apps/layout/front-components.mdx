...
- **Custom settings tab** — When referenced via `settingsCustomTabFrontComponentUniversalIdentifier` in [`defineApplication()`](/developers/extend/apps/config/application), the front component renders inside the app's settings page as a **Custom** tab.
...
- **Wire it as a custom settings tab** — set `settingsCustomTabFrontComponentUniversalIdentifier` in [`defineApplication()`](/developers/extend/apps/config/application) to render it as a **Custom** tab on the app's settings page.
