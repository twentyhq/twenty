...
| `scope` *(optional)* | `WORKSPACE` (default) or `WORKSPACE_MEMBER` |
...
### Workspace and workspace member scope
...
- **`WORKSPACE_MEMBER`** — configured by each member for themselves, under their own user settings.

A `WORKSPACE_MEMBER` page runs as the member viewing it. Note that `applicationVariables` are workspace-wide: one value shared by everyone. A page that stores something per person should persist it through the app's own objects rather than a variable, otherwise one member's value overwrites another's.

<Note>
`defineSettingsFrontComponent()` is deprecated in favor of `defineSettingPage()`. An app still using it keeps working — its component is treated as a single **Variables** page — but it can only ever declare one, which is what `defineSettingPage()` exists to lift.
</Note>
