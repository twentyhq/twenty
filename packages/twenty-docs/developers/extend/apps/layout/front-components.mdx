...
It always acts as the person looking at the page. `runAs: 'application'` and `runAs: { workspaceMemberId }` are logic-function options only: a component never receives your application's own token, so asking for either here throws. Put work that needs the app's own access behind a logic function and call that instead.
