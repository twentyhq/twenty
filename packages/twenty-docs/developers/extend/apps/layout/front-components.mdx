...
Older releases that still export `CodeEditor` from `twenty-ui/primitives/input` require `monaco-editor` at build time, even when the editor is unused. For those versions, run `yarn add -D monaco-editor`.
