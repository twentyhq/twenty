...
<Note>
`requestAnimationFrame`, `fetch`, `setTimeout` and `queueMicrotask` work, with or without the `window.` prefix, and so do `performance` and `requestIdleCallback` (backed by a timer, so the idle deadline is approximate).
</Note>
...
`CSS.escape()` works. `CSS.supports()` answers truthfully for a short allowlist of common layout properties (`display`, `position`, `flex-wrap`, ...) and returns `false` for everything else, even values the browser supports, so write the CSS directly instead of feature-detecting it.
