...
`getApplicationVariable` always returns a **string** (or `undefined`), regardless of the variable's declared `type`. The `type` only drives the settings-UI input and build-time validation — parse the string yourself (`Number(...)`, `JSON.parse(...)`, `=== 'true'`). See [Variable types](/developers/extend/apps/config/application#variable-types).
