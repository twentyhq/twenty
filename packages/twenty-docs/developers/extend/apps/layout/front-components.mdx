...
<Note>
`requestAnimationFrame`, `fetch`, `setTimeout` and `queueMicrotask` work, with or without the `window.` prefix, and so do `console`, `crypto`, `performance`, `structuredClone`, `btoa`/`atob`, `URL`, `AbortController`, `Headers`/`Request`/`Response`, `Blob`, `FormData` and `TextEncoder`/`TextDecoder`.
