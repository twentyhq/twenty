...
| `localStorage.get(key)` | Read a device-local value |
| `localStorage.set(key, value)` | Write a device-local value |
| `localStorage.delete(key)` | Delete a device-local value |
| `localStorage.keys()` | List the keys your app has stored |
| `localStorage.clear()` | Delete every value your app has stored |
...
`localStorage` from `twenty-sdk/front-component` stores small JSON values on the current device, scoped to your app install and the signed-in user. Nobody else's app can read your keys, and another user signing into the same browser starts from an empty store. Nothing is synced: the values do not follow the user to another browser or machine, so use it for UI state, drafts and preferences, and use a logic function's [key-value store](/developers/extend/apps/logic/key-value-store) for anything that must survive a device change.

Every method is async, and values go through `JSON.stringify` / `JSON.parse`:
...
const NoteDraft = () => {
  ...
  useEffect(() => {
    localStorage.get<string>('draft').then((storedDraft) => {
      if (storedDraft !== null) {
  ...
  const handleChange = (nextDraft: string) => {
    setDraft(nextDraft);
    localStorage.set('draft', nextDraft);
  };
...
Writes are capped, and every limit counts characters rather than bytes: keys are at most 512 characters, a single serialized value at most 262,144 characters, and your app's whole store at most 1,048,576 characters per user. A write that breaks a limit rejects with an error whose `code` is one of `FRONT_COMPONENT_STORAGE_KEY_TOO_LONG`, `FRONT_COMPONENT_STORAGE_VALUE_TOO_LARGE`, `FRONT_COMPONENT_STORAGE_QUOTA_EXCEEDED` or `FRONT_COMPONENT_STORAGE_UNAVAILABLE`. Use `keys()` and `delete()` to make room.

The global `window.localStorage` is backed by the same store, so a dependency that reads or writes it keeps working and its data persists too. It stays synchronous like the browser API, which means a write is applied locally right away and saved in the background; use the SDK methods when you need to know the write reached the host. Because that store is shared, `get()` returns the raw string for a value a dependency wrote as plain text instead of JSON.

`window.sessionStorage` is also available, but it stays in memory for the lifetime of the component and is never written to the device.
...
`localStorage` is provided by Twenty rather than the browser: the component runs in a worker at an opaque origin, so the host stores the values on your app's behalf. See [device-local storage](#device-local-storage) for the API, its scoping and its limits. `sessionStorage` is a shim that keeps values in memory for the lifetime of the component and never persists them. IndexedDB, cookies, the Cache API and `BroadcastChannel` remain unavailable. To persist state across devices, call a [logic function](/developers/extend/apps/logic/logic-functions) and use its [key-value store](/developers/extend/apps/logic/key-value-store).
