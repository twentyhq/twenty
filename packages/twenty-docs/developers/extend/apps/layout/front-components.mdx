...
- **App settings** — Defined with [`defineSettingsFrontComponent()`](#custom-settings-tabs), the front component renders as its own tab in the app's settings page.
...
- **Define it with [`defineSettingsFrontComponent()`](#custom-settings-tabs)** — renders it as its own tab in the app's settings page.
...
Your app's settings page opens on a **General** tab holding the settings every app shares — auto-upgrade, App URL and connections. Those are system-managed and an app cannot override them. App URL only shows with **Advanced mode** on.

Every other tab comes from your app. To add one, define a front component with `defineSettingsFrontComponent` instead of `defineFrontComponent`. It takes the same [configuration fields](#configuration-fields) (except `isHeadless`, which is not accepted since a settings tab always renders visible UI), plus an optional `tab` object describing how the tab is presented.
...
export default defineSettingsFrontComponent({
  ...
  name: 'app-settings',
  description: "Custom UI for the app's settings page",
...
| Field | Required | Description |
|-------|----------|-------------|
| `tab.label` | No | Tab title. `General` is reserved for the built-in tab and fails the build |
| `tab.icon` | No | Icon name, for example `IconRefresh`. An unknown name falls back to a default icon |
| `tab.position` | No | Sort key among your app's tabs; may be negative |

Omitting `tab` entirely gives the tab the generated **Variables** tab's own presentation — label `Variables`, icon `IconVariable`, position `0` — since a single settings tab stands in for that tab rather than sitting beside it.

### Ordering

Declare as many settings tabs as you need. They render after **General**, sorted by ascending `position`; tabs sharing a position fall back to their `universalIdentifier` in alphabetical order, so the order is stable across syncs.

Once you declare more than one, each has to carry its own `tab` — the build fails otherwise, since two components falling back to the same default would render as identically labelled tabs on the same position.

### Application variables

An app that declares no settings tab of its own gets a generated **Variables** tab holding its [application variables](/developers/extend/apps/config/application). An app with no variables either shows only the **General** tab content, with no tab bar at all.

A settings tab lays out its own configuration UI, so declaring one replaces that generated tab — read and write your variables from your component instead.
