...
- **App settings** — Defined with [`defineSettingsTabFrontComponent()`](#custom-settings-tab), the front component renders as the app's **Settings** tab content, replacing the default variable configuration UI.
...
- **Define it with [`defineSettingsTabFrontComponent()`](#custom-settings-tab)** — renders it as the app's **Settings** tab content, replacing the default variable configuration UI.
...
To replace an app's default **Settings** tab (the auto-generated variable configuration UI) with your own component, define it with `defineSettingsTabFrontComponent` instead of `defineFrontComponent`. It takes the same [configuration fields](#configuration-fields) (except `isHeadless`, which is not accepted since a settings tab always renders visible UI) and additionally marks the component as the app's settings tab.

```tsx src/front-components/app-settings.tsx
import { defineSettingsTabFrontComponent } from 'twenty-sdk/define';
...
export default defineSettingsTabFrontComponent({
...
Only one settings tab front component is allowed per app; declaring more than one fails the build. When present, the app's **Settings** tab renders this component instead of the default variable configuration UI.
