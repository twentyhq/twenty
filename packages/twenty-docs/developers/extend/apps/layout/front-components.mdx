...
| `MutationObserver` | Works, including `subtree`, `attributeFilter`, old values and `takeRecords()`; `class` changes on elements you rendered are the one exception, they never produce a record |
...
A `class` write on an element you rendered reaches the page through the `className` property rather than the attribute, so it never shows up in a `MutationObserver` record. Watch your own state instead of observing the attribute.
