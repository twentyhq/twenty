...
<Note>
`requestAnimationFrame` and `cancelAnimationFrame` work, with or without the `window.` prefix. Only Chrome runs a real animation frame inside a worker; Firefox and Safari fall back to a 16 ms timer, so callbacks are not aligned to the display refresh and have no ordering relationship with the host's geometry push. Use it to defer a re-read past a write, not to drive a smooth animation, and prefer CSS transitions, which run on the host.
</Note>
...
`requestIdleCallback` and `cancelIdleCallback` are backed by a timer rather than real idle detection: the callback runs about a frame later and its deadline advertises a fixed 5 ms budget, against 50 ms natively, so treat it as "soon", not as "when the thread is free". Each callback gets its own budget, so `timeRemaining()` never winds down across an idle period. `timeout` is not a deadline: it can only pull the callback earlier than that frame, and `didTimeout` is true only when the callback really did run at or after the timeout you asked for. As in the spec, `timeout: 0` means "no timeout".
...
`CSS.supports()` is a stub: it answers `true` for custom properties and for a short table of exact property/value pairs (`display: flex`, `position: sticky`, `container-type: inline-size`, ...), so `display: ruby` is `false` even though `display: flex` is `true`. Everything else returns `false`, including values the browser supports (`gap`, `aspect-ratio`) and any condition combining `and`, `or` or `not`, so write the CSS directly instead of feature-detecting it. `CSS.escape()` follows the spec, but the sandbox's own `querySelector` does not understand the escapes it emits, so use it for stylesheet text you hand to the host, not to build a selector you then query.
