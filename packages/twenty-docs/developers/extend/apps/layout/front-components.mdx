...
By default, every front component bundles its own copy of the libraries it imports, so an app with five components ships React five times. Declare shared dependencies to build those libraries once and have every component of the app load them from a single cached file:

```ts src/front-component-shared-dependencies.ts
