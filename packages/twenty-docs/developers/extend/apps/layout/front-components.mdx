...
<Note>
`requestAnimationFrame` works, with or without the `window.` prefix.
</Note>
...
`requestIdleCallback` is backed by a timer rather than real idle detection: the callback runs almost immediately and its deadline advertises a small fixed budget, so treat it as "soon", not as "when the thread is free". A `timeout` option is honoured.
...
`CSS.escape()` works, and `CSS.supports()` answers `true` for custom properties and for a short allowlist of common layout properties (`display`, `position`, `flex-wrap`, ...). Everything else returns `false`, including values the browser supports and any condition combining `and`, `or` or `not`, so write the CSS directly instead of feature-detecting it.
...
- **Node built-ins.** `fs`, `path` and `node:crypto` fail the build, so move that work into a [logic function](/developers/extend/apps/logic/logic-functions). Their web equivalents are available, see [global scope](#global-scope).
