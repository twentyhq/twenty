...
Declare `microphone`, `camera`, or both in the `requestedCapabilities` array passed to `defineApplication()` before using `navigator.mediaDevices.getUserMedia()`. Users approve these capabilities when installing the app. Twenty does not ask again for each capture, but browser and operating system permissions still apply. App updates cannot add media capabilities beyond those approved at installation. An orange dot in the workspace header indicates active capture. Hover over or select the dot to see the apps using the microphone or camera and stop their capture. Pending requests appear as an outlined dot with a cancellation control. A missing capability rejects the request before the browser or operating system is asked for permission.
