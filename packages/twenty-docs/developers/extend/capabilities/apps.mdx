...
- Build front components with custom UI, actions, and command menu items
- Define views, navigation menu items, and page layouts
...
| `definePageLayout()` | Page layout definitions |
...
| `definePageLayout()` | Define page layouts with tabs and widgets |
...
Each function file uses `defineLogicFunction()` to export a configuration with a handler and optional trigger settings.
...
export default defineLogicFunction({
  ...
  timeoutSeconds: 2,
  handler,
  // HTTP route trigger — exposes function at /s/post-card/create
...
Here is an example with a cron trigger:
...
And with a database event trigger:

```typescript
export default defineLogicFunction({
  universalIdentifier: '203f1df3-4a82-4d06-a001-b8cf22a31156',
  name: 'on-person-updated',
  handler: personUpdatedHandler,
...
Trigger types:
- **`httpRouteTriggerSettings`**: Exposes your function on an HTTP path and method **under the `/s/` endpoint**:
> e.g. `path: '/post-card/create'` → call on `<APP_URL>/s/post-card/create`
- **`cronTriggerSettings`**: Runs your function on a schedule using a CRON expression.
- **`databaseEventTriggerSettings`**: Runs on workspace object lifecycle events. When the event operation is `updated`, specific fields to listen to can be specified in the `updatedFields` array. If left undefined or empty, any update will trigger the function.
> e.g. `eventName: 'person.updated'`

Notes:
- All trigger settings are optional. Functions without triggers can be used as utility functions called by other functions.
- Each trigger type is a separate property on the config. A function can have at most one trigger of each type.
...
```bash filename="Terminal"
yarn twenty function:execute -n pre-install
```
...
- The function's `universalIdentifier` is automatically set as `preInstallLogicFunctionUniversalIdentifier` on the application manifest during the build. You can also set it explicitly in `defineApplication()` via the `preInstallLogicFunctionUniversalIdentifier` property.
- The default timeout is set to 300 seconds (5 minutes) to allow for longer preparation tasks.
- Pre-install functions do not need triggers — they are invoked by the platform before installation or manually via `function:execute -n <name>`.
...
- The function's `universalIdentifier` is automatically set as `postInstallLogicFunctionUniversalIdentifier` on the application manifest during the build — you do not need to set it in `defineApplication()`.
...
- You can combine `isTool` with trigger settings — a function can be both a tool (callable by AI agents) and triggered by events (cron, database events, routes) at the same time.
...
Views define saved, reusable ways to display records from a specific object. Use `defineView()` to define views with built-in validation:

```typescript
// src/views/all-post-cards.view.ts
import { defineView } from 'twenty-sdk';
import { POST_CARD_UNIVERSAL_IDENTIFIER, CONTENT_FIELD_UNIVERSAL_IDENTIFIER } from 'src/objects/post-card.object';
...
Key points:
- `objectUniversalIdentifier` links the view to the object it displays.
- `fields` controls which columns are visible and their order.
- `filters`, `filterGroups`, `groups`, and `fieldGroups` are optional for advanced filtering and grouping.
- Views are typically paired with a navigation menu item so they appear in the sidebar.

### Standalone fields

Standalone fields let you extend an existing object (custom or standard) with additional fields, without modifying the object's definition file. Use `defineField()` to define standalone fields:

```typescript
// src/fields/postCardNumber.field.ts
...
You can also define relation fields to link objects together:

```typescript
// src/fields/recipient-relation.field.ts
import { defineField, FieldType, RelationType, OnDeleteAction } from 'twenty-sdk';

export default defineField({
  universalIdentifier: 'a1b2c3d4-e5f6-7890-abcd-ef1234567890',
  objectUniversalIdentifier: '<source-object-uuid>',
  type: FieldType.RELATION,
  name: 'recipient',
  label: 'Recipient',
  relationTargetObjectMetadataUniversalIdentifier: '<target-object-uuid>',
  relationTargetFieldMetadataUniversalIdentifier: '<inverse-field-uuid>',
  universalSettings: {
...
Key points:
- `objectUniversalIdentifier` specifies which object this field extends.
- Standalone fields are useful for extending standard objects or keeping field definitions separate from the object file.
- The SDK exports `STANDARD_OBJECT_UNIVERSAL_IDENTIFIERS` (aliased as `STANDARD_OBJECT`) with universal identifiers for all built-in objects and their fields, so you can reference them when extending standard objects.

### Navigation menu items

Navigation menu items add entries to the workspace sidebar. Use `defineNavigationMenuItem()` to define sidebar links:

```typescript
// src/navigation-menu-items/post-cards.navigation-menu-item.ts
import { defineNavigationMenuItem } from 'twenty-sdk';
import { ALL_POST_CARDS_VIEW_ID } from 'src/views/all-post-cards.view';
...
Key points:
- `viewUniversalIdentifier` links the menu item to a specific view.
- `position` controls the ordering in the sidebar.
- Optional properties: `name`, `icon`, `link` (for external URLs), `targetObjectUniversalIdentifier`.
- Without a navigation menu item, your views won't appear in the sidebar — users will have no way to navigate to them.

### Page layouts

Page layouts define the layout structure for record detail pages. Use `definePageLayout()` to define layouts with tabs and widgets:

```typescript
// src/page-layouts/post-card-detail.page-layout.ts
import { definePageLayout } from 'twenty-sdk';
import { POST_CARD_UNIVERSAL_IDENTIFIER } from 'src/objects/post-card.object';

export default definePageLayout({
  universalIdentifier: 'd1e2f3a4-b5c6-7d8e-9f0a-1b2c3d4e5f6a',
  name: 'Post Card Detail',
  objectUniversalIdentifier: POST_CARD_UNIVERSAL_IDENTIFIER,
  tabs: [
    {
      universalIdentifier: 'e1f2a3b4-c5d6-7e8f-9a0b-1c2d3e4f5a6b',
      title: 'Overview',
      position: 0,
      icon: 'IconList',
      widgets: [],
...
Key points:
- `objectUniversalIdentifier` links the layout to a specific object type.
- `tabs` define the tab structure; each tab can contain `widgets`.
- Each widget requires a `universalIdentifier`, `title`, `type`, and `configuration`.
- `defaultTabToFocusOnMobileAndSidePanelUniversalIdentifier` optionally sets which tab is shown first on mobile and in the side panel.

### Front component API

The SDK provides hooks and utility functions for front components to interact with Twenty's UI. These are available inside components defined with `defineFrontComponent()`.

#### Hooks

| Hook | Return type | Description |
|------|-------------|-------------|
| `useFrontComponentExecutionContext(selector)` | `T` | Subscribe to the front component's execution context with a selector function. The context contains `frontComponentId` and `userId`. |
| `useFrontComponentId()` | `string` | Get the current front component instance ID. |
| `useUserId()` | `string \| null` | Get the current user's ID. |

```typescript
import { defineFrontComponent, useFrontComponentExecutionContext, useUserId } from 'twenty-sdk';
...
const MyComponent = () => {
  ...
  return (
    <div>
      <p>User: {userId}</p>
      <pre>{JSON.stringify(fullContext, null, 2)}</pre>
    </div>
  );
};
...
|----------|-------------|
| `navigate(to, params?, queryParams?, options?)` | Navigate to a Twenty app path. |
| `openSidePanelPage({ page, pageTitle, pageIcon, shouldResetSearchState? })` | Open a page in the side panel. |
| `closeSidePanel()` | Close the side panel. |
| `enqueueSnackbar({ message, detailedMessage?, variant, dedupeKey? })` | Show a notification toast. |
| `unmountFrontComponent()` | Unmount the current front component. |

### Action components

Action components are headless components that execute a single action and then unmount themselves. They handle error reporting via snackbar notifications automatically.

| Component | Props | Description |
|-----------|-------|-------------|
| `Action` | `execute: () => void \| Promise<void>` | Runs a custom function, then unmounts. |
| `ActionLink` | `to`, `params?`, `queryParams?`, `options?` | Navigates to a Twenty app path, then unmounts. |
| `ActionOpenSidePanelPage` | `page`, `pageTitle`, `pageIcon`, `onClick?`, `shouldResetSearchState?` | Opens a side panel page, then unmounts. |

```typescript
import { defineFrontComponent, Action } from 'twenty-sdk';
import { CoreApiClient } from 'twenty-sdk/generated';
...
const CreateTask = () => {
  const execute = async () => {
    ...
    await client.mutation({
      createTask: { __args: { data: { title: 'New task' } }, id: true },
    });
  };
...
### Front component commands

Front components can be registered as command menu items, making them accessible from Twenty's command palette. Add a `command` property to your `defineFrontComponent()` call:

```typescript
import { defineFrontComponent } from 'twenty-sdk';

const QuickSearch = () => {
  return <div>Quick search UI...</div>;
};
...
Key points:
- `label` is displayed in the command menu.
- `isPinned` keeps the command visible at the top of the command palette.
- `availabilityType` can be `'GLOBAL'`, `'SINGLE_RECORD'`, or `'BULK_RECORDS'` to control when the command is available.
- `availabilityObjectUniversalIdentifier` restricts the command to a specific object type when using record-level availability.
...
Explore a minimal, end-to-end example that demonstrates objects, logic functions, front components, and trigger settings [here](https://github.com/twentyhq/twenty/tree/main/packages/twenty-apps/hello-world):

<Note>
The hello-world example app may still use the legacy `FunctionConfig` / `triggers` array format. For the latest API patterns using `defineLogicFunction()` with individual trigger properties (`httpRouteTriggerSettings`, `cronTriggerSettings`, `databaseEventTriggerSettings`), refer to the documentation on this page.
</Note>
