...
# Install Yarn (via Corepack) and project dependencies
yarn
...
- **yarn.lock**, **.yarnrc.yml**: Lock and configure the Yarn 4 toolchain used by the project. Yarn is managed by Corepack based on the `packageManager` field in `package.json`.
