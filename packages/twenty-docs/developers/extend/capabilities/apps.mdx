...
| `queryStringParameters` | `Record<string, string \| string[] \| undefined>` | Query string parameters |
| `pathParameters` | `Record<string, string \| string[] \| undefined>` | Path parameters extracted from the route pattern (e.g., `/users/:id` → `{ id: '123' }`) |
...
<Note>
Header names are case-insensitive. `'Content-Type'`, `'content-type'`, and `'CONTENT-TYPE'` are equivalent.
</Note>
