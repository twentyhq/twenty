...
```bash filename="Terminal"
# Default (minimal): only core files (application-config.ts and default-role.ts)
npx create-twenty-app@latest my-app

# Exhaustive: all examples (object, field, logic function, front component, view, navigation menu item, skill, agent)
npx create-twenty-app@latest my-app --exhaustive
```
...
A freshly scaffolded app with `--exhaustive` mode looks like this:
...
| `--api-key <api-key>` | Authentication api-key for the target server |
