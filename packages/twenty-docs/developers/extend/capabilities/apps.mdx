...
Here is the `postCard` object from the Hello World app:

```ts filename="src/objects/postCard.ts"
...
- Each `@Field` decorator defines a field on the object with a type, label, and its own stable `universalIdentifier`. Some field types (like `FULL_NAME` and `ADDRESS`) use helper types such as `FullNameField` and `AddressField`.
...
Every app has a single `application.config.ts` file in `src/` that describes:
...
```ts filename="src/role.config.ts"
