...
Both REST and GraphQL support creating up to 200 records per request. GraphQL also supports batch upsert (create-or-update in one call) using plural names like `CreateCompanies`.

## Request limits

Twenty does not impose a general per-minute request limit on the Core or Metadata APIs. Bulk create and upsert requests accept up to 200 records. REST list requests return 60 records by default and allow up to 200; nested relation lists return up to 60 records per parent.
