...
Twenty exposes an [MCP](https://modelcontextprotocol.io/) server so that AI assistants — Claude Desktop, Windsurf, Cursor, and others — can read and write your CRM data through natural language.

The server is available at:

| Environment | MCP Endpoint |
|-------------|-------------|
| **Cloud** | `https://api.twenty.com/mcp` |
...
<Note>
OAuth requires an MCP client that supports the [MCP Authorization specification](https://modelcontextprotocol.io/specification/2025-03-26/basic/authorization). Claude Desktop, Windsurf, and Cursor all support it.
</Note>
...
Go to **Settings > AI > More > MCP Server** in Twenty. Choose your authentication method (OAuth or API Key), copy the JSON snippet, and paste it into your MCP client's config file.
...
For self-hosted instances, replace `https://api.twenty.com` with your server URL. Make sure `SERVER_URL` in your environment matches the public URL of your Twenty instance — this is used to generate the OAuth discovery metadata.
...
- The AI feature flag must be enabled for your workspace. Contact your workspace admin.
