...
<Warning>
**初めてインストールしますか？** [Docker Compose インストールガイド](/ja/developers/self-host/capabilities/docker-compose)に従ってTwentyを起動し、その後はここに戻り構成してください。
</Warning>
...
ダウンタイムなしで `ENCRYPTION_KEY` をローテーションする方法については、[キー ローテーション ガイド](/ja/developers/self-host/capabilities/key-rotation) を参照してください。
...
<Note>
"Mail.ReadWrite" と "Mail.Send" は、ワークフローアクションを使用してメールを送信する場合にのみ必須です。 メールを受信するだけの場合は、"Mail.Read"を使用できます。
</Note>

<Note>
ワークフローアクションを使用してカレンダーイベントを作成する場合は、"Calendars.ReadWrite" が必須です。 カレンダーイベントを同期するだけの場合は、"Calendars.Read" を使用してください。
</Note>
...
## API とフロントエンドが別ホスト上にある場合

バックエンドにフロントエンドを配信させない場合にのみ関連します。 同一オリジンでのデプロイの場合、ここで設定する必要はありません。

API と通信するすべてのブラウザーオリジンを、カンマ区切りで指定してください。 `SERVER_URL`
および `FRONTEND_URL` は既に信頼済みです。 ブラウザーが宣言されていないオリジンからサーバーを呼び出すと、サーバーは警告をログに記録します。
...
<Warning>
フロントエンドはすべてのリクエストでクレデンシャルを送信し、ブラウザーは宣言されていないオリジンからのクレデンシャル付きレスポンスを拒否するため、この設定を行うまでアプリは読み込めません。
</Warning>

2 つのホストが異なる登録可能ドメイン（単なる異なるサブドメインではない）の場合は、`AUTH_COOKIE_SAME_SITE=none` も設定してください。これは HTTPS を必須とします。

マルチワークスペースが有効な場合、ここにすべてのワークスペースのサブドメインを列挙しないでください。
代わりに、各ワークスペースのホスト名ごとに API パス（`/graphql`、`/metadata`、`/rest`、`/auth/`、`/client-config`、`/file`）をバックエンドにルーティングし、すべてのワークスペースが同一オリジンを維持するようにします。Twenty Cloud はこの方式で稼働しています。
