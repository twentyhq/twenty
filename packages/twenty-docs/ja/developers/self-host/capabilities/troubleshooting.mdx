...
本番環境では、ロジック関数はデフォルトで無効になっています。 有効にするには、`LOGIC_FUNCTION_TYPE` 環境変数を `LOCAL` または `LAMBDA` に設定します。 これは、環境変数経由、または管理画面のデータベース変数から設定できます。 詳細は[ロジック関数のセットアップガイド](/ja/developers/self-host/capabilities/setup#logic-functions-available-drivers)を参照してください。
...
#### ワークフローで Logic 関数をテストする際の「ファイルが見つかりません」エラー

不足している依存関係の問題を解決するには、twenty-server コンテナで `yarn command:prod application:rebuild-default-deps` コマンドを実行します。
