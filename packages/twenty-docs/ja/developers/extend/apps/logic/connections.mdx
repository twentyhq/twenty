...
そこから `getConnection(connectedAccountId)` を使用して最新のアクセストークンを読み取り、プロバイダーの API（例: Slack `auth.test`）を呼び出すか、[key-value store](/ja/developers/extend/apps/logic/key-value-store) を使ってマッピングを永続化します。
...
フックが実行される時点では `ConnectedAccount` はすでに削除されているため、`getConnection(connectedAccountId)` はもう解決されません。 クリーンアップに必要なもの（`team_id` や外部サブスクリプション ID など）は、接続時に `connectedAccountId` をキーとして [key-value store](/ja/developers/extend/apps/logic/key-value-store) に書き込まれている必要があります。
...
| `name`            | 接続の表示名（OAuth コールバック時に自動取得。ユーザーが名称変更可能）                                                                        |
| `handle`          | 接続された上流アカウントのメールアドレスは、再接続ごとに更新されます。 トークン交換時に返されたOIDC `id_token`を解決し、プロバイダが1つを返さないときに接続するTwentyユーザーの電子メールに戻ります |
