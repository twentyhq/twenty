...
<Note>
`enqueueJobs` は、ジョブが受け付けられた時点ですぐに戻り、ジョブが実行された時点では戻りません。 対象の結果は返しません。結果を後から読み戻す必要がある場合は、各対象で生成したものを [key-value store](/ja/developers/extend/apps/logic/key-value-store) またはワークスペースレコードに書き込ませてください。
</Note>
