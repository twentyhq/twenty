...
<CardGroup cols={2}>
  <Card title="オブジェクト" icon="table" href="/ja/developers/extend/apps/data/objects">
    `defineObject` — 固有のフィールドを持つ新しいレコードタイプを宣言します。
  </Card>
  <Card title="オブジェクトの拡張" icon="wand-magic-sparkles" href="/ja/developers/extend/apps/data/extending-objects">
    `defineField` — 標準オブジェクトや他のアプリのオブジェクトにフィールドを追加します。
  </Card>
  <Card title="関連" icon="diagram-project" href="/ja/developers/extend/apps/data/relations">
    オブジェクト間の双方向 `MANY_TO_ONE` / `ONE_TO_MANY` 接続。
  </Card>
  <Card title="タイムライン活動タイプ" icon="clock-rotate-left" href="/ja/developers/extend/apps/data/timeline-activity-types">
    レコードタイムライン用の自動監査イベントと明示的なアプリイベントを定義します。
  </Card>
</CardGroup>
...
| **タイムライン活動タイプ** | レコードタイムラインに表示される自動監査イベントまたは明示的なドメインイベント                                         | `defineTimelineActivityType()`         |

SDK はビルド時に AST 解析を通じてこれらを検出するため、ファイル構成は自由です — 慣例としては `src/objects/`、`src/fields/`、`src/indexes/`、`src/timeline-activity-types/` です。 安定した `universalIdentifier` UUID が、デプロイをまたいであらゆるものを結び付けます。
...
<Note>
**Application Config** や **Roles & Permissions** をお探しですか？ それらは、アプリが追加するデータではなくアプリ自体を記述するものであり、[Config](/ja/developers/extend/apps/config/overview) 配下にあります。 **Connections**（Linear、GitHub、Slack OAuth）をお探しですか？ それらはロジック関数*から*呼び出されるために存在し、[Logic](/ja/developers/extend/apps/logic/connections) 配下にあります。
</Note>
