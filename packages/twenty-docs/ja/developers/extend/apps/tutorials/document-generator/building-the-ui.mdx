...
[view](/ja/developers/extend/apps/layout/views) は、指定されたオブジェクトのリストです。
[navigation menu item](/ja/developers/extend/apps/layout/navigation-menu-items)
はそのビューをサイドバーに配置します。 サーバーはすでに各オブジェクトのデフォルトの「All ...」ビューを自動で用意しているため、アプリが定義するビューは追加の厳選されたビューとなります。
...
[front component](/ja/developers/extend/apps/layout/front-components) は Twenty 内でサンドボックス化された React
...
`availabilityType: 'RECORD_SELECTION'` を指定した [command menu item](/ja/developers/extend/apps/layout/command-menu-items) は、Person が選択されているときに表示され、サイドパネルでコンポーネントを開きます。
...
[page layout](/ja/developers/extend/apps/layout/page-layouts) でマウントします。 `RECORD_PAGE` レイアウトは、オブジェクトのレコードビューにタブを追加します。単一の
...
`FIELD` ウィジェットは、他にもさまざまな表示モードやオプションをサポートしています。たとえば、2 ホップ先の入れ子になったリレーションのレコードを一覧表示することもできます。詳しくは、[フィールドウィジェットリファレンス](/ja/developers/extend/apps/layout/page-layouts#field-widgets)を参照してください。
...
<Card title="次へ: AI エージェント →" icon="robot" href="/ja/developers/extend/apps/tutorials/document-generator/ai-agent">
