...
コア: [ロジック関数](/ja/developers/extend/apps/logic/logic-functions)
...
<Tip>
副作用がないため、高速ユニットテスト
(`yarn test:unit`) でカバーできます。 [Testing](/ja/developers/extend/apps/operations/testing) を参照してください。
</Tip>

## The handler

ハンドラは生成された [`CoreApiClient`](/ja/developers/extend/apps/logic/logic-functions)
...
詳細な権限については、 [Roles](/ja/developers/extend/apps/config/roles) を参照してください。
...
<Card title="次へ: HTTP ルート →" icon="グローブ" href="/ja/developers/extend/apps/tutorials/document-generator/http-routes">
