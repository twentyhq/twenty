...
* タブ内の各 `widget` は、[front component](/ja/developers/extend/apps/layout/front-components)、リレーションリスト、その他のビルトインウィジェットタイプをレンダリングできます。
...
* `widgets` はこのタブのみにスコープされます。`definePageLayout` 内でインライン定義されたウィジェットとまったく同様に、[front components](/ja/developers/extend/apps/layout/front-components)、views などを参照します。
