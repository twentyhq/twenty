...
<CardGroup cols={2}>
  <Card title="ビュー" icon="list" href="/ja/developers/extend/apps/layout/views">
    `defineView` — 表示カラム、フィルター、グループなどを含む、保存済みリスト構成。
  </Card>
  <Card title="ナビゲーションメニュー項目" icon="bars" href="/ja/developers/extend/apps/layout/navigation-menu-items">
    `defineNavigationMenuItem` — ビューまたは外部 URL を指すサイドバー項目。
  </Card>
  <Card title="ページレイアウト" icon="table-columns" href="/ja/developers/extend/apps/layout/page-layouts">
    `definePageLayout` および `definePageLayoutTab` — レコード詳細ページ上のタブとウィジェット。
  </Card>
  <Card title="フロントコンポーネント" icon="window-maximize" href="/ja/developers/extend/apps/layout/front-components">
    `defineFrontComponent` — Twenty 内でレンダリングされる、サンドボックス化された React コンポーネント。
  </Card>
  <Card title="コマンドメニュー項目" icon="terminal" href="/ja/developers/extend/apps/layout/command-menu-items">
...
これらは依然として積極的に開発中です。サンドボックスは DOM を部分的にのみ実装しているため、高度な利用方法では失敗する場合があります。 [現在の制限](/ja/developers/extend/apps/layout/front-components#current-limitations) を参照してください。
