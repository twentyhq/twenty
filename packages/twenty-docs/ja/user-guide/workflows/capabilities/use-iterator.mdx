...
ほとんどの配列ソース（Search Records、webhook の配列フィールド、Bulk manual trigger）は、変数ピッカーで直接選択できます。

**Code** ステップや **Logic Function** ステップが*トップレベル配列*を返す場合、その出力は変数ピッカー内でインデックス付きエントリ（`0`、`1`、`2`、…）として表示されます。 配列全体をループ処理するには、そのステップの **Whole list** オプションを選択します。Iterator は、そのリストから各アイテムの構造を自動的に推論します。
...
変数ピッカーでは、現在のアイテムの特定のフィールドまでドリルダウンして選択するか、**Use the whole item** を選択して、現在のアイテム全体（`{{iterator.currentItem}}`）を参照できます。 アイテム全体を選択すると、フィールドを 1 つずつ再構築するのではなく、完全なレコードをそのまま後続ステップに渡したい場合に便利です。
...
* [ワークフローのアクション](/ja/user-guide/workflows/capabilities/workflow-actions)
* [ブランチの使い方](/ja/user-guide/workflows/capabilities/use-branches-in-workflows)
* [ワークフロー FAQ](/ja/user-guide/workflows/how-tos/need-more-help/workflows-faq)
