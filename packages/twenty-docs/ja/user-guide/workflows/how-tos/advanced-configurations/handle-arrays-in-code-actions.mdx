...
<Tip>
**配列からフィールドを取り出す代わりに、配列自体をループ処理したいですか？** Code ステップまたは Logic Function ステップがトップレベルの配列を返す場合は、それをそのまま [Iterator](/ja/user-guide/workflows/capabilities/use-iterator) に渡せます。Iterator の入力としてそのステップの **Whole list** オプションを選択し、ループ内では `{{iterator.currentItem}}` を使って各要素を参照します。 その場合、配列をオブジェクトに再構成する必要はありません。
</Tip>
