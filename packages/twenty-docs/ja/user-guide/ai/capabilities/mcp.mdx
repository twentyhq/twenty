...
API キーを作成するには、**Settings → MCP & APIs → API → + Create key** に移動します。 詳細は[API](/ja/developers/extend/api#create-an-api-key)を参照してください。
...
* **API Key**: **Settings → Members → Roles** で API キーにロールを割り当てます。 詳しくは[権限](/ja/user-guide/permissions-access/capabilities/permissions)を参照してください。
