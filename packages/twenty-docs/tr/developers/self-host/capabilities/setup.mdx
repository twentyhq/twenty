...
<Warning>
**İlk kez mi kuruyorsunuz?** Twenty'yi çalıştırmak için [Docker Compose kurulum kılavuzunu](/tr/developers/self-host/capabilities/docker-compose) izleyin, ardından yapılandırma için buraya dönün.
</Warning>
...
`ENCRYPTION_KEY`'i kesinti olmadan döndürmek için [Anahtar döndürme rehberi](/tr/developers/self-host/capabilities/key-rotation) bölümüne bakın.
...
<Note>
"Mail.ReadWrite" ve "Mail.Send", yalnızca iş akışı eylemlerimizi kullanarak e-posta göndermek istiyorsanız zorunludur. Yalnızca e-posta almak istiyorsanız "Mail.Read" kullanabilirsiniz.
</Note>

<Note>
İş akışı eylemlerimizi kullanarak takvim etkinlikleri oluşturmak istiyorsanız "Calendars.ReadWrite" zorunludur. Yalnızca takvim etkinliklerini eşzamanlamak istiyorsanız "Calendars.Read" kullanın.
</Note>
...
## API ve ön uç ayrı ana makinelerde

Yalnızca arka ucun (back-end) ön ucu (front-end) sunmasına izin vermiyorsanız geçerlidir. Aynı kökene (same-origin) sahip dağıtımlar için burada hiçbir şeye gerek yoktur.

API ile iletişim kuran her tarayıcı kökenini (origin) virgülle ayırarak belirtin. `SERVER_URL`
ve `FRONTEND_URL` zaten güvenilirdir. Bir tarayıcı bildirilmeyen bir kökenden
çağrı yaptığında sunucu bir uyarı kaydeder.
...
<Warning>
Ön uç her istekte kimlik bilgileri gönderir ve tarayıcılar, bildirilmemiş bir kökenden gelen kimlik bilgileri içeren
yanıtı reddeder, bu nedenle uygulama,
bunu ayarlayana kadar yüklenemez.
</Warning>

İki ana makine farklı kayıt edilebilir etki alanlarındaysa (sadece farklı
alt etki alanları değilse), ayrıca HTTPS gerektiren `AUTH_COOKIE_SAME_SITE=none` değişkenini de ayarlayın.

Çoklu çalışma alanı etkinleştirildiğinde, burada her çalışma alanının alt alan adını listelemeyin.
Bunun yerine, API yollarını (`/graphql`, `/metadata`, `/rest`, `/auth/`,
`/client-config`, `/file`) her çalışma alanı ana makine adında arka uca yönlendirin; böylece her çalışma alanı aynı kökende kalır — Twenty Cloud tam olarak böyle çalışır.
