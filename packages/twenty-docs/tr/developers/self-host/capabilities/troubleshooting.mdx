...
Üretimde, mantık işlevleri varsayılan olarak devre dışıdır. Bunları etkinleştirmek için `LOGIC_FUNCTION_TYPE` ortam değişkenini `LOCAL` veya `LAMBDA` olarak ayarlayın. Bu, ortam değişkenleri aracılığıyla veya yönetici panelindeki veritabanı değişkenleri üzerinden yapılandırılabilir. Ayrıntılar için [Mantık İşlevleri kurulum kılavuzu](/tr/developers/self-host/capabilities/setup#logic-functions-available-drivers) bölümüne bakın.
...
#### İş akışında Logic işlevini test ederken 'Dosya Bulunamadı' hatası

Eksik bağımlılıklar sorununu çözmek için twenty-server container'ınızda `yarn command:prod application:rebuild-default-deps` komutunu çalıştırın.
