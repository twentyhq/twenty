...
Geri doldurmanın, en başından itibaren satırları onun altında yazabilmesi için, v2.5 yükseltmesinden **önce** özel bir `ENCRYPTION_KEY` ayarlamalısınız. Geri doldurmadan sonra anahtarları değiştirmek bir [anahtar döndürme](/tr/developers/self-host/capabilities/key-rotation) gerektirir.

## Gizli verilerin ve imzalama anahtarlarının döndürülmesi

`ENCRYPTION_KEY` döndürme, JWT imzalama anahtarını döndürme veya sızdırılmış bir imzalama anahtarını iptal etme gibi günlük operasyonel görevler için özel [Anahtar döndürme kılavuzu](/tr/developers/self-host/capabilities/key-rotation) bölümüne bakın.
