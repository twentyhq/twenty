...
Bir bağlantı nesnesi, bir kaynak kaydını bir veya daha fazla hedef kayda bağlar. Örneğin, `PostCardRecipient` hem bir `postCard` ilişkisini hem de bir `recipient` ilişkisini tutabilir. Kaynak nesnenin `ONE_TO_MANY` alanında, `junctionTargetFieldUniversalIdentifier` değerini bağlantı nesnesinde hedefi işaret eden ilişki alanına ayarlayın:
...
`RECIPIENT_FIELD_ID`, bağlantı nesnesindeki bir `MANY_TO_ONE` veya morph ilişkisini tanımlamalıdır. [zaman çizelgesi etkinliği yayılımı](/tr/developers/extend/apps/data/timeline-activity-types#automatic-and-explicit-events) gibi bir özellik bağlantı nesnesinden geçtiğinde, Twenty'ye hangi ilişkili kaydın hedef olduğunu bildirir.
...
| `universalSettings.junctionTargetFieldUniversalIdentifier` | Yalnızca bağlantı nesnesi ONE_TO_MANY | Yayılım hedefini işaret eden bağlantı nesnesindeki ilişki alanı                              |

## Satır içi ilişki alanları

Bir ilişkiyi doğrudan [`defineObject`](/tr/developers/extend/apps/data/objects) içinde de tanımlayabilirsiniz. Satır içi olduğunda, `objectUniversalIdentifier`'ı atlayın — üst nesneden devralınır:
