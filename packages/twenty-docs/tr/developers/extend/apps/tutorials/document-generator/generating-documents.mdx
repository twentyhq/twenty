...
Şimdi esas kısma geçelim: bir şablon ve bir kaydı yükleyen, yer tutucuları dolduran ve yeni bir belge kaydeden bir [mantık işlevi](/tr/developers/extend/apps/logic/logic-functions).
...
<Tip>
Bu dosyanın yan etkisi olmadığından, onu hızlı birim testleriyle kapsayabilirsiniz (`yarn test:unit`). Bkz. [Testing](/tr/developers/extend/apps/operations/testing).
</Tip>

## İşleyici

Handler, CRM verilerini okumak ve yazmak için oluşturulmuş [`CoreApiClient`](/tr/developers/extend/apps/logic/logic-functions) kullanır. Şablonu yükler, hedef kaydı yükler, gövdeyi doldurur ve bir `document` oluşturur.
...
Daha ayrıntılı izinler için bkz. [Roles](/tr/developers/extend/apps/config/roles).
...
<Card title="Sırada: HTTP rotaları →" icon="globe" href="/tr/developers/extend/apps/tutorials/document-generator/http-routes">
