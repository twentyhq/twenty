...
Bir API anahtarı oluşturmak için **Settings → MCP & APIs → API → + Create key** bölümüne gidin. Ayrıntılar için [API'ler](/tr/developers/extend/api#create-an-api-key) bölümüne bakın.
...
* **API Anahtarı**: **Settings → Members → Roles** altında API anahtarına bir rol atayın. Bkz. [İzinler](/tr/user-guide/permissions-access/capabilities/permissions).
