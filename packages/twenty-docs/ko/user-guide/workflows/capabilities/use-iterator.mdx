...
대부분의 배열 소스(레코드 검색, 웹훅 배열 필드, 일괄 수동 트리거)는 변수 선택기에서 직접 선택할 수 있습니다.

**Code** 또는 **Logic Function** 단계가 *최상위 배열*을 반환하면, 그 출력은 변수 선택기에서 인덱스가 매겨진 항목들(`0`, `1`, `2`, …)로 표시됩니다. 배열 전체를 순회하려면 해당 단계에서 **Whole list** 옵션을 선택하세요. 그러면 Iterator가 목록에서 각 항목의 구조를 자동으로 추론합니다.
...
변수 선택기에서 현재 항목의 특정 필드로 들어가거나, **Use the whole item**을 선택해 현재 항목 전체(`{{iterator.currentItem}}`)를 참조할 수 있습니다. 전체 항목을 선택하면, 필드를 하나씩 다시 구성하는 대신 전체 레코드를 그대로 후속 단계로 전달하고자 할 때 유용합니다.
...
* [워크플로우 액션](/ko/user-guide/workflows/capabilities/workflow-actions)
* [브랜치 사용 방법](/ko/user-guide/workflows/capabilities/use-branches-in-workflows)
* [워크플로우 FAQ](/ko/user-guide/workflows/how-tos/need-more-help/workflows-faq)
