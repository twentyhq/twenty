...
[뷰](/ko/developers/extend/apps/layout/views)는 특정 객체에 대해 저장된 목록입니다.
[내비게이션 메뉴 항목](/ko/developers/extend/apps/layout/navigation-menu-items)은 해당 뷰를 사이드바에 배치합니다. 서버는 이미 각 객체의 기본 "All ..." 보기를 자동으로 생성하므로, 앱이 정의하는 보기는 추가로 제공되는 선별된 보기입니다.
...
[프런트 컴포넌트](/ko/developers/extend/apps/layout/front-components)는 Twenty 안에 샌드박스된 React 컴포넌트입니다. 우리 컴포넌트는 선택된 레코드를 읽고, `CoreApiClient`를 통해 person 템플릿을 로드한 뒤, 이전 장에서 만든 라우트로 POST 요청을 보냅니다.
...
`availabilityType: 'RECORD_SELECTION'`이 있는 [커맨드 메뉴 항목](/ko/developers/extend/apps/layout/command-menu-items)은 Person 이 선택되었을 때 표시되며, 컴포넌트를 사이드 패널에서 엽니다.
...
[페이지 레이아웃](/ko/developers/extend/apps/layout/page-layouts)으로 이를 마운트합니다. `RECORD_PAGE` 레이아웃은 객체의 레코드 뷰에 탭을 추가합니다. 단일 `FRONT_COMPONENT` 위젯이 있는 탭은 컴포넌트를 전체 폭으로 호스팅합니다:
...
`FIELD` 위젯은 중첩된 관계의 레코드를 두 단계 떨어진 곳까지 나열하는 것을 포함하여, 다른 표시 모드와 옵션도 지원합니다. 자세한 내용은 [필드 위젯 레퍼런스](/ko/developers/extend/apps/layout/page-layouts#field-widgets)를 참고하세요.
...
<Card title="다음: AI 에이전트 →" icon="robot" href="/ko/developers/extend/apps/tutorials/document-generator/ai-agent">
