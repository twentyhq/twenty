...
이제 핵심입니다. 템플릿과 레코드를 불러와 플레이스홀더를 채우고 새 문서를 저장하는 [로직 함수](/ko/developers/extend/apps/logic/logic-functions)입니다.
...
<Tip>
이 파일은 부작용이 없으므로, 빠른 단위 테스트(`yarn test:unit`)로 검증할 수 있습니다. [테스트](/ko/developers/extend/apps/operations/testing)를 참조하세요.
</Tip>

## 핸들러

핸들러는 생성된 [`CoreApiClient`](/ko/developers/extend/apps/logic/logic-functions)를 사용해 CRM 데이터를 읽고 씁니다. 템플릿을 불러오고, 대상 레코드를 불러온 뒤, 본문을 채우고 `document`를 생성합니다.
...
보다 세분화된 권한에 대해서는 [Roles](/ko/developers/extend/apps/config/roles)를 참조하세요.
...
<Card title="다음: HTTP 경로 →" icon="globe" href="/ko/developers/extend/apps/tutorials/document-generator/http-routes">
