...
[skill](/ko/developers/extend/apps/logic/skills-and-agents)은(는) 재사용 가능한 지침으로,
...
[agent](/ko/developers/extend/apps/logic/skills-and-agents)는 프롬프트와 모델을
...
<Note>
에이전트는 자신의 역할이 허용할 때만 그 도구를 호출할 수 있습니다. 우리는 이미 [2장](/ko/developers/extend/apps/tutorials/document-generator/generating-documents#grant-it-access)에서 앱의 역할에 `canAccessAllTools: true` 및 `canBeAssignedToAgents: true`를 설정했습니다.
</Note>
...
<Card title="다음: 게시 →" icon="rocket" href="/ko/developers/extend/apps/tutorials/document-generator/publishing">
