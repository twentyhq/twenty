...
<CardGroup cols={2}>
  <Card title="개체" icon="table" href="/ko/developers/extend/apps/data/objects">
    `defineObject` — 자체 필드를 가진 새로운 레코드 타입을 선언합니다.
  </Card>
  <Card title="객체 확장하기" icon="wand-magic-sparkles" href="/ko/developers/extend/apps/data/extending-objects">
    `defineField` — 표준 개체 또는 다른 앱의 개체에 필드를 추가합니다.
  </Card>
  <Card title="관계" icon="diagram-project" href="/ko/developers/extend/apps/data/relations">
    개체 간의 양방향 `MANY_TO_ONE` / `ONE_TO_MANY` 연결입니다.
  </Card>
  <Card title="타임라인 활동 유형" icon="clock-rotate-left" href="/ko/developers/extend/apps/data/timeline-activity-types">
    레코드 타임라인의 자동 감사 이벤트 및 명시적 앱 이벤트를 정의합니다.
  </Card>
</CardGroup>
...
| **타임라인 활동 유형** | 레코드 타임라인에 표시되는 자동 감사 이벤트 또는 명시적 도메인 이벤트                                     | `defineTimelineActivityType()`             |

SDK는 빌드 시 AST 분석을 통해 이를 감지하므로 파일 구성은 자유롭게 정할 수 있습니다. 규칙은 `src/objects/`, `src/fields/`, `src/indexes/`, `src/timeline-activity-types/`입니다. 안정적인 `universalIdentifier` UUID가 배포 간 모든 것을 하나로 연결합니다.
...
<Note>
**Application Config** 또는 **Roles & Permissions**를 찾고 계신가요? 이러한 항목은 앱이 추가하는 데이터가 아니라 앱 자체를 설명하며, [Config](/ko/developers/extend/apps/config/overview) 아래에 있습니다. **Connections**(Linear, GitHub, Slack OAuth)를 찾고 계신가요? 이들은 로직 함수 *내부에서* 호출되기 위한 것이며, [Logic](/ko/developers/extend/apps/logic/connections) 아래에 있습니다.
</Note>
