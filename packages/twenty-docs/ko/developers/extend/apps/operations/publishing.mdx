...
앱을 [로컬에서 빌드하고 테스트](/ko/developers/extend/apps/getting-started/concepts)하면, 배포 경로는 두 가지입니다:
...
마켓플레이스 필드 전체 목록(`author`, `category`, `aboutDescription`, `websiteUrl`, `termsUrl` 등)은 Building Apps 페이지의 [defineApplication 아코디언](/ko/developers/extend/apps/config/application#marketplace-metadata)을 참조하세요.
