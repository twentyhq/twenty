...
* 탭 내부의 각 `widget`은 [front component](/ko/developers/extend/apps/layout/front-components), 관계 목록 또는 기타 기본 제공 위젯 타입을 렌더링할 수 있습니다.
...
* `widgets`는 이 탭에만 범위가 국한됩니다. `definePageLayout`에 인라인으로 정의된 위젯과 완전히 동일한 방식으로 [front components](/ko/developers/extend/apps/layout/front-components), 뷰 등을 참조합니다.
