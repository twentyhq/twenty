...
**내비게이션 메뉴 항목**은 왼쪽 사이드바의 항목입니다. `defineNavigationMenuItem()`을 사용해 사용자 지정 사이드바 링크를 추가하세요 — 일반적으로 배포하는 각 [보기](/ko/developers/extend/apps/layout/views)마다 하나씩 추가하거나 외부 URL을 가리키도록 설정합니다.
...
* 열거형에는 사용자가 만든 레코드 즐겨찾기에 내부적으로 사용되는 `NavigationMenuItemType.RECORD`도 포함되어 있습니다. 이는 앱 매니페스트에서는 사용할 수 없습니다(레코드를 참조할 필드가 없기 때문입니다).
...
<Note>
**흔한 오류:** 연결된 내비게이션 메뉴 항목 없이 오브젝트를 생성하면 해당 오브젝트는 사이드바에서 접근할 수 없습니다. 기술적/내부용 오브젝트가 아닌 한, 모든 사용자 지정 오브젝트에는 이를 가리키는 사이드바 항목이 있어야 합니다.
</Note>
