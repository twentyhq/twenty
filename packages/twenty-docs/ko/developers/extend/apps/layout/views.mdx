...
* `fields`는 어떤 열을 어떤 순서로 표시할지를 제어합니다. 각 필드는 `fieldMetadataUniversalIdentifier`를 참조합니다. 자동 생성된 시스템 필드(예: `createdAt`)를 참조하는 방법은 [시스템 메타데이터 대상으로 지정하기](/ko/developers/extend/apps/data/system-fields)를 참조하세요.
...
| `openRecordIn`                                                                                              | 사용 중단됨                                                                             | 더 이상 읽지 않음: 레코드가 어디에서 열리는지는 이제 [object](/ko/developers/extend/apps/data/objects)의 속성(`defineObject()`의 `openRecordIn`)으로 정의되며, 설정되지 않은 경우 각 멤버의 개별 기본 설정을 따릅니다. |
...
보기만으로는 사이드바에서 직접 접근할 수 없습니다. 사이드바에 표시하려면, 해당 보기의 `universalIdentifier`를 가리키는 `VIEW` 타입의 [탐색 메뉴 항목](/ko/developers/extend/apps/layout/navigation-menu-items)과 연결해야 합니다. 이것이 정형화된 패턴입니다. 모든 사용자 정의 객체는 생성 시 기본 인덱스 뷰가 자동으로 생성되므로, 이를 여는 사이드바 항목만 제공하면 됩니다.
