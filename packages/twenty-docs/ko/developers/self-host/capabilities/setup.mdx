...
<Warning>
**처음 설치하시나요?** [Docker Compose 설치 가이드](/ko/developers/self-host/capabilities/docker-compose)를 따라 Twenty를 실행하고, 이곳으로 돌아와 구성을 진행하세요.
</Warning>
...
중단 없이 `ENCRYPTION_KEY`를 회전하는 방법은 [키 회전 가이드](/ko/developers/self-host/capabilities/key-rotation)를 참고하세요.
...
<Note>
"Mail.ReadWrite"와 "Mail.Send"는 워크플로우 작업을 통해 이메일을 보내려는 경우에만 필수입니다. 이메일 수신만 원하시면 대신 "Mail.Read"를 사용하세요.
</Note>

<Note>
워크플로우 작업을 사용하여 캘린더 이벤트를 생성하려면 "Calendars.ReadWrite"가 필수입니다. 캘린더 이벤트만 동기화하려면 "Calendars.Read"를 사용하세요.
</Note>
...
## 별도 호스트에서 동작하는 API와 프런트엔드

백엔드가 프런트엔드를 제공하지 않는 경우에만 해당됩니다. 동일 출처
배포의 경우 여기에서 추가로 설정할 내용이 없습니다.

API와 통신하는 모든 브라우저 오리진을 쉼표로 구분하여 선언하세요. `SERVER_URL`
과 `FRONTEND_URL`은 이미 신뢰된 상태입니다. 브라우저가 선언되지 않은 오리진에서 서버를 호출하면,
서버는 경고를 로그에 기록합니다.
...
<Warning>
프런트엔드는 모든 요청에 자격 증명을 함께 보내고, 브라우저는 선언되지 않은 오리진에서 오는 자격 증명 포함 응답을 거부하므로, 이 값을 설정하기 전까지 앱이 로드되지 않습니다.
</Warning>

두 호스트가 서로 다른 등록 가능한 도메인(단순히 서로 다른 서브도메인이 아닌 경우)에 있다면, `AUTH_COOKIE_SAME_SITE=none`도 설정해야 하며, 이때는 HTTPS가 필요합니다.

멀티 워크스페이스가 활성화된 경우, 여기에는 모든 워크스페이스 서브도메인을 나열하지 마세요.
대신, 각 워크스페이스 호스트 이름에서 API 경로(`/graphql`, `/metadata`, `/rest`, `/auth/`,
`/client-config`, `/file`)를 백엔드로 라우팅하여 모든 워크스페이스가 동일 오리진을 유지하도록 하세요. Twenty Cloud는 이렇게 동작합니다.
