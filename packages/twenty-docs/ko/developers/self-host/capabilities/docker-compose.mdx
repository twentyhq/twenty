...
<Warning>
Docker 컨테이너는 프로덕션 호스팅 또는 셀프 호스팅용입니다. 기여하려면 [로컬 설정](/ko/developers/contribute/capabilities/local-setup)을 확인하세요.
</Warning>
...
고급 설정을 위해 [환경 변수 설정](/ko/developers/self-host/capabilities/setup) 문서를 참조하세요. 모든 환경 변수는 변수에 따라 서버 및/또는 작업자 수준에서 `docker-compose.yml` 파일에 선언되어야 합니다.
...
3. **`.env` 업데이트**
   ...
   중단 없이 키를 회전하는 방법은 [키 회전 가이드](/ko/developers/self-host/capabilities/key-rotation)를 참조하세요.
...
기본적으로 Twenty는 `localhost`의 포트 `3000`에서 실행됩니다. UI는 브라우저 자체의 origin에서 API를 확인하므로, 컨테이너에 도달할 수 있는 어떤 호스트명에서도 로드됩니다. 하지만 여전히 `SERVER_URL`은 공개용 URL로 설정해야 합니다. 서버는 이를 생성된 링크(이메일, OAuth 콜백)와 HTTPS 감지(보안 쿠키용)에 사용합니다.
...
문제가 발생하면 [문제 해결](/ko/developers/self-host/capabilities/troubleshooting)에서 해결책을 확인하십시오.

## 관리형 호스팅

<Tip>
  직접 Twenty를 실행하고 유지 관리하고 싶지 않으신가요? 인스턴스의 구축, 호스팅, 관리를 처음부터 끝까지 맡길 [공인 Twenty 호스팅 파트너](https://twenty.com/partners/list?categories=HOSTING\&ref=docs-docker-compose)를 찾아보세요. *(또는 저희 팀에 문의하세요: [contact@twenty.com](mailto:contact@twenty.com).)*
</Tip>
