...
<Note>
  **Tasks and Notes limitation**: Relations on Tasks and Notes are hardcoded as
  many-to-many and are not yet available in workflow triggers or actions. To
  access these relations, use the [API](/developers/extend/api) instead.
</Note>
...
<Note>
  Be careful with bidirectional syncs to avoid infinite loops. Use filters to
