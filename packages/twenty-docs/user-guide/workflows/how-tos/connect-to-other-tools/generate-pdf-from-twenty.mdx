...
export const main = async (
  ...
  // Step 3: Request an upload URL for the file
  ...
  const createFileUploadResponse = await fetch(metadataGraphqlEndpoint, {
    method: 'POST',
    headers: {
      Authorization: `Bearer ${authToken}`,
    ...
    body: JSON.stringify({
      ...
      variables: {
        ...
        fieldMetadataId: uploadFileFieldMetadataId,
      },
    }),
  });

  if (!createFileUploadResponse.ok) {
    throw new Error(`Upload failed with status ${createFileUploadResponse.status}`);
  }

  const createFileUploadResult = await createFileUploadResponse.json();

  if (createFileUploadResult.errors?.length) {
    throw new Error(`Upload failed: ${createFileUploadResult.errors[0].message}`);
  }

  const uploadTarget = createFileUploadResult.data?.createFileUpload;

  if (!uploadTarget) {
    throw new Error('No upload target returned from createFileUpload');
  }

  const { fileId, uploadUrl, contentType } = uploadTarget;

  // Step 4: Send the file bytes to the upload URL
  ...
  // Step 5: Confirm the upload so the file becomes usable
  const completeFileUploadMutation = `
  ...
  const completeFileUploadResponse = await fetch(metadataGraphqlEndpoint, {
    method: 'POST',
    headers: {
      Authorization: `Bearer ${authToken}`,
      'Content-Type': 'application/json',
  ...
  if (!completeFileUploadResponse.ok) {
    throw new Error(`Upload failed with status ${completeFileUploadResponse.status}`);
  }

  const completeFileUploadResult = await completeFileUploadResponse.json();

  if (completeFileUploadResult.errors?.length) {
    throw new Error(`Upload failed: ${completeFileUploadResult.errors[0].message}`);
  }

  if (!completeFileUploadResult.data?.completeFileUpload?.id) {
    throw new Error('No file id returned from completeFileUpload');
  }

  // Step 6: Create the attachment linked to the company
