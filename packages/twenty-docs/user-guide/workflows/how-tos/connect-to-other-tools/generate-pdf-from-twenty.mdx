...
export const main = async (
  ...
  // Cloud: https://api.twenty.com/rest/metadata/objects
  // Self-hosted: https://your-domain.com/rest/metadata/objects
  const restEndpoint = 'https://api.twenty.com/rest/metadata/objects';
  ...
  // (it doesn't have universal identifier like fields of standard objects)
  ...
  const uploadFileFieldMetadataId = result.data.objects.find((object) => object.nameSingular === 'attachment').fields.find((field) => field.name === 'file').id;
  ...
  uploadForm.append('map', JSON.stringify({ '1': ['variables.file'] }));
  uploadForm.append('1', pdfFile);
