...
- Rename actions to make your workflow easier to understand
</Tip>
...
- Choose the specific record to update.
  ...
  - Or you can have the record dynamically selected, by designating a record found in a previous step, using teh `(x+)`. You cannot search for the record based on different criteria at this stage. If you've not yet identified the record, add a `Search Record` step before this `Update Record` step.
...
<Note>
The fields canot be made mandatory.
</Note>
...
#### Handling Arrays as Input

When working with arrays in Code actions — common when processing data from external systems or preparing data for an [Iterator](/user-guide/workflows/capabilities/workflow-actions#iterator) — you may encounter arrays passed as strings or JSON rather than actual arrays.

<Note>
We're actively working on making array handling easier. For now, use the pattern below to ensure your input is properly formatted as an array.
</Note>

**Example: Forcing input to be treated as an array**
...
**Key pattern**: The line `typeof users === "string" ? JSON.parse(users) : users` checks if the input is a string and parses it, or uses it directly if it's already an array.
