...
- **It is not recommended for records created manually**: with this trigger, worfklows start as soon as the record is created. Since Twenty UI offers auto-save on the fly (there is not an edit mode and then a validation to save records), the workflow will be triggerred before the user inputs all the fields.
...
- **Single**: The selected record(s) will be passed to your workflow. This is configured for a given object. Several records can be selected before triggering the workflow. The workflow will runfrom beginning to end as many times as there are records selected.
