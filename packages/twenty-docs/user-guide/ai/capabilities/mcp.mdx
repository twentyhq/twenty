...
<Warning>
MCP is currently in **alpha** and is only available on some workspaces. If you don't see the MCP Server section in Settings > APIs & Webhooks, it may not be enabled for your workspace yet.
</Warning>
...
Add this to your MCP client configuration, replacing `{your-workspace-url}` with the URL you use to access Twenty (e.g. `https://app.twenty.com`):
...
Go to **Settings > APIs & Webhooks** in Twenty and scroll to the **MCP Server** section. Choose your authentication method (OAuth or API Key), copy the JSON snippet (it will already use your workspace URL), and paste it into your MCP client's config file.
...
| **ChatGPT** | Add via **Settings > Connectors > Add custom connector** (Pro/Plus with Developer Mode) |
