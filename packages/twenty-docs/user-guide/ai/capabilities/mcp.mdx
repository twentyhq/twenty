...
<Note>
  OAuth requires an MCP client that supports the [MCP Authorization
  specification](https://modelcontextprotocol.io/specification/2025-03-26/basic/authorization).
  Claude Desktop, Claude Code, Cursor, and ChatGPT support it.
</Note>
...
<Warning>
  Your API key grants access to workspace data. Keep it out of version control
  and shared dotfiles.
</Warning>
...
Go to **Settings → MCP & APIs → MCP** in Twenty. Choose your authentication method (OAuth or API Key), copy the JSON snippet (it will already use your workspace URL), and paste it into your MCP client's config file.
...
- _"Show me the 5 most recently created companies"_
- _"Create a new person named Jane Doe at Acme Corp"_
- _"Find all open opportunities worth more than $10k"_
...
Record operation names follow a grammar, so an assistant can build them without a discovery call: `find_many_companies`, `find_one_person`, `update_one_opportunity`. If a name is wrong, `learn_tools` reports it under `notFound` along with the closest matching names.

For anything else, `get_tool_catalog` browses the remaining tools one category at a time. It is a fallback: an unfiltered call returns hundreds of operations and most assistants will truncate the response.
