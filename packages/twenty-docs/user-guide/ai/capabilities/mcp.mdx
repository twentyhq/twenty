...
For anything else, `get_tool_catalog` finds tools from a short `query` (for example "workflow runs") or lists one category. It is a fallback: a call with no arguments returns hundreds of operations and most assistants will truncate the response.
