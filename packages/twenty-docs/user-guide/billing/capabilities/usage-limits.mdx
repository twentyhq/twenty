...
| Where | What happens |
|-------|--------------|
| **AI chat** | The message is answered with the limit that was reached |
| **API and app calls** | The call returns a `QUOTA_EXHAUSTED` error (HTTP 402) |
| **Email campaigns** | Sending stops and the remaining deliveries are marked as skipped |
| **Workflows and logic functions** | The run fails with the limit as its reason |
