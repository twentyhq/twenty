...
<Accordion title="Why my seats are different than the number of users in my workspace?">
We synchronize workspaces with payment system every 6 hours and adjust invoices based on the difference.
</Accordion>

<Accordion title="Why did I get an invoice for new workspace member?">
When new workspace member is added to workspace, new invoice is immediately sent to cover the difference. When workspace member is removed, change is applied in standard monthly/yearly invoice.
</Accordion>
</AccordionGroup>
