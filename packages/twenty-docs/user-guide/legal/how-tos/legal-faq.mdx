...
<Accordion title="Are applications built on Twenty required to be AGPL licensed?">
Applications that only interact with Twenty via its published APIs/SDKs can be licensed under terms of your choice (see the “Twenty Application Exception” in the repository LICENSE). Twenty itself is mostly AGPLv3, and some packages (including the Twenty SDK) are MIT licensed.
</Accordion>
</AccordionGroup>
