...
<Note>
  The `id` field is Twenty's internal identifier, auto-generated for each
  record. It uses UUID format (e.g., `c776ee49-f608-4a77-8cc8-6fe96ae1e43f`).
</Note>
...
<Note>
  The field name `id` is reserved for Twenty's internal ID. Use a different name
  like `externalId` or `legacyId` for external identifiers.
</Note>
...
<Warning>
  To **update existing records**, it is recommended to **only match one unique
  field**.
</Warning>
