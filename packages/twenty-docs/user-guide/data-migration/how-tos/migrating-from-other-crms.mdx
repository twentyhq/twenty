...
<Note>
  Views, workflows, and permissions must be recreated manually after migration.
  Plan time for this configuration work.
</Note>
...
<Note>
  Keep this mapping document handy during import—you'll reference it when
  mapping columns.
</Note>
...
<Accordion title="What about file attachments?">
  File attachments are not included in CSV exports. You'll need to re-upload
  them manually, migrate via API, or contact our team for assistance.
</Accordion>

<Accordion title="Can I keep both systems running during migration?">
  Yes, we recommend keeping your old CRM running until you've verified the
  migration is complete. Just be careful not to create new data in both places.
</Accordion>
