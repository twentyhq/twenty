...
<Note>
  **Why export first?** The exported file has the correct format, includes
  unique identifiers, and maps automatically during import.
</Note>
...
<Note>
  **Use only ONE unique identifier.** Don't map both `email` AND `id`. This can
...
<Accordion title="Can I clear/empty a field value?">
  Yes, leave the cell empty in your CSV. The import will clear that field's
  value on the existing record.
</Accordion>

<Accordion title="What happens to fields I don't include in the import?">
  Fields not in your import file remain unchanged on existing records. Only
  fields you include are updated.
</Accordion>

<Accordion title="Can I update relation fields (like Company)?">
  Yes! Include the relation's unique identifier (e.g., `companyDomain`) and map
  it to the relation field. The relation will be updated.
</Accordion>

<Accordion title="How do I know which records will be updated vs. created?">
  During the import review step, Twenty shows you how many records will be
  updated vs. created based on unique identifier matches.
</Accordion>
