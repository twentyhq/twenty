...
Twenty does not impose a general per-minute request limit on workspace data API calls. Bulk create and upsert requests accept up to 200 records per call.
...
- **Batch operations** — create or upsert up to 200 records per call

**For imports, use batch operations** to maximize throughput.
...
- Group up to **200 records per API call**
- This maximizes throughput

### Handle Transient Errors
- Use bounded concurrency so imports do not overload your deployment
- Retry transient failures with exponential backoff
- Monitor failed requests and resume imports safely
