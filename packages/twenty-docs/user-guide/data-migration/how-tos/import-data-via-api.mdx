...
<Note>
  For datasets in the hundreds of thousands, the API is significantly faster and
  more reliable than multiple CSV imports.
</Note>
...
<Accordion title="Can I update existing records via API?">
  Yes! Use update mutations (GraphQL) or PUT/PATCH requests (REST) with the
  record's `id`.
</Accordion>

<Accordion title="How do I handle duplicates?">
  Query for existing records first using unique identifiers (email, domain).
