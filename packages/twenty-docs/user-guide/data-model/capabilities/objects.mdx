...
- **Owner field**: a relation to workspace members. The member a record points to gets full access to it.
- **Sharing rules**: grant `Can view` or `Can edit` on the records of the object to everyone, a role or a member. A rule without criteria covers every record; criteria narrow it down to matching records. Switching an object to `Private` asks for a first rule so the existing records stay visible.
