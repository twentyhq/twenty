...
### Sharing
The **Sharing** tab of an object decides who can read its records:

- **Level**: `Open` objects are readable by every member the role lets in, which is the default. `Private` objects only show a record to the members who hold access to it, through ownership, a manual share or a sharing rule. A few standard objects such as attachments inherit the level of their parent record; that level is shown but cannot be changed.
- **Owner field**: a relation to workspace members. The member a record points to gets full access to it. Changing the owner field rebuilds that access from the current records: the members named by the previous field lose it, the members named by the new field gain it.
- **Sharing rules**: grant `Can view` or `Can edit` on the records of the object to everyone, a role or a member. A rule without criteria covers every record; criteria narrow it down to matching records. A rule created from the settings starts inactive so its criteria can be set before it grants anything. Switching an object to `Private` asks for a first rule so the existing records stay visible, and that last rule cannot be removed, deactivated or narrowed while the object stays `Private`.

The level and owner field shown in the settings and returned by the API are the effective values: on a standard object, the workspace's choice replaces the level the application declared.
