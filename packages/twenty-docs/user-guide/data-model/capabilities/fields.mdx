...
Indexes are not free: every write has to update them, and each one uses disk space. There's a soft cap of 10 custom indexes per object. Add an index only when you've identified a slow query that would benefit from it.
