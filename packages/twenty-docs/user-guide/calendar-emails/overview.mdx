...
<Note>
**Syncing internal emails**: The "internal emails not synced" behavior is the default, but it can be turned off. Go to **Settings → Security**, expand **Advanced** settings, and enable the **Sync Internal Emails** toggle to include emails where all participants share the same domain. This is a workspace-wide setting (useful for universities or shared-domain organizations).
</Note>
...
- **Internal Emails**: Not synced by default (same domain emails remain private). Enable **Sync Internal Emails** in **Settings → Security → Advanced** to include them workspace-wide.
