...
Um einen API-Schlüssel zu erstellen, gehen Sie zu **Settings → MCP & APIs → API → + Create key**. Details finden Sie unter [APIs](/de/developers/extend/api#create-an-api-key).
...
* **API-Schlüssel**: Weisen Sie dem API-Schlüssel unter **Settings → Members → Roles** eine Rolle zu. Siehe [Berechtigungen](/de/user-guide/permissions-access/capabilities/permissions).
