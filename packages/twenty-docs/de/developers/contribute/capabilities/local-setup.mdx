...
Verwenden Sie Umgebungsvariablen oder `.env`-Dateien, um Ihr Projekt zu konfigurieren. Weitere Informationen [hier](/de/developers/self-host/capabilities/setup).
...
<Info>
**Multi-Workspace-Modus:** Standardmäßig läuft Twenty im Einzel-Workspace-Modus, in dem nur ein Workspace erstellt werden kann. Um die Multi-Workspace-Unterstützung zu aktivieren (nützlich zum Testen von Subdomain-basierten Funktionen), setzen Sie `IS_MULTIWORKSPACE_ENABLED=true` in der `.env`-Datei Ihres Servers. Siehe [Multi-Workspace-Modus](/de/developers/self-host/capabilities/setup#multi-workspace-mode) für Details.
</Info>
...
Sollten Sie auf ein Problem stoßen, schauen Sie sich die [Fehlerbehebung](/de/developers/self-host/capabilities/troubleshooting) für Lösungen an.
