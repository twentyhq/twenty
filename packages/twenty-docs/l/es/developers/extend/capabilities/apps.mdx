...
* `roleUniversalIdentifier` debe coincidir con el rol que defines en tu archivo `*.role.ts` (ver abajo).

#### Roles y permisos

Las aplicaciones pueden definir roles que encapsulan permisos sobre los objetos y acciones de tu espacio de trabajo. El campo `roleUniversalIdentifier` en `application.config.ts` designa el rol predeterminado que usan las funciones de lógica de tu aplicación.
...
El `universalIdentifier` de este rol se referencia luego en `application.config.ts` como `roleUniversalIdentifier`. En otras palabras:
...
* Los permisos de la clave de API están determinados por el rol referenciado en tu `application.config.ts` mediante `roleUniversalIdentifier`. Este es el rol predeterminado que usan las funciones de lógica de tu aplicación.
* Las aplicaciones pueden definir roles para seguir el principio de mínimo privilegio. Concede solo los permisos que necesitan tus funciones y después apunta `roleUniversalIdentifier` al identificador universal de ese rol.
