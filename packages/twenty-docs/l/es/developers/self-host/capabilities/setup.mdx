...
Con el modo multi-workspace habilitado, no incluyas aquí cada subdominio de workspace.
En su lugar, enruta las rutas de la API (`/graphql`, `/metadata`, `/rest`, `/auth/`,
`/client-config`, `/file`) al back-end en cada nombre de host de workspace, de modo que cada
workspace permanezca con el mismo origen; así es como Twenty Cloud se ejecuta.
