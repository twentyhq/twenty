...
<Info>
  **Modo de múltiples espacios de trabajo:** De forma predeterminada, Twenty se
  ejecuta en modo de un solo espacio de trabajo, donde solo se puede crear un
  espacio de trabajo. Para habilitar la compatibilidad con múltiples espacios de
  trabajo (útil para probar funciones basadas en subdominios), establece
  `IS_MULTIWORKSPACE_ENABLED=true` en el archivo `.env` de tu servidor. Consulta
  [Modo de múltiples espacios de
  trabajo](/l/es/developers/self-host/capabilities/setup#multi-workspace-mode)
  para más detalles.
</Info>
