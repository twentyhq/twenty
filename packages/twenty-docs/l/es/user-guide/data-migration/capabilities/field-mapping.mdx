...
<Note>
  **Consejo rápido:** Exporta algunas filas del objeto que quieres importar. El
  archivo exportado tendrá los nombres de columna exactos que Twenty espera, lo
  que hará que la asignación automática sea fluida durante la importación.
</Note>
...
<Note>
  **Los campos deben existir antes de la importación.** La importación crea
  registros, no campos. Crea campos personalizados en **Settings → Data Model**
  antes de importar.
</Note>
...
<Note>
  **Algunos campos tienen una sintaxis especial.** Recomendamos descargar el
  archivo de muestra antes de preparar la importación para ver la sintaxis
  esperada para cada tipo de campo.
</Note>
...
<Note>
  Si proporcionas un `id`, debe estar en formato UUID (p. ej.,
...
<Note>
  Las nuevas opciones de selección no se crearán automáticamente mediante la
  importación. Deben agregarse en **Settings → Data Model** antes de importar.
</Note>
...
<Note>
  Las nuevas opciones de Select no se crearán automáticamente mediante la
  importación. Deben agregarse en **Configuración → Modelo de datos** antes de
  importar.
</Note>
