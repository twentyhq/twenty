...
<Warning>
  Supprimer votre espace de travail supprime définitivement toutes les données
  et ne peut pas être annulé. Toutes les données de l'espace de travail seront
  perdues à jamais, tous les membres perdront immédiatement l'accès et cette
  action ne peut pas être inversée.
</Warning>
