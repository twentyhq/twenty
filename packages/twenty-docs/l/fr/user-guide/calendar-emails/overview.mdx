...
<Warning>
  Seules les vraies boîtes aux lettres peuvent être connectées (par exemple,
  support@domain.com avec sa propre boîte de réception). Les alias email qui
  redirigent vers une autre boîte aux lettres ne peuvent pas être connectés à
  Twenty.
</Warning>
...
<Note>
  Lorsque cette option est activée, les contacts sont automatiquement liés à
  leurs enregistrements Entreprise en fonction de leur domaine de messagerie. Si
  l'entreprise n'existe pas encore, Twenty la crée pour vous.
</Note>
...
<Note>
  **Durée de la synchronisation initiale**: La synchronisation du calendrier se
  termine rapidement (généralement en quelques minutes), tandis que la
  synchronisation des emails prend plus de temps pour les grandes boîtes aux
  lettres. Twenty importe des emails à environ **400 messages par minute**
  (soumis aux limites de débit de l'API Gmail), de sorte qu'une boîte aux
  lettres contenant 10,000 emails prend environ 25 minutes. Ne vous inquiétez
  pas si vous voyez des contacts provenant d'événements de calendrier apparaître
