...
<Note>
  Pour les agents IA s’exécutant au sein de flux de travail, l’attribution d’un
  rôle garantit que l’agent ne peut pas accéder ni modifier des données en
  dehors de sa portée prévue—même si le flux de travail dispose d’autorisations
  plus étendues.
</Note>
