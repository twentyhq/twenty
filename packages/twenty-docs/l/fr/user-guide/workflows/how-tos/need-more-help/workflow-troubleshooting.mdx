...
<Tip>
  Si vous souhaitez que le flux de travail ne s'exécute qu'une seule fois par
  enregistrement, ajoutez une action Filter pour vérifier un champ comme
  `createdAt equals updatedAt` (première sauvegarde) ou utilisez un champ à case
  à cocher personnalisé pour suivre si le flux de travail s'est déjà exécuté.
</Tip>
