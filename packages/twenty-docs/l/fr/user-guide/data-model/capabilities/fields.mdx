...
3. Continuez en cliquant sur `+ Nouveau champ`.
...
Les index de base de données sont gérés automatiquement — ajouter les vôtres est rarement nécessaire et il est facile de se tromper. Lorsque le mode avancé est activé, chaque objet possède une section **Index** sous `Paramètres → Modèle de données → <object>` pour les cas où vous savez que vous en avez besoin.
