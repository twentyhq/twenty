...
<Note>
  **Limites d'affichage** : Les diagrammes en barres peuvent afficher au maximum
  100 barres, et 50 groupes par barre lors de l'utilisation de Group By. Si vous
  voyez l'avertissement "Données non affichées : X barres max par graphique",
  ajoutez des filtres pour restreindre vos données ou modifiez le regroupement
  (par ex., regroupez par semaine plutôt que par jours).
</Note>
...
<Note>
  **Bientôt disponible** : Les jauges et les tableaux ne sont pas encore
  disponibles, mais figurent sur notre feuille de route.
</Note>
