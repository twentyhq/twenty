...
#### Erreur « File Not Found » lors du test de la fonction Logic dans le workflow

Exécutez la commande `yarn command:prod application:rebuild-default-deps` dans votre conteneur twenty-server pour résoudre le problème de dépendances manquantes.
