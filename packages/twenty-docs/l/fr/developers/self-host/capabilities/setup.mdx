...
et `FRONTEND_URL` sont déjà approuvées. Le serveur consigne un avertissement lorsque un navigateur
...
Avec le mode multi-workspace activé, ne répertoriez pas ici chaque sous-domaine de workspace.
Au lieu de cela, acheminez les chemins d’API (`/graphql`, `/metadata`, `/rest`, `/auth/`,
`/client-config`, `/file`) vers le back-end sur chaque nom d’hôte de workspace, afin que chaque
workspace reste en same-origin — c’est ainsi que Twenty Cloud fonctionne.
