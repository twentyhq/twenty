...
<Warning>
Les conteneurs Docker sont destinés à l'hébergement en production ou à l'auto-hébergement. Pour contribuer, veuillez consulter la [configuration locale](/l/fr/developers/contribute/capabilities/local-setup).
</Warning>
...
Consultez [Configurer les variables d'environnement](/l/fr/developers/self-host/capabilities/setup) pour la configuration avancée. Toutes les variables d'environnement doivent être déclarées dans le fichier `docker-compose.yml` au niveau du serveur et/ou du worker, selon la variable.
...
2. **Générer une clé de chiffrement**
   ...
   **Important :** Gardez cette valeur secrète / ne la partagez pas. La perte de `ENCRYPTION_KEY` signifie la perte d’accès à chaque secret stocké dans la base de données (jetons OAuth, variables d’application, secrets TOTP, etc.).
...
3. **Mettez à jour le `.env`**
   ...
   Voir le [guide de rotation des clés](/l/fr/developers/self-host/capabilities/key-rotation) pour obtenir des instructions sur la rotation de la clé sans interruption de service.
...
Par défaut, Twenty fonctionne sur `localhost` au port `3000`. L’interface utilisateur résout l’API à partir de l’origine propre du navigateur, elle se charge donc à partir de n’importe quel nom d’hôte qui atteint le conteneur — mais définissez tout de même `SERVER_URL` sur votre URL publique : le serveur l’utilise pour les liens générés (e‑mails, callbacks OAuth) et pour détecter le HTTPS pour les cookies sécurisés.
...
<Tip>
  Vous préférez ne pas héberger et maintenir Twenty vous-même ? [Trouvez un partenaire d’hébergement Twenty certifié](https://twenty.com/partners/list?categories=HOSTING\&ref=docs-docker-compose) pour déployer, héberger et gérer votre instance de bout en bout. *(Ou contactez notre équipe : [contact@twenty.com](mailto:contact@twenty.com).)*
</Tip>
