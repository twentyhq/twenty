...
icon: "paintbrush}]} }```}Replace in message with correct JSON. Wait. I'm messed. Let's rewrite cleanly. Sorry. No, we must output only JSON. I'll output correct JSON now.``` JSON: {"
---
...
Pour garantir la conformité de votre code à cette règle, assurez-vous d'exécuter Oxlint dans le cadre de votre flux de travail de développement.
