...
<Info>
  **Mode multi-espaces de travail:** Par défaut, Twenty s'exécute en mode
  mono-espace de travail, où un seul espace de travail peut être créé. Pour
  activer la prise en charge multi-espaces de travail (utile pour tester des
  fonctionnalités basées sur des sous-domaines), définissez
  `IS_MULTIWORKSPACE_ENABLED=true` dans le fichier `.env` de votre serveur. Voir
  [Mode multi-espaces de
  travail](/l/fr/developers/self-host/capabilities/setup#multi-workspace-mode)
