...
<Warning>
  Yalnızca gerçek posta kutuları bağlanabilir (ör. support@domain.com kendi
  gelen kutusu ile). Başka bir posta kutusuna yönlendirilen e-posta takma adları
  Twenty'ye bağlanamaz.
</Warning>
...
<Note>
  Etkinleştirildiğinde, kişiler e-posta alan adlarına göre Şirket kayıtlarına
  otomatik olarak bağlanır. Şirket henüz mevcut değilse, Twenty onu sizin için
  oluşturur.
</Note>
...
<Note>
  **İlk senkronizasyon zamanlaması**: Takvim senkronizasyonu hızlı tamamlanır
  (genellikle birkaç dakika içinde), e-posta senkronizasyonu ise büyük posta
  kutuları için daha uzun sürer. Twenty, e-postaları yaklaşık olarak dakikada
  **400 ileti** hızında içe aktarır (Gmail API hız sınırlaması tarafından
  kısıtlanır), bu nedenle 10,000 e-postalı bir posta kutusunun içe aktarılması
  yaklaşık 25 dakika sürer. Takvim etkinliklerinden gelen kişilerin e-posta
  kişilerinizden önce göründüğünü görürseniz endişelenmeyin; bu normal bir
  davranıştır.
</Note>
