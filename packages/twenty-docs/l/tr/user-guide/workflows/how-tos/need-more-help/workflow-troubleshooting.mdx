...
<Tip>
  Kayıt başına iş akışının yalnızca bir kez çalışmasını istiyorsanız, `createdAt
  equals updatedAt` (ilk kaydetme) gibi bir alanı kontrol etmek için bir Filtre
  eylemi ekleyin veya iş akışının daha önce çalışıp çalışmadığını izlemek için
  özel bir onay kutusu alanı kullanın.
</Tip>
