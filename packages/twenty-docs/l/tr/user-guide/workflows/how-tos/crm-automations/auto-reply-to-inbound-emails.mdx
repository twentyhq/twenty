...
<Tip>
  **In-Reply-To**, tetikleyiciden bir `message.headerMessageId` bekler — bu bir
  alıcı adresi değil, e-postanın benzersiz parmak izidir. Bunu boş bırakırsanız
  e-posta yine de gönderilir, yalnızca bağımsız bir ileti olarak.
</Tip>

<Warning>
  Gmail, iletileri diziler halinde gruplamak için konu satırını kullanır.
  Gmail’in yanıtı özgün dizinin içinde göstermesi için konu mutlaka `Re:` ile
  başlamalıdır (iki nokta ve boşluk dahil). Bu olmadan, In-Reply-To başlığı
  doğru ayarlanmış olsa bile yanıt ayrı bir konuşma olarak görünür.
</Warning>
