...
<Note>
  **İçe aktarılabilecekler:** Yalnızca tek bir nesne türüne işaret eden
  bire-çoğa ilişkiler. Birden fazla nesne türüne işaret eden ilişkiler (ör.
  Notların Kişilere VE Şirketlere bağlanması) henüz içe aktarmada
  desteklenmiyor.
</Note>
...
<Note>
  **Önerilir:** Şirketler için `domain`, Kişiler için `email` kullanın. Bunlar
  insan tarafından okunabilirdir ve e-tablonuzda doğrulanması kolaydır.
</Note>
...
<Accordion title="Mevcut kayıtlardaki ilişkileri güncelleyebilir miyim?">
  Evet! Alt kaydın benzersiz tanımlayıcısını (ör. Kişiler için `email`) ve yeni
  ilişki değerini ekleyin. İçe aktarma ilişkiyi günceller.
</Accordion>

<Accordion title="Çoktan-çoka ilişkileri içe aktarabilir miyim?">
  Çoktan-çoka ilişkiler henüz içe aktarmada desteklenmiyor. Bu, 2026'nın ilk
  yarısı için planlanıyor.
</Accordion>

<Accordion title="Birden fazla nesne türüne bağlantı verebilir miyim (ör. Notları Kişilere VE Şirketlere)?">
  Birden fazla nesne türüne işaret eden ilişkiler henüz içe/dışa aktarma için
  desteklenmiyor. Bu özellik yol haritamızda.
</Accordion>

<Accordion title="Ana kayıt mevcut değilse ne olur?">
  İçe aktarma, o satır için bir hata gösterir. Aşağıdakileri yapabilirsiniz: *
  Önce ana kaydı içe aktarın, ardından yeniden içe aktarın * Referans değerini
  düzeltin * Satırı içe aktarmadan çıkarın
</Accordion>
