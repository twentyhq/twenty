...
<Note>
  **Hataları arayüzde düzeltin.** E-tablonuza geri dönmenize gerek yok. Zamandan
  tasarruf etmek için içe aktarma sırasında hücreleri doğrudan düzenleyin.
</Note>
...
<Note>
  Tüm biçimler geçerlidir, ancak e-posta/takvim senkronizasyonunun kullandığı
  biçimle eşleştiği için `https://domain.com` önerilir. Diğer biçimlerin
  kullanılması yinelenen şirketler oluşturabilir.
</Note>
...
<Note>
  Diğer hatalardan farklı olarak, bu hata veri inceleme adımında yakalanmaz. İçe
  aktarma başlar ve eksik ilişkiyle karşılaştığında başarısız olur.
</Note>
...
<Note>
  CSV içe aktarma alanları değil, kayıtları oluşturur. İçe aktarmadan önce tüm
