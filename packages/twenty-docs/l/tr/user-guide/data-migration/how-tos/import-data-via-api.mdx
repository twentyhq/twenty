...
<Note>
  Yüz binlerce kayıttan oluşan veri kümeleri için API, birden çok CSV içe
  aktarımından belirgin şekilde daha hızlı ve daha güvenilirdir.
</Note>
...
<Accordion title="API aracılığıyla mevcut kayıtları güncelleyebilir miyim?">
  Evet! Kayıdın `id`si ile güncelleme mutasyonları (GraphQL) veya PUT/PATCH
  isteklerini (REST) kullanın.
</Accordion>

<Accordion title="Yinelenenleri nasıl ele alırım?">
  Önce benzersiz tanımlayıcıları (e-posta, etki alanı) kullanarak mevcut
  kayıtları sorgulayın. Varsa güncelleyin, yoksa oluşturun.
</Accordion>
