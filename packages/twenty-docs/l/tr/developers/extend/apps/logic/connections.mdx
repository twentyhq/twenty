...
| `name`            | Bağlantının görünen adı (OAuth geri çağrısında otomatik türetilir, kullanıcı tarafından yeniden adlandırılabilir)                                                                                                                                   |
| `tanıtıcı`        | Bağlı yukarı akış hesabının e-postası, her yeniden bağlanışta yenilenir; sağlayıcı bir e-posta döndürmediğinde, jeton değişiminde döndürülen OIDC `id_token` değerinden çözülür ve yedek olarak bağlanan Twenty kullanıcısının e-postası kullanılır |
