...
<CardGroup cols={2}>
  ...
  <Card title="Zaman Çizelgesi Aktivite Türleri" icon="clock-rotate-left" href="/l/tr/developers/extend/apps/data/timeline-activity-types">
    Kayıt zaman çizelgeleri için otomatik denetim olaylarını ve açık uygulama olaylarını tanımlayın.
  </Card>
</CardGroup>
...
| **Zaman Çizelgesi Aktivite Türü** | Kayıt zaman çizelgelerinde gösterilen otomatik denetim olayı veya açık etki alanı olayı.                                              | `defineTimelineActivityType()`           |

SDK bunları derleme zamanında AST analizi aracılığıyla algılar; dolayısıyla dosya organizasyonu size kalmıştır — gelenek `src/objects/`, `src/fields/`, `src/indexes/` ve `src/timeline-activity-types/` dizinlerini kullanmaktır. Kararlı `universalIdentifier` UUID’leri, dağıtımlar arasında her şeyi birbirine bağlar.
