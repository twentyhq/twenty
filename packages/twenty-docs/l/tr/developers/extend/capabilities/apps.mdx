...
Uygulamalar, Twenty özelleştirmelerini **kod olarak** oluşturup yönetmenizi sağlar. Instead of configuring everything through the UI, you define your data model and logic functions in code — making it faster to build, maintain, and roll out to multiple workspaces.
...
* **src/**: The main place where you define your application-as-code:
  ...
  * `*.role.ts`: Role definitions used by your logic functions. Aşağıda "Varsayılan fonksiyon rolü"ne bakın.
  * `*.object.ts`: Özel nesne tanımları.
...
Uygulamalar, çalışma alanınızdaki nesneler ve eylemler üzerindeki izinleri kapsülleyen roller tanımlayabilir. The field `functionRoleUniversalIdentifier` in `application.config.ts` designates the default role used by your app's logic functions.
...
When a route trigger invokes your logic function, it receives a `RoutePayload` object that follows the AWS HTTP API v2 format. Türü `twenty-sdk` içinden içe aktarın:
...
By default, HTTP headers from incoming requests are **not** passed to your logic function for security reasons. Belirli başlıklara erişmek için bunları açıkça `forwardedRequestHeaders` dizisinde listeleyin:
...
* API anahtarının izinleri, `application.config.ts` içinde `functionRoleUniversalIdentifier` aracılığıyla referans verilen role göre belirlenir. This is the default role used by logic functions of your application.
