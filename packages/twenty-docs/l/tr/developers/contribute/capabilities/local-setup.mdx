...
<Warning>
  `npm` çalışmaz, bunun yerine `yarn` kullanmalısınız. Yarn artık Node.js ile
  birlikte geliyor, bu yüzden ayrı bir yüklemeye gerek yoktur. Eğer henüz
  yapmadıysanız Yarn'ı etkinleştirmek için sadece `corepack enable` komutunu
...
<Warning>
  Doğru `node` versiyonunu yüklemek için `nvm` kullanın. `.nvmrc` tüm katkıda
  bulunanların aynı versiyonu kullanmasını sağlar.
</Warning>
...
<Info>
  **Çoklu Çalışma Alanı Modu:** Varsayılan olarak, Twenty yalnızca bir çalışma
  alanının oluşturulabildiği tek çalışma alanı modunda çalışır. Çoklu çalışma
  alanı desteğini etkinleştirmek için (alt alan adına dayalı özellikleri test
  etmek açısından kullanışlıdır), sunucunuzdaki `.env` dosyanızda
  `IS_MULTIWORKSPACE_ENABLED=true` değerini ayarlayın. Ayrıntılar için [Çoklu
  Çalışma Alanı
  Modu](/l/tr/developers/self-host/capabilities/setup#multi-workspace-mode)
  bölümüne bakın.
</Info>
...
<Tabs>
  <Tab title="Linux">
    ...
    Değilse, dağıtımınız için [Redis Kurulum
    Kılavuzu](https://redis.io/docs/latest/operate/oss_and_stack/install/install-redis/)
    üzerinden kontrol edin.
  </Tab>
  <Tab title="Mac OS">
    Redis zaten çalışıyor olmalıdır. Değilse, şu komutu çalıştırın: ```bash brew
