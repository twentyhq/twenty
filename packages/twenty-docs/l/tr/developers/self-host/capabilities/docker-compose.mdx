...
Varsayılan olarak, Twenty `localhost` üzerinde `3000` portunda çalışır. UI, API'yi tarayıcının kendi origin'inden çözümler; bu nedenle container'a erişen herhangi bir hostname'den yüklenir — ancak yine de `SERVER_URL` değerini genel (public) URL'inize ayarlayın: sunucu bunu oluşturulan bağlantılar (e-postalar, OAuth geri aramaları) ve güvenli çerezler için HTTPS'i algılamak amacıyla kullanır.
