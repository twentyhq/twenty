...
Приложения позволяют создавать и управлять настройками Twenty **в виде кода**. Instead of configuring everything through the UI, you define your data model and logic functions in code — making it faster to build, maintain, and roll out to multiple workspaces.
...
* **src/**: The main place where you define your application-as-code:
  ...
  * `*.role.ts`: Role definitions used by your logic functions. См. раздел «Роль функции по умолчанию» ниже.
  * `*.object.ts`: Определения пользовательских объектов.
...
Приложения могут определять роли, инкапсулирующие права на объекты и действия в вашем рабочем пространстве. The field `functionRoleUniversalIdentifier` in `application.config.ts` designates the default role used by your app's logic functions.
...
When a route trigger invokes your logic function, it receives a `RoutePayload` object that follows the AWS HTTP API v2 format. Импортируйте тип из `twenty-sdk`:
...
By default, HTTP headers from incoming requests are **not** passed to your logic function for security reasons. Чтобы получить доступ к определённым заголовкам, явно перечислите их в массиве `forwardedRequestHeaders`:
...
* Права ключа API определяются ролью, на которую ссылается ваш `application.config.ts` через `functionRoleUniversalIdentifier`. This is the default role used by logic functions of your application.
