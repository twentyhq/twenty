...
Активно разрабатываются передние компоненты. Рендеринг, стиль и обработка событий хорошо работают. Все, что достигает *прошлого* рендеринга (измерение элемента, вызов метода DOM на ref, портирование за пределами дерева, прикосновение к хранилищу браузера) сегодня отсутствует или незавершено и большинство из них бесшумно: без исключения, и никакой ошибки TypeScript, так как строительные площадки набраны в полном DOM браузере.

Если один из этих блоков вас, [откройте задачу](https://github.com/twentyhq/twenty/issues/new/choose), чтобы получить приоритет.

### Макет и измерение

Ничто не может измерять сам себя.

| API                                                           | Что происходит                                                              |
| ------------------------------------------------------------- | --------------------------------------------------------------------------- |
| `getBoundingClientRect()`, `getClientRects()`                 | Бросает                                                                     |
| `offsetWidth`, `clientWidth`, `scrollWidth`, `offsetTop`, ... | Тихо `undefined`, так `width ?? 0-й урожай `0`и`ширина > 600\` всегда false |
| `ResizeObserver`, `IntersectionObserver`                      | `Справочная ошибка` (защита `typeof` делает работу)                         |
| `window.matchMedia()`, `window.getComputedStyle()`            | Бросает                                                                     |
| `window.innerWidth`, `innerHeight`, `devicePixelRatio`        | Молчально «неопределенно»                                                   |
| `new MutationObserver(fn)`                                    | Построит, затем выбрасывает `.observe()`                                    |

Таким образом, перекодирует `ResponsiveContainer`, плавающий UI / Popper, список виртуализации и перетаскивание не работают. Сделайте макет в CSS вместо: ваш стиль достигает реальной страницы, так что flexbox, сетка, `aspect-ratio`, `clamp()` и `@container` все ведут себя нормально.
...
| `document.getElementById()`, `getElementsByClassName()`, `createTreeWalker()`                               | Бросает                                                            | `querySelector()` / `querySelectorAll()`, который работает                                           |
