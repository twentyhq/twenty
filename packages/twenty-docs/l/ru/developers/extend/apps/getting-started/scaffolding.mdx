...
| Вкладка компоновки страницы     | `yarn twenty dev:add pageLayoutTab`        | `src/page-layout-tabs/\<name>.ts`        |
| Элемент меню команд             | `yarn twenty dev:add commandMenuItem`      | `src/command-menu-items/\<name>.ts`      |
| Поле представления              | `yarn twenty dev:add viewField`            | `src/view-fields/\<name>.ts`             |
| Провайдер подключения           | `yarn twenty dev:add connectionProvider`   | `src/connection-providers/\<name>.ts`    |
| Тип действия на временной шкале | `yarn twenty dev:add timelineActivityType` | `src/timeline-activity-types/\<name>.ts` |
