...
## Связи через промежуточный объект

Промежуточный объект связывает одну исходную запись с одной или несколькими целевыми записями. Например, `PostCardRecipient` может содержать связь `postCard` и связь `recipient`. В поле `ONE_TO_MANY` исходного объекта задайте для `junctionTargetFieldUniversalIdentifier` поле связи в промежуточном объекте, указывающее на цель:
...
`RECIPIENT_FIELD_ID` должен идентифицировать связь `MANY_TO_ONE` или морфную связь в промежуточном объекте. Это сообщает Twenty, какая связанная запись является целевой, когда функция, например [распределение действий временной шкалы](/l/ru/developers/extend/apps/data/timeline-activity-types#automatic-and-explicit-events), проходит через промежуточный объект.
...
| `universalSettings.junctionTargetFieldUniversalIdentifier` | Только для промежуточного объекта ONE_TO_MANY | Поле связи в промежуточном объекте, указывающее на цель распределения                           |
