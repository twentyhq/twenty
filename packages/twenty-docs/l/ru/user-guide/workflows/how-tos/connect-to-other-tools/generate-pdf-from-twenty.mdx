...
<Tip>
  С помощью ручного триггера пользователи могут запускать этот рабочий процесс с
  кнопки, которая появляется в правом верхнем углу после выбора записи, чтобы
  сгенерировать и прикрепить PDF.
</Tip>
...
<Note>
  Если прикрепляете к другому объекту (Person, Opportunity и т. д.),
  переименуйте параметр соответствующим образом (например, `personId`,
  `opportunityId`) и обновите логическую функцию.
</Note>
