...
## Aggiornamento alla v2.34+ — PostgreSQL 15 minimo

Twenty v2.34 richiede PostgreSQL 15 o una versione successiva. La configurazione ufficiale di Docker Compose utilizza PostgreSQL 16. Se la tua istanza si connette a un database gestito esternamente, aggiornalo prima di avviare il server v2.34.

## Aggiornamenti tra versioni (v1.23+)

A partire da **v1.23**, Twenty supporta gli aggiornamenti tra versioni. Puoi passare direttamente da qualsiasi versione supportata all'ultima versione senza dover attraversare ogni versione intermedia.

Ad esempio, l'aggiornamento da v1.23 direttamente a v2.0 è pienamente supportato.
