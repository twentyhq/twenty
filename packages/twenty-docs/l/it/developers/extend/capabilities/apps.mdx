...
Le app ti consentono di creare e gestire le personalizzazioni di Twenty **come codice**. Instead of configuring everything through the UI, you define your data model and logic functions in code — making it faster to build, maintain, and roll out to multiple workspaces.
...
* **src/**: The main place where you define your application-as-code:
  ...
  * `*.role.ts`: Role definitions used by your logic functions. Vedi "Ruolo funzione predefinito" qui sotto.
  * `*.object.ts`: Definizioni di oggetti personalizzati.
...
Le applicazioni possono definire ruoli che incapsulano i permessi sugli oggetti e sulle azioni del tuo spazio di lavoro. The field `functionRoleUniversalIdentifier` in `application.config.ts` designates the default role used by your app's logic functions.
...
When a route trigger invokes your logic function, it receives a `RoutePayload` object that follows the AWS HTTP API v2 format. Importa il tipo da `twenty-sdk`:
...
By default, HTTP headers from incoming requests are **not** passed to your logic function for security reasons. Per accedere a intestazioni specifiche, elencale esplicitamente nell'array `forwardedRequestHeaders`:
...
* I permessi della chiave API sono determinati dal ruolo referenziato in `application.config.ts` tramite `functionRoleUniversalIdentifier`. This is the default role used by logic functions of your application.
