...
3. Procedi cliccando su `+ Nuovo Campo`.
...
Gli indici del database sono gestiti automaticamente — aggiungerne di propri è raramente necessario ed è facile commettere errori. Con la modalità Avanzata attiva, ogni oggetto ha una sezione **Indici** in `Impostazioni → Modello dati → <object>` per i casi in cui sai di averne bisogno.
