...
Per creare una chiave API, vai su **Impostazioni → MCP e API → API → + Crea chiave**. Vedi [API](/l/it/developers/extend/api#create-an-api-key) per i dettagli.
...
* **Chiave API**: assegna un ruolo alla chiave API in **Impostazioni → Membri → Ruoli**. Vedi [Autorizzazioni](/l/it/user-guide/permissions-access/capabilities/permissions).
