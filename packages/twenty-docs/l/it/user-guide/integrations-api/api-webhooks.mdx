...
Ad esempio, quando un'opportunità passa a "Chiuso Vinto", un webhook può automaticamente attivare la creazione della fattura nel tuo sistema contabile. Nota che questo tipo di automazione può essere ottenuto anche utilizzando la funzione [Workflows di Twenty](/user-guide/workflows/getting-started-workflows) in-app, che offre trigger basati su aggiornamenti di campo per automazione interna.
