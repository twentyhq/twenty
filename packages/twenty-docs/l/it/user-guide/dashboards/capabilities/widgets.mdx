...
<Note>
  **Limiti di visualizzazione**: I grafici a barre possono mostrare al massimo
  100 barre e, quando si usa Raggruppa per, 50 gruppi per barra. Se visualizzi
  l'avviso "Undisplayed data: max X bars per chart", aggiungi filtri per
  restringere i dati o modifica il raggruppamento (ad es., raggruppa per
  settimana invece che per giorni).
</Note>
...
<Note>
  **In arrivo**: I grafici a indicatore e le tabelle non sono ancora disponibili
  ma sono nella nostra roadmap.
</Note>
