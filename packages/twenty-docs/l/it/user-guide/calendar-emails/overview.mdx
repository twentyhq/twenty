...
<Warning>
  Solo vere caselle di posta possono essere collegate (es. supporto@dominio.com
  con la propria inbox). Alias email che inoltrano ad un'altra casella non
  possono essere collegati a Twenty.
</Warning>
...
<Note>
  Quando è attivata, i contatti vengono collegati automaticamente ai relativi
  record di Azienda in base al loro dominio email. Se l'Azienda non esiste
  ancora, Twenty la crea per te.
</Note>
...
<Note>
  **Tempistiche di sincronizzazione iniziale**: La sincronizzazione del
  calendario si completa rapidamente (di solito in pochi minuti), mentre la
  sincronizzazione delle email richiede più tempo per le caselle di posta di
  grandi dimensioni. Twenty importa email a circa **400 messaggi al minuto**
  (limitato dal rate limiting dell'API di Gmail), quindi una casella di posta
  con 10.000 email impiega circa 25 minuti. Non preoccuparti se vedi contatti
  provenienti dagli eventi del calendario apparire prima dei tuoi contatti
  email; è un comportamento normale.
</Note>
