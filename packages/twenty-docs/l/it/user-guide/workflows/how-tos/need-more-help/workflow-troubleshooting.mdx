...
<Tip>
  Se desideri che il flusso di lavoro venga eseguito solo una volta per record,
  aggiungi un'azione Filter per verificare un campo come `createdAt equals
  updatedAt` (primo salvataggio) oppure usa un campo casella di controllo
  personalizzato per tracciare se il flusso di lavoro è già stato eseguito.
</Tip>
