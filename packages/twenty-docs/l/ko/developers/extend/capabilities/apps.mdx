...
- **.oxlintrc.json** 및 **tsconfig.json**: 앱의 TypeScript 소스에 대한 린팅 및 TypeScript 구성을 제공합니다.
...
<Note>
  **기본 필드는 자동으로 생성됩니다.** 사용자 정의 객체를 정의하면 Twenty가
  `name`, `createdAt`, `updatedAt`, `createdBy`, `position`, `deletedAt` 등의
  표준 필드를 자동으로 추가합니다. 이 필드들은 `fields` 배열에 정의할 필요가
  없습니다. 사용자 정의 필드만 추가하세요.
</Note>
