...
훅이 실행될 때는 `ConnectedAccount`가 이미 제거된 상태이므로 `getConnection(connectedAccountId)`는 더 이상 해결(resolve)되지 않습니다. 정리(cleanup)에 필요한 모든 것(예: `team_id`, 외부 구독 ID)은 연결 시점에 `connectedAccountId`를 키로 사용해 [key-value store](/l/ko/developers/extend/apps/logic/key-value-store)에 미리 기록되어 있어야 합니다.
...
| `handle`          | 각 재연결 시 새로 고쳐지는 연결된 업스트림 계정의 이메일로, 토큰 교환 시 반환된 OIDC `id_token`에서 확인되며, 공급자가 이메일을 반환하지 않는 경우 연결 중인 Twenty 사용자의 이메일로 대체됩니다 |
