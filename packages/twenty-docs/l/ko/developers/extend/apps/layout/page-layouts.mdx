...
* 각 `tab`은 `title`, `position`, `layoutMode`를 사용해 페이지의 섹션을 정의합니다. 레코드 페이지와 독립형 페이지에는 `VERTICAL_LIST`를, 대시보드에는 `GRID`를 사용합니다. `VERTICAL_LIST` 탭에서는 단일 위젯이 전체 너비로 렌더링되며 탭 전체를 차지합니다. 여러 개의 위젯이 있으면 카드 형태로 위에서 아래로 쌓입니다. `GRID` 탭은 위젯의 개수와 관계없이 항상 12열 그리드에서 카드 형태로 배치되므로, 하나의 위젯이 페이지 전체를 채우도록 하려면 `VERTICAL_LIST`를 선택하세요.
