...
[내비게이션 메뉴 항목](/l/ko/developers/extend/apps/layout/navigation-menu-items)은 해당 뷰를 사이드바에 배치합니다. 서버는 이미 각 객체의 기본 "All ..." 보기를 자동으로 생성하므로, 앱이 정의하는 보기는 추가로 제공되는 선별된 보기입니다.
...
`FIELD` 위젯은 중첩된 관계의 레코드를 두 단계 떨어진 곳까지 나열하는 것을 포함하여, 다른 표시 모드와 옵션도 지원합니다. 자세한 내용은 [필드 위젯 레퍼런스](/l/ko/developers/extend/apps/layout/page-layouts#field-widgets)를 참고하세요.
