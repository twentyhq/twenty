...
Twenty supports logic functions for workflows and custom logic. `SERVERLESS_TYPE` 환경 변수를 통해 실행 환경을 구성합니다.

<Warning>
  **Security Notice:** The local driver (`SERVERLESS_TYPE=LOCAL`) runs code directly on the host in a Node.js process with no sandboxing. 개발 환경에서 신뢰할 수 있는 코드에만 사용해야 합니다. 신뢰할 수 없는 코드를 처리하는 프로덕션 배포의 경우 `SERVERLESS_TYPE=LAMBDA` 또는 `SERVERLESS_TYPE=DISABLED` 사용을 강력히 권장합니다.
</Warning>
...
| ------ | -------------------------- | -------------------------------- | --------------- |
| 비활성화   | `SERVERLESS_TYPE=DISABLED` | Disable logic functions entirely | 해당 없음           |
