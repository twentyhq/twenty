...
예, Settings → Data model → 원하는 object → Layout으로 이동한 다음 **Customize record page**를 클릭하세요. 이 버튼이 비활성화되어 있다면, 레코드를 하나 생성해야 합니다.
</Accordion>
</AccordionGroup>
...
<Accordion title="하나의 object에는 필드를 최대 몇 개까지 만들 수 있나요?">
하나의 object에 가질 수 있는 필드 수의 최대값은 Postgres 제한에 따라, 기존 필드와 삭제된 필드를 모두 포함해 1600개입니다. 1600개 필드 제한에 도달하면, 이는 당사의 공정 사용 Fair Usage 정책에 해당하므로 Twenty에서는 워크스페이스를 수정해 주지 않는다는 점에 유의하세요.
</Accordion>
</AccordionGroup>
