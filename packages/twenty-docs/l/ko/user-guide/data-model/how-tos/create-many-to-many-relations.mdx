...
<Warning>
**베타 기능**: 정션 관계는 현재 베타 단계입니다. 이 가이드를 따르기 전에 **Settings → Community → Features**에서 이를 활성화하세요.
</Warning>

<Note>
이 기능을 사용하려면 **Advanced mode**도 활성화되어 있어야 합니다(설정 사이드바 왼쪽 하단의 토글).
</Note>
...
1. **베타 기능에서 Junction Relations 활성화**: **Settings → Community → Features**로 이동하여 **Junction Relations**를 활성화하세요
2. **Advanced mode 활성화**: 설정 사이드바 왼쪽 하단에서 **Advanced mode**를 켭니다
3. 데이터 모델을 계획하세요:
   * 어떤 두 오브젝트를 연결하나요?
...
4. "이름 필드 생성을 건너뛰기"를 켭니다

<img src="/images/user-guide/fields/new-pivot-object.png" alt="새 피벗 오브젝트" />
...
## 2단계: 오브젝트와 정션 간 관계 만들기

두 개의 오브젝트 각각에서 정션 오브젝트로 관계 필드를 추가합니다.

### 첫 번째 관계(오브젝트 A → 정션)

1. **Settings → Data Model**에서 첫 번째 오브젝트를 선택하세요
2. **+ 관계 추가**를 클릭합니다
3. 정션 오브젝트를 선택하세요(예: "Project Assignments")
4. 관계 유형을 **One-To-Many**로 설정합니다(한 사람이 여러 할당에 연결될 수 있음)
5. 필드 이름 지정:
...
1. **Settings → Data Model**에서 두 번째 오브젝트를 선택하세요
2. **+ 관계 추가**를 클릭합니다
3. 정션 오브젝트를 선택하세요(예: "Project Assignments")
4. 관계 유형을 **One-To-Many**로 설정합니다(하나의 프로젝트가 여러 할당에 연결될 수 있음)
