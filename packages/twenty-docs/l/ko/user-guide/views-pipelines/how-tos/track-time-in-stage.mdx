...
<Note>
  "Closed Won"과 "Closed Lost"는 마지막 단계이므로 "Days in" 필드는 필요하지
  않습니다.
</Note>
...
<Note>
  새 브랜치를 만들려면 워크플로 캔버스에서 마우스 오른쪽 버튼을 클릭하고 **New
  action**을 클릭합니다. 그런 다음 이전 노드에서 이 새 액션으로 화살표를 끌어
  이전 노드와 이 액션을 연결합니다.
</Note>
