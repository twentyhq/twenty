...
이 워크플로우는 **수동 트리거**를 사용하므로 사용자가 선택한 레코드에 대해 필요할 때 PDF를 생성할 수 있습니다. **로직 함수**가 다음을 처리합니다:
...
<Tip>
  수동 트리거를 사용하면 사용자가 레코드를 선택한 후 우측 상단에 표시되는
  버튼으로 이 워크플로우를 실행해 PDF를 생성하고 첨부할 수 있습니다.
</Tip>
...
<Note>
  다른 객체(Person, Opportunity 등)에 첨부하는 경우 해당 매개변수 이름을
  바꾸고(예: `personId`, `opportunityId`) 로직 함수를 업데이트하세요.
</Note>
...
- [로직 함수](/l/ko/user-guide/workflows/capabilities/workflow-actions#code)
