...
1. 在 Twenty 運行的主機中，關閉 Twenty：`docker compose down`

2. 更改 `.env` 文件中的 `TAG` 值來升級版本。 （我們建議使用 `major.minor` 版本，例如 `v0.53` ）

3. 使用命令 `docker compose up -d` 讓 Twenty 上線
...
如果在升級後遇到運行時問題，您可能需要刷新緩存以確保與最新更改同步。 在您的 twenty-server 容器中運行以下命令：
...
請更新您的本地 `docker-compose.yml` 以使用 [v0.50.0 docker-compose.yml](https://github.com/twentyhq/twenty/blob/v0.50.0/packages/twenty-docker/docker-compose.yml)。

### v0.43.0到v0.44.0

升級您的 Twenty 實例以使用 v0.44.0 映像
...
2. 升級你的 docker-compose.yml 以使用 postgres:16 映像檔，如 [docker-compose.yml](https://raw.githubusercontent.com/twentyhq/twenty/main/packages/twenty-docker/docker-compose.yml) 檔案中一樣。
...
將你的 Twenty 實例升級到使用 v0.42.0 的映像檔
...
將你的 Twenty 實例升級到使用 v0.41.0 的映像檔
...
將你的 Twenty 實例升級到使用 v0.40.0 的映像檔
...
將你的 Twenty 實例升級到使用 v0.35.0 的映像檔
...
將你的 Twenty 實例升級到使用 v0.34.0 的映像檔
...
將你的 Twenty 實例升級到使用 v0.33.0 的映像檔
...
將你的 Twenty 實例升級到使用 v0.32.0 的映像檔
...
將你的 Twenty 實例升級到使用 v0.31.0 的映像檔
...
將你的 Twenty 實例升級到使用 v0.30.0 的映像檔

**Breaking change**:
To enhance performances, Twenty now requires redis cache to be configured. 我們已更新 [docker-compose.yml](https://raw.githubusercontent.com/twentyhq/twenty/main/packages/twenty-docker/docker-compose.yml) 以反映這一點。
確保更新您的配置並相應地更新環境變數：
...
將你的 Twenty 實例升級到使用 v0.24.0 的映像檔
...
將你的 Twenty 實例升級到使用 v0.23.0 的映像檔
...
將你的 Twenty 實例升級到使用 v0.22.0 的映像檔
