...
* **src/**: The main place where you define your application-as-code:
  ...
  * `*.role.ts`: Role definitions used by your logic functions. 参见下方“默认函数角色”。
  * `*.object.ts`：自定义对象定义。
...
应用可以定义角色，以封装对工作空间对象与操作的权限。 The field `functionRoleUniversalIdentifier` in `application.config.ts` designates the default role used by your app's logic functions.
...
When a route trigger invokes your logic function, it receives a `RoutePayload` object that follows the AWS HTTP API v2 format. 从 `twenty-sdk` 导入该类型：
...
By default, HTTP headers from incoming requests are **not** passed to your logic function for security reasons. 如需访问特定请求头，请在 `forwardedRequestHeaders` 数组中显式列出：
...
* API 密钥的权限由 `application.config.ts` 中通过 `functionRoleUniversalIdentifier` 引用的角色决定。 This is the default role used by logic functions of your application.
