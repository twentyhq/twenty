...
| **共享依赖项** | 由应用的前端组件共享的依赖项，在 package.json 中声明 | [共享依赖项](/l/zh/developers/extend/apps/layout/front-components#sharing-dependencies-across-front-components) |
| **命令菜单项** | 快速操作和 Cmd+K 条目                    | [命令菜单项](/l/zh/developers/extend/apps/layout/command-menu-items)                                            |

## 沙盒化

* **逻辑函数** 在服务器上的独立 Node.js 进程中运行。 它们仅通过类型化 API 客户端访问数据，该客户端会以触发运行的人员或应用本身的身份执行操作。 请参阅[调用使用谁的访问权限](/l/zh/developers/extend/apps/logic/logic-functions#whose-access-a-call-uses)。
