...
<CardGroup cols={2}>
  ...
  <Card title="键值存储" icon="database" href="/l/zh/developers/extend/apps/logic/key-value-store">
    在逻辑函数的多次运行之间持久化状态——缓存、游标和跨工作区声明。
  </Card>
  <Card title="后台作业" icon="layer-group" href="/l/zh/developers/extend/apps/logic/background-jobs">
    将逻辑函数排入队列，由工作进程运行，以避开单次运行超时限制。
  </Card>
</CardGroup>
