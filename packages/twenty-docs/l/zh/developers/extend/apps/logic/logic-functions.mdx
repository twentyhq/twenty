...
<Note>
  `databaseEventTriggerSettings.updatedFields`
  会筛选出哪些更新事件会触发该函数。 `event.properties.updatedFields`
  告诉你在当前事件中哪些字段实际发生了变化。
</Note>
...
<Note>
  **安装 hooks**——预安装和后安装处理程序——共享此运行时，但使用它们自己的 define
  函数进行声明，并且不接受触发器设置。 有关 `definePreInstallLogicFunction` 和
  `definePostInstallLogicFunction`，请参阅 [Install
  Hooks](/l/zh/developers/extend/apps/config/install-hooks)。
</Note>
...
<Note>
  **CoreApiClient 在开发/构建时生成。** 如果在未先运行 `yarn twenty dev` 或
  `yarn twenty dev:build` 的情况下尝试使用它，将会抛出错误。
