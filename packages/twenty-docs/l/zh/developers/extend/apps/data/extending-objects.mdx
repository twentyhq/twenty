...
* `writability` 也可以按字段单独设置：`MetadataWritability.APPLICATION` 将写入限制为你应用自身的逻辑函数，`MetadataWritability.SYSTEM` 保留用于平台管理的数据。 字段的可写性只能比其所属对象的级别更严格，读取操作不会受到影响。 有关完整语义，请参见[对象](/l/zh/developers/extend/apps/data/objects)。
