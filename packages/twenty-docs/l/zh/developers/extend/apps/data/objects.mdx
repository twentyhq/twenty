...
* `openRecordIn` 设置在点击此对象的记录时，它们会在何处打开：`ObjectOpenRecordIn.USER_CHOICE`（默认，遵循每位工作区成员在 Settings → Experience 中的个人偏好）、`ObjectOpenRecordIn.SIDE_PANEL`，或 `ObjectOpenRecordIn.RECORD_PAGE`。 将其固定为 `RECORD_PAGE`，用于那些需要完整页面才能正常使用的记录（就像工作流和仪表盘那样），或者固定为 `SIDE_PANEL`，用于那些只适合作为快速面板显示的记录（就像日历事件那样）。
* `writability` 控制在角色权限生效之前，哪些主体可以对该对象的记录进行任何写入操作：`MetadataWritability.OPEN`（默认值 — 由工作区角色决定）、`MetadataWritability.APPLICATION`（只有你的应用自身的逻辑函数可以创建、更新或删除记录；将其用于类似配置的对象，这类对象的记录定义行为，因此具有广泛记录访问权限的工作区成员无法通过 API 编辑这些记录）、或者 `MetadataWritability.SYSTEM`（保留用于平台管理的数据）。 读取不受影响 — 这是在服务器端强制执行的，不同于 `isUIEditable`，后者只会隐藏 UI 的可操作入口。 它同样存在于每个字段上，但其限制只能比对象级别更严格。
...
* **在界面中显示此对象** — 请参阅 [导航菜单项](/l/zh/developers/extend/apps/layout/navigation-menu-items) 以添加侧边栏条目；请参阅 [视图](/l/zh/developers/extend/apps/layout/views) 以添加自定义列表配置。
