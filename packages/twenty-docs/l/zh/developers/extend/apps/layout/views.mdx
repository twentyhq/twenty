...
| `openRecordIn`                                                                                              | 已弃用                                                                                | 不再读取：记录的打开位置现在是[对象](/l/zh/developers/extend/apps/data/objects)的一个属性（`defineObject()` 上的 `openRecordIn`），如果未设置该属性，则回退到每个成员自己的偏好设置。 |
...
单独一个视图无法从侧边栏访问。 要让它显示在侧边栏中，请将其与类型为 `VIEW`、指向该视图 `universalIdentifier` 的[导航菜单项](/l/zh/developers/extend/apps/layout/navigation-menu-items)配对。 这是规范做法：每个自定义对象在创建时都会自动生成一个默认的列表视图，因此你只需提供用于打开它的侧边栏条目。
