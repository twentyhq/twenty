...
正在开发前台组件。 渲染、样式和处理活动效果良好。 任何达到*过去* 渲染(测量元素，在ref上调用DOM方法) 在你的树外端传输端口，触摸浏览器存储设备)今天不存在或不完整。 并且大部分失败：没有例外，也没有错误的 TypeScript ，因为scaffold 是与完整的浏览器 DOM键入的。
...
| API                                                           | 发生了什么                                           |
| ------------------------------------------------------------- | ----------------------------------------------- |
| `getBoundingClientRect()`，`getClientRects()`                  | 投出                                              |
| `offsetWidth`, `clientWidth`, `scrollWidth`, `offsetTop`, ... | 静默`未定义`, 所以`宽度 ?? 0` 当量`0` 当量时，`宽度 > 600` 时总是假的 |
| `ResizeObserver`, `IntersectionObserver`                      | `ReferenceError` (`typeof`警卫干工作)                |
| `window.matchMedia()`, `window.getComputedStyle()`            | 投出                                              |
| `window.innerWidth`, `innerHight`, `devicePixelRatio`         | 静默`未定义`                                         |
| `新的 MutationObserver(fn)`                                     | 构造，然后扔出 `.observe()`                            |

因此，调整导航“ResponsiveContainer”、浮动界面/弹出窗口、列表虚拟化和拖动到调整大小尚不可用。 在 CSS 中选择布局：您的样式表到达了真正的页面，所以弹性框、网格、`aspect-ratio`、`clamp()`和`@container`都是正常行为。
...
| `document.getElementById()`, `getElementsByClassName()`, `createTreeWalker()`                               | 投出                            | `querySelector()` / `querySelectorAll()`，它起作用。 |
