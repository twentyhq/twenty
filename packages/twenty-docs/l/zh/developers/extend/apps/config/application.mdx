...
* 两种类型的变量都可以接受 `isDeprecated: true`。 使用它来停用变量而不是删除它：保留已声明的键可以保留存储的值（删除它会销毁操作员输入的值），并且变量仍然会被注入，所以你的代码仍然可以回退到它 — `process.env.NEW_API_KEY ?? process.env.API_KEY`。 一旦弃用变量没有任何值，它就会从应用的设置中消失，并且永远不会计入应用的配置检查中，因此 `isDeprecated` 的优先级高于 `isRequired`。
...
* 注入到您的逻辑函数中的运行时令牌来源于该角色。 以某个人的身份执行的调用会进一步受限于该人能够执行的操作，因此其权限永远不会超过两者中的任一方。 请参阅[调用使用谁的访问权限](/l/zh/developers/extend/apps/logic/logic-functions#whose-access-a-call-uses)。
