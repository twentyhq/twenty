...
<Note>
  ...
  Docker 启动后，你可以通过 `yarn twenty server start`
  继续——无需重新生成脚手架。
</Note>
...
<Warning>
  开发模式仅适用于以开发模式运行的 Twenty 实例（`NODE_ENV=development`）。
  生产实例会拒绝开发同步请求——请使用 `yarn twenty deploy` 部署到生产服务器。
  参见[发布应用](/l/zh/developers/extend/apps/publishing)。
</Warning>
