...
<Note>
“Mail.ReadWrite”和“Mail.Send”仅在你想使用我们的工作流程操作发送电子邮件时是必需的。 如果你只想接收邮件，你可以使用“Mail.Read”。
</Note>

<Note>
如果你想使用我们的工作流操作创建日历事件，则必须使用 "Calendars.ReadWrite"。 如果你只想同步日历事件，请使用 "Calendars.Read"。
</Note>
...
仅当你不让后端来提供前端资源时，此项才相关。 同源部署在这里无需任何设置。

声明每一个与 API 通信的浏览器来源（origin），使用逗号分隔。 `SERVER_URL`
和 `FRONTEND_URL` 已被信任。 当浏览器从未声明的来源调用它时，服务器会记录一条警告日志。
...
<Warning>
前端会在每个请求中发送凭据，而浏览器会拒绝来自未声明源的携带凭据的响应，因此在你完成此设置之前，应用将无法加载。
</Warning>

如果这两个主机位于不同的可注册域（不仅仅是不同的子域），还需要设置 `AUTH_COOKIE_SAME_SITE=none`，并且这要求使用 HTTPS。

在启用多工作区功能时，不要在此列出每个工作区的子域名。
相反，请在每个工作区主机名上将 API 路径（`/graphql`、`/metadata`、`/rest`、`/auth/`、`/client-config`、`/file`）路由到后端，这样每个工作区都保持同源 —— 这正是 Twenty Cloud 的运行方式。
