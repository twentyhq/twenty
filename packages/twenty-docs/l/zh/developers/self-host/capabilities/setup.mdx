...
Twenty supports logic functions for workflows and custom logic. 执行环境通过 `SERVERLESS_TYPE` 环境变量进行配置。

<Warning>
  **Security Notice:** The local driver (`SERVERLESS_TYPE=LOCAL`) runs code directly on the host in a Node.js process with no sandboxing. 它应仅在开发环境中用于可信代码。 对于在生产环境中处理不受信任代码的部署，我们强烈建议使用 `SERVERLESS_TYPE=LAMBDA` 或 `SERVERLESS_TYPE=DISABLED`。
</Warning>
...
| 驱动     | 环境变量                       | 用例                               | 安全级别     |
| ------ | -------------------------- | -------------------------------- | -------- |
| 禁用     | `SERVERLESS_TYPE=DISABLED` | Disable logic functions entirely | 不适用      |
