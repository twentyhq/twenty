...
<Warning>
**安全提示：** 本地驱动（`LOGIC_FUNCTION_TYPE=LOCAL` 或 `CODE_INTERPRETER_TYPE=LOCAL`）会在没有沙箱的情况下，在主机上的 Node.js 进程中直接运行代码。 它应仅在开发环境中用于可信代码。 For production deployments handling untrusted code, use `LOGIC_FUNCTION_TYPE=LAMBDA` or `CODE_INTERPRETER_TYPE=E2B` (with sandboxing), or keep them disabled.
</Warning>
...
| 驱动  | 环境变量                             | 用例                                  | 安全级别                    |
| --- | -------------------------------- | ----------------------------------- | ----------------------- |
| 禁用  | `CODE_INTERPRETER_TYPE=DISABLED` | Disable AI code execution           | 不适用                     |
| 本地  | `CODE_INTERPRETER_TYPE=LOCAL`    | Development only                    | 低（无沙箱）                  |
