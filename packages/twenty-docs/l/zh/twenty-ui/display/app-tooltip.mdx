...
| place            | 來自 `react-tooltip` 的 `PlacesType` 字符串       | 指定工具提示的位置。 值包括 `bottom`, `left`, `right`, `top`, `top-start`, `top-end`, `right-start`, `right-end`, `bottom-start`, `bottom-end`, `left-start`, 和 `left-end` |
