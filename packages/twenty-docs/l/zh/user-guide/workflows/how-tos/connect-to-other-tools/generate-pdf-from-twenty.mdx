...
<Tip>
  使用手动触发器，用户在选择记录后可通过右上角出现的按钮运行此工作流，以生成并附加
  PDF。
</Tip>
...
<Note>
  If attaching to a different object (Person, Opportunity, etc.), rename the
  parameter accordingly (e.g., `personId`, `opportunityId`) and update the logic
  function.
</Note>
