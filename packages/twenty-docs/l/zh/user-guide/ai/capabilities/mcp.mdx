...
<Note>
  OAuth 需要支持[MCP
  授权规范](https://modelcontextprotocol.io/specification/2025-03-26/basic/authorization)的
  MCP 客户端。 Claude Desktop、Claude Code、Cursor 和 ChatGPT 均支持它。
</Note>
...
<Warning>
  您的 API 密钥可访问工作区数据。 请不要将其纳入版本控制，也不要放入共享的
  dotfiles。
</Warning>
...
- _"给我展示最近创建的 5 家公司"_
- _"在 Acme Corp 新建一位名为 Jane Doe 的联系人"_
- _"查找所有价值超过 $10k 的未关闭商机"_
