...
<Accordion title="我可以限制对特定记录的访问（行级权限）吗？">
  行级权限将在 2026 年第一季度于 **Organization** 方案中提供。
  这将允许您根据条件限制对特定记录的访问（例如，仅查看自己的商机）。
</Accordion>
...
<Accordion title="支持哪些身份提供商？">
  Twenty 支持： * **SAML 2.0**（适用于大多数企业身份提供商） * **Google
  Workspace** * **Microsoft Entra ID**（原 Azure AD）
</Accordion>

<Accordion title="什么是即时（JIT）预配？">
  启用 JIT 预配后，用户首次通过 SSO 登录时，会在 Twenty 中自动创建其账户。
  系统会自动为其分配默认角色。
</Accordion>
