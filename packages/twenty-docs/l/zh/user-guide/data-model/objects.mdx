...
3. 單擊頂部的「+ 新物件」。 輸入名稱（單數和複數），選擇一個圖標，為自訂物件添加描述，然後點擊保存（在右上角）。 Using Listing as an example of custom object, the singular would be "listing" and the plural would be "listings" along with a description like "Listings that hosts created to showcase their property."
...
\*\*注意：\*\*如不確定是否需要新物件或字段，查閱[這篇文章](/user-guide/data-model/customize-your-data-model)以獲得設計數據模型的指導。
