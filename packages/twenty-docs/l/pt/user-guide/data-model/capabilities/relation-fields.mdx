...
<Warning>
  **Limitação de Importação/Exportação**: Relações que apontam para vários tipos
  de objeto ainda não são suportadas para importação/exportação CSV. Isto está
  no nosso roadmap.
</Warning>
...
<Warning>
  **Recurso de laboratório**: As relações de junção devem ser ativadas em
  **Configurações → Atualizações → Lab** antes do uso.
</Warning>
