...
<Check>O seu ficheiro está no formato CSV, XLSX ou XLS</Check>

<Check>O ficheiro tem menos de 10 000 registos</Check>

<Check>Sem domínios duplicados no seu ficheiro</Check>
...
<Note>
  Precisa de importar mais de 10 000 empresas? Divida em vários ficheiros ou use
  a [importação via
  API](/l/pt/user-guide/data-migration/how-tos/import-data-via-api).
</Note>
...
<Note>
  **Dica profissional:** Clique em **Download sample file** durante a importação
  para ver os nomes de colunas exatos que o Twenty espera.
</Note>
...
<Note>
  As opções de Select usam **nomes de API**, não os rótulos de apresentação. Em
  **Settings → Data Model** → Ative o **Modo avançado** para ver os nomes de
  API.
</Note>
...
<Accordion title="E se tiver empresas sem domínio?">
  Pode deixar o domínio em branco. No entanto, recomendamos adicionar domínios
  quando possível para melhorar a qualidade dos dados e a ligação automática de
  e-mails.
</Accordion>

<Accordion title="Posso importar empresas sem quaisquer Pessoas associadas?">
  Sim! Pode importar as empresas primeiro, depois importar as Pessoas e
  associá-las usando o domínio da empresa.
</Accordion>

<Accordion title="O que acontece se importar um domínio que já existe?">
  Se incluir um identificador único (domínio ou id) que corresponda a uma
  empresa existente, a Twenty atualiza essa empresa em vez de criar um
  duplicado.
</Accordion>
