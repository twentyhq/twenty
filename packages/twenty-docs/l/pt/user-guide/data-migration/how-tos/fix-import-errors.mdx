...
<Note>
  **Corrija os erros na interface.** Não precisa de voltar à sua folha de
  cálculo. Edite as células diretamente durante a importação para poupar tempo.
</Note>
...
<Note>
  Todos os formatos são válidos, mas `https://domain.com` é recomendado porque
  corresponde ao formato usado pela sincronização de email/calendário. Usar
  outros formatos pode criar empresas duplicadas.
</Note>
...
<Note>
  Ao contrário de outros erros, este não é detetado durante a etapa de revisão
  dos dados. A importação começará e depois falhará quando encontrar a relação
  em falta.
</Note>
...
<Note>
  A importação de CSV cria registos, não campos. Todos os campos devem existir
  antes de importar.
</Note>
