...
<Note>
  Para conjuntos de dados na casa das centenas de milhares, a API é
  significativamente mais rápida e mais fiável do que múltiplas importações CSV.
</Note>
...
<Accordion title="Posso atualizar registos existentes via API?">
  Sim! Utilize mutations de atualização (GraphQL) ou pedidos PUT/PATCH (REST)
  com o `id` do registo.
</Accordion>

<Accordion title="Como devo lidar com duplicados?">
  Pesquise primeiro por registos existentes usando identificadores únicos
  (email, domínio). Atualize se existir, crie se não existir.
</Accordion>
