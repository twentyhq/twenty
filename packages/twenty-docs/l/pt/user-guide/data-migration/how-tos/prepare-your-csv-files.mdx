...
<Note>
  Para conjuntos de dados maiores que 10.000 registros, divida em vários
  arquivos ou use a [importação via
...
<Note>
  **Dica de especialista:** Em vez disso, exporte alguns registros existentes.
  Isso fornece exemplos reais de como os dados devem ser formatados, e os nomes
  das colunas serão mapeados automaticamente durante a importação.
</Note>
...
<Warning>
  **Registros excluídos logicamente contam para a unicidade.** Registros em Menu
  de Comando → Ver registros excluídos causarão erros de duplicação. Exclua-os
  permanentemente ou restaure e atualize-os.
</Warning>
...
<Note>
  Novas opções de seleção não são criadas automaticamente. Adicione-as em
  **Configurações → Modelo de Dados** antes de importar.
</Note>
