...
<Note>
  O campo `id` é o identificador interno do Twenty, gerado automaticamente para
  cada registo. Utiliza o formato UUID (por exemplo,
...
<Note>
  O nome do campo `id` é reservado para o ID interno do Twenty. Utilize um nome
  diferente, como `externalId` ou `legacyId`, para identificadores externos.
</Note>
...
<Warning>
  Para **atualizar registos existentes**, recomenda-se **corresponder apenas a
  um campo único**.
</Warning>
