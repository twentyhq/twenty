...
<Note>
  Para conjuntos de dados muito grandes (centenas de milhares de registos),
  utilize a API. Os nossos [parceiros de
  implementação](/l/pt/user-guide/getting-started/capabilities/implementation-services)
  podem ajudar a executar estes scripts, se necessário.
</Note>
...
<Note>
  **Os campos têm de existir antes da importação.** Carregar um CSV cria
  registos, mas não cria campos. Se precisar de campos personalizados, crie-os
  primeiro em **Definições → Modelo de dados**.
</Note>
...
<Note>
  **Os registos eliminados contam para a unicidade.** Os registos eliminados
  logicamente (visíveis no Menu de Comandos → Ver registos eliminados) são
  incluídos nas verificações de unicidade. Se importar um registo com o mesmo
  valor único que um registo eliminado, o registo eliminado será restaurado.
</Note>
...
<Note>
  **Apenas as colunas visíveis são exportadas.** O ficheiro CSV conterá apenas
  as colunas apresentadas na sua visualização atual. Adicione ou oculte colunas
  antes de exportar para controlar que dados são incluídos.
</Note>
