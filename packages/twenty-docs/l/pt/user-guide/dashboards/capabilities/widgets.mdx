...
<Note>
  **Limites de exibição**: gráficos de barras podem mostrar no máximo 100 barras
  e 50 grupos por barra ao usar Agrupar por. Se você vir o aviso "Undisplayed
  data: max X bars per chart", adicione filtros para reduzir seus dados ou
  altere o agrupamento (por exemplo, agrupe por semana em vez de por dia).
</Note>
...
<Note>
  **Em breve**: gráficos de medidor e tabelas ainda não estão disponíveis, mas
  estão em nosso roteiro.
</Note>
