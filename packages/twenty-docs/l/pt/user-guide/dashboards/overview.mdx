...
<Note>
  Os painéis estão atualmente em versão beta. Ative-os em **Configurações →
  Atualizações → Acesso antecipado**.
</Note>
...
<Note>
  **Limitações atuais**: * A exportação de painéis e o compartilhamento com
  usuários externos (usuários não-Twenty) não estão disponíveis no momento. *
