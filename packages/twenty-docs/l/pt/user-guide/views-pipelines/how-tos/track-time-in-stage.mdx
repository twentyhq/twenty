...
<Tip>
  Este é um exemplo de criação de [Campos de
  fórmula](/l/pt/user-guide/workflows/how-tos/crm-automations/formula-fields)
  usando fluxos de trabalho — especificamente cálculos de datas.
</Tip>
...
<Note>
  Você não precisa de campos "Dias em" para Fechado Ganho e Fechado Perdido,
  pois são etapas finais.
</Note>
...
<Note>
  Para criar uma nova ramificação, clique com o botão direito no canvas do
  workflow e clique em **Nova ação**. Em seguida, vincule esta ação ao nó
  anterior arrastando a seta do nó anterior para esta nova ação.
</Note>
