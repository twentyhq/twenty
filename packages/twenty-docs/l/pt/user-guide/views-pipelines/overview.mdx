...
<Warning>
  Visualizações excluídas não podem ser recuperadas. Certifique-se de que deseja
  removê-la antes de confirmar.
</Warning>
...
<Note>
  As visualizações padrão "Todos(as) [Nome do objeto]" não podem ter sua
