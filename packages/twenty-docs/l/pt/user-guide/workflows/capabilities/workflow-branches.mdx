...
<Note>
  Cada ramificação é independente. Adicionar uma ramificação não afeta outros
  caminhos existentes desse nó.
</Note>
...
<Note>
  Embora você possa usar um nó
  [Atraso](/l/pt/user-guide/workflows/capabilities/workflow-actions#delay) para
  pausar a execução, atualmente não é possível configurá-lo para esperar "até
  que outra ramificação termine".
</Note>
