...
<Note>
  **Limitação de Tarefas e Notas**: As relações em Tarefas e Notas são
  pré-definidas como muitos-para-muitos e ainda não estão disponíveis em
  gatilhos ou ações de fluxos de trabalho. Para aceder a estas relações, use
  antes a [API](/l/pt/developers/extend/api).
</Note>
...
<Note>
  Tenha cuidado com sincronizações bidirecionais para evitar ciclos infinitos.
  Use filtros para verificar se o valor realmente mudou antes de atualizar.
</Note>
