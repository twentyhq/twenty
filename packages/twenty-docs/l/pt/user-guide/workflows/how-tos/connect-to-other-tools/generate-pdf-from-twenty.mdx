...
<Tip>
  Com um Gatilho Manual, os utilizadores podem executar este fluxo de trabalho
  utilizando um botão que aparece no canto superior direito quando um registo é
  selecionado, para gerar e anexar um PDF.
</Tip>
...
<Note>
  Se estiver a anexar a um objeto diferente (Pessoa, Oportunidade, etc.),
  renomeie o parâmetro em conformidade (por exemplo, `personId`,
  `opportunityId`) e atualize a função lógica.
</Note>
...
export const main = async (params: { companyId: string }) => {
  ...
  const uploadFileFieldMetadataId = result.data.objects.edges
    .find((object) => object.node.nameSingular === 'attachment')
    .node.fieldsList.find((field) => field.name === 'file').id;
...
2. Passe o URL do PDF devolvido para a função lógica como parâmetro
...
- [Funções lógicas](/l/pt/user-guide/workflows/capabilities/workflow-actions#code)
