...
* **Workspaces ilimitados**: Crie quantos workspaces quiser (somente self-hosted)
* **Código-fonte privado**: Nenhuma exigência de publicar alterações em um repositório público (somente self-hosted)
</Accordion>
...
Você pode fazer isso em `Settings → Billing → Plans`. Depois, clique em `Alterar para Organização`.
</Accordion>

<Accordion title="Onde posso mudar minha assinatura para o plano Pro?">
Você pode fazer isso em `Settings → Billing → Plans`. Depois, clique em `Switch to Pro`.
</Accordion>

<Accordion title="Onde posso mudar minha assinatura para anual?">
Você pode fazer isso em `Configurações → Cobrança`. Depois, clique em `Upgrade to yearly`.
</Accordion>

<Accordion title="Onde posso mudar minha assinatura para mensal?">
Você pode fazer isso em `Configurações → Cobrança`. Depois, clique em `Downgrade to monthly`.
</Accordion>
...
Os créditos são deduzidos em tempo real à medida que os workflows ou apps são executados. Você pode monitorar seu uso em **Configurações → Cobrança** para acompanhar o consumo e os créditos restantes.
</Accordion>
...
<Accordion title="Posso comprar mais créditos de workflow apenas por 1 mês?">
Sim, acesse `Settings → Billing`, clique no botão `Increase`, escolha quantos créditos você quer e confirme; eles serão aplicados ao período de cobrança atual. Depois, clique novamente no botão `Increase`, altere o valor de volta ao original e confirme; o downgrade será aplicado a partir do próximo período de cobrança.
</Accordion>
</AccordionGroup>
