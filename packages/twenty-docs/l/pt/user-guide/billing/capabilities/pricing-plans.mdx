---
title: Planos de preços
description: Saiba mais sobre os planos de preços do Twenty e como alternar entre eles.
---

## Visão Geral

O Twenty oferece preços flexíveis para atender equipes de todos os tamanhos, quer você prefira hospedagem em nuvem ou auto-hospedagem.

## Planos na nuvem

### Pro (Nuvem)

Para equipes prontas para escalar:

* Todos os recursos centrais do CRM
* Sincronização de email e calendário
* Fluxos de trabalho e automações
* Suporte padrão

<Note>
Os recursos Premium (SSO, permissões em nível de linha, dados de uso de IA e mais) não estão incluídos no plano Pro.
</Note>

### Organização (Nuvem)

Para equipes maiores com necessidades avançadas:

* Tudo do Pro
* **Recursos Premium**: integração com SSO, permissões em nível de linha, dados de uso de IA e mais
* Suporte prioritário

## Planos auto-hospedados

### Gratuito (Auto-hospedado)

Hospede o Twenty na sua própria infraestrutura sem custo:

* Todos os recursos do Pro incluídos
* Suporte da comunidade via Discord
* Controle total sobre seus dados

### Organização (Auto-hospedado)

Para equipes que precisam de recursos premium enquanto se auto-hospedam:

* Todos os recursos do Pro
* **Recursos Premium**: integração com SSO, permissões em nível de linha, dados de uso de IA e mais
* Suporte da equipe Twenty
* Sem necessidade de publicar código personalizado como código aberto antes de distribuir

## Recursos premium

Os recursos premium estão disponíveis apenas nos planos Organização (Nuvem ou Auto-hospedado):

* **Integração SSO**: Single Sign-On com seu provedor de identidade
* **Permissões em nível de linha**: controle de acesso granular no nível de registro
* **Dados de uso de IA**: Acompanhe o consumo de IA em todo o espaço de trabalho
* **Registros de auditoria**: verifique o que cada membro do workspace tem feito
* **Campanhas de email**: planeje quais emails devem ser enviados (disponível em breve)
* **Workspaces ilimitados**: Crie quantos workspaces quiser (somente para auto-hospedagem)
* **Código-fonte privado**: Nenhuma exigência de publicar alterações em um repositório público (somente para auto-hospedagem)

## Mudança de planos

### Atualizar para Organização

1. Vá para **Configurações → Faturamento → Planos**
2. Clique em **Mudar para Organização**
3. Confirme o upgrade

### Fazer downgrade para Pro

1. Vá para **Configurações → Faturamento → Planos**
2. Clique em **Mudar para Pro**
3. Confirme o downgrade

### Mudar para faturamento anual

1. Vá para **Configurações → Faturamento**
2. Clique em **Fazer upgrade para Anual**
3. Economize com faturamento anual

### Mudar para faturamento mensal

1. Vá para **Configurações → Faturamento**
2. Clique em **Alterar para Mensal**
3. Economize com faturamento mensal

## Obtenha uma chave Enterprise para Organization (Self-Hosted)

Para usar o plano Organization (Self-Hosted), você precisa obter uma chave Enterprise:

1. Vá para **Configurações → Painel de Administração → Enterprise**

<img src="/images/user-guide/billing/organization-key.png" alt="Chave Enterprise" />

2. Clique em **Obter chave Enterprise**
3. Quando você for redirecionado ao Stripe, insira seus dados de pagamento e confirme
4. Quando sua chave Enterprise for exibida, cole-a na página de configurações do Enterprise e ative a licença do Organization
