...
## Atualização para v2.34+ — PostgreSQL 15 no mínimo

O Twenty v2.34 requer PostgreSQL 15 ou mais recente. A configuração oficial do Docker Compose usa PostgreSQL 16. Se a sua instância se conectar a um banco de dados gerenciado externamente, atualize-o antes de iniciar o servidor v2.34.

## Atualizações entre versões (v1.23+)

A partir da **v1.23**, o Twenty oferece suporte a atualizações entre versões. Você pode pular diretamente de qualquer versão compatível para a versão mais recente sem passar por cada versão intermediária.

Por exemplo, a atualização da v1.23 diretamente para a v2.0 é totalmente suportada.
...
Se a sua instância for anterior à v1.23, você deve atualizar incrementalmente por cada versão principal marcada (da v1.6 para a v1.7, depois da v1.7 para a v1.8 e assim por diante) até chegar à v1.23. A partir daí, você pode pular diretamente para a versão mais recente.
