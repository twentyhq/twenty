...
<Warning>
  Os contêineres Docker são para hospedagem em produção ou auto-hospedagem. Para
  contribuir, consulte a [Configuração
  local](/l/pt/developers/contribute/capabilities/local-setup).
</Warning>
