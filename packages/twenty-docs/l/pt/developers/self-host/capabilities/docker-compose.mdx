...
Por padrão, o Twenty é executado em `localhost` na porta `3000`. A interface acessa a API a partir da origem do próprio navegador, portanto pode ser carregada de qualquer nome de host que alcance o contêiner — ainda assim, defina `SERVER_URL` como sua URL pública: o servidor a utiliza para gerar links (e-mails, callbacks de OAuth) e para detectar HTTPS e definir cookies seguros.
...
<Tip>
  Prefere não executar e manter o Twenty você mesmo? [Encontre um parceiro Twenty certificado de hospedagem](https://twenty.com/partners/list?categories=HOSTING\&ref=docs-docker-compose) para implantar, hospedar e gerir a sua instância de ponta a ponta. *(Ou entre em contato com a nossa equipe: [contact@twenty.com](mailto:contact@twenty.com).)*
</Tip>
