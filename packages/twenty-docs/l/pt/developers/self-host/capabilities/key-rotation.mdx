...
- **Enterprise (automático)** — um cron diário (`'15 3 * * *'` UTC) emite uma nova chave atual assim que a existente tiver sido atual por `SIGNING_KEY_ROTATION_DAYS` (padrão `90`). A chave anterior _não_ é revogada, então tokens assinados sob ela continuam sendo verificados. Registre-o uma vez com `yarn command:prod cron:register:all`.

  <Note>
    O cron do Enterprise e `SIGNING_KEY_ROTATION_DAYS` são disponibilizados a
    partir da v2.6+.
  </Note>
...
<Note>
  O comando `secret-encryption:rotate` descrito abaixo é disponibilizado a
  partir da v2.6+.
</Note>
