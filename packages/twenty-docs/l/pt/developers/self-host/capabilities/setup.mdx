...
<Warning>
**Aviso de segurança:** O driver local (`LOGIC_FUNCTION_TYPE=LOCAL` ou `CODE_INTERPRETER_TYPE=LOCAL`) executa código diretamente no host em um processo Node.js sem sandbox. Deve ser usado apenas para código confiável em desenvolvimento. For production deployments handling untrusted code, use `LOGIC_FUNCTION_TYPE=LAMBDA` or `CODE_INTERPRETER_TYPE=E2B` (with sandboxing), or keep them disabled.
</Warning>
...
| Driver     | Variável de ambiente             | Caso de uso                         | Nível de segurança      |
| ---------- | -------------------------------- | ----------------------------------- | ----------------------- |
| Desativado | `CODE_INTERPRETER_TYPE=DISABLED` | Disable AI code execution           | N/A                     |
| Local      | `CODE_INTERPRETER_TYPE=LOCAL`    | Development only                    | Baixo (sem sandbox)     |
