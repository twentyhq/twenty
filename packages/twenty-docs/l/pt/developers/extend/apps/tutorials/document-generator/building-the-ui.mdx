...
coloca essa visualização na barra lateral. O servidor já provisiona automaticamente a visualização padrão "All ..." de cada objeto, portanto, as visualizações que um app define são adicionais e selecionadas.
...
Widgets de `FIELD` também oferecem suporte a outros modos de exibição e opções, incluindo
a listagem de registros de uma relação aninhada a dois níveis de distância — veja a
[referência do widget de campo](/l/pt/developers/extend/apps/layout/page-layouts#field-widgets).
