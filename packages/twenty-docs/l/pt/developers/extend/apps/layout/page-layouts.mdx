...
* Cada `tab` define uma seção da página com um `title`, `position` e `layoutMode`: `VERTICAL_LIST` para páginas de registro e páginas independentes, `GRID` para dashboards. Em uma aba `VERTICAL_LIST`, um único widget é renderizado em largura total e ocupa toda a aba; com vários widgets, eles são empilhados como cards. Uma aba `GRID` sempre distribui seus widgets como cards em uma grade de 12 colunas, independentemente da quantidade, portanto escolha `VERTICAL_LIST` quando quiser que um único widget preencha a página.
