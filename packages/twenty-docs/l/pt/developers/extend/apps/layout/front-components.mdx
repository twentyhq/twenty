...
Um componente de front-end por si só não é acessível pela UI — é preciso _exibi-lo_. As duas maneiras de fazer isso são:
...
<div style={{ textAlign: 'center' }}>
  <img
    ...
    alt="Botão de ação rápida no canto superior direito"
  />
</div>
...
<Warning>
  Variáveis secretas (`isSecret: true`) **não** são expostas aos componentes de
  front. Elas estão disponíveis apenas em [funções de
  lógica](/l/pt/developers/extend/apps/logic/logic-functions), que são
  executadas no lado do servidor. Isso impede que valores sigilosos, como chaves
  de API, sejam enviados para o navegador.
</Warning>
