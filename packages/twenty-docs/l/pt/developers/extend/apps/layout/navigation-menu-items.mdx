...
<Note>
  **Armadilha comum:** criar um objeto sem uma visualização associada + item do
  menu de navegação torna esse objeto invisível para os usuários. A menos que
  seja um objeto técnico/interno, todo objeto personalizado deve ter uma
  visualização padrão *e* uma entrada na barra lateral apontando para ela.
</Note>
