...
| `openRecordIn`                                                                                              | obsoleto                                                                           | Não é mais lido: o local onde os registros são abertos agora é uma propriedade do [objeto](/l/pt/developers/extend/apps/data/objects) (`openRecordIn` em `defineObject()`), recorrendo à preferência de cada membro. |
...
Uma visualização por si só não é acessível a partir da barra lateral. Para fazê-la aparecer lá, associe-a a um [item de menu de navegação](/l/pt/developers/extend/apps/layout/navigation-menu-items) do tipo `VIEW` que aponte para o `universalIdentifier` da visualização. Esse é o padrão canônico: todo objeto personalizado recebe uma exibição de índice padrão gerada automaticamente no momento da criação, então você só precisa incluir o item da barra lateral que a abre.
