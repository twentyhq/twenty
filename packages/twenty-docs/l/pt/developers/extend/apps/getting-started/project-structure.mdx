...
<Note>
  **A organização de arquivos fica a seu critério.** As pastas acima são
  convenções — o SDK detecta entidades por meio de análise de AST em chamadas a
  `export default defineEntity(...)`, independentemente de onde o arquivo
  esteja.
</Note>
