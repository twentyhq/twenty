...
* A função padrão é detectada automaticamente a partir do arquivo de função marcado com [`defineApplicationRole()`](/l/pt/developers/extend/apps/config/roles) — você não precisa referenciá-la em `defineApplication()`.
...
* Passar `defaultRoleUniversalIdentifier` explicitamente ainda é compatível para retrocompatibilidade, mas está obsoleto em favor de `defineApplicationRole()`.

## Papel de função padrão

A função declarada com [`defineApplicationRole()`](/l/pt/developers/extend/apps/config/roles) controla ao que as funções de lógica e os componentes de interface do aplicativo podem acessar:
