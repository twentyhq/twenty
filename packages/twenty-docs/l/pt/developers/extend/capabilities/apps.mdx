...
Os aplicativos permitem criar e gerenciar personalizações do Twenty **como código**. Instead of configuring everything through the UI, you define your data model and logic functions in code — making it faster to build, maintain, and roll out to multiple workspaces.
...
* **src/**: The main place where you define your application-as-code:
  ...
  * `*.role.ts`: Role definitions used by your logic functions. Veja "Papel de função padrão" abaixo.
  * `*.object.ts`: Definições de objetos personalizados.
...
Os aplicativos podem definir papéis que encapsulam permissões sobre os objetos e ações do seu espaço de trabalho. The field `functionRoleUniversalIdentifier` in `application.config.ts` designates the default role used by your app's logic functions.
...
When a route trigger invokes your logic function, it receives a `RoutePayload` object that follows the AWS HTTP API v2 format. Importe o tipo de `twenty-sdk`:
...
By default, HTTP headers from incoming requests are **not** passed to your logic function for security reasons. Para acessar cabeçalhos específicos, liste-os explicitamente no array `forwardedRequestHeaders`:
...
* As permissões da chave de API são determinadas pelo papel referenciado no seu `application.config.ts` via `functionRoleUniversalIdentifier`. This is the default role used by logic functions of your application.
