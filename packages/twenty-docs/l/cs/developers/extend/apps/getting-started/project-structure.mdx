...
Vlastní runtime závislosti vaší aplikace (knihovny, které vaše logické funkce skutečně importují za běhu) přidejte jako obvykle pod `dependencies`. Udržujte strom štíhlý: `dependencies` se instalují do běhové vrstvy s limitem 250 MB po rozbalení a při jeho překročení synchronizace nebo instalace selže s chybou překročení velikosti závislostí — UI knihovny a vývojářské nástroje patří do `devDependencies`.
