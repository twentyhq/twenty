...
| **Sdílené závislosti**        | Závislosti sdílené frontendovými komponentami aplikace, definované v souboru package.json | [Sdílené závislosti](/l/cs/developers/extend/apps/layout/front-components#sharing-dependencies-across-front-components) |
...
* **Logické funkce** běží v izolovaných procesech Node.js na serveru. K datům přistupují pouze prostřednictvím typovaného klienta API, který vystupuje buď jako osoba, jež spuštění vyvolala, nebo jako samotná aplikace. Viz [čí přístup volání používá](/l/cs/developers/extend/apps/logic/logic-functions#whose-access-a-call-uses).
...
* **Oprávnění** jsou vynucována na úrovni API. Vkládají se dva tokeny běhového prostředí, oba odvozené z role deklarované pomocí `defineApplicationRole()`; výchozí token je dále omezen na to, co může provést osoba, která spuštění vyvolala, kdykoli za spuštěním stojí nějaká osoba.
...
* **Pre/post-install a odinstalační hooky** — volitelné funkce, které běží během instalace nebo těsně před odinstalací. Podrobnosti viz [Install Hooks](/l/cs/developers/extend/apps/config/install-hooks).
