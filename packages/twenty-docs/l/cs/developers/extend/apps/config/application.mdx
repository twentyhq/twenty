...
* Oba druhy proměnných přijímají `isDeprecated: true`. Použijte ji k vyřazení proměnné namísto jejího smazání: ponechání deklarovaného klíče zachová uloženou hodnotu (smazání ji zničí, tedy hodnotu, kterou operátor zadal) a proměnná je stále injektována, takže váš kód se k ní může vrátit — `process.env.NEW_API_KEY ?? process.env.API_KEY`. A deprecated variable disappears from the app's settings once it has no value, and never counts toward the app's configuration check, so `isDeprecated` beats `isRequired`.
