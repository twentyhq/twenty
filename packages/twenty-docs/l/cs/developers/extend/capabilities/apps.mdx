...
The `universalIdentifier` of this role is then referenced in `application.config.ts` as `roleUniversalIdentifier`. Jinými slovy:
...
* The API key's permissions are determined by the role referenced in your `application.config.ts` via `roleUniversalIdentifier`. Toto je výchozí role používaná logickými funkcemi vaší aplikace.
* Aplikace mohou definovat role podle principu nejmenších oprávnění. Grant only the permissions your functions need, then point `roleUniversalIdentifier` to that role's universal identifier.
