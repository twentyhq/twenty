...
Ve výchozím nastavení běží Twenty na `localhost` na portu `3000`. UI řeší API z vlastního původu prohlížeče, takže se načítá z libovolného hostnamu, který dosáhne na kontejner — ale přesto nastavte `SERVER_URL` na svou veřejnou URL: server ji používá pro generované odkazy (e-maily, OAuth callbacky) a k detekci HTTPS pro zabezpečené cookies.
