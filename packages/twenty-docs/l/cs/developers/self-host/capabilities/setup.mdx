...
<Note>
"Mail.ReadWrite" a "Mail.Send" jsou povinné pouze tehdy, pokud chcete odesílat e-maily pomocí akcí v našich pracovních postupech. Můžete použít "Mail.Read" místo toho, pokud chcete pouze přijímat e-maily.
</Note>

<Note>
"Calendars.ReadWrite" je povinné, pokud chcete vytvářet události v kalendáři pomocí akcí v našich pracovních postupech. Pokud chcete pouze synchronizovat události kalendáře, použijte "Calendars.Read".
</Note>
...
## API a front-end na samostatných serverech

Platí pouze v případě, že back-end neposkytuje front-end. Nasazení se stejným původem
zde nic nepotřebují.

Uveďte každý původ prohlížeče, který komunikuje s API, oddělený čárkami. `SERVER_URL`
a `FRONTEND_URL` jsou již důvěryhodné. Server zapisuje varování do logu, když jej prohlížeč
volá z nedeklarovaného původu.
...
<Warning>
Front-end odesílá přihlašovací údaje u každého požadavku a prohlížeče odmítnou
odpověď s přihlašovacími údaji z nedeklarovaného původu, takže se aplikace nenačte, dokud toto nenastavíte.
</Warning>

Pokud jsou dva hostitelé na různých registrovatelných doménách (nejen na různých subdoménách), nastavte také `AUTH_COOKIE_SAME_SITE=none`, což vyžaduje HTTPS.

Pokud je povolen vícepracovní prostředí (multi-workspace), neuvádějte zde každý dílčí doménový název pracovního prostoru.
Místo toho směrujte API cesty (`/graphql`, `/metadata`, `/rest`, `/auth/`,
`/client-config`, `/file`) na back-end na každém hostiteli pracovního prostoru, takže každý
pracovní prostor zůstává stejného původu (same-origin) — takto běží Twenty Cloud.
