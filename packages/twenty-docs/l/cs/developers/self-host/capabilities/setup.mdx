...
Je-li povoleno více pracovních prostorů (multi-workspace), neuvádějte zde každou subdoménu pracovního prostoru.
Místo toho směrujte cesty API (`/graphql`, `/metadata`, `/rest`, `/auth/`,
`/client-config`, `/file`) na back-end na každém hostname pracovního prostoru, aby každý
pracovní prostor zůstal ve stejném původu (same-origin) — takto běží Twenty Cloud.
