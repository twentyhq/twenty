...
### Rată de sincronizare

Twenty importă e-mailuri cu aproximativ **400 de mesaje pe minut**. Această rată este în prezent limitată de restricțiile de rată ale API-ului Gmail (și se aplică în mod similar altor furnizori).

Pentru referință:

* 1,000 de e-mailuri → ~2.5 minute
* 10,000 de e-mailuri → ~25 minute
* 50,000 de e-mailuri → ~2 ore
...
* **Actualizări**: Noile e-mailuri apar imediat

### Ce nu se sincronizează

* **E-mailuri interne**: În mod implicit, e-mailurile dintre colegi (același domeniu) rămân private (configurate în Setări → General → Securitate)
...
Alegeți dacă e-mailurile interne sunt sincronizate în Twenty:

1. Accesați **Setări → General → Securitate**
2. Comutați **Sincronizează e-mailurile interne** activat/dezactivat

### Sincronizare selectivă a folderelor
...
2. Configurați folderele în secțiunea Email
...
* **Întârzieri la sincronizare**: E-mailurile apar imediat, dar importurile inițiale durează mai mult
