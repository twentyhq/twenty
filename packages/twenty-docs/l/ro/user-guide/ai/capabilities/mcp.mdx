...
Pentru a crea o cheie API, mergi la **Settings → MCP & APIs → API → + Create key**. Vezi [API-uri](/l/ro/developers/extend/api#create-an-api-key) pentru detalii.

## Pornire rapidă

### 1. Copiază configurația

Mergi la **Settings → MCP & APIs → MCP** în Twenty. Alege metoda de autentificare (OAuth sau Cheie API), copiază fragmentul JSON (va folosi deja URL-ul spațiului tău de lucru) și lipește-l în fișierul de configurare al clientului tău MCP.
...
* **Cheie API**: Atribuie un rol cheii API în **Settings → Members → Roles**. Vezi [Permisiuni](/l/ro/user-guide/permissions-access/capabilities/permissions).
