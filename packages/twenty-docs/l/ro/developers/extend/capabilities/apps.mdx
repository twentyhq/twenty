...
* **Scaffolded**: Run `yarn app:create-entity` and choose the option to add a new function. Aceasta generează un fișier inițial cu un handler și o configurație.
* **Manual**: Creați un fișier nou `*.function.ts` și folosiți `defineFunction()`, urmând același model.

### Client tipizat generat

Run yarn app:generate to create a local typed client in generated/ based on your workspace schema. Folosiți-l în funcțiile dvs.:
