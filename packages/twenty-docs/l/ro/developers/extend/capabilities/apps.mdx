...
The `universalIdentifier` of this role is then referenced in `application.config.ts` as `roleUniversalIdentifier`. Cu alte cuvinte:
...
* The API key's permissions are determined by the role referenced in your `application.config.ts` via `roleUniversalIdentifier`. Acesta este rolul implicit folosit de funcțiile de logică ale aplicației.
* Aplicațiile pot defini roluri pentru a urma principiul celui mai mic privilegiu. Grant only the permissions your functions need, then point `roleUniversalIdentifier` to that role's universal identifier.
