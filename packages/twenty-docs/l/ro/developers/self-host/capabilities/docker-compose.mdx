...
Implicit, Twenty rulează pe `localhost` la portul `3000`. Interfața de utilizator rezolvă API-ul din origin-ul propriu al browserului, astfel încât se încarcă de pe orice hostname care ajunge la container — dar setează în continuare `SERVER_URL` la URL-ul tău public: serverul îl folosește pentru linkuri generate (emailuri, callback-uri OAuth) și pentru a detecta HTTPS pentru cookie-uri securizate.
