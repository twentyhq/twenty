...
Um deine Parameter **einmal** zu deklarieren und beide Oberflächen zu bedienen, definiere ein einziges JSON Schema (`InputJsonSchema`) und konvertiere es für die Workflow-Aktion mit `jsonSchemaToInputSchema` aus `twenty-sdk/logic-function`. `toolTriggerSettings.inputSchema` nimmt das JSON Schema direkt, während `workflowActionTriggerSettings.inputSchema` Twenty's `InputSchema` erwartet:
