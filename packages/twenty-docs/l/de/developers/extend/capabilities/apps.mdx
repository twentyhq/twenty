...
Mit Apps können Sie Twenty-Anpassungen **als Code** erstellen und verwalten. Instead of configuring everything through the UI, you define your data model and logic functions in code — making it faster to build, maintain, and roll out to multiple workspaces.
...
* **src/**: The main place where you define your application-as-code:
  ...
  * `*.role.ts`: Role definitions used by your logic functions. Siehe unten „Standard-Funktionsrolle“.
  * `*.object.ts`: Benutzerdefinierte Objektdefinitionen.
...
Anwendungen können Rollen definieren, die Berechtigungen für die Objekte und Aktionen Ihres Workspaces kapseln. The field `functionRoleUniversalIdentifier` in `application.config.ts` designates the default role used by your app's logic functions.
...
When a route trigger invokes your logic function, it receives a `RoutePayload` object that follows the AWS HTTP API v2 format. Importieren Sie den Typ aus `twenty-sdk`:
...
By default, HTTP headers from incoming requests are **not** passed to your logic function for security reasons. Um auf bestimmte Header zuzugreifen, listen Sie diese explizit im Array `forwardedRequestHeaders` auf:
...
* Die Berechtigungen des API-Schlüssels werden durch die Rolle bestimmt, auf die in Ihrer `application.config.ts` über `functionRoleUniversalIdentifier` verwiesen wird. This is the default role used by logic functions of your application.
