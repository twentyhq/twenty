...
<Warning>
  ...
  **So migrieren Sie bestehende Funktionen:** Aktualisieren Sie Ihren Handler, sodass er statt direkt aus dem params-Objekt aus `event.body`, `event.queryStringParameters` oder `event.pathParameters` destrukturiert.
</Warning>
