...
Anwendungen können Rollen definieren, die Berechtigungen für die Objekte und Aktionen Ihres Workspaces kapseln. The field `roleUniversalIdentifier` in `application.config.ts` designates the default role used by your app's logic functions.
...
The `universalIdentifier` of this role is then referenced in `application.config.ts` as `roleUniversalIdentifier`. Anders ausgedrückt:
...
* The API key's permissions are determined by the role referenced in your `application.config.ts` via `roleUniversalIdentifier`. Dies ist die Standardrolle, die von den Logikfunktionen Ihrer Anwendung verwendet wird.
* Anwendungen können Rollen definieren, um das Least-Privilege-Prinzip einzuhalten. Grant only the permissions your functions need, then point `roleUniversalIdentifier` to that role's universal identifier.
