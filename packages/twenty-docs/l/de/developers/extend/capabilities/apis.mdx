...
<Note>
  Ihre personalisierte API-Dokumentation ist nach dem Erstellen eines
  API-Schlüssels unter **Einstellungen → API & Webhooks** verfügbar. Da Twenty
  APIs erzeugt, die Ihrem benutzerdefinierten Datenmodell entsprechen, ist die
  Dokumentation für Ihren Arbeitsbereich einzigartig.
</Note>
...
<Warning>
  Ihr API-Schlüssel gewährt Zugriff auf sensible Daten. Teilen Sie ihn nicht mit
  nicht vertrauenswürdigen Diensten. Wenn er kompromittiert wurde, deaktivieren
  Sie ihn umgehend und erstellen Sie einen neuen.
</Warning>
...
<Tip>
  Verwenden Sie Batch-Vorgänge, um den Durchsatz zu maximieren — verarbeiten Sie
  bis zu 60 Datensätze in einem einzelnen API-Aufruf, statt einzelne Anfragen zu
  senden.
</Tip>
