...
<Warning>
  `npm` wird nicht funktionieren, Sie sollten stattdessen `yarn` verwenden. Yarn
  wird jetzt mit Node.js geliefert, so dass Sie es nicht separat installieren
  müssen. Sie müssen nur `corepack enable` ausführen, um Yarn zu aktivieren,
...
<Warning>
  Verwenden Sie `nvm`, um die korrekte `node`-Version zu installieren. Die
  `.nvmrc` stellt sicher, dass alle Mitwirkenden die gleiche Version verwenden.
</Warning>
...
<Info>
  **Multi-Workspace-Modus:** Standardmäßig läuft Twenty im
  Einzel-Workspace-Modus, in dem nur ein Workspace erstellt werden kann. Um die
  Multi-Workspace-Unterstützung zu aktivieren (nützlich zum Testen von
  Subdomain-basierten Funktionen), setzen Sie `IS_MULTIWORKSPACE_ENABLED=true`
  in der `.env`-Datei Ihres Servers. Siehe
  [Multi-Workspace-Modus](/l/de/developers/self-host/capabilities/setup#multi-workspace-mode)
  für Details.
</Info>
...
<Tabs>
  <Tab title="Linux">
    Je nach Ihrer Linux-Distribution könnte der Redis-Server automatisch
    gestartet werden. Wenn nicht, überprüfen Sie den [Redis
    Installationsleitfaden](https://redis.io/docs/latest/operate/oss_and_stack/install/install-redis/)
    für Ihre Distribution.
  </Tab>
  <Tab title="Mac OS">
    Redis sollte bereits laufen. Falls nicht, führen Sie aus: ```bash brew
