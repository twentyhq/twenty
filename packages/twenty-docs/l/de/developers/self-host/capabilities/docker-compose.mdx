...
Standardmäßig läuft Twenty auf `localhost` an Port `3000`. Die UI ruft die API vom ursprünglichen Ursprung des Browsers auf, sodass sie von jedem Hostnamen geladen wird, der den Container erreicht – setze jedoch trotzdem `SERVER_URL` auf deine öffentliche URL: Der Server verwendet sie für generierte Links (E-Mails, OAuth-Callbacks) und zur Erkennung von HTTPS für sichere Cookies.
