...
<CardGroup cols={2}>
  <Card title="كائنات" icon="table" href="/l/ar/developers/extend/apps/data/objects">
  ...
  <Card title="أنواع أنشطة الجدول الزمني" icon="clock-rotate-left" href="/l/ar/developers/extend/apps/data/timeline-activity-types">
    حدّد أحداث التدقيق التلقائية وأحداث التطبيق المحددة صراحةً للجداول الزمنية للسجلات.
  </Card>
</CardGroup>
...
| **نوع نشاط الجدول الزمني** | حدث تدقيق تلقائي أو حدث مجال صريح يظهر في المخططات الزمنية للسجلات.                                     | `defineTimelineActivityType()`          |

يكتشف SDK هذه العناصر عبر تحليل AST أثناء وقت الإنشاء، لذا تنظيم الملفات يعود إليك — الاصطلاح هو `src/objects/` و `src/fields/` و `src/indexes/` و `src/timeline-activity-types/`. معرّفات UUID ثابتة من نوع `universalIdentifier` تربط كل شيء معًا عبر عمليات النشر.
