...
* يُعرّف كل `tab` قسمًا من الصفحة مع `title` و`position` و`layoutMode`: `VERTICAL_LIST` لصفحات السجلات والصفحات المستقلة، و`GRID` للوحات المعلومات. في علامة تبويب من نوع `VERTICAL_LIST`، يعرض عنصر واجهة واحد ممتدًا حتى الحواف ويملك علامة التبويب بالكامل؛ وعند وجود عدة عناصر واجهة، فإنها تتكدس كبطاقات. تعرض علامة تبويب من نوع `GRID` دائمًا عناصر الواجهة كبطاقات على شبكة من 12 عمودًا مهما كان عددها، لذا اختر `VERTICAL_LIST` عندما تريد لعنصر واجهة واحد أن يملأ الصفحة.
