...
* يحتوي التعداد أيضًا على `NavigationMenuItemType.RECORD`، ويُستخدم داخليًا للمفضّلات التي ينشئها المستخدم للسجلات — ولا يمكن استخدامه من بيان التطبيق (app manifest) لعدم وجود حقل للإشارة إلى سجل.
...
<Note>
**مشكلة شائعة:** إنشاء كائن بدون عنصر قائمة تنقّل مرتبط يجعل ذلك الكائن غير قابل للوصول من الشريط الجانبي. ما لم يكن كائنًا تقنيًا/داخليًا، يجب أن يحتوي كل كائن مخصّص على عنصر في الشريط الجانبي يشير إليه.
</Note>
