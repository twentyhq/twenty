...
تحمل كل رسالة بريد إلكتروني ترويسة `Message-ID` مخفية — بصمة فريدة يعيّنها خادم بريد المرسل. When you reply to an email, your mail client sets an `In-Reply-To` header referencing that fingerprint. That's how Gmail, Outlook, and every other client groups messages into threads.
...
| الحقل       | القيمة                                          |
| ----------- | ----------------------------------------------- |
| **الشرط**   | AI Agent `response` **does not contain** `SKIP` |
