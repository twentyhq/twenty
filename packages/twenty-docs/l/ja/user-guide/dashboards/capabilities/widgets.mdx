...
<Note>
  **Display limits**: Bar charts can show a maximum of 100 bars, and 50 groups
  per bar when using Group By. 警告 "Undisplayed data: max X bars per chart"
  が表示された場合は、フィルターを追加してデータを絞り込むか、グルーピングを変更します（例:
  日単位ではなく週単位でグループ化）。
</Note>
...
<Note>
  **近日提供予定**:
  ゲージチャートとテーブルはまだ利用できませんが、ロードマップに含まれています。
</Note>
