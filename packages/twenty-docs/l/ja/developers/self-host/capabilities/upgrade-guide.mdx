...
## v2.34+ へのアップグレード — PostgreSQL 15 以降が必要

Twenty v2.34 には PostgreSQL 15 以降が必要です。 公式の Docker Compose 設定では PostgreSQL 16 を使用します。 インスタンスが外部で管理されているデータベースに接続している場合は、v2.34 サーバーを起動する前にアップグレードしてください。

## クロスバージョンのアップグレード (v1.23+)

**v1.23** 以降、Twenty はクロスバージョンのアップグレードをサポートします。 サポートされている任意のバージョンから、中間バージョンを段階的に経ることなく、最新リリースへ直接移行できます。

たとえば、v1.23 から v2.0 へ直接アップグレードすることが完全にサポートされています。
...
インスタンスが v1.23 より前の場合は、v1.23 に到達するまで、各メジャーのタグ付きバージョンを順に（v1.6 から v1.7、次に v1.7 から v1.8、…）段階的にアップグレードする必要があります。 そこからは、最新バージョンに直接アップグレードできます。
