...
代わりに、各ワークスペースのホスト名ごとに API パス（`/graphql`、`/metadata`、`/rest`、`/auth/`、`/client-config`、`/file`）をバックエンドへルーティングし、すべてのワークスペースが same-origin を維持するようにします。これは Twenty Cloud が動作している方式です。
