...
`PageChangeEffect` sidecar 组件只包含一个 `useEffect` ，它持有页面更改时要执行的所有逻辑。

这样您就知道只有一个地方可以触发再渲染。

### 在代码库中添加 `useEffect` 之前总是考虑一次

重新渲染往往是由不必要的“使用效果”造成的。

您应该想一想您是否需要`useEffect`，或者您是否可以在事件处理函数中移动逻辑。

你会发现移动一个 "handleClick" 或 "handleChange" 函数中的逻辑通常很容易。

您还可以在如 Apollo 的库中找到它们：`onCompleted`，`onError` 等。

### 使用一个兄弟组件提取`useEffect`或数据获取逻辑。

如果你觉得需要在你的根组件中添加 "useEffect" ，你应该考虑在侧边组件中提取它。
