...
## 升级到 v2.34+ — 最低要求 PostgreSQL 15

Twenty v2.34 要求 PostgreSQL 15 或更高版本。 官方 Docker Compose 配置使用 PostgreSQL 16。 如果您的实例连接到外部托管的数据库，请在启动 v2.34 服务器之前对其进行升级。

## 跨版本升级（v1.23+）

自 **v1.23** 起，Twenty 支持跨版本升级。 您可以从任意受支持的版本直接跳到最新版本，而无需逐一经过每个中间版本。

例如，从 v1.23 直接升级到 v2.0 完全受支持。
...
你应该在 v2.5 升级**之前**设置专用的 `ENCRYPTION_KEY`，这样回填从一开始就会在该密钥下写入行。 在回填完成后再更换密钥，则需要进行一次[轮换](/zh/developers/self-host/capabilities/key-rotation)。

## 轮换机密和签名密钥

对于日常运维任务，例如轮换 `ENCRYPTION_KEY`、轮换 JWT 签名密钥或吊销泄露的签名密钥，请参阅专门的[密钥轮换指南](/zh/developers/self-host/capabilities/key-rotation)。
...
如果您的实例早于 v1.23，您必须按顺序逐步升级经过每个主要标记版本（从 v1.6 到 v1.7，再从 v1.7 到 v1.8，依此类推），直到达到 v1.23。 届时，您可以直接跳到最新版本。
