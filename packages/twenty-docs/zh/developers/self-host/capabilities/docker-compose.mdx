...
<Warning>
Docker 容器适用于生产环境托管或自托管。 如需参与贡献，请查看[本地设置](/zh/developers/contribute/capabilities/local-setup)。
</Warning>
...
有关高级配置，请参阅[设置环境变量](/zh/developers/self-host/capabilities/setup)。 所有环境变量必须在 `docker-compose.yml` 文件中声明，具体在服务器和/或 worker 层级，取决于变量。
...
3. **更新`.env`**
   ...
   有关在不停机的情况下轮换密钥的说明，请参阅[密钥轮换指南](/zh/developers/self-host/capabilities/key-rotation)。
...
默认情况下，Twenty在`localhost`的端口`3000`上运行。 UI 会基于浏览器自身的源（origin）解析 API 地址，因此它可以从任何能访问该容器的主机名加载——但仍需要将 `SERVER_URL` 设置为你的公共 URL：服务器会用它来生成链接（邮件、OAuth 回调），并据此检测是否启用 HTTPS，以设置安全 Cookie。
...
如果您遇到任何问题，请检查[故障排除](/zh/developers/self-host/capabilities/troubleshooting)以获取解决方案。

## 托管服务

<Tip>
  不想自己运行和维护 Twenty 吗？ [寻找经过认证的 Twenty 托管合作伙伴](https://twenty.com/partners/list?categories=HOSTING\&ref=docs-docker-compose) 来端到端部署、托管和管理你的实例。 *(或者联系我们的团队： [contact@twenty.com](mailto:contact@twenty.com)。)*
</Tip>
