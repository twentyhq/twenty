...
**导航菜单项** 是左侧边栏中的一个条目。 使用 `defineNavigationMenuItem()` 发布自定义侧边栏链接——通常为你发布的每个[视图](/zh/developers/extend/apps/layout/views)提供一个链接——或指向外部 URL。
...
<Note>
**常见陷阱：** 创建一个没有关联导航菜单项的对象会使该对象无法从侧边栏访问。 除非是技术性/内部对象，否则每个自定义对象都应在侧边栏中有一个指向它的条目。
</Note>
