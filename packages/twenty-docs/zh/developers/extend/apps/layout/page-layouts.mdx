...
* 选项卡内的每个 `widget` 可以渲染一个[前端组件](/zh/developers/extend/apps/layout/front-components)、关系列表或其他内置小部件类型。
...
* `widgets` 仅作用于此选项卡——它们引用[前端组件](/zh/developers/extend/apps/layout/front-components)、视图等，其方式与在 `definePageLayout` 中内联定义的小部件完全相同。
