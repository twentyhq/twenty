...
* `fields` 控制显示哪些列以及它们的顺序。 每个字段引用一个 `fieldMetadataUniversalIdentifier`。 若要引用自动创建的系统字段（例如 `createdAt`），请参阅[定位系统元数据](/zh/developers/extend/apps/data/system-fields)。
...
| 属性                                                                                                          | 值                                                                                  | 描述                                                                                                                           |
| ----------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------- |
| `type`                                                                                                      | `ViewType.TABLE`（默认），`ViewType.LIST`，`ViewType.KANBAN`，`ViewType.CALENDAR`         | 记录的排布方式。 （`FIELDS_WIDGET`、`TABLE_WIDGET`、`KANBAN_WIDGET`、`LIST_WIDGET` 和 `CALENDAR_WIDGET` 也存在，但由页面布局小部件在内部使用。）              |
| `visibility`                                                                                                | `ViewVisibility.WORKSPACE`（默认），`ViewVisibility.UNLISTED`                           | 视图是对整个工作区可见，还是在选择器中隐藏。                                                                                                       |
| `openRecordIn`                                                                                              | 已弃用                                                                                | 不再读取：记录的打开位置现在是[对象](/zh/developers/extend/apps/data/objects)的一个属性（`defineObject()` 上的 `openRecordIn`），如果未设置该属性，则回退到每个成员自己的偏好设置。 |
...
单独一个视图无法从侧边栏访问。 要让它显示在侧边栏中，请将其与类型为 `VIEW`、指向该视图 `universalIdentifier` 的[导航菜单项](/zh/developers/extend/apps/layout/navigation-menu-items)配对。 这是规范做法：每个自定义对象在创建时都会自动生成一个默认的列表视图，因此你只需提供用于打开它的侧边栏条目。
