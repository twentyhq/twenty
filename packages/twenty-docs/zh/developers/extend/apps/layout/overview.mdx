...
<CardGroup cols={2}>
  <Card title="视图" icon="list" href="/zh/developers/extend/apps/layout/views">
    `defineView` — 已保存的列表配置：可见列、筛选器、分组。
  </Card>
  <Card title="导航菜单项" icon="bars" href="/zh/developers/extend/apps/layout/navigation-menu-items">
    `defineNavigationMenuItem` — 指向视图或外部 URL 的侧边栏条目。
  </Card>
  <Card title="页面布局" icon="table-columns" href="/zh/developers/extend/apps/layout/page-layouts">
    `definePageLayout` 和 `definePageLayoutTab` — 记录详情页上的选项卡和小部件。
  </Card>
  <Card title="前端组件" icon="window-maximize" href="/zh/developers/extend/apps/layout/front-components">
    `defineFrontComponent` — 在 Twenty 内部渲染的沙箱化 React 组件。
  </Card>
  <Card title="命令菜单项" icon="terminal" href="/zh/developers/extend/apps/layout/command-menu-items">
...
它们仍在积极开发中：沙盒仅实现了部分 DOM，因此高级用法可能会失败。 请参阅[当前限制](/zh/developers/extend/apps/layout/front-components#current-limitations)。
