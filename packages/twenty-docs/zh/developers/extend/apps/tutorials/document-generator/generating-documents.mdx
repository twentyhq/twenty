...
现在核心：一个[逻辑函数](/zh/developers/extend/apps/logic/logic-functions)
...
（`yarn test:unit`）。 见 [Testing](/zh/developers/extend/apps/operations/testing)。
</Tip>

## 处理程序

处理程序使用生成的 [`CoreApiClient`](/zh/developers/extend/apps/logic/logic-functions)
...
请参阅 [Roles](/zh/developers/extend/apps/config/roles) 以获取更精细的权限。
...
<Card title="下一步：HTTP路由 →" icon="全局模式" href="/zh/developers/extend/apps/tutorials/document-generator/http-route">
