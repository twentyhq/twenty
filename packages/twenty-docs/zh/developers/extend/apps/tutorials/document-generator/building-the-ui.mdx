...
[view](/zh/developers/extend/apps/layout/views) 是一个已保存的对象列表。
[导航菜单项](/zh/developers/extend/apps/layout/navigation-menu-items)
将此视图放置在侧边栏中。 服务器已经为每个对象自动预配了默认的 "All ..." 视图，因此应用定义的视图是额外的、经过精心筛选的视图。
...
[前端组件](/zh/developers/extend/apps/layout/front-components)是一个被沙箱隔离在 Twenty 内部的 React 组件。 我们读取了选中的记录，通过 `CoreApiClient` 和 POSTs 将
人模板加载到最后一章的
路线。
...
当选中某个 Person 时，带有 `availabilityType: 'RECORD_SELECTION'` 的[命令菜单项](/zh/developers/extend/apps/layout/command-menu-items)会出现，并在侧边面板中打开该组件。
...
用[页面布局](/zh/developers/extend/apps/layout/page-layouts)挂载它。 `RECORD_PAGE` 布局会在对象的记录视图中添加选项卡；包含单个 `FRONT_COMPONENT` 小部件的选项卡会以全宽方式承载该组件：
...
`FIELD\` 小部件也支持其他显示模式和选项，包括
列出相隔两跳的嵌套关联记录——请参阅
[字段小部件参考](/zh/developers/extend/apps/layout/page-layouts#field-widgets).
...
<Card title="下一步：一个 AI 代理 →" icon="robot" href="/zh/developers/extend/apps/tutorials/document-generator/ai-agent">
  添加代理和技能来呼叫您的工具。
</Card>
