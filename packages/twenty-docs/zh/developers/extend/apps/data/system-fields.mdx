...
存在于每个对象上的标量字段，你不会使用 [`defineField()`](/zh/developers/extend/apps/data/extending-objects) 来声明其中任何一个：

`id`, `createdAt`, `updatedAt`, `deletedAt`, `createdBy`, `updatedBy`, `position`, `searchVector`

那么，如何在[视图](/zh/developers/extend/apps/layout/views)中将 `createdAt` 作为一列来引用呢？
...
* `applicationUniversalIdentifier` 是你的应用标识符，也就是你传递给 [`defineApplication()`](/zh/developers/extend/apps/config/application) 的那个。
...
解析得到的 id 可用于任何需要 `viewUniversalIdentifier` 的地方，例如侧边栏中的 [`NavigationMenuItemType.VIEW`](/zh/developers/extend/apps/layout/navigation-menu-items) 条目。 如果只是要打开某个对象的主列表，优先使用带有 `targetObjectUniversalIdentifier` 的 `NavigationMenuItemType.OBJECT`：它不需要派生。
...
<Warning>
  系统视图及其列是由**服务器所有**的：解析它们的标识符是为了引用它们，而不是为了声明它们。 [`defineView()`](/zh/developers/extend/apps/layout/views) 上的 `key` 已被弃用且会被忽略，因此 manifest 视图永远无法占用 `INDEX` 键，并且服务器已经为你添加的每个字段预配置了一列，所以在系统视图上为同一个字段声明你自己的 `defineViewField()` 会与其发生冲突。
</Warning>
...
当对象是由**你的应用**使用 [`defineObject()`](/zh/developers/extend/apps/data/objects) 定义、且不存在此类常量时，再使用上面的辅助方法。
