...
* 此处定义的内联字段**不**需要 `objectUniversalIdentifier`——它会从父对象继承。 使用[`defineField()`](/zh/developers/extend/apps/data/extending-objects)为你不拥有的对象添加字段。
* 你可以使用 `yarn twenty dev:add object` 脚手架创建新对象，它会引导你完成命名、字段和关系。 参见 [Architecture → Scaffolding entities](/zh/developers/extend/apps/getting-started/scaffolding)。
...
| 标识符和关联 | `UUID`、`RELATION`、`MORPH_RELATION`（参见 [Relations](/zh/developers/extend/apps/data/relations)）                  |
...
* **将此对象与其他对象关联**——关于双向关系模式，参见 [Relations](/zh/developers/extend/apps/data/relations)。
* **为其他应用的对象添加字段**——关于 `defineField()`，参见 [Extending Objects](/zh/developers/extend/apps/data/extending-objects)。
* **在界面中显示此对象** — 请参阅 [导航菜单项](/zh/developers/extend/apps/layout/navigation-menu-items) 以添加侧边栏条目；请参阅 [视图](/zh/developers/extend/apps/layout/views) 以添加自定义列表配置。
