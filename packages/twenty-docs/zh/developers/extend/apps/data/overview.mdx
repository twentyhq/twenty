...
<CardGroup cols={2}>
  <Card title="对象" icon="table" href="/zh/developers/extend/apps/data/objects">
    `defineObject` —— 声明具有自有字段的新记录类型。
  </Card>
  <Card title="扩展对象" icon="wand-magic-sparkles" href="/zh/developers/extend/apps/data/extending-objects">
    `defineField` —— 向标准对象或其他应用的对象添加字段。
  </Card>
  <Card title="关系" icon="diagram-project" href="/zh/developers/extend/apps/data/relations">
    对象之间的双向 `MANY_TO_ONE` / `ONE_TO_MANY` 连接。
  </Card>
  <Card title="时间线活动类型" icon="clock-rotate-left" href="/zh/developers/extend/apps/data/timeline-activity-types">
    为记录时间线定义自动审计事件和显式应用事件。
  </Card>
</CardGroup>
...
| **时间线活动类型** | 显示在记录时间线中的自动审计事件或显式域事件                                | `defineTimelineActivityType()`              |

SDK 会在构建时通过 AST 分析检测这些内容，因此文件组织方式由你决定——约定是使用 `src/objects/`、`src/fields/`、`src/indexes/` 和 `src/timeline-activity-types/`。 稳定的 `universalIdentifier` UUID 在不同部署之间将一切关联在一起。
...
<Note>
在找 **Application Config** 或 **Roles & Permissions** 吗？ 这些描述的是应用本身而不是它添加的数据——相关内容位于 [Config](/zh/developers/extend/apps/config/overview) 下。 在找 **Connections**（Linear、GitHub、Slack OAuth）吗？ 这些用于*从*逻辑函数中调用，并位于 [Logic](/zh/developers/extend/apps/logic/connections) 下。
</Note>
