...
要创建 API 密钥，请前往 **Settings → MCP & APIs → API → + Create key**。 详见[API](/zh/developers/extend/api#create-an-api-key)。

## 快速开始

### 1. 复制配置

在 Twenty 中前往 **Settings → MCP & APIs → MCP**。 选择您的身份验证方式（OAuth 或 API 密钥），复制 JSON 片段（其中已包含您的工作区 URL），并将其粘贴到 MCP 客户端的配置文件中。
...
* **API 密钥**：在 **Settings → Members → Roles** 下为该 API 密钥分配角色。 参见[权限](/zh/user-guide/permissions-access/capabilities/permissions)。
