...
<Warning>
**Première installation ?** Suivez le [Guide d’installation de Docker Compose](/fr/developers/self-host/capabilities/docker-compose) pour faire fonctionner Twenty, puis revenez ici pour la configuration.
</Warning>
...
Certains paramètres d'infrastructure comme les connexions de base de données (`PG_DATABASE_URL`), les URL du serveur (`SERVER_URL`) et les secrets (`ENCRYPTION_KEY`, `FALLBACK_ENCRYPTION_KEY`) ne peuvent être configurés que via le fichier `.env`.
...
Pour assurer la rétrocompatibilité, si `ENCRYPTION_KEY` n'est pas défini, Twenty se rabat sur `APP_SECRET` pour le chiffrement au repos, ce qui correspond au comportement historique des anciens déploiements. Les nouvelles installations doivent toujours définir une `ENCRYPTION_KEY` dédiée.
...
Pour faire pivoter `ENCRYPTION_KEY` sans temps d'arrêt, consultez le [guide de rotation des clés](/fr/developers/self-host/capabilities/key-rotation).
...
<Note>
"Mail.ReadWrite" et "Mail.Send" ne sont obligatoires que si vous souhaitez envoyer des e-mails en utilisant nos actions de flux de travail. Vous pouvez utiliser "Mail.Read" à la place si vous souhaitez uniquement recevoir des e-mails.
</Note>

<Note>
"Calendars.ReadWrite" est obligatoire si vous souhaitez créer des événements de calendrier en utilisant nos actions de flux de travail. Si vous souhaitez uniquement synchroniser des événements de calendrier, utilisez "Calendars.Read".
</Note>
...
Définissez `STORAGE_TYPE=S_3` et configurez les variables `STORAGE_S3_*` via le panneau d'administration ou `.env`. Voir la [référence config-variables.ts](https://github.com/twentyhq/twenty/blob/main/packages/twenty-server/src/engine/core-modules/twenty-config/config-variables.ts) pour la liste complète des variables S3.
...
**En production (NODE_ENV=production):** Les fonctions logiques et l'interpréteur de code sont par défaut sur **Désactivé**. Vous devez les activer explicitement avec `LOGIC_FUNCTION_TYPE` et `CODE_INTERPRETER_TYPE` si vous avez besoin de ces fonctionnalités.
...
## API et interface front-end sur des hôtes séparés

Uniquement pertinent si le back-end ne sert pas l’interface front-end. Les déploiements de même origine
n’ont besoin de rien ici.

Déclarez toutes les origines de navigateur qui communiquent avec l’API, séparées par des virgules. `SERVER_URL`
et `FRONTEND_URL` sont déjà approuvées. Le serveur consigne un avertissement lorsqu’un navigateur
l’appelle depuis une origine non déclarée.
...
<Warning>
Le front-end envoie les identifiants à chaque requête, et les navigateurs rejettent une
réponse avec des identifiants provenant d’une origine non déclarée, de sorte que l’application ne parvient pas à se charger tant que vous n’avez pas défini ce paramètre.
</Warning>

Si les deux hôtes se trouvent sur des domaines enregistrables différents (et pas seulement sur des sous-domaines différents), définissez également `AUTH_COOKIE_SAME_SITE=none`, ce qui nécessite HTTPS.

Avec le mode multi-espaces de travail activé, ne répertoriez pas ici chaque sous-domaine d’espace de travail.
Au lieu de cela, acheminez les chemins d’API (`/graphql`, `/metadata`, `/rest`, `/auth/`,
`/client-config`, `/file`) vers le back-end sur chaque nom d’hôte d’espace de travail, afin que chaque
espace de travail reste de même origine — c’est ainsi que fonctionne Twenty Cloud.
