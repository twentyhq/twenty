...
Une [vue](/fr/developers/extend/apps/layout/views) est une liste enregistrée d’un objet donné.
Un [élément du menu de navigation](/fr/developers/extend/apps/layout/navigation-menu-items)
place cette vue dans la barre latérale. Le serveur provisionne déjà automatiquement, pour chaque objet, la vue par défaut « Tous ... » ; les vues définies par une application sont donc des vues supplémentaires, sélectionnées.
...
Un [composant frontal](/fr/developers/extend/apps/layout/front-components) est un composant React
...
Un [lien de menu de commande](/fr/developers/extend/apps/layout/command-menu-items) avec
...
Les widgets `FIELD` prennent également en charge d'autres modes d'affichage et options, notamment l’énumération des enregistrements d’une relation imbriquée à deux sauts — voir la [référence des widgets de champ](/fr/developers/extend/apps/layout/page-layouts#field-widgets).
