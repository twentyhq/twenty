...
<CardGroup cols={2}>
  <Card title="Objets" icon="table" href="/fr/developers/extend/apps/data/objects">
    `defineObject` — déclarez de nouveaux types d’enregistrements avec leurs propres champs.
  </Card>
  <Card title="Extension des objets" icon="wand-magic-sparkles" href="/fr/developers/extend/apps/data/extending-objects">
    `defineField` — ajoutez des champs aux objets standard ou à ceux d’autres applications.
  </Card>
  <Card title="Relations" icon="diagram-project" href="/fr/developers/extend/apps/data/relations">
    Connexions bidirectionnelles `MANY_TO_ONE` / `ONE_TO_MANY` entre objets.
  </Card>
  <Card title="Types d'activités de la chronologie" icon="clock-rotate-left" href="/fr/developers/extend/apps/data/timeline-activity-types">
    Définissez des événements d'audit automatiques et des événements d'application explicites pour les chronologies d'enregistrements.
  </Card>
</CardGroup>
...
| **Type d'activité de la chronologie** | Un événement d'audit automatique ou un événement de domaine explicite affiché dans les chronologies des enregistrements.                    | `defineTimelineActivityType()`            |

Le SDK les détecte via une analyse AST au moment de la compilation, donc l’organisation des fichiers vous appartient — la convention est `src/objects/`, `src/fields/`, `src/indexes/` et `src/timeline-activity-types/`. Des UUID `universalIdentifier` stables relient tout d’un déploiement à l’autre.
...
<Note>
Vous cherchez **Application Config** ou **Roles & Permissions** ? Ces éléments décrivent l’application elle-même plutôt que les données qu’elle ajoute — ils se trouvent sous [Config](/fr/developers/extend/apps/config/overview). Vous cherchez **Connections** (Linear, GitHub, Slack OAuth) ? Ceux-ci existent pour être appelés *depuis* des fonctions logiques et se trouvent sous [Logic](/fr/developers/extend/apps/logic/connections).
</Note>
