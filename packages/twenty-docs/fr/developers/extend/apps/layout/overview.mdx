...
<CardGroup cols={2}>
  <Card title="Vues" icon="list" href="/fr/developers/extend/apps/layout/views">
    `defineView` — configurations de listes enregistrées : colonnes visibles, filtres, groupes.
  </Card>
  <Card title="Éléments de menu de navigation" icon="bars" href="/fr/developers/extend/apps/layout/navigation-menu-items">
    `defineNavigationMenuItem` — entrées de barre latérale pointant vers des vues ou des URL externes.
  </Card>
  <Card title="Mises en page" icon="table-columns" href="/fr/developers/extend/apps/layout/page-layouts">
    `definePageLayout` et `definePageLayoutTab` — onglets et widgets sur la page de détails d’un enregistrement.
  </Card>
  <Card title="Composants frontaux" icon="window-maximize" href="/fr/developers/extend/apps/layout/front-components">
    `defineFrontComponent` — composants React isolés rendus à l’intérieur de Twenty.
  </Card>
  <Card title="Éléments du menu de commande" icon="terminal" href="/fr/developers/extend/apps/layout/command-menu-items">
...
Elles sont encore en cours de développement actif : le bac à sable implémente un DOM partiel, donc les usages avancés peuvent échouer. Voir [Limitations actuelles](/fr/developers/extend/apps/layout/front-components#current-limitations).
