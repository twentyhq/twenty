...
<CardGroup cols={2}>
  <Card title="Fonctions logiques" icon="bolt" href="/fr/developers/extend/apps/logic/logic-functions">
    Le bloc de construction principal — types de déclencheurs, charges utiles et client API typé.
  </Card>
  <Card title="Compétences et agents" icon="robot" href="/fr/developers/extend/apps/logic/skills-and-agents">
    Instructions d’agent IA réutilisables et assistants avec invites système personnalisées.
  </Card>
  <Card title="Connexions" icon="plug" href="/fr/developers/extend/apps/logic/connections">
    Identifiants OAuth détenus par votre application pour des services tiers — Linear, GitHub, Slack, et plus encore.
  </Card>
  <Card title="Stockage clé-valeur" icon="database" href="/fr/developers/extend/apps/logic/key-value-store">
    Conservez l’état entre les exécutions de fonctions logiques — caches, curseurs et revendications inter-espaces de travail.
  </Card>
  <Card title="Tâches en arrière-plan" icon="layer-group" href="/fr/developers/extend/apps/logic/background-jobs">
    Mettez en file d’attente l’exécution d’une fonction logique sur les workers afin de contourner la limite de temps par exécution.
  </Card>
</CardGroup>
...
Les fonctions s’exécutent dans un environnement isolé dans des processus Node.js sandboxés et accèdent à l’espace de travail via un client API typé, limité au rôle déclaré sur [`defineApplication()`](/fr/developers/extend/apps/config/application).

<Note>
Les **hooks au moment de l’installation** — du code qui s’exécute avant ou après l’installation — partagent ce runtime mais utilisent leurs propres fonctions define et se trouvent sous [Config → Install Hooks](/fr/developers/extend/apps/config/install-hooks).
</Note>
