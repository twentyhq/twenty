...
Une fois que votre application est [créée et testée en local](/fr/developers/extend/apps/getting-started/concepts), vous avez deux options pour sa distribution :
...
Voir l'[accordéon defineApplication](/fr/developers/extend/apps/config/application#marketplace-metadata) dans la page Building Apps pour la liste complète des champs de la place de marché (`author`, `category`, `aboutDescription`, `websiteUrl`, `termsUrl`, etc.).
