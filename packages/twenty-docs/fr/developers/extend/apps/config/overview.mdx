...
<CardGroup cols={2}>
  <Card title="Configuration de l'application" icon="rocket" href="/fr/developers/extend/apps/config/application">
    `defineApplication` — identité, rôle par défaut, variables, métadonnées de la marketplace.
  </Card>
  <Card title="Rôles et autorisations" icon="shield-halved" href="/fr/developers/extend/apps/config/roles">
    `defineRole` — déclarez ce que les fonctions logiques de votre application peuvent lire et écrire.
  </Card>
  <Card title="Hooks d'installation" icon="wrench" href="/fr/developers/extend/apps/config/install-hooks">
...
<Note>
Les hooks d'installation partagent l'environnement d'exécution des [fonctions logiques](/fr/developers/extend/apps/logic/logic-functions) — même signature de handler, mêmes variables d'environnement, même client d'API typé — mais ils sont déclarés avec leurs propres fonctions `define` et vivent en dehors du modèle de déclencheurs habituel (HTTP, cron, événements de base de données).
</Note>
