...
Para criar uma chave de API, vá em **Settings → MCP & APIs → API → + Create key**. Veja [APIs](/pt/developers/extend/api#create-an-api-key) para detalhes.
...
* **Chave de API**: Atribua uma função à chave de API em **Settings → Members → Roles**. Veja [Permissões](/pt/user-guide/permissions-access/capabilities/permissions).
