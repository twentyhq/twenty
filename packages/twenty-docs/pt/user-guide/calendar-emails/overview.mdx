...
### Configuração IMAP/SMTP/CalDAV (Outros Provedores)

Para outros provedores de email e calendário:

1. Vá para **Configurações → Contas**
2. Configure as configurações de IMAP para sincronizar emails recebidos e as configurações de SMTP para enviar emails
3. Configure as definições CalDAV para o calendário
4. Teste a ligação

<Note>
**Self-hosting em uma rede isolada (air-gapped) ou interna**: Por padrão, Twenty recusa conexões de saída para endereços IP privados/internos (proteção contra SSRF). Se o seu servidor de email ou calendário estiver em um IP local/privado (por exemplo, um servidor local instalado em uma LAN), as conexões para ele serão bloqueadas. Para permiti-las, defina a seguinte variável de ambiente no servidor:

```
OUTBOUND_HTTP_SAFE_MODE_ENABLED=false
```

Isso desativa o modo seguro para **todas** as solicitações de saída (ações de fluxo de trabalho HTTP, webhooks e conexões IMAP/SMTP/CalDAV), portanto, defina essa configuração apenas em redes confiáveis e isoladas, onde a proteção contra SSRF não é necessária.
</Note>
...
<Note>
**Sincronizando emails internos**: O comportamento de "emails internos não sincronizados" é o padrão, mas pode ser desativado. A alternância fica em configurações avançadas:
1. Abra **Configurações** e ative a alternância **Avançado** na parte inferior da página de configurações
2. Vá para **Geral → Segurança**
3. Ative a alternância **Sincronizar emails internos** para incluir emails em que todos os participantes compartilham o mesmo domínio

Essa é uma configuração em todo o workspace (útil para universidades ou organizações com domínio compartilhado).
</Note>
...
* **Emails internos**: Não sincronizados por padrão (emails do mesmo domínio permanecem privados). Ative a alternância **Avançado** na parte inferior de **Configurações** e, em seguida, ative **Sincronizar emails internos** em **Geral → Segurança** para incluí-los em todo o workspace.
...
* [Funcionalidades da caixa de correio](/pt/user-guide/calendar-emails/capabilities/mailbox)
* [Resolver problemas de emails em falta](/pt/user-guide/calendar-emails/how-tos/i-dont-see-emails-on-records)
