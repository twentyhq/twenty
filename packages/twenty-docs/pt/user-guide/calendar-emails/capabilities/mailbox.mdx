...
**Nota**: Para conectar suas contas de email e configurar as configurações de sincronização, visite [Configuração de Email & Calendário](/pt/user-guide/calendar-emails/overview).
...
* **Atualizações**: Novos emails aparecem imediatamente

### O Que Não É Sincronizado

* **E-mails internos**: Por padrão, e-mails entre colegas (mesmo domínio) permanecem privados (configurado em Configurações → Geral → Segurança)
...
Escolha se os e-mails internos são sincronizados no Twenty:

1. Vá para **Configurações → Geral → Segurança**
2. Alterne **Sincronizar e-mails internos** ligado/desligado
...
* **Atrasos na Sincronização**: Emails aparecem imediatamente, mas as importações iniciais levam mais tempo
