...
<Warning>
Os contêineres Docker são para hospedagem em produção ou auto-hospedagem. Para contribuir, consulte a [Configuração local](/pt/developers/contribute/capabilities/local-setup).
</Warning>
...
Consulte [Configurar Variáveis de Ambiente](/pt/developers/self-host/capabilities/setup) para configuração avançada. Todas as variáveis de ambiente devem ser declaradas no arquivo `docker-compose.yml` no nível do servidor e/ou do trabalhador, dependendo da variável.
...
3. **Atualize o `.env`**
   ...
   Consulte o [guia de rotação de chaves](/pt/developers/self-host/capabilities/key-rotation) para obter instruções sobre como fazer a rotação sem tempo de inatividade.
