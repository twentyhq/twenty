...
<Warning>
**Primeira vez instalando?** Siga o [guia de instalação do Docker Compose](/pt/developers/self-host/capabilities/docker-compose) para iniciar o Twenty, depois retorne aqui para configurar.
</Warning>
...
Para rotacionar a `ENCRYPTION_KEY` sem tempo de inatividade, consulte o [guia de rotação de chaves](/pt/developers/self-host/capabilities/key-rotation).
...
<Note>
"Mail.ReadWrite" e "Mail.Send" são obrigatórios apenas se você quiser enviar emails usando nossas ações de fluxo de trabalho. Você pode usar "Mail.Read" em vez disso se quiser apenas receber emails.
</Note>

<Note>
"Calendars.ReadWrite" é obrigatório se você quiser criar eventos de calendário usando nossas ações de fluxo de trabalho. Se você quiser apenas sincronizar eventos de calendário, use "Calendars.Read".
</Note>
...
## API e front-end em hosts separados

Apenas relevante se você não permitir que o back-end sirva o front-end. Implantações de mesma origem
não precisam de nada aqui.

Declare todas as origens de navegador que se comunicam com a API, separadas por vírgula. `SERVER_URL`
e `FRONTEND_URL` já são confiáveis. O servidor registra um aviso quando um navegador
faz uma chamada a partir de uma origem não declarada.
...
<Warning>
O front-end envia credenciais em todas as solicitações, e os navegadores rejeitam uma
resposta com credenciais vinda de uma origem não declarada, portanto o app não carrega até
que você defina isso.
</Warning>

Se os dois hosts estiverem em domínios registráveis diferentes (não apenas em
subdomínios diferentes), também defina `AUTH_COOKIE_SAME_SITE=none`, o que requer HTTPS.

Com o multi-workspace ativado, não liste aqui todos os subdomínios dos workspaces.
Em vez disso, roteie os caminhos da API (`/graphql`, `/metadata`, `/rest`, `/auth/`,
`/client-config`, `/file`) para o back-end em cada hostname de workspace, para que cada
workspace permaneça na mesma origem — é assim que o Twenty Cloud funciona.
