...
* Campos inline definidos aqui **não** precisam de `objectUniversalIdentifier` — ele é herdado do objeto pai. Use [`defineField()`](/pt/developers/extend/apps/data/extending-objects) para adicionar campos a objetos que não pertencem a você.
* Você pode criar novos objetos com `yarn twenty dev:add object`, que orienta você na definição de nomes, campos e relacionamentos. Veja [Arquitetura → Scaffolding de entidades](/pt/developers/extend/apps/getting-started/scaffolding).
...
| Identificadores e relações | `UUID`, `RELATION`, `MORPH_RELATION` (veja [Relações](/pt/developers/extend/apps/data/relations))                                   |
...
* **Conecte este objeto a outros** — veja [Relações](/pt/developers/extend/apps/data/relations) para o padrão de relação bidirecional.
* **Adicione campos a objetos de outros apps** — veja [Extensão de objetos](/pt/developers/extend/apps/data/extending-objects) para `defineField()`.
* **Exibir este objeto na interface do usuário (UI)** — consulte [Itens do menu de navegação](/pt/developers/extend/apps/layout/navigation-menu-items) para adicionar uma entrada na barra lateral; consulte [Views](/pt/developers/extend/apps/layout/views) para adicionar configurações de lista personalizadas.
