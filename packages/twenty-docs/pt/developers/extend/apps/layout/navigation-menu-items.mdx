...
Um **item do menu de navegação** é uma entrada na barra lateral esquerda. Use `defineNavigationMenuItem()` para disponibilizar links personalizados na barra lateral — normalmente um por [visualização](/pt/developers/extend/apps/layout/views) que você disponibiliza — ou para apontar para URLs externas.
