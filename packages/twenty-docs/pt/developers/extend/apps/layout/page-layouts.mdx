...
* Cada `widget` dentro de uma aba pode renderizar um [front component](/pt/developers/extend/apps/layout/front-components), uma lista de relações ou outros tipos de widget nativos.
...
* `widgets` têm escopo apenas para esta aba — eles referenciam [front components](/pt/developers/extend/apps/layout/front-components), visualizações etc., exatamente como widgets definidos inline em `definePageLayout`.
