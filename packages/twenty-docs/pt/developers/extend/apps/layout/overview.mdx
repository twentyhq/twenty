...
<CardGroup cols={2}>
  <Card title="Visualizações" icon="list" href="/pt/developers/extend/apps/layout/views">
    `defineView` — configurações salvas de lista: colunas visíveis, filtros, grupos.
  </Card>
  <Card title="Itens do menu de navegação" icon="bars" href="/pt/developers/extend/apps/layout/navigation-menu-items">
    `defineNavigationMenuItem` — entradas da barra lateral apontando para visualizações ou URLs externas.
  </Card>
  <Card title="Layouts de Página" icon="table-columns" href="/pt/developers/extend/apps/layout/page-layouts">
    `definePageLayout` e `definePageLayoutTab` — abas e widgets na página de detalhes de um registro.
  </Card>
  <Card title="Componentes de front-end" icon="window-maximize" href="/pt/developers/extend/apps/layout/front-components">
    `defineFrontComponent` — componentes React em sandbox que são renderizados dentro do Twenty.
  </Card>
  <Card title="Itens do menu de comandos" icon="terminal" href="/pt/developers/extend/apps/layout/command-menu-items">
...
Eles ainda estão em desenvolvimento ativo: o sandbox implementa um DOM parcial, portanto usos avançados podem falhar. Veja as [limitações atuais](/pt/developers/extend/apps/layout/front-components#current-limitations).
