...
* **Hooks de pré-instalação/pós-instalação/desinstalação** *(opcional)* — consulte [Funções de lógica](/pt/developers/extend/apps/logic/logic-functions).
...
* O papel padrão é detectado automaticamente a partir do arquivo de definição de papel marcado com [`defineApplicationRole()`](/pt/developers/extend/apps/config/roles) — você não precisa referenciá-lo em `defineApplication()`.
...
O papel declarado com [`defineApplicationRole()`](/pt/developers/extend/apps/config/roles) controla o que as funções de lógica e os componentes de front-end do aplicativo podem acessar:

* Os tokens de tempo de execução injetados nas suas funções de lógica são derivados desse papel. Uma chamada que atua como uma pessoa é ainda mais limitada pelo que essa pessoa pode fazer, portanto nunca pode exceder nenhum dos dois. Consulte [Qual acesso uma chamada usa](/pt/developers/extend/apps/logic/logic-functions#whose-access-a-call-uses).
...
Ao criar um novo app com o scaffold, a CLI cria um arquivo de papel inicial em `src/roles/default-role.ts`. Consulte [Papéis e permissões](/pt/developers/extend/apps/config/roles) para a referência completa.

## Metadados do Marketplace

Se você planeja [publicar seu app](/pt/developers/extend/apps/operations/publishing), estes campos opcionais controlam como seu app aparece no marketplace:
