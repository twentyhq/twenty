...
- If a configured server exists but the matching MCP tools are not visible in the current thread, use the `setup-mcp` skill's troubleshooting workflow instead of falling back to a different workspace.
