...
const assertMarketplaceTemplate = (fail) => {
  ...
  if (twentyEntry.source?.source !== 'local') {
    fail(`${templatePath} twenty.source.source must be local`);
  }

  if (twentyEntry.source?.type) {
    fail(`${templatePath} twenty.source.type is deprecated; use source.source`);
  }

  if (twentyEntry.policy?.installation !== 'AVAILABLE') {
    fail(`${templatePath} twenty.policy.installation must be AVAILABLE`);
  }

  if (twentyEntry.policy?.authentication !== 'ON_INSTALL') {
    fail(`${templatePath} twenty.policy.authentication must be ON_INSTALL`);
  }
