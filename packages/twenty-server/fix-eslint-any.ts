...
const report = JSON.parse(fs.readFileSync(reportPath, 'utf8')) as Record<string, { position: string; rule: string }[]>;

// Count total errors
const totalErrors = Object.values(report).reduce((acc, occurrences) => acc + occurrences.length, 0);
  ...
  try {
    ...
    for (const occurrence of sortedOccurrences) {
      ...
      if (lineIndex < 0 || lineIndex >= lines.length) {
        console.log(`Line index out of bounds in ${filePath}: ${lineIndex} (max: ${lines.length - 1})`);
        continue;
      }
      ...
      lines.splice(lineIndex, 0, '// eslint-disable-next-line @typescript-eslint/no-explicit-any');
    ...
    if (processedErrors % 50 === 0 || processedErrors === totalErrors) {
      console.log(`Progress: ${processedErrors}/${totalErrors} errors processed`);
    }
  } catch (error) {
