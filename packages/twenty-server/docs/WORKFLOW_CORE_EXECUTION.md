...
1. Include the parent-link repair and core mutation prerequisites. The core API prerequisite owns the 2.42 version-alias schema and backfill; both APIs and execution use that same alias. Apply that schema command and the 2.42 execution-compatibility schema command before deploying consumers that select the alias. The latter makes the legacy parent nullable and enforces unambiguous version aliases; it does not create a second alias column.
...
3. With that barrier held, run the 2.42 parent-link repair, shared version-alias backfill, workflow execution mapping backfill, and projection-nullability command. Dry-run and resolve reported mapping conflicts, then commit and verify the backfill. The execution backfill requires the shared aliases to be populated, then validates live definitions, pending runs and published pointers transactionally; it does not overwrite or independently populate version aliases (a dry run replays the alias backfill inside its rolled-back transaction). Soft-deleted versions and versions of deleted workflows are not validated. A run core id whose core row no longer exists is re-derived from the aliases; one whose row exists is kept.
