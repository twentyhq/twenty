...
This generates a timestamped file and auto-registers it in `instance-commands.constant.ts` — do not edit that file manually. Nothing is generated when the database already matches the entity metadata.

### Checking for pending schema changes

```bash
npx nx database:migrate:check twenty-server
```

The core schema is built exclusively by legacy TypeORM migrations and instance commands — `synchronize` is off — so an entity change that ships without a matching instance command leaves the database behind the code. This command diffs the entity metadata against the connected database, prints the DDL that is missing, and exits non-zero when there is any. It writes nothing, so it is safe to run against any database the server can read.

CI runs it in the `server-validation` job against a freshly migrated database. When it fails, generate an instance command for the reported changes and commit both the generated file and its `instance-commands.constant.ts` entry — never commit a placeholder generated by the check itself.
