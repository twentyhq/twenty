...
Deploy the initial-company reconciliation listener before running this repair so new assignments are handled while historical gaps are repaired.
...
Existing company targets remain unchanged, including manual attribution and soft-deleted targets representing explicit removals. The repair only inserts missing company targets. It does not run general participant reconciliation or derive new person/opportunity targets. It is a separately invoked command, outside the automatic upgrade sequence.

From the server package, preview a workspace:

```bash
yarn command:prod workspace:repair-initial-company-targets --workspace-id <workspace-id> --dry-run
```
...
```bash
yarn command:prod workspace:repair-initial-company-targets --workspace-id <workspace-id>
```
