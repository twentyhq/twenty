...
type IsOneMinorVersionHigherTestCase = EachTestingContext<{
  from: string;
  to: string;
  expected?: boolean;
}>;
describe('is-one-minor-version-higher', () => {
  const validTestCases: IsOneMinorVersionHigherTestCase[] = [
    {
      context: {
        from: '1.0.0',
        to: '1.1.0',
  ...
  const invalidTestCases: IsOneMinorVersionHigherTestCase[] = [
    {
      context: {
        from: '1.0.0',
        to: '1.2.0',
