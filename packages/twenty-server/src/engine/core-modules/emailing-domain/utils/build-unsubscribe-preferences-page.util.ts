...
const buildTrackingSection = ({ decision }: TrackingPreference): string => {
  ...
  return `<div class="topics"><p class="section-title">Tracking</p><p class="hint">This sender measures whether its emails are opened and which links are clicked. You can turn this off for your address.</p><label class="topic"><input type="radio" name="tracking" value="${MessageTrackingConsentDecision.GRANTED}"${grantedAttribute} />Measure my opens and clicks</label><label class="topic"><input type="radio" name="tracking" value="${MessageTrackingConsentDecision.DENIED}"${deniedAttribute} />Don't measure anything for me</label></div>`;
};
