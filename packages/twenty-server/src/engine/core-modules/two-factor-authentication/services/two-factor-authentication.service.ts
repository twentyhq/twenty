...
export class TwoFactorAuthenticationService {
  ...
  async checkIf2FARequired(
    ...
    const isTwoFactorAuthenticationGloballyEnforced = this.twentyConfigService.get(
      'IS_TWO_FACTOR_AUTHENTICATION_GLOBALLY_ENFORCED'
    )

    const shouldEnforce2FA = isTwoFactorAuthenticationEnabled && (
  ...
  async verifyToken(userId: string, token: string, workspaceId: string): Promise<User> {
    console.log({userId,token,workspaceId})
    ...
    if (!userTwoFactorAuthenticationMethod?.userWorkspace.user || !userTwoFactorAuthenticationMethod.context) {
      throw new AuthException(
        'Malformed Request',
    ...
    await this.twoFactorAuthenticationMethodRepository.save(userTwoFactorAuthenticationMethod);
