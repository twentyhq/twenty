...
export class EmailComposerService {
  ...
  private async findSelectableConnectedAccounts({
    workspaceId,
    userWorkspaceId,
    callerType,
  }: {
    ...
    Pick<ConnectedAccountEntity, 'id' | 'visibility' | 'userWorkspaceId'>[]
  > {
    if (isCallerBypassingAccountVisibility(callerType)) {
      const authContext = buildSystemAuthContext(workspaceId);
    ...
    return this.connectedAccountMetadataService.findUsableByCaller({
      workspaceId,
      userWorkspaceId,
    });
  }
  ...
  private async getDefaultConnectedAccountIdOrThrow({
    ...
    const selectableAccounts = await this.findSelectableConnectedAccounts({
      workspaceId,
      userWorkspaceId,
      callerType,
    });
    ...
    const connectedAccountId = isDefined(userWorkspaceId)
      ? selectConnectedAccountIdForCaller({
          connectedAccounts: selectableAccounts,
          userWorkspaceId,
        })
    ...
    if (!isDefined(connectedAccountId)) {
      throw new EmailToolException(
        isDefined(userWorkspaceId)
          ? `No connected account usable by user workspace '${userWorkspaceId}'`
          : 'No connected account shared with this workspace',
        EmailToolExceptionCode.NO_USABLE_CONNECTED_ACCOUNT,
      );
    }
