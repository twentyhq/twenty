...
export class OAuthService {
  ...
  async exchangeAuthorizationCode(params: {
    ...
    // Atomically consume the single-use code: only the request that flips
    // revokedAt away from null proceeds. This closes the check-then-use race
    // where two concurrent redemptions of the same code both pass the
    // revokedAt check above and each receive a token pair.
