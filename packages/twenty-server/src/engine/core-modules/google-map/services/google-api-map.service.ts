...
export class GoogleApiMapService {
  ...
  public async getAutoCompleteAddress(
    ...
    try {
      let url = `https://maps.googleapis.com/maps/api/place/autocomplete/json?input=${address}&sessiontoken=${token}&key=${this.apiMapKey}`;
  ...
  public async getAddressDetails(
    ...
    try {
      ...
      if (result.data.status === 'OK') {
        return sanitisePlaceDetailsResults(
