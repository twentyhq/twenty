...
describe('WorkspaceDomainsService', () => {
  ...
  describe('resolveWorkspaceAndPublicDomain - isolated public function domain', () => {
    const mockMultiWorkspaceConfig = () => {
      jest
        .spyOn(twentyConfigService, 'get')
        .mockImplementation((key: string) => {
          const env = {
    ...
    it('resolves the default per-workspace function subdomain by workspace subdomain', async () => {
      mockMultiWorkspaceConfig();
      ...
      const result =
        await workspaceDomainsService.resolveWorkspaceAndPublicDomain(
          'https://acme.withtwenty.com',
        );

      expect(result.workspace?.id).toEqual('workspace-id1');
      expect(result.publicDomain).toBeNull();
      expect(result.isIsolatedOrigin).toBe(true);
    });

    it('prefers an explicitly registered (app-scoped) public domain row', async () => {
      mockMultiWorkspaceConfig();

      jest.spyOn(publicDomainRepository, 'findOne').mockResolvedValueOnce({
        domain: 'myapp.withtwenty.com',
        applicationId: 'app-1',
        workspace: { id: 'workspace-id2' },
      } as unknown as PublicDomainEntity);

      const result =
        await workspaceDomainsService.resolveWorkspaceAndPublicDomain(
          'https://myapp.withtwenty.com',
        );

      expect(result.workspace?.id).toEqual('workspace-id2');
      expect(result.publicDomain?.applicationId).toEqual('app-1');
      expect(result.isIsolatedOrigin).toBe(true);
    });

    it('does not flag the main app subdomain as isolated', async () => {
      mockMultiWorkspaceConfig();
  ...
  describe('buildPublicFunctionUrl', () => {
    it('builds the isolated function URL from the workspace subdomain', () => {
      jest
        .spyOn(twentyConfigService, 'get')
        .mockImplementation((key: string) =>
      ...
      const result = workspaceDomainsService.buildPublicFunctionUrl({
        workspace: { subdomain: 'acme' },
        path: '/webhook/stripe',
      });

      expect(result).toBe('https://acme.withtwenty.com/webhook/stripe');
    });

    it('returns undefined when no public domain base is configured', () => {
      jest
        .spyOn(twentyConfigService, 'get')
