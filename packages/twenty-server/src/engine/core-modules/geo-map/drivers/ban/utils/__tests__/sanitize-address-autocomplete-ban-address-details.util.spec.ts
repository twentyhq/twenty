...
describe('sanitizeAddressAutocompleteBanAddressDetails', () => {
  it('should return empty object when no feature is given', () => {
    expect(sanitizeAddressAutocompleteBanAddressDetails(undefined)).toEqual({});
  });

  it('should map a housenumber feature to address fields', () => {
    expect(
      sanitizeAddressAutocompleteBanAddressDetails(HOUSENUMBER_FEATURE),
    ).toEqual({
  ...
  it('should leave street empty for a municipality feature', () => {
    const result =
      sanitizeAddressAutocompleteBanAddressDetails(MUNICIPALITY_FEATURE);
  ...
  it('should omit location and state when the feature lacks them', () => {
    const result = sanitizeAddressAutocompleteBanAddressDetails({
      properties: {
        id: '80021_6590_00008',
        label: '8 Boulevard du Port 80000 Amiens',
        type: 'housenumber',
