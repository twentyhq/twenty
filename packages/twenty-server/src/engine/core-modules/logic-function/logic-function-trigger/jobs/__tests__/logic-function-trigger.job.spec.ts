...
describe('LogicFunctionTriggerJob', () => {
  ...
  it('swallows user errors by default so the job is not retried', async () => {
  ...
  it('throws on user error when shouldRetryOnUserError is set so the queue retries', async () => {
    logicFunctionExecutorService.execute.mockResolvedValue(
      buildExecuteResult({ errorMessage: 'boom' }),
    );
