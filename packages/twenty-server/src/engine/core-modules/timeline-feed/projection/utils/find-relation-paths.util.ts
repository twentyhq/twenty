...
const DEFAULT_MAX_PROJECTION_DEPTH = 3;
...
export type RelationPath = RelationHop[];

// A projection path is a relation path between two object types. Junctions
// (e.g. messageParticipant, noteTarget) are ordinary intermediate objects, so
// a participation hop is just a path through a junction — no special casing.
export const findRelationPaths = ({
  fromObjectNameSingular,
  toObjectNameSingular,
  flatObjectMetadataMaps,
  flatFieldMetadataMaps,
  maxDepth = DEFAULT_MAX_PROJECTION_DEPTH,
}: {
  ...
  const rootObjectId = idByNameSingular[fromObjectNameSingular];

  if (!isDefined(rootObjectId)) {
    return [];
  }

  let frontier = [{ objectId: rootObjectId, path: [] as RelationPath }];
  ...
  for (let depth = 0; depth < maxDepth; depth++) {
    ...
    for (const { objectId, path } of frontier) {
      ...
      for (const field of getFlatFieldsFromFlatObjectMetadata(
        ...
        if (
          relation.targetObjectMetadata.nameSingular === toObjectNameSingular
        ) {
          paths.push(nextPath);
