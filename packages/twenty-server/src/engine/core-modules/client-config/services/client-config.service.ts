...
export class ClientConfigService {
  ...
  // Local is the fallback rather than an error state: browser speech recognition
  // needs no provider, so an instance with none still gets dictation.
  private resolveDictationMode(): DictationMode {
    if (!this.twentyConfigService.get('IS_DICTATION_ENABLED')) {
      return DICTATION_MODES.disabled;
    }

    return this.aiModelRegistryService.hasTranscriptionModel()
      ? DICTATION_MODES.cloud
      : DICTATION_MODES.local;
  }
}
