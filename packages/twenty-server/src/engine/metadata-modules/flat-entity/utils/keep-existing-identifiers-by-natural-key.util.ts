...
type FlatEntityWithUniversalIdentifier = NonNullable<
  FlatEntityMapsByUniversalIdentifier['byUniversalIdentifier'][string]
>;

const computeNaturalKey = ({
  flatEntity,
  naturalKeyProperties,
}: {
...
>({
  ...
  const existingUniversalIdentifierByNaturalKey = new Map(
    Object.values(fromFlatEntityMaps.byUniversalIdentifier)
      .filter(isDefined)
      .map((fromFlatEntity) => [
        computeNaturalKey({ flatEntity: fromFlatEntity, naturalKeyProperties }),
  ...
  for (const toFlatEntity of toFlatEntities) {
    const existingUniversalIdentifier =
      existingUniversalIdentifierByNaturalKey.get(
        computeNaturalKey({ flatEntity: toFlatEntity, naturalKeyProperties }),
      );
