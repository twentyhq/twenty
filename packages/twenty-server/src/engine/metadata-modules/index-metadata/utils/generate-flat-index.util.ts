...
export const generateFlatIndexMetadataWithNameOrThrow = ({
  ...
  const orderedFlatFields = flatIndex.universalFlatIndexFieldMetadatas
    ...
    .map((flatIndexField) => {
      ...
      // Composite parent picked at a specific sub-property -> single
      // sub-column. The deterministic name must reflect the actual
      // physical column so two indexes on (address > city) and
      // (address > postcode) don't collide.
      if (
        ...
        return {
          name: computeCompositeColumnName(
            {
