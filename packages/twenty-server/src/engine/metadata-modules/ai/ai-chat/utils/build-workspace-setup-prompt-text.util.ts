...
3. Build the dashboard: load the dashboard-building skill, create_complete_dashboard with graph widgets, repair anything in widgetErrors. Then introduce roles and propose one or two that match how this team works, each in one line: what it can reach and what it cannot. Call list_roles first, since every workspace already has Admin and Member, and propose roles that add something rather than repeat them.
4. Create the approved roles: no skill covers this, so learn the role tools, then create_role for each, followed by upsert_object_permissions for per-object limits. That call replaces the role's whole override list, so send every override you want to keep, and never grant write without read. Assigning teammates stays with the admin in Settings > Members > Roles. Close with a short recap of what was built.
