  ...
  async executeAgent({
    ...
    try {
      ...
      if (agent) {
        ...
        // Registry tools are exposed lazily through the learn_tools /
        // execute_tool meta-tools instead of inlining every tool's full JSON
        // schema on each step. Only a compact catalog (names + descriptions)
        // goes into the system prompt; the model resolves the schema it needs
        // on demand. Scoped exclusively by the agent permission-tab role. No
        // role means no registry tools.
        if (isDefined(agentRoleId)) {
          ...
          // Deny the meta-tools access to anything outside the shown catalog
          // (other categories, output-navigation tools). This preserves the
          // recursion guard that keeps workflow tools out of agent execution.
