	...
	implements Required<FieldMetadataEntity>
{
	...
	@OneToMany(
		() => ViewFieldEntity,
		(viewField) => viewField.fieldMetadata,
	)
	viewFields: Relation<ViewFieldEntity[]>;

	@OneToMany(
		() => ViewFilterEntity,
		(viewFilter) => viewFilter.fieldMetadata,
	)
	...
	@OneToMany(
		() => ViewEntity,
		(view) => view.calendarEndFieldMetadata,
	)
	calendarEndViews: Relation<ViewEntity[]>;

	@OneToMany(
		() => ViewEntity,
		(view) => view.mainGroupByFieldMetadata,
	)
	mainGroupByFieldMetadataViews: Relation<ViewEntity[]>;

	@OneToMany(
		() => ViewSortEntity,
		(viewSort) => viewSort.fieldMetadata,
	)
