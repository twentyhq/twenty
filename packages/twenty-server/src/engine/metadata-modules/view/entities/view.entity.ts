...
import { ADD_MINE_FILTER_FIELD_METADATA_ID_TO_VIEW_UPGRADE_COMMAND_NAME } from 'src/database/commands/upgrade-version-command/2-43/add-mine-filter-field-metadata-id-to-view-upgrade-command-name.constant';
...
export type ViewOverrides = {
  ...
  mineFilterFieldMetadataId?: SerializedRelation | null;
...
@Index('IDX_VIEW_MINE_FILTER_FIELD_METADATA', ['mineFilterFieldMetadataId'])
  ...
  implements Required<ViewEntity>
{
  ...
  @WasIntroducedInUpgrade({
    upgradeCommandName:
      ADD_MINE_FILTER_FIELD_METADATA_ID_TO_VIEW_UPGRADE_COMMAND_NAME,
  })
  @Column({ nullable: true, type: 'uuid' })
  ...
  @ManyToOne(
    () => FieldMetadataEntity,
    (fieldMetadata) => fieldMetadata.mineFilterViews,
    {
      onDelete: 'SET NULL',
      nullable: true,
    },
  )
  @JoinColumn({ name: 'mineFilterFieldMetadataId' })
  mineFilterFieldMetadata: Relation<FieldMetadataEntity> | null;
