...
} from 'src/engine/metadata-modules/view/exceptions/view-field.exception';
...
export class ViewFieldService {
  ...
  async create(viewFieldData: Partial<ViewField>): Promise<ViewField> {
    ...
    if (!viewFieldData.fieldMetadataId) {
      throw new ViewFieldException(
  ...
  async update(
    ...
    await this.viewFieldRepository.update(id, updateData);

    return this.findById(id, workspaceId);
  }

  async delete(id: string, workspaceId: string): Promise<ViewField | null> {
    const viewField = await this.findById(id, workspaceId);
  ...
  async validateViewField(id: string, workspaceId: string): Promise<ViewField> {
    const viewField = await this.findById(id, workspaceId);
  ...
  async bulkDeleteByFieldMetadataId(
    ...
    await this.viewFieldRepository.softDelete({
      workspaceId,
      fieldMetadataId,
    });
  }
