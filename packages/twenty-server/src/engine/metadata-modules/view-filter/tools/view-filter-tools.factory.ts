...
const CreateViewFilterInputSchema = z.object({
  ...
  fieldMetadataId: z
    ...
    .uuid()
    .describe(
      'ID of the field to filter on. Use get_fields_metadata to find field IDs.',
    ),
  operand: z
...
export class ViewFilterToolsFactory {
  ...
  generateWriteTools(workspaceId: string): ToolSet {
    return {
      create_view_filter: {
        description:
          'Add a filter to a view. Use get_fields_metadata to get fieldMetadataId values.',
      ...
      create_many_view_filters: {
        description:
          'Add multiple filters to a view in one call. Use get_fields_metadata to get fieldMetadataId values.',
