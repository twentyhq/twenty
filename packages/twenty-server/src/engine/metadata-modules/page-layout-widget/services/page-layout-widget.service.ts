...
export class PageLayoutWidgetService {
  ...
  async deleteByTabIds({
    ...
    const widgetsToDelete = Object.values(existingFlatPageLayoutWidgetMaps.byId)
      .filter(isDefined)
      .filter(
        (widget) =>
          tabIds.includes(widget.pageLayoutTabId) &&
          !isDefined(widget.deletedAt),
      )
      .map((widget) => ({
        ...widget,
    ...
    await this.validateAndRunWidgetMigration({
      ...
      operations: {
        flatEntityToCreate: [],
        flatEntityToUpdate: widgetsToDelete,
        flatEntityToDelete: [],
      },
      errorMessage:
        'Multiple validation errors occurred while deleting page layout widgets by tab ids',
    });
  }

  async restoreByTabIds({
    tabIds,
    workspaceId,
  }: {
    tabIds: string[];
    ...
    const widgetsToRestore = Object.values(
      existingFlatPageLayoutWidgetMaps.byId,
    )
      .filter(isDefined)
      .filter(
        (widget) =>
    ...
    await this.validateAndRunWidgetMigration({
      ...
      operations: {
        flatEntityToCreate: [],
        flatEntityToUpdate: widgetsToRestore,
        flatEntityToDelete: [],
      },
      errorMessage:
        'Multiple validation errors occurred while restoring page layout widgets by tab ids',
    });
  }
}
