...
describe('ObjectNavigationCommandOnCreateSideEffectHandlerService', () => {
  ...
  it('still provisions when a caller-authored row targets the object, so the engine records the collision', () => {
    const result = expectSuccess(
      handler.buildSideEffects(
        buildArgs({
          pendingFlatCommandMenuItems: [
  ...
  it('still provisions when a caller-authored row squats the derived identifier', () => {
    const result = expectSuccess(
      handler.buildSideEffects(
        buildArgs({
          pendingFlatCommandMenuItems: [
    ...
    const [navigationCommand] = Object.values(
      ...
      { universalIdentifier: string; isSystemSideEffect: boolean },
    ];
  ...
  it('still provisions when a synced caller-authored row sits in the object aggregator', () => {
    const result = expectSuccess(
      handler.buildSideEffects(
        buildArgs({
          flatObjectMetadata: buildFlatObjectMetadata({
            commandMenuItemUniversalIdentifiers: ['caller-authored-identifier'],
          }),
          syncedFlatCommandMenuItems: [
            {
              universalIdentifier: 'caller-authored-identifier',
              position: 3,
              engineComponentKey: EngineComponentKey.NAVIGATION,
              payload: { objectMetadataItemId: OBJECT_ID },
              isSystemSideEffect: false,
  ...
  it('does not treat commands outside the object aggregator as duplicates', () => {
    const result = expectSuccess(
      handler.buildSideEffects(
        buildArgs({
          syncedFlatCommandMenuItems: [
