...
@Injectable()
export class SharingRuleLastBackfillOnDeleteSideEffectHandlerService extends MetadataSideEffectHandler(
  {
    operation: 'delete',
    metadataName: 'sharingRule',
    name: 'sharingRuleLastBackfillOnDelete',
    description:
      'Deleting the last active sharing rule without criteria granting EVERYONE or a ROLE on a PRIVATE object would hide every record from everyone, so the deletion is refused unless the same migration keeps another backfill rule or moves the object off PRIVATE. Noop on objects that had no backfill rule, which are already locked out.',
  },
) {
  buildSideEffects({
      ...
      errors: [
        {
          ...
          message: t`Cannot delete the last sharing rule keeping ${objectLabel} readable while the object is private`,
