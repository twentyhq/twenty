...
describe('ExternalEntityLink standard metadata build', () => {
  ...
  it('marks the externalEntityLink object as system', () => {
    const externalEntityLink =
      allFlatEntityMaps.flatObjectMetadataMaps.byUniversalIdentifier[
        STANDARD_OBJECTS.externalEntityLink.universalIdentifier
      ];

    expect(externalEntityLink?.isSystem).toBe(true);
  });
  ...
  it('builds all expected fields for externalEntityLink', () => {
    ...
    const fieldIdentifiers = Object.values(
      STANDARD_OBJECTS.externalEntityLink.fields,
    )
      .filter(isDefined)
  ...
  it('builds all expected indexes for externalEntityLink', () => {
    ...
    const indexIdentifiers = Object.values(
      STANDARD_OBJECTS.externalEntityLink.indexes,
    )
      .filter(isDefined)
