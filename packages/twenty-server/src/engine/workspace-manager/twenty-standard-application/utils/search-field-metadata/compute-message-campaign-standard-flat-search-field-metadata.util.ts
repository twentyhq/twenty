...
export const buildMessageCampaignStandardFlatSearchFieldMetadatas = ({
  now,
  objectName,
  workspaceId,
  ...
  CreateStandardSearchFieldArgs<'messageCampaign'>,
  'context'
>): FlatSearchFieldMetadata[] =>
  SEARCH_FIELDS_FOR_MESSAGE_CAMPAIGN.map((searchField, position) =>
    createStandardSearchFieldFlatMetadata({
      objectName,
      workspaceId,
      context: {
        fieldName:
          searchField.name as AllStandardObjectFieldName<'messageCampaign'>,
        position,
      },
      standardObjectMetadataRelatedEntityIds,
      dependencyFlatEntityMaps,
