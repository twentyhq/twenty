...
import { WorkspaceSchemaMigrationException, WorkspaceSchemaMigrationExceptionCode } from '../exceptions/workspace-schema-migration.exception';
...
export const collectEnumOperationsForField = ({
  ...
  if (isCompositeFieldMetadataType(fieldMetadata.type)) {
    ...
    for (const property of compositeType.properties) {
      if (property.type === FieldMetadataType.RELATION || !isEnumFieldMetadataType(property.type)) {
        continue;
      }

      const columnName = computeCompositeColumnName(fieldMetadata.name, property);
      ...
      if (operation === 'create') {
        ...
        const newColumnName = computeCompositeColumnName(options.newFieldName, property);
        const newEnumName = computePostgresEnumName({ tableName, columnName: newColumnName });
    ...
    if (operation === 'create') {
      const enumName = computePostgresEnumName({ tableName, columnName: fieldMetadata.name });
      enumOperations.push({
        enumName,
      ...
      const enumName = computePostgresEnumName({ tableName, columnName: fieldMetadata.name });
      const newEnumName = computePostgresEnumName({ tableName, columnName: options.newFieldName });
...
export const collectEnumOperationsForObject = ({
  ...
  const enumFields = Object.values(objectMetadata.fieldsById)
    .filter(isDefined)
    .filter((field: any) => isEnumFieldMetadataType(field.type));

  for (const enumField of enumFields) {
    const field = enumField as any;

    if (operation === 'drop') {
      const enumName = computePostgresEnumName({ tableName, columnName: field.name });

      enumOperations.push({ enumName });
    } else if (operation === 'rename' && options?.newTableName) {
      const fromEnumName = computePostgresEnumName({ tableName, columnName: field.name });
      const toEnumName = computePostgresEnumName({ tableName: options.newTableName, columnName: field.name });
