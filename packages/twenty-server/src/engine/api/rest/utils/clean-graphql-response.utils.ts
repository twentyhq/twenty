...
export const cleanGraphQLResponse = (input: any) => {
  ...
  }; // Initialize the output with a data key at the top level
    ...
    Object.keys(obj).forEach((key) => {
      if (isObject(obj[key])) {
        if (obj[key].edges) {
          ...
          cleanedObj[key] = obj[key].edges.map((edge: { node: unknown }) =>
  ...
  Object.keys(input).forEach((key) => {
    if (isObject(input[key]) && input[key].edges) {
      ...
      (output.data as Record<string, unknown>)[key] = input[key].edges.map(
        (edge: { node: unknown }) => cleanObject(edge.node),
      );
      ...
      (output.data as Record<string, unknown>)[key] = cleanObject(input[key]);
    } else if (Array.isArray(input[key])) {
      const itemsWithEdges = input[key].filter((item) => item.edges);
      const cleanedObjArray = itemsWithEdges.map(({ edges, ...item }) => {
        return {
          ...item,
          [key]: edges.map((edge: { node: unknown }) => cleanObject(edge.node)),
        };
      });
      ...
      (output.data as Record<string, unknown>)[key] = input[key];
    }
  });
