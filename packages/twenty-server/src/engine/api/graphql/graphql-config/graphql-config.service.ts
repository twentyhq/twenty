  ...
  implements GqlOptionsFactory<YogaDriverConfig<'express'>>
{
  ...
  async createSchema(
    ...
    // Resolve the GraphQLSchemaFactory for the contextId
    const graphQLSchemaFactory = await this.moduleRef.resolve(
    ...
    return await graphQLSchemaFactory.create(data.workspace.id, data.user?.id);
  }
}
