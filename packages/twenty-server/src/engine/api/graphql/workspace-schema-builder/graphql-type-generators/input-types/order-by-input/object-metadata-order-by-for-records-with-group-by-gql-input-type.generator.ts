...
@Injectable()
export class ObjectMetadataOrderByForRecordsWithGroupByGqlInputTypeGenerator {
  private readonly logger = new Logger(
    ObjectMetadataOrderByForRecordsWithGroupByGqlInputTypeGenerator.name,
  );
  constructor(
  ...
  public buildAndStore({
    ...
    const inputType = new GraphQLInputObjectType({
      name: `${pascalCase(objectMetadata.nameSingular)}${GqlInputTypeDefinitionKind.OrderByForRecordsWithGroupBy.toString()}Input`,
    ...
    const key = computeObjectMetadataInputTypeKey(
      ...
      GqlInputTypeDefinitionKind.OrderByForRecordsWithGroupBy,
    );
