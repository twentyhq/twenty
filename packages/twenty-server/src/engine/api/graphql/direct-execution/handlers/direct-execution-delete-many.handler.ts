...
import { assertDeleteManyArgs } from 'src/engine/api/common/guards/assert-delete-many-args.guard';
...
@Injectable()
export class DirectExecutionDeleteManyHandler extends DirectExecutionBaseHandler {
  constructor(
  ...
  async handle(
    ...
    assertDeleteManyArgs(args);

    const records = await this.commonDeleteManyService.execute(args, context);
