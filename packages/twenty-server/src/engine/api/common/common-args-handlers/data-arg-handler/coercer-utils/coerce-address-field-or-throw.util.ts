...
export const coerceAddressFieldOrThrow = (
  value: unknown,
  fieldName?: string,
) => {
  ...
  try {
      ...
      switch (subField) {
        ...
        default:
          throw new CommonDataCoercerException(
            ...
            CommonDataCoercerExceptionCode.INVALID_ADDRESS,
          );
      }
    }
    ...
    throw new CommonDataCoercerException(
      `Invalid value ${inspect(value)} for address field "${fieldName}" - ${error.message}`,
      CommonDataCoercerExceptionCode.INVALID_ADDRESS,
    );
  }
};
