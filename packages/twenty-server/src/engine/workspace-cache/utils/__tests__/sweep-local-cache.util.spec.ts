...
const ORM_PREFIX = 'orm:entity-metadatas';

const entry = (lastReadAt: number): WorkspaceLocalCacheEntry<unknown> => ({
  versions: new Map([['h', { data: {}, lastReadAt }]]),
  latestHash: 'h',
...
describe('sweepLocalCache', () => {
  it('drops entries idle past the TTL and keeps recent ones', () => {
    const cache = new Map([
      [`${ORM_PREFIX}:ws-idle`, entry(0)],
      [`${ORM_PREFIX}:ws-recent`, entry(TTL_MS)],
    ]);

    const evicted = sweepLocalCache(cache, TTL_MS + 1, config());

    expect(evicted).toBe(1);
    expect(cache.has(`${ORM_PREFIX}:ws-idle`)).toBe(false);
    expect(cache.has(`${ORM_PREFIX}:ws-recent`)).toBe(true);
  });

  it('trims a provider over its cap, evicting the least-recently-read', () => {
    const cache = new Map([
      [`${ORM_PREFIX}:ws-a`, entry(1)],
      [`${ORM_PREFIX}:ws-b`, entry(2)],
      [`${ORM_PREFIX}:ws-c`, entry(3)],
    ]);

    const evicted = sweepLocalCache(
      cache,
      100,
      config({ maxEntriesByPrefix: new Map([[ORM_PREFIX, 2]]) }),
    );

    expect(evicted).toBe(1);
    expect(cache.size).toBe(2);
    expect(cache.has(`${ORM_PREFIX}:ws-a`)).toBe(false); // oldest
    expect(cache.has(`${ORM_PREFIX}:ws-c`)).toBe(true); // newest
  });
  ...
  it('expires first so the per-provider cap only counts survivors', () => {
    const cache = new Map([
      [`${ORM_PREFIX}:ws-idle`, entry(0)], // expired by TTL
      [`${ORM_PREFIX}:ws-a`, entry(TTL_MS + 1)],
      [`${ORM_PREFIX}:ws-b`, entry(TTL_MS + 2)],
    ]);
