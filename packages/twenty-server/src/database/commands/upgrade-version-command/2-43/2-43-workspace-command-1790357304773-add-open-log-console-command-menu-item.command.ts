...
@RegisteredWorkspaceCommand('2.43.0', 1790357304773)
@Command({
  name: 'upgrade:2-43:add-open-log-console-command-menu-item',
  description:
    'Add the Open Logs Console command menu item to workspaces that predate it',
})
export class AddOpenLogConsoleCommandMenuItemCommand extends ProvisionedWorkspaceCommandRunner {
  constructor(
  ...
  override async runOnWorkspace({
    ...
    const commandMenuItemsToCreate = buildMissingStandardCommandMenuItemsToCreate(
      {
        commandMenuItemNames: ['openLogConsole'],
    ...
    if (options.dryRun) {
      this.logger.log(
        `Would add the Open Logs Console command menu item for workspace ${workspaceId}`,
      );
