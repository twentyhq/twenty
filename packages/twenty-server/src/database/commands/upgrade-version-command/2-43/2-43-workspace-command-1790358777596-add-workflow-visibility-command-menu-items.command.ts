...
@RegisteredWorkspaceCommand('2.43.0', 1790358777596)
@Command({
  name: 'upgrade:2-43:add-workflow-visibility-command-menu-items',
  description:
    'Add the Make Private and Share with Workspace workflow commands to workspaces that predate them',
})
export class AddWorkflowVisibilityCommandMenuItemsCommand extends ProvisionedWorkspaceCommandRunner {
  ...
  override async runOnWorkspace({
    ...
    if (options.dryRun) {
      this.logger.log(
        `Would add ${commandMenuItemsToCreate.length} workflow visibility command(s) for workspace ${workspaceId}`,
      );
