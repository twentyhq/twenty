...
const WORKSPACE_SCOPED_HANDLE_INDEX_UNIVERSAL_IDENTIFIER =
  BLOCKLIST.indexes.workspaceScopedHandleUniqueIndex.universalIdentifier;

@RegisteredWorkspaceCommand('2.33.0', 1787203841000)
@Command({
  name: 'upgrade:2-33:add-blocklist-scope-field',
  description:
    'Add the blocklist.scope field and its workspace-scoped unique handle index, make blocklist.workspaceMember nullable so a handle can be blocked workspace-wide, and switch that relation from SET_NULL to CASCADE so a destroyed workspace member takes their entries with them',
})
export class AddBlocklistScopeFieldCommand extends ProvisionedWorkspaceCommandRunner {
  ...
  override async runOnWorkspace({
    ...
    const isWorkspaceScopedHandleIndexMissing = !isDefined(
      findFlatEntityByUniversalIdentifier<FlatIndexMetadata>({
        flatEntityMaps: flatIndexMaps,
        universalIdentifier: WORKSPACE_SCOPED_HANDLE_INDEX_UNIVERSAL_IDENTIFIER,
      }),
    );
    ...
    if (
      !isScopeFieldMissing &&
      !isWorkspaceScopedHandleIndexMissing &&
      !isWorkspaceMemberRelationOutdated
    ) {
      return;
    }
    ...
    const standardWorkspaceScopedHandleIndex =
      findFlatEntityByUniversalIdentifier<FlatIndexMetadata>({
        flatEntityMaps: standardAllFlatEntityMaps.flatIndexMaps,
        universalIdentifier: WORKSPACE_SCOPED_HANDLE_INDEX_UNIVERSAL_IDENTIFIER,
      });

    if (!isDefined(standardWorkspaceScopedHandleIndex)) {
      throw new Error(
        'Standard application is missing blocklist index workspaceScopedHandleUniqueIndex',
      );
    }

    if (isDryRun) {
      this.logger.log(
        `[DRY RUN] Would apply the blocklist workspace-scope migration for workspace ${workspaceId}: scope field ${isScopeFieldMissing ? 'missing' : 'present'}, unique index ${isWorkspaceScopedHandleIndexMissing ? 'missing' : 'present'}, workspaceMember relation ${isWorkspaceMemberRelationOutdated ? 'outdated' : 'up to date'}`,
      );
    ...
    if (isWorkspaceScopedHandleIndexMissing) {
      flatIndexMetadataToCreate.push(standardWorkspaceScopedHandleIndex);
    }
