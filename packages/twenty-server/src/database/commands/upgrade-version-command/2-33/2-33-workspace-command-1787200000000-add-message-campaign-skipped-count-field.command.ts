...
const SKIPPED_COUNT_FIELD_UNIVERSAL_IDENTIFIER =
  MESSAGE_CAMPAIGN.fields.skippedCount.universalIdentifier;

@RegisteredWorkspaceCommand('2.33.0', 1787200000000)
@Command({
  name: 'upgrade:2-33:add-message-campaign-skipped-count-field',
  description:
    'Add the messageCampaign.skippedCount field on existing workspaces that predate it',
})
export class AddMessageCampaignSkippedCountFieldCommand extends ProvisionedWorkspaceCommandRunner {
  constructor(
  ...
  override async runOnWorkspace({
    ...
    if (
      isDefined(
        findFlatEntityByUniversalIdentifier<FlatFieldMetadata>({
          ...
          universalIdentifier: SKIPPED_COUNT_FIELD_UNIVERSAL_IDENTIFIER,
        }),
      )
    ) {
      return;
    ...
    if (result.status === 'fail') {
      this.logger.error(
        `Failed to add skippedCount field:\n${JSON.stringify(result, null, 2)}`,
      );

      throw new Error(
        `Failed to add skippedCount field for workspace ${workspaceId}`,
      );
    }

    this.logger.log(`Added skippedCount field for workspace ${workspaceId}`);
  }
}
