...
export class AddMessageCampaignStatFieldsCommand extends ActiveOrSuspendedWorkspaceCommandRunner {
  ...
  override async runOnWorkspace({
    ...
    const hasMetadataChanges =
      fieldsToCreate.length > 0 || viewFieldsToCreate.length > 0;

    if (isDryRun) {
      this.logger.log(
        `[DRY RUN] Workspace ${workspaceId}: ${fieldsToCreate.length} field(s), ${viewFieldsToCreate.length} view column(s), then backfill campaign counts`,
      );
