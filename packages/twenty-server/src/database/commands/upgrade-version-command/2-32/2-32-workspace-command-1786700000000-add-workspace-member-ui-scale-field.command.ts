...
const UI_SCALE_FIELD_UNIVERSAL_IDENTIFIER =
  WORKSPACE_MEMBER.fields.uiScale.universalIdentifier;

@RegisteredWorkspaceCommand('2.32.0', 1786700000000)
@Command({
  name: 'upgrade:2-32:add-workspace-member-ui-scale-field',
  description:
    'Add the workspaceMember.uiScale system field on existing workspaces that predate it',
})
export class AddWorkspaceMemberUiScaleFieldCommand extends ProvisionedWorkspaceCommandRunner {
  constructor(
  ...
  override async runOnWorkspace({
    ...
    if (
      isDefined(
        flatFieldMetadataMaps.byUniversalIdentifier[
          UI_SCALE_FIELD_UNIVERSAL_IDENTIFIER
        ],
      )
    ) {
      return;
    ...
    if (!isDefined(standardField)) {
      throw new Error(
        'Standard application is missing workspaceMember field uiScale',
      );
    }

    if (isDryRun) {
      this.logger.log(
        `[DRY RUN] Would add uiScale field for workspace ${workspaceId}`,
      );
    ...
    if (result.status === 'fail') {
      this.logger.error(
        `Failed to add uiScale field:\n${JSON.stringify(result, null, 2)}`,
      );

      throw new Error(
        `Failed to add uiScale field for workspace ${workspaceId}`,
      );
    }
