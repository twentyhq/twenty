...
@Command({
  ...
  description:
    'Seed one regular table view per object alongside its engine-owned INDEX view. The seeded view copies the INDEX view field layout once and is written with isSystemSideEffect: false under the workspace-custom application, so the user owns it and no application sync reaps it. It carries ViewKey.DEFAULT, the transitional marker the client uses to know an object has been split and INDEX can be hidden. Idempotent per entity on deterministic identifiers: the seeded view and each copied view field are gated independently, so a retry after a partial failure creates only what is missing. Shares SeedObjectDefaultViewService with the provisioning path, so a workspace created after this release is seeded without the backfill running again.',
})
