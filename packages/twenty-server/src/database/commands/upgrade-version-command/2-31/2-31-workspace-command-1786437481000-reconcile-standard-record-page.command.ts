...
import { collectRecordPageStackFlatEntities } from 'src/database/commands/upgrade-version-command/2-31/utils/collect-record-page-stack-flat-entities.util';
...
@RegisteredWorkspaceCommand('2.31.0', 1786437481000)
@Command({
    ...
    for (const flatPageLayout of Object.values(
      ...
      if (
        ...
        if (
          ...
          this.logger.warn(
            `Standard-owned record-page layout ${flatPageLayout.id} on standard object ${flatObjectMetadata.universalIdentifier} in workspace ${workspaceId} matches neither the pre-2.31 literal nor the derived identifier, skipping`,
          );
        }
        continue;
      }
