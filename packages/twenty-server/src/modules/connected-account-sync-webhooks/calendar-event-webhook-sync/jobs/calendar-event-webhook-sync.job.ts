...
import { CALENDAR_EVENT_WEBHOOK_SYNC_MAX_RETRY_DELAY_MS } from 'src/modules/connected-account-sync-webhooks/calendar-event-webhook-sync/constants/calendar-event-webhook-sync-max-retry-delay-ms.constant';
...
export class CalendarEventWebhookSyncJob {
  ...
  async handle(data: CalendarEventWebhookSyncJobData): Promise<void> {
    const [latestWebhookEventId] = await this.cacheStorage
      .mget<string>([getCalendarEventWebhookSyncDebounceCacheKey(data)])
      .catch(() => [undefined]);
    ...
    if (
      !shouldRetry ||
      (data.retryAttempt ?? 0) >= CALENDAR_EVENT_WEBHOOK_SYNC_RETRY_LIMIT
    ) {
      return;
    }
