...
// §8.3 stage gate — institutionalOpportunity.updateOne. Blocks a forward stage move unless the
// current stage's task is DONE. Logic in StageGateService; config auto-derived.
@WorkspaceQueryHook(`institutionalOpportunity.updateOne`)
export class InstitutionalOpportunityStageGatePreQueryHook implements WorkspacePreQueryHookInstance {
  ...
  async execute(
    ...
    const { stageField, cfg } = STAGE_GATE_CONFIGS['institutionalOpportunity'];
