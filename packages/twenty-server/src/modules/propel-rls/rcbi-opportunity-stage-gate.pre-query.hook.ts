...
// §8.3 stage gate — rcbiOpportunity.updateOne. Blocks a forward stage move unless the
// current stage's task is DONE. Logic in StageGateService; config auto-derived.
@WorkspaceQueryHook(`rcbiOpportunity.updateOne`)
export class RcbiOpportunityStageGatePreQueryHook implements WorkspacePreQueryHookInstance {
  ...
  async execute(
    ...
    const { stageField, cfg } = STAGE_GATE_CONFIGS['rcbiOpportunity'];
