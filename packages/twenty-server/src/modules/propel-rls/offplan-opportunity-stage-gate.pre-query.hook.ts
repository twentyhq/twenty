...
// §8.3 stage gate — offPlanOpportunity.updateOne. Blocks a forward stage move unless the
// current stage's task is DONE. Logic in StageGateService; config auto-derived.
@WorkspaceQueryHook(`offPlanOpportunity.updateOne`)
export class OffPlanOpportunityStageGatePreQueryHook implements WorkspacePreQueryHookInstance {
  ...
  async execute(
    ...
    const { stageField, cfg } = STAGE_GATE_CONFIGS['offPlanOpportunity'];
