...
// §8.3 stage gate — secondaryOpportunity.updateOne. Blocks a forward stage move unless the
// current stage's task is DONE. Logic in StageGateService; config auto-derived.
@WorkspaceQueryHook(`secondaryOpportunity.updateOne`)
export class SecondaryOpportunityStageGatePreQueryHook implements WorkspacePreQueryHookInstance {
  ...
  async execute(
    ...
    const { stageField, cfg } = STAGE_GATE_CONFIGS['secondaryOpportunity'];
