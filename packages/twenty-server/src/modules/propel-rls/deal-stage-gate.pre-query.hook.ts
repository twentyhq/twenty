...
// §8.3 stage gate — deal.updateOne. Blocks a forward stage move unless the
// current stage's task is DONE. Logic in StageGateService; config auto-derived.
@WorkspaceQueryHook(`deal.updateOne`)
export class DealStageGatePreQueryHook implements WorkspacePreQueryHookInstance {
  ...
  async execute(
    ...
    const { stageField, cfg } = STAGE_GATE_CONFIGS['deal'];
