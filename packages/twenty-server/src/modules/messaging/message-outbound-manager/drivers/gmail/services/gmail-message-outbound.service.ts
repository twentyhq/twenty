...
export class GmailMessageOutboundService implements MessageOutboundDriver {
  ...
  async sendDraft(
    ...
    await this.deleteDraftByMessageId(connectedAccount, draftExternalId).catch(
      (error) => {
        this.logger.warn(
          `Failed to delete sent Gmail draft (sync will recover): ${error}`,
        );
      },
    );
  ...
  private async deleteDraftByMessageId(
    ...
    if (!isDefined(draftId)) {
      this.logger.warn(
        `Could not find Gmail draft with message id ${messageId} to delete after send`,
      );
