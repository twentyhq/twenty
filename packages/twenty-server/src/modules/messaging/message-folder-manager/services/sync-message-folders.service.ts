  ...
  private async syncFolderChanges(
    ...
    for (const discoveredFolder of discoveredFolders) {
      ...
      if (!deepEqual(newData, currentData)) {
        foldersToUpdate.set(existingFolder.id, newData);
      }
    }
      ...
      async () => {
        ...
        return workspaceDataSource.transaction(
          async (transactionManager: WorkspaceEntityManager) => {
            ...
            const remainingFolders = existingFolders
              .filter(
                (existingFolder) =>
                  !folderIdsToDelete.includes(existingFolder.id),
              )
              .map((existingFolder) => {
                const update = foldersToUpdate.get(existingFolder.id);

                if (update) {
                  return { ...existingFolder, ...update };
                }

                return existingFolder;
              });

            return [...remainingFolders, ...createdFolders];
          },
        );
      },
    );
