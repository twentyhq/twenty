...
export class WorkflowValidationWorkspaceService {
  ...
  async assertWorkflowVersionStructureIsValidOrThrow({
    ...
    const structuralErrors = validateWorkflowStepParams({
      trigger,
      steps,
    }).filter((issue) => issue.severity === 'error');

    const richTextErrors = (
      await this.validateWorkspaceMetadata({ workspaceId, steps: steps ?? [] })
    ).filter((issue) => issue.code === 'INVALID_RICH_TEXT_FIELD');

    const blockingErrors = [...structuralErrors, ...richTextErrors];
