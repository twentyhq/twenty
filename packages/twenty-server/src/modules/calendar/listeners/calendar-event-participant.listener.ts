...
export class CalendarEventParticipantListener {
  ...
  public async handleCalendarEventParticipantCreatedEvent(payload: {
    workspaceId: string;
    userId: string;
    calendarEventParticipants: ObjectRecord<CalendarEventParticipantWorkspaceEntity>[];
  }): Promise<void> {
    const calendarEventParticipant = payload.calendarEventParticipants;
    ...
    const calendarEventParticipantWithPersonId =
      calendarEventParticipant.filter((participant) => participant.personId);

    await this.timelineActivityRepository.insertTimelineActivitiesForObject(
      'person',
      calendarEventParticipantWithPersonId.map((participant) => ({
        dataSourceSchema,
        name: participant.isOrganizer
          ? 'calendarEvent.sent'
          : 'calendarEvent.received',
        properties: null,
