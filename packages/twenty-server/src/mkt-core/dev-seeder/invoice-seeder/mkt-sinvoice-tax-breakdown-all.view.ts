...
export const mktSInvoiceTaxBreakdownsAllView = (
  objectMetadataItems: ObjectMetadataEntity[],
) => {
  const itemObjectMetadata = objectMetadataItems.find(
    (object) => object.standardId === MKT_OBJECT_IDS.mktSInvoiceTaxBreakdown,
  );

  if (!itemObjectMetadata) {
    throw new Error('SInvoice Tax Breakdown object metadata not found');
  }

  return {
    name: 'All SInvoice Tax Breakdowns',
