...
export const mktSInvoiceItemsAllView = (
  objectMetadataItems: ObjectMetadataEntity[],
) => {
  const itemObjectMetadata = objectMetadataItems.find(
    (object) => object.standardId === MKT_OBJECT_IDS.mktSInvoiceItem,
  );

  if (!itemObjectMetadata) {
    throw new Error('SInvoice Item object metadata not found');
  }

  return {
    name: 'All SInvoice Items',
    ...
    fields: [
      {
        fieldMetadataId:
          itemObjectMetadata.fields.find(
            (field) => field.standardId === MKT_SINVOICE_ITEM_FIELD_IDS.name,
          )?.id ?? '',
        position: 0,
        isVisible: true,
        size: 180,
