...
export class SInvoiceIntegrationService {
  ...
  public filterSInvoiceToPayload(sInvoice: MktSInvoiceWorkspaceEntity): sInvoicePayload {
    // handle payments
    ...
    const itemInfo = sInvoice.mktSInvoiceItems?.map(item => ({
    ...
    const taxBreakdowns = sInvoice.mktSInvoiceTaxBreakdowns?.map(tax => ({
    ...
    const metadata = sInvoice.mktSInvoiceMetadata?.map(meta => ({
      keyTag: meta.keyTag || '',
    ...
    return {
      generalInvoiceInfo: {
        ...
        exchangeRate: sInvoice.exchangeRate ? parseFloat(sInvoice.exchangeRate) : 1,
      ...
      taxBreakdowns,
      summarizeInfo: {
        sumOfTotalLineAmountWithoutTax: sInvoice.sumOfTotalLineAmountWithoutTax || 0,
  ...
  async syncSInvoice(orderId: string) {
    this.logger.log(`[S-INVOICE SERVICE] Starting syncSInvoice for order: ${orderId}`);
    ...
    try {
      ...
      const sInvoice = await sInvoiceRepository.findOne(
        ...
        relations: ['mktSInvoicePayments', 'mktSInvoiceItems', 'mktSInvoiceTaxBreakdowns', 'mktSInvoiceMetadata'],
      });

      if (!sInvoice) {
        this.logger.warn(`[S-INVOICE SERVICE] S-Invoice not found for order ${orderId} when sync S-Invoice`);
        return {} as sInvoiceType;
      }
      ...
      this.logger.log(`[S-INVOICE SERVICE] Completed syncSInvoice for order: ${orderId}`);
    } catch (error) {
      this.logger.error(`[S-INVOICE SERVICE] Failed to sync S-Invoice for order ${orderId}: ${error}`);
  ...
  async sendSInvoice(payload: sInvoicePayload, orderId: string) {
    ...
    try {
      ...
      this.logger.log(`[S-INVOICE] Response: ${JSON.stringify(sInvoiceUpdate)}`);
   ...
   this.logger.log(`[S-INVOICE SERVICE] Starting saveSInvoice for ID: ${sInvoiceId}`);
   ...
   try {
     ...
     if (!existingSInvoice) {
       this.logger.warn(`[S-INVOICE SERVICE] S-Invoice with ID ${sInvoiceId} not found`);
       return;
     }
     ...
     await sInvoiceRepository.update(sInvoiceId, updateData as MktSInvoiceWorkspaceEntity);
     ...
     if (sInvoiceUpdate.orderSInvoiceStatus && existingSInvoice.mktOrderId) {
       ...
       await orderRepository.update(existingSInvoice.mktOrderId, {
         sInvoiceStatus: sInvoiceUpdate.orderSInvoiceStatus as ORDER_SINVOICE_STATUS
       });
       this.logger.log(`[S-INVOICE SERVICE] Updated order sInvoiceStatus to ${sInvoiceUpdate.orderSInvoiceStatus} for order ID: ${existingSInvoice.mktOrderId}`);
     }

     this.logger.log(`[S-INVOICE SERVICE] Successfully saved S-Invoice with ID: ${sInvoiceId}`);
     this.logger.log(`[S-INVOICE SERVICE] Updated data: ${JSON.stringify(updateData)}`);

   } catch (error) {
     this.logger.error(`[S-INVOICE SERVICE] Failed to save S-Invoice with ID ${sInvoiceId}: ${error}`);
     this.logger.error(`[S-INVOICE SERVICE] Error details: ${error.message}`, error.stack);
     throw error;
   }
 }
