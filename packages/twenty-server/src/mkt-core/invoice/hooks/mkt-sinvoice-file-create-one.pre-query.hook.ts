...
import { SINVOICE_FILE_STATUS,SINVOICE_FILE_TYPE } from 'src/mkt-core/invoice/invoice.constants';
...
{
  private readonly logger = new Logger(MktSInvoiceFileCreateOnePreQueryHook.name);
  ...
  async execute(
    ...
    this.logger.log(`Creating SInvoiceFile with input: ${JSON.stringify(input)}`);

    if (!workspaceId || !input?.mktSInvoiceId) {
      this.logger.warn('Missing workspaceId or mktSInvoiceId, skipping API call');
      return payload;
    }
    ...
    try {
      ...
      if (!sInvoice.supplierTaxCode || !sInvoice.invoiceNo || !sInvoice.templateCode) {
        this.logger.warn(`SInvoice missing required fields for API call: ${input.mktSInvoiceId}`);
        return payload;
      }
      ...
      const updatedInput = {
        ...
        fileName: `${input.fileName}.${input.fileType?.toLowerCase()}` || `invoice-${sInvoice.invoiceNo}.${SINVOICE_FILE_TYPE.PDF.toLowerCase()}`,
      ...
      this.logger.log(`[S-INVOICE FILE HOOK] Prepared file data for SInvoice: ${input.mktSInvoiceId}`);
      ...
      this.logger.error(`[S-INVOICE FILE HOOK] Failed to prepare file data for SInvoice: ${input.mktSInvoiceId}`, error);
