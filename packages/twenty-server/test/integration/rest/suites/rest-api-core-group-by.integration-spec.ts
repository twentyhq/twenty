...
describe('REST API Core Group By endpoint', () => {
  ...
  it('groups by stage and closeDate with records', async () => {
    ...
    const response = await makeRestAPIRequest({
      method: 'get',
      path: `/opportunities/groupBy?group_by=${encodeURIComponent(groupByQuery)}&aggregate=${encodeURIComponent(AGGREGATE_FIELDS)}&filter=${encodeURIComponent(FILTER_2020)}&include_records_sample=true&limit=10`,
      body: {},
    });
  ...
  it('groups by stage and closeDate with records and filters', async () => {
    ...
    const response = await makeRestAPIRequest({
      method: 'get',
      path: `/opportunities/groupBy?group_by=${encodeURIComponent(groupByQuery)}&aggregate=${encodeURIComponent(AGGREGATE_FIELDS)}&filter=${encodeURIComponent(filterQuery)}&include_records_sample=true&limit=10`,
      body: {},
    });
  ...
  describe('order by for records', () => {
    const getGroupByRequestWithOrderByForRecords = (
      ...
      return {
        method: 'get' as const,
        path: `/opportunities/groupBy?group_by=${encodeURIComponent(groupByQuery)}&filter=${encodeURIComponent(FILTER_2020)}&order_by_for_records=${encodeURIComponent(`name[${orderByForRecords}]`)}&include_records_sample=true&limit=10`,
        body: {},
      };
    };
