...
describe('Calendar event save operations (integration)', () => {
  ...
  it('should link the participant to a person that already existed before the import', async () => {
    const eventExternalId = `google-calendar-event-${randomUUID()}`;
    const title = `Calendar event ${randomUUID()}`;
    const attendee = `attendee-existing-person-${randomUUID()}@acme.com`;

    const personResponse = await makeGraphqlAPIRequest(
      createOneOperationFactory({
        objectMetadataSingularName: 'person',
        gqlFields: 'id',
        data: { emails: { primaryEmail: attendee } },
      }),
    );

    expect(personResponse.body.errors).toBeUndefined();

    const personId = personResponse.body.data.createPerson.id as string;
    ...
    const participants = await findRecordNodesByFilter<
      ParticipantNode & { personId: string | null }
    >(
      'calendarEventParticipant',
      'calendarEventParticipants',
      'id handle personId',
      { calendarEventId: { eq: event.id } },
    );
