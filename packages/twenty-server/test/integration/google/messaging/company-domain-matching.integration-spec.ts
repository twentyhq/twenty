...
describe('Gmail contact auto-creation company domain matching (integration)', () => {
  ...
  const createCompany = async (primaryLinkUrl: string) => {
    const createResponse = await makeGraphqlAPIRequest(
      createManyOperationFactory({
        ...
        gqlFields: 'id domainName { primaryLinkUrl }',
        data: [
          {
  ...
  const mergeTwoCompanies = async ({
    ...
    const survivingCompany = await createCompany(
      `https://${survivingDomainName}`,
    );
    const mergedAwayCompany = await createCompany(
      `https://www.${mergedAwayDomainName}`,
    );
  ...
  it('stores a domain name as its bare domain whatever spelling it was written with', async () => {
    const domainName = `written-as-url-${randomUUID()}.com`;

    const company = await createCompany(
      `HTTPS://WWW.${domainName.toUpperCase()}/careers?utm=1`,
    );

    expect(company.domainName.primaryLinkUrl).toBe(domainName);
  }, 120000);

  it('attaches a contact to a company whose domain was written as a full url', async () => {
    const domainName = `full-url-${randomUUID()}.com`;

    const company = await createCompany(`https://www.${domainName}/careers`);
  ...
  it('attaches a contact to the surviving company when its domain was merged into secondary links', async () => {
    ...
    expect(mergedCompany.domainName.secondaryLinks).toEqual([
      expect.objectContaining({ url: mergedAwayDomainName }),
    ]);
