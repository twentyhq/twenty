...
const EXPECTED_READABILITIES = {
  message: MetadataReadability.PRIVATE,
  messageThread: MetadataReadability.PRIVATE,
  calendarEvent: MetadataReadability.PRIVATE,
  messageParticipant: MetadataReadability.INHERITED,
  calendarEventParticipant: MetadataReadability.INHERITED,
};
...
describe('2-40 workspace command 1789100300000 - MakeMessagingAndCalendarPrivateCommand (integration)', () => {
  ...
  const setReadabilities = async (
    readabilityByObjectName: Record<string, MetadataReadability>,
  ) => {
    for (const [nameSingular, objectMetadataId] of Object.entries(
      objectMetadataIdByName,
    )) {
      await setObjectReadability(
        objectMetadataId,
        readabilityByObjectName[nameSingular],
      );
    }
  };
  ...
  beforeAll(async () => {
    ...
    objectMetadataIdByName = Object.fromEntries(
      objectMetadataItems.map((objectMetadata) => [
        objectMetadata.nameSingular,
        objectMetadata.id,
      ]),
  ...
  afterAll(async () => {
    await setReadabilities(EXPECTED_READABILITIES);
  });
  ...
  it('leaves the objects OPEN on a dry run', async () => {
    ...
    expect(await findReadabilities()).toEqual({
      ...
      messageParticipant: MetadataReadability.OPEN,
      calendarEventParticipant: MetadataReadability.OPEN,
    });
  });

  it('makes the messaging and calendar objects PRIVATE, their participants INHERITED, and skips them on a second run', async () => {
    await runCommand();
    await runCommand();

    expect(await findReadabilities()).toEqual(EXPECTED_READABILITIES);
  });
});
