...
describe('workflow version core workflow id dual write (e2e)', () => {
  ...
  describe('when the workflow no longer resolves to a core workflow', () => {
    const manualTrigger = {
      name: 'Manual Trigger',
      type: 'MANUAL',
      settings: { outputSchema: {} },
    ...
    const countMirroredCoreVersionRows = async (): Promise<number> => {
      const rows = await global.testDataSource.query(
    ...
    beforeAll(async () => {
      const resolvedCoreWorkflowId = await waitForWorkspaceCoreWorkflowId();

      assertIsDefinedOrThrow(resolvedCoreWorkflowId);

      coreWorkflowId = resolvedCoreWorkflowId;

      await global.testDataSource.query(
        `UPDATE "${WORKSPACE_SCHEMA}"."workflow" SET "coreWorkflowId" = NULL WHERE "id" = $1`,
        [workflowId],
      );
      await global.testDataSource.query(
    ...
    afterAll(async () => {
      ...
      await global.testDataSource.query(
        `UPDATE "${WORKSPACE_SCHEMA}"."workflow" SET "coreWorkflowId" = $2 WHERE "id" = $1`,
    ...
    it('keeps the existing core workflow link on mirror writes', async () => {
      await updateWorkflowVersionTrigger({
        workflowVersionId,
        trigger: manualTrigger,
      });

      expect(await countCoreVersionRowsPointingAt(coreWorkflowId)).toBe(1);
    });

    it('rejects a mirror write that has no core workflow to link', async () => {
      await global.testDataSource.query(
      ...
      expect(await countMirroredCoreVersionRows()).toBe(0);
    });
  });
});
