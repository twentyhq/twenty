...
export const getInviteSuggestions = async ({
  expectToFail = false,
  token,
  ...
  if (expectToFail === true) {
    warnIfNoErrorButExpectedToFail({
      response,
      errorMessage:
        'Reading the onboarding invite suggestions should have failed but did not',
    });
  }
  ...
  if (expectToFail === false) {
    warnIfErrorButNotExpectedToFail({
      response,
      errorMessage:
        'Reading the onboarding invite suggestions has failed but should not',
    });
  }
