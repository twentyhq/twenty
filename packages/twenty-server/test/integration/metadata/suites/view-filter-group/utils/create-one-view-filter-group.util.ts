...
import { createViewFilterGroupQueryFactory } from 'test/integration/metadata/suites/view-filter-group/utils/create-view-filter-group-query-factory.util';
...
export const createOneViewFilterGroup = async ({
  input,
  gqlFields,
  expectToFail,
}: PerformMetadataQueryParams<CreateViewFilterGroupInput>): CommonResponseBody<{
  createViewFilterGroup: ViewFilterGroupDTO;
}> => {
  const graphqlOperation = createViewFilterGroupQueryFactory({
    input,
    gqlFields,
  });
