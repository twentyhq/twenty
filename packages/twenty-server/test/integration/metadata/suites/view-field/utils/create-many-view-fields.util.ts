...
import { createManyViewFieldsQueryFactory } from 'test/integration/metadata/suites/view-field/utils/create-many-view-fields-query-factory.util';
...
export const createManyViewFields = async ({
  inputs,
  gqlFields,
  expectToFail,
}: {
  ...
  createManyViewFields: ViewFieldEntity[];
}> => {
  const graphqlOperation = createManyViewFieldsQueryFactory({
    inputs,
    gqlFields,
  });
