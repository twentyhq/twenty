...
import { createViewFieldQueryFactory } from 'test/integration/metadata/suites/view-field/utils/create-view-field-query-factory.util';
...
export const createOneViewField = async ({
  input,
  gqlFields,
  expectToFail,
}: PerformMetadataQueryParams<CreateViewFieldInput>): CommonResponseBody<{
  createViewField: ViewFieldEntity;
}> => {
  const graphqlOperation = createViewFieldQueryFactory({
    input,
    gqlFields,
  });
