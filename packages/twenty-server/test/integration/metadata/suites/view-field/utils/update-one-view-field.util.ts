...
import { updateViewFieldQueryFactory } from 'test/integration/metadata/suites/view-field/utils/update-view-field-query-factory.util';
...
export const updateOneViewField = async ({
  input,
  gqlFields,
  expectToFail,
}: PerformMetadataQueryParams<UpdateViewFieldInput>): CommonResponseBody<{
  updateViewField: ViewFieldEntity;
}> => {
  const graphqlOperation = updateViewFieldQueryFactory({
    input,
    gqlFields,
  });
