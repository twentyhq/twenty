...
import { deleteViewFieldQueryFactory } from 'test/integration/metadata/suites/view-field/utils/delete-view-field-query-factory.util';
...
export const deleteOneViewField = async ({
  input,
  gqlFields,
  expectToFail,
}: PerformMetadataQueryParams<DeleteViewFieldInput>): CommonResponseBody<{
  deleteViewField: ViewFieldEntity;
}> => {
  const graphqlOperation = deleteViewFieldQueryFactory({
    input,
    gqlFields,
  });
