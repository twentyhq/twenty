...
import { destroyViewFieldQueryFactory } from 'test/integration/metadata/suites/view-field/utils/destroy-view-field-query-factory.util';
...
export const destroyOneViewField = async ({
  input,
  gqlFields,
  expectToFail,
}: PerformMetadataQueryParams<DestroyViewFieldInput>): CommonResponseBody<{
  destroyViewField: ViewFieldEntity;
}> => {
  const graphqlOperation = destroyViewFieldQueryFactory({
    input,
    gqlFields,
  });
