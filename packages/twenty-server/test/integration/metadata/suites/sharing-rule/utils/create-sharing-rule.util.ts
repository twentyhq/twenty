...
export const createSharingRule = async ({
  input,
  gqlFields = SHARING_RULE_GQL_FIELDS,
  expectToFail = false,
  token,
}: PerformMetadataQueryParams<CreateSharingRuleInput>): CommonResponseBody<{
  createSharingRule: SharingRuleDTO;
}> => {
  ...
  if (expectToFail === true) {
    warnIfNoErrorButExpectedToFail({
      response,
      errorMessage: 'Sharing rule creation should have failed but did not',
    });
  }
  ...
  if (expectToFail === false) {
    warnIfErrorButNotExpectedToFail({
      response,
      errorMessage: 'Sharing rule creation has failed but should not',
    });
  }
