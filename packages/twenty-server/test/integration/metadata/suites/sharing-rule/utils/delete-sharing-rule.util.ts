...
export const deleteSharingRule = async ({
  input,
  gqlFields = SHARING_RULE_GQL_FIELDS,
  expectToFail = false,
  token,
}: PerformMetadataQueryParams<{ id: string }>): CommonResponseBody<{
  deleteSharingRule: SharingRuleDTO;
}> => {
  ...
  if (expectToFail === true) {
    warnIfNoErrorButExpectedToFail({
      response,
      errorMessage: 'Sharing rule deletion should have failed but did not',
    });
  }
  ...
  if (expectToFail === false) {
    warnIfErrorButNotExpectedToFail({
      response,
      errorMessage: 'Sharing rule deletion has failed but should not',
    });
  }
