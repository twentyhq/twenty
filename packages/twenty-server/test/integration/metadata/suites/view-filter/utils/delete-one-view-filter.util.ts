...
import { deleteViewFilterQueryFactory } from 'test/integration/metadata/suites/view-filter/utils/delete-view-filter-query-factory.util';
...
export const deleteOneViewFilter = async ({
  input,
  gqlFields,
  expectToFail,
}: PerformMetadataQueryParams<DeleteViewFilterInput>): CommonResponseBody<{
  deleteViewFilter: ViewFilterDTO;
}> => {
  const graphqlOperation = deleteViewFilterQueryFactory({
    input,
    gqlFields,
  });
