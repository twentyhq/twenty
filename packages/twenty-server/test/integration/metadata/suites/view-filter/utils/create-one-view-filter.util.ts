...
import { createViewFilterQueryFactory } from 'test/integration/metadata/suites/view-filter/utils/create-view-filter-query-factory.util';
...
export const createOneViewFilter = async ({
  ...
  createViewFilter: ViewFilterDTO;
}> => {
  const graphqlOperation = createViewFilterQueryFactory({
    input,
    gqlFields,
  });
