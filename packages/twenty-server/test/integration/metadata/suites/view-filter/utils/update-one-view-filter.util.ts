...
import { updateViewFilterQueryFactory } from 'test/integration/metadata/suites/view-filter/utils/update-view-filter-query-factory.util';
...
export const updateOneViewFilter = async ({
  input,
  gqlFields,
  expectToFail,
}: PerformMetadataQueryParams<UpdateViewFilterInput>): CommonResponseBody<{
  updateViewFilter: ViewFilterDTO;
}> => {
  const graphqlOperation = updateViewFilterQueryFactory({
    input,
    gqlFields,
  });
