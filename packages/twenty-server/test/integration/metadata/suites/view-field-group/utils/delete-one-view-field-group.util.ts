...
import { deleteViewFieldGroupQueryFactory } from 'test/integration/metadata/suites/view-field-group/utils/delete-view-field-group-query-factory.util';
...
export const deleteOneViewFieldGroup = async ({
  input,
  gqlFields,
  expectToFail,
}: PerformMetadataQueryParams<DeleteViewFieldGroupInput>): CommonResponseBody<{
  deleteViewFieldGroup: ViewFieldGroupDTO;
}> => {
  const graphqlOperation = deleteViewFieldGroupQueryFactory({
    input,
    gqlFields,
  });
