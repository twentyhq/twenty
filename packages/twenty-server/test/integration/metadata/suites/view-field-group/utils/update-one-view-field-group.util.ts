...
import { updateViewFieldGroupQueryFactory } from 'test/integration/metadata/suites/view-field-group/utils/update-view-field-group-query-factory.util';
...
export const updateOneViewFieldGroup = async ({
  input,
  gqlFields,
  expectToFail,
}: PerformMetadataQueryParams<UpdateViewFieldGroupInput>): CommonResponseBody<{
  updateViewFieldGroup: ViewFieldGroupDTO;
}> => {
  const graphqlOperation = updateViewFieldGroupQueryFactory({
    input,
    gqlFields,
  });
