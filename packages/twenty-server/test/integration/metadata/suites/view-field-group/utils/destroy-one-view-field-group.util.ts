...
import { destroyViewFieldGroupQueryFactory } from 'test/integration/metadata/suites/view-field-group/utils/destroy-view-field-group-query-factory.util';
...
export const destroyOneViewFieldGroup = async ({
  input,
  gqlFields,
  expectToFail,
}: PerformMetadataQueryParams<DestroyViewFieldGroupInput>): CommonResponseBody<{
  destroyViewFieldGroup: ViewFieldGroupDTO;
}> => {
  const graphqlOperation = destroyViewFieldGroupQueryFactory({
    input,
    gqlFields,
  });
