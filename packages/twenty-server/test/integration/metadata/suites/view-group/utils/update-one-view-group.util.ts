...
import { updateViewGroupQueryFactory } from 'test/integration/metadata/suites/view-group/utils/update-view-group-query-factory.util';
...
export const updateOneViewGroup = async ({
  input,
  gqlFields,
  expectToFail,
}: PerformMetadataQueryParams<UpdateViewGroupInput>): CommonResponseBody<{
  updateViewGroup: ViewGroupEntity;
}> => {
  const graphqlOperation = updateViewGroupQueryFactory({
    input,
    gqlFields,
  });
