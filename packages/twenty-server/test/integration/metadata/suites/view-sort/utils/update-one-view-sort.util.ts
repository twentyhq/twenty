...
import { updateViewSortQueryFactory } from 'test/integration/metadata/suites/view-sort/utils/update-view-sort-query-factory.util';
...
export const updateOneViewSort = async ({
  input,
  gqlFields,
  expectToFail,
}: PerformMetadataQueryParams<UpdateViewSortInput>): CommonResponseBody<{
  updateViewSort: ViewSortDTO;
}> => {
  const graphqlOperation = updateViewSortQueryFactory({
    input,
    gqlFields,
  });
