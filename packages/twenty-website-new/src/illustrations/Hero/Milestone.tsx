...
function resolveMilestoneSettings(nextSettings = {}) {
  return {
    ...DEFAULT_SETTINGS,
    ...nextSettings,
    lighting: {
      ...DEFAULT_SETTINGS.lighting,
      ...nextSettings.lighting,
    },
    material: {
      ...DEFAULT_SETTINGS.material,
      ...nextSettings.material,
    },
    halftone: {
      ...DEFAULT_SETTINGS.halftone,
      ...nextSettings.halftone,
    },
    background: {
    ...
    animation: {
      ...
      ...nextSettings.animation,
    },
  };
}

async function mountHalftoneCanvas(options) {
  const { container, imageUrl, settingsRef } = options;
  const getSettings = () => resolveMilestoneSettings(settingsRef?.current);
  ...
  const illustrationColor = settings.halftone.dashColor;
  const renderer = new THREE.WebGLRenderer({ antialias: false, alpha: true });
  renderer.outputColorSpace = THREE.SRGBColorSpace;
  renderer.setPixelRatio(1);
  renderer.setClearColor(
    settings.background.color,
    settings.background.transparent ? 0 : 1,
  );
  ...
  const halftoneMaterial = new THREE.ShaderMaterial({
    ...
    uniforms: {
      ...
      dashColor: { value: new THREE.Color(illustrationColor) },
      hoverDashColor: {
        value: new THREE.Color(illustrationColor),
      },
      time: { value: 0 },
      waveAmount: { value: 0 },
  ...
  const renderFrame = (timestamp) => {
    ...
    renderer.setClearColor(
      ...
      currentSettings.background.transparent ? 0 : 1,
    );
    imageMaterial.uniforms.contrast.value =
      currentSettings.halftone.imageContrast;
    halftoneMaterial.uniforms.tile.value = currentSettings.halftone.scale;
    halftoneMaterial.uniforms.s_3.value = currentSettings.halftone.power;
    halftoneMaterial.uniforms.s_4.value = currentSettings.halftone.width;
    halftoneMaterial.uniforms.applyToDarkAreas.value =
      currentSettings.halftone.toneTarget === 'dark' ? 1 : 0;
    halftoneMaterial.uniforms.dashColor.value.set(
      currentSettings.halftone.dashColor,
    );
    halftoneMaterial.uniforms.hoverDashColor.value.set(
      currentSettings.halftone.hoverDashColor,
    );
    halftoneMaterial.uniforms.time.value = elapsedTime;
    halftoneMaterial.uniforms.waveAmount.value = currentSettings.animation
      .waveEnabled
      ? currentSettings.animation.waveAmount
      : 0;
    halftoneMaterial.uniforms.waveSpeed.value =
      currentSettings.animation.waveSpeed;
    const hoverEasing =
      1 -
      Math.exp(
        -deltaSeconds *
    ...
    halftoneMaterial.uniforms.hoverHalftoneActive.value = currentSettings
      .animation.hoverHalftoneEnabled
      ? interaction.hoverStrength
      : 0;
    halftoneMaterial.uniforms.hoverHalftonePowerShift.value = currentSettings
      .animation.hoverHalftoneEnabled
      ? currentSettings.animation.hoverHalftonePowerShift
      : 0;
    halftoneMaterial.uniforms.hoverHalftoneRadius.value =
      currentSettings.animation.hoverHalftoneRadius;
    halftoneMaterial.uniforms.hoverHalftoneWidthShift.value = currentSettings
      .animation.hoverHalftoneEnabled
      ? currentSettings.animation.hoverHalftoneWidthShift
      : 0;
    halftoneMaterial.uniforms.hoverLightStrength.value = currentSettings
      .animation.hoverLightEnabled
      ? currentSettings.animation.hoverLightIntensity *
        interaction.hoverStrength
      : 0;
    halftoneMaterial.uniforms.hoverLightRadius.value =
      currentSettings.animation.hoverLightRadius;
...
export function Milestone({
  imageUrl = '/illustrations/generated/milestone.jpg',
  settings,
  style,
}: MilestoneProps) {
  const mountReference = useRef<HTMLDivElement>(null);
  const resolvedSettings = resolveMilestoneSettings(settings);
  const settingsReference = useRef(resolvedSettings);

  settingsReference.current = resolvedSettings;
