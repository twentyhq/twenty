...
async function mountHalftoneCanvas(options) {
  ...
  try {
    geometry = await createGeometry(modelUrl, modelOverrides?.importedGeometry);
  } catch (error) {
    onError?.(error);
  ...
  canvas.style.cursor = resolvedAnimation.followDragEnabled
    ? 'grab'
    : 'default';
  ...
  const handleWindowPointerMove = (event) => {
    ...
    const deltaX =
      (event.clientX - interaction.pointerX) * resolvedAnimation.dragSens;
    const deltaY =
      (event.clientY - interaction.pointerY) * resolvedAnimation.dragSens;
  ...
  const renderFrame = (timestamp) => {
    ...
    if (autoRotateEnabled) {
      ...
      baseRotationX +=
        Math.sin(interaction.autoElapsed * 0.2) * resolvedAnimation.autoWobble;
    }
    ...
    if (rotateEnabled) {
      ...
      if (resolvedAnimation.rotatePreset === 'axis') {
        const axisDirection = resolvedAnimation.rotateAxis.startsWith('-')
          ? -1
          : 1;
    ...
    if (resolvedAnimation.lightSweepEnabled) {
      ...
      lightHeight +=
        Math.cos(lightPhase * 0.85) * resolvedAnimation.lightSweepHeightRange;
    }
